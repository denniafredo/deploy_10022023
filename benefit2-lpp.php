		...
		if(is_null($hasilpremi)||is_null($hasilbenefit)||is_null($hasilexpirasi)) {
		  if ((strlen($rumuspremi)==0) && (strlen($rumusbenefit)==0)) {
			...
			$hasilexpirasi = NULL;
					...
					$FM->parse($rumuspremi);
		    $hasilpremi=$FM->execute($DB); //echo "sin $rumuspremi";
			
			$FM->parse("(* JUA 1)");
			$debugpremi=$FM->execute($DB); 
			//echo "$debugpremi";
...
echo "<form action=\"benefit1-lpp.php?vara=$vara&premijua=$premijua&jua1=$jua1&prefixpertanggungan=$prefixpertanggungan&nopertanggungan=$nopertanggungan&noproposal=$noproposal&kdper=$kdper\" name=\"porm\" method=\"post\">";
