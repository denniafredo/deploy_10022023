			...
			try {
				...
				if ($scope.data.isAdaTertanggungTambahan1) {
					if ($scope.data.noKtpTertanggungTambahan1 == '') $scope.messages.push({
						'message': "Nomor KTP tertanggung tambahan 1 harus benar! (16 digit)"
					});
	...
	function ($state, $scope, $stateParams, spajProvider, dataFactory, $store, $ionicPopup) {
		...
		$scope.init_data = function () {
			$scope.data = {
				...
				'masaBayarPremi': (prospek.kd_produk == 'JL4BLN' || prospek.kd_produk == 'JL4BLN_' || prospek.kd_produk == 'JL4XN') ? "99" : (prospek.kd_produk == 'JSDMPPN')? "10":"5",
				'premi': prospek.jumlah_premi,
				'uangAsuransi': prospek.jua,
				'jangkaWaktuAsuransi': (prospek.kd_produk == 'JL4BLN' || prospek.kd_produk == 'JL4BLN_' || prospek.kd_produk == 'JL4XN') ? "99" : (prospek.kd_produk == 'JSDMPPN')? "10":"5", //data.jenisAsuransi == 'JL4BLN' || data.jenisAsuransi == 'JL4BLN_' || data.jenisAsuransi == 'JL4XN'
				'nomorRekening': '',
		...
		$scope.setProspekRider = function(){
			...
			try{
				...
				$scope.data.SPOUSE_TPDUA = parseInt(prospek.SPOUSE_TPD);
				$scope.data.SPOUSE_DEATHUA = parseInt(prospek.SPOUSE_DEATH);
			}catch(e){
...
.controller('daftarSPAJOnlineCtrl', ['$scope', '$state', '$stateParams', 'spajProvider', 'dataFactory', '$ionicPopup', '$http', '$ionicLoading', '$location', '$store', '$ionicScrollDelegate',
	function ($scope, $state, $stateParams, spajProvider, dataFactory, $ionicPopup, $http, $ionicLoading, $location, $store, $ionicScrollDelegate) {
		...
		$scope.isValidToSubmit = function(guid){
			...
			if ($scope.sumberDataPempol1 == null) {
				//$scope.messages.push({	'message': 'Silakan lengkapi data identitas pempol.'	})
			} else {bol1 = true;}			
			if ($scope.sumberDataPempol2 == null) {
...
/* 			console.log('1 ' + isValidDataTertanggung);
		...
		$scope.showProfile = function(idagen){
			...
			url = 'http://jaim.jiwasraya.co.id/mobileapi/spaj_bridge/get.php?act=get_profile&idagen=';
			$http({
				method: "GET",
				...
				if (response.data == null) {}
					...
					tpl += '<img style="width:80%;" src="https://jaim.jiwasraya.co.id/asset/avatar/'+idagen+'.jpg">';
					tpl += '</div>';
					tpl += '<div class="col">';
