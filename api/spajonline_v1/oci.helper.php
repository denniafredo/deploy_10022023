<?php
$oracle_config = array(
		"ORACLE_HOST" => "database.ifg-life.id",
		"ORACLE_SERVICE" => "IFGRETAIL",
		"ORACLE_USER" => "NADM",
		"ORACLE_PASSWORD" => "ifg#dbs#nadm#2020",
);
...
class OracleObject {
	...
	*/
	private function csv_enclosed_value($value = null) {
		if ($value !== null && $value != '' ) {
			...
			$csv_enclosure = self::CSV_ENCLOSURE;
	...
	*/
	public function csv() {
		...
		return implode(self::CSV_DELIMITER, $entry).self::CSV_LINEFEED;
	}
...
class OracleConnection {
	...
	public function __construct($config, $debug = false, $debug_filename = null) {
		...
		if (isset($config['ORACLE_SERVICE']))
			$this->ORACLE_SERVICE = $config['ORACLE_SERVICE'];
	...
	*/
	public function connect($force = false) {
		...
		if (!$this->connected) {
			...
			if($this->ORACLE_SERVICE != ""){
				$service = " (SERVICE_NAME  = $this->ORACLE_SERVICE) ";
			} else {
				$service = " (SID  = $this->ORACLE_SID) ";
			}
			
			$ORACLE_DSN = "
			(DESCRIPTION =
				(ADDRESS = (PROTOCOL = TCP)(HOST = $this->ORACLE_HOST)(PORT = $this->ORACLE_PORT))
				(CONNECT_DATA = ".$service." ) )";
				
			$this->connection = oci_connect($this->ORACLE_USER, 
				$this->ORACLE_PASSWORD, 
				$ORACLE_DSN, $this->ORACLE_NLS_CHARACTERSET);
...
public function query_array($sql, $params = null, $output_inserted_id = false){
	$result = false;
	...
	$pres = @oci_execute($this->statement, OCI_NO_AUTO_COMMIT);
	
	$this->num_rows = oci_num_rows($this->statement);
	
	while (($row = oci_fetch_array($this->statement,OCI_ASSOC)) != false) {
		$result[] = $row;
	}
	...
	*/
	public function rows() {
		...
		$rows = $this->convert_from_latin1_to_utf8_recursively($rows);
