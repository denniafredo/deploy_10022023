                            ...
                            <?php if (!$proposal['error'] && $proposal) {
                                foreach($proposal['message'] as $i => $v) { ?>
									<tr bgcolor="<?=$v['SUSPEND'] ? "yellow" : "";?>">
										...
										<td align="center"><?=$v['NAMASTATUS'].($v['SUSPEND'] ? " (Pending) - {$v['KETERANGAN_SUSPEND']}" : null)?></td>
										...
										<td class="text-center" width="95">
                                            ...
                                            <!--upload button-->
                                            <?php if ($v['KDSTATUS'] == 2) { ?> 
                                            <a href="<?="$this->url/uploadfile?buildid=$v[BUILDID]&id=$v[NOID]"?>" class="btn btn-xs blue tooltips" style="margin:0px;" data-container="body" data-placement="top" data-original-title="Upload File"><i class="fa fa-file-pdf-o btn-xs-tbl"></i></a>
                                            <?php } ?>     
    ...
    jQuery(document).ready(function() {
		...
		$('#btnProposal').click(function() {
			...
			if (kdproduk) {
				...
				window.location.href = `<?=base_url("simulasi/produk")?>?noagen=${noagen}&token=${token}&noid=${noid}&kdproduk=${kdproduk}`;
			} else {
