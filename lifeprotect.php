						...
						<div class="portlet-title" align="center">
							<div class="caption">
								...
								<span class="step-title">
									 Asuransi IFG Life Protection Platinum
								</span>
							</div>
						</div>
									          	...
									          	<div class="row">
									            	...
									            	<div class="col-md-5">
											            ...
											            <div class="form-group">
															...
															<div class="col-md-3">
																<input readonly type="text" class="form-control" name="usiapemegangpolis" id="usiapemegangpolis" value="<?= $usia; ?>" />
															</div>
									          	...
									          	<div class="row">
										            ...
										            <div class="col-md-5">
														...
														<div class="form-group">
															...
															<div class="col-md-4">
																<select class="form-control " name="hubungandenganpempol" id="hubungandenganpempol" >
																	...
																	<option value="5">Adik Kandung</option>
																	<option value="6">Kakek/Nenek</option>
																	<option value="7">Cucu</option>
																	<option value="8">Paman/Bibi</option>
																	<option value="9">Keponakan</option>
																</select>
															</div>
														</div>
														...
														<div class="form-group">
															...
															<div class="col-md-4">
																<input type="text" class="form-control" name="noktpcalontertanggung" id="noktpcalontertanggung" oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);" maxlength = "16" required />
															</div>
											            </div>
	...
	<!-- END CONTENT -->
		...
		<script>
			...
			$('#tanggallahircalontertanggung').on('change', function() {
    			...
    			let ageInMonths = ageInDays / 30.44; // rata-rata jumlah hari dalam setiap bulan
    			let ageInYears = ageInMonths / 12;

    			let years = Math.floor(ageInYears);
			    let months = Math.floor(ageInMonths % 12);
			    let days = Math.floor(ageInDays % 30.44);
				...
				var age = hariini.getFullYear() - birthday_ctt.getFullYear() - tahun + x;
			...
			function getAllTarif(){
				...
				var kode_produk = 'JL4BIFG';

				$.ajax({
					type	: "POST",
					url		: "<?=base_url('jspromapannew_2019/cgetTarifPremi');?>",
					data	: "usiacalontertanggung="+usiasekarangctt+"&kdproduk="+kode_produk,
					success : function(msg) {
						...
						if(jeniskelaminctt == 'M'){
							var tariftermlife = a.TERMLIFE_L;
						}else{
							var tariftermlife = a.TERMLIFE_P;
						}
						...
						console.log(nilaiekstrapremilife_ctt, nilaiekstrapremilifehobi_ctt,tariftermlife);
						...
						if(kdjenispekerjaanctt ==75 || kdjenispekerjaanctt ==165 || kdjenispekerjaanctt ==109){
							if (ra_wp > 200000000){
						...
						if (jumlahnilaibiaya > ceknilairiderbenefitpasti){
							...
							if(nilaia < nilaib){
								...
								if(hasilhitungjua < 100000000){
									alert("Minimal uang asuransi adalah seratus juta (Rp 100.000.000,-)");
									location.reload();
								}else{
						...
						/* FUNGSI MENGHITUNG UA BARU - 22/03/2022 */
						//var minimum_ua = totalpremi * 12 * 5 <= 100000000 ? 100000000 : totalpremi * 12 * 5;
						/*var minimum_ua = totalpremi * 12 * 5 * 10;
						console.log("Min. UA : "+minimum_ua);
						var hasilhitungjua = minimum_ua;
						...
						if (($('#kdjnspekerjaancalontertanggung').val() == '165' || $('#kdjnspekerjaancalontertanggung').val() == '75' || $('#kdjnspekerjaancalontertanggung').val() == '109') && totalresikoawalctt > 500000000) {
							...
							$('#uangasuransi').val(500000000);
							$('#uangasuransidasar').val(500000000);
							$('#resikoawaluangasuransidasar').val(500000000);
						}
			...
			/**
				Rider ADDB di hide, ketika user memilih ADB
			**/
			$('#pil_jsadb').on('change', function() {
				let x = $("#pil_jsadb").is(":checked");
				if (x == true){
					$("#barisjsaddb").hide();
				}else{
					$("#barisjsaddb").show();
			...
			/**
				Rider ADB di hide, ketika user memilih ADDB
			**/
			$('#pil_jsaddb').on('change', function() {
				let x = $("#pil_jsaddb").is(":checked");
				if (x == true){
					$("#barisjsadb").hide();
				}else{
					$("#barisjsadb").show();
			...
			$('#totalpremi').on('change', function() {
				...
				if (carabayarpremi == 1){
					var totalpremi = document.getElementById("totalpremi").value;
					if(totalpremi<500000){ 
						alert("Minimal Premi per Bulan Rp 500.000,-");
						document.getElementById("totalpremi").value ="";
					}else{
			...
			$('#submitProposal').on('click', function() {
				...
				$.ajax({
					...
					data	: "hubungandenganpempol="+hubungandenganpempol+"&perokokpempol="+perokokpempol+"&namalengkap="+namalengkap+"&tanggallahir="+tanggallahir+"&gender="+gender+"&email="+email+"&handphone="+handphone+"&phone="+phone+"&kdjnspekerjaanpempol="+kdjnspekerjaanpempol+"&noktpcpp="+noktpcpp+"&kdhobicalonpempol="+kdhobipemegangpolis+"&kdproduk=JL4BPRO",
					success : function(msg) {
						...
						if(msg == '1' || msg == 1){
							...
							$.ajax({
								...
								success : function(msg) {				
									...
									window.location.href= "<?=base_url('jspromapannew_2019/hasilifg')?>?buildid="+<?=$this->session->userdata('build_id');?>+'&filepdf='+filepdf+'&kodeprospek='+kodeprospek+'&kodeproduk=JL4BPRO'; 
									$.LoadingOverlay("hide");
