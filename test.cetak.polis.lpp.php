<?php
	...
	$data['paragraph1']		 = "Penanggung dengan ini menyatakan setuju untuk membayarkan Manfaat Asuransi atas diri Tertanggung sebagaimana "
							 . "yang tercantum dalam Polis ini berdasarkan syarat dan ketentuan Data Polis, Syarat - Syarat Umum Polis, Ketentuan Khusus, "
	...
	$data['paragraph3']		 = "Dalam hal masa Free Look Provision tersebut telah terlewati, maka \"Pemegang Polis dengan ini menyatakan bahwa "
							 . "Pertanggungan sudah disetujui sesuai dengan Data Polis, Syarat - Syarat Umum Polis, Ketentuan Khusus, ketentuan tambahan "
							 . "dan atau ketentuan lainnya (bila ada)\".";
	...
	class CetakPolis extends FPDF {
        ...
        private $kelasrs	= null;
        private $col = 0;
		
		public function __construct($data) {
			parent::__construct('P', 'mm', 'A4');
            $this->SetMargins(10, 35);
		...
		public function Init() {
			...
			$sql = "SELECT a.NOPOLBARU, a.PREFIXPERTANGGUNGAN, a.TGLSP, TO_CHAR(a.MULAS, 'DD/MM/YYYY') AS MULAS, a.USIA_TH, a.NOPERTANGGUNGAN, 
						...
						a.PREMI1, a.PREMI2, /*CASE WHEN SUBSTR(a.NOSP, 0, 1) = 'O' THEN k.nomorspajcetak ELSE a.NOSP END*/ a.NOSP, 
						...
						(SELECT COUNT(kdbenefit) FROM $DBUser.tabel_223_transaksi_produk WHERE prefixpertanggungan = a.prefixpertanggungan AND nopertanggungan = a.nopertanggungan AND kdbenefit IN ('JSHCA', 'JSHCB', 'JSHCC', 'JSHCD', 'JSHCE')) rideraskes,
                        (SELECT NAMAPEJABAT FROM $DBUser.TABEL_002_PEJABAT WHERE KDKANTOR = 'KP' AND KDORGANISASI = '100') AS NAMADIREKTUR,
                        (SELECT NAMAJABATAN FROM $DBUser.TABEL_002_PEJABAT WHERE KDKANTOR = 'KP' AND KDORGANISASI = '100') AS JABATANDIREKTUR,
						l.NOACCOUNT NOH2H, m.NOACCOUNT NOVABNI
					...
					LEFT OUTER JOIN $DBUser.TABEL_100_KLIEN_ACCOUNT l ON a.prefixpertanggungan = l.prefixpertanggungan
						...
						AND l.jenis = 'HH'
						AND l.kdbank = 'BMRI'
					LEFT OUTER JOIN $DBUser.TABEL_100_KLIEN_ACCOUNT m ON a.prefixpertanggungan = m.prefixpertanggungan
						AND a.nopertanggungan = m.nopertanggungan
						AND m.jenis = 'VA'
						AND m.kdbank = 'BNI'
            ...
            /*===== tertanggung rider lampiran polis JL4B hospital care =====*/
            ...
            /*===== nilai benefit rawat inap rider JL4B hospital care =====*/
		...
		public function SPAJ() {
			...
			$y1 = $this->GetY();
			$this->font_body();
			$this->MultiCell($this->pg_w-($this->num_space*2)-($x2-$x1)-($this->pg_w-$this->num_space-102), 5, ", beserta semua pernyataan dan keterangan ****************************", 0, 'J');
			$this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
			$this->SetXY($this->GetX(), $y1+5);
			$this->SetFillColor(255,255,255);
			$this->MultiCell($this->pg_w-($this->num_space*2), 5, "yang disampaikan oleh :", 0, 'J', true);
			...
			$y1 = $this->GetY();
			$this->font_ttd();
			//$this->MultiCell(($this->pg_w-($this->num_space*2))/3, 3, "Bea Materai Lunas Rp10.000 berdasarkan keputusan\nKantor Pelayanan Pajak Tanggal : 15 Oktober 2021\nNomor SI-00006/SK/WPJ.30/KP.0103/2021", 0, 'C', false);
			$y2 = $this->GetY();
			$yH = $y2 - $y1;
			...
			$this->Image($filename, 158, $lastY, 30, 30);
            $this->Ln(44);
		...
		public function DataPolis() {
			...
			//echo $this->arr['TGLAKHIRPREMI']."AAAA";
			$this->font_title('B');
			...
			$this->Cell(($this->pg_w-($this->num_space*2))/25, $height, ":", 0, 0, 'L');
			$this->SetFont('Arial','',9);
			$this->Cell(($this->pg_w-($this->num_space*2))*17/25, $height, $this->tgl_indonesia($arr["MULAS"])." SAMPAI ".$this->tgl_indonesia($arr["EXPIRASI"]), 0, 1, 'L');
			//
			$this->SetFont('Arial','B',9);
			$this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
			...
			$this->Cell(($this->pg_w-($this->num_space*2))/25, $height, ":", 0, 0, 'L');
			$this->SetFont('Arial','',9);
			$this->Cell(($this->pg_w-($this->num_space*2))*17/25, $height, $arr["NOH2H"], 0, 1, 'L');
			//
			$this->SetFont('Arial','B',9);
			$this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
			$this->Cell(($this->pg_w-($this->num_space*2))*7/25, $height, "Nomor Virtual Account BNI", 0, 0, 'L');
			$this->Cell(($this->pg_w-($this->num_space*2))/25, $height, ":", 0, 0, 'L');
			$this->SetFont('Arial','',9);
			$this->Cell(($this->pg_w-($this->num_space*2))*17/25, $height, $arr["NOVABNI"], 0, 1, 'L');
			//
			...
			$this->SetFont('Arial','B',9);
			$this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
			if ($kd_cara_bayar == "X" || $kd_cara_bayar == "E" || $kd_cara_bayar == "J" || $kd_cara_bayar == "HT" || $lama_premi < 5) {
                $this->Cell(($this->pg_w-($this->num_space*2))*7/25, $height, "Premi Single", 0, 0, 'L');
                $this->Cell(($this->pg_w-($this->num_space*2))/25, $height, ":", 0, 0, 'L');
                $this->SetFont('Arial','',9);
				$this->Cell(($this->pg_w-($this->num_space*2))*17/25, $height, $arr['NOTASI'].number_format($arr['PREMISTD'],2), 0, 1, 'L');
			}
			else if ($premi2 > 0) {
                $this->Cell(($this->pg_w-($this->num_space*2))*7/25, $height, "Premi Regular", 0, 0, 'L');
                $this->Cell(($this->pg_w-($this->num_space*2))/25, $height, ":", 0, 0, 'L');
                $this->SetFont('Arial','',9);
				$this->Cell(($this->pg_w-($this->num_space*2))*17/25, $height, $arr['NOTASI'].number_format($arr['PREMISTD'],2,",","."), 0, 1, 'L');
			}
			...
			if(substr($arr["KDPRODUK"], 0, 4) == "JL4B") {
				...
				$this->Cell(($this->pg_w-($this->num_space*2))/25, $height, ":", 0, 0, 'L');
				$this->SetFont('Arial','',9);
				$this->Cell(($this->pg_w-($this->num_space*2))/25, $height, $arr['NOTASI'].number_format($arrs["PREMITUB"],2,",","."), 0, 0, 'L');
				//$this->Cell(($this->pg_w-($this->num_space*2))*16/25, $height, $ss,2,",","."), 0, 1, 'L');
			} else if (substr($arr['KDPRODUK'], 0, 3) == 'JL4') {
                $this->SetFont('Arial','B',9);
                $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
                $this->Cell(($this->pg_w-($this->num_space*2))*7/25, $height, "Top Up Single", 0, 0, 'L');
                $this->Cell(($this->pg_w-($this->num_space*2))/25, $height, ":", 0, 0, 'L');
                $this->SetFont('Arial','',9);
                $this->Cell(($this->pg_w-($this->num_space*2))/25, $height, $arr['NOTASI'].number_format($arrs["PREMITUP"],2,",","."), 0, 0, 'L');
            }
			//
			$this->Ln(5);
			$this->SetFont('Arial','B',9);
			...
			$x1 = $this->GetX();
			$y1 = $this->GetY();
			if(substr($kd_produk,0,4)=="JL4B")
				$this->MultiCell(($this->pg_w-($this->num_space*2))*3/25, 4, "SUP\nKK", 0, 'C');
			else if ($kd_produk=="JL4XG")
                $this->MultiCell(($this->pg_w-($this->num_space*2))*3/25, $h, "SUP JS GA\nKK JS GA", 0, 'C');
			else
				$this->MultiCell(($this->pg_w-($this->num_space*2))*3/25, $h, "SUP PRO-I\nKK PRO-I", 0, 'C');
			$y2 = $this->GetY();
			$yH = $y2 - $y1;
			...
			$this->Cell(($this->pg_w-($this->num_space*2))*4/25, $h, "UTAMA", 0, 0, 'C');
			$this->Cell(($this->pg_w-($this->num_space*2))*4/25, $h, $arr["EXPIRASI"], 0, 0, 'C');
			$this->Cell(($this->pg_w-($this->num_space*2))*5/25, $h, $arr["EXPIRASI"], 0, 1, 'C');
			...
			if (!empty($this->art)) {
				...
				foreach($this->art as $i => $value) {
					...
					$atasdrtertanggung = substr($kd_produk, 0, 3) == "JL3" ? $tgl_mulas : trim($value["ATAS"]);

					/*Update sesuai email dari OBR 13/04/2020 untuk mengubah kode TERM RIDER dari 'TI' ke 'TR' - Teguh*/
					if($kdbnfx == 'TI'){
						$kdbnfx = 'TR';
					}
					...
					$x1 = $this->GetX();
					$y1 = $this->GetY();
					$this->MultiCell(($this->pg_w-($this->num_space*2))*5/25, 4, trim($value["NAMABENEFIT"]), 0, 'L');
					$y2 = $this->GetY();
					$yH = $y2 - $y1;
					...
					$this->Cell(($this->pg_w-($this->num_space*2))*4/25, $h, $atasdrtertanggung, 0, 0, 'C');
					$this->Cell(($this->pg_w-($this->num_space*2))*4/25, $h, $value["EXP"], 0, 0, 'C');
					$this->Cell(($this->pg_w-($this->num_space*2))*5/25, $h, $value["EXP"], 0, 1, 'C');
					$this->Ln(1);
				}
			}
		}
		    ...
		    if ($this->arr['RIDERASKES'] > 0) {
                ...
                foreach($this->arrt2 as $i => $value) {
                    if ($i < $k && $this->arr['KDPRODUK'] != 'JSKEL') {
                        ...
                        if ($this->arrt['KDPRODUK'] == "JSSPO1" || $this->arrt['KDPRODUK'] == "JSSPO2" || $this->arrt['KDPRODUK'] == "JSSPO7A" || $this->arrt['KDPRODUK'] == "JSSPO8" || $this->arrt['KDPRODUK'] == "JSSPO9") {
                            $data[$j]['NO2']	   = $value['T'];
                            $data[$j]['NILAI2'] = substr_replace(floor(0.001 * ($value['TARIF'] * $this->arrt['JUA'] + $value['TARIFS'] * $this->arrt['JUABP'])), '000', -3);
                        }
                        else if ($this->arrt['KDPRODUK'] == "JSPEIPN" || $this->arrt['KDPRODUK'] == "JSPEIP" || $this->arrt['KDPRODUK'] == "JSPEI5") {
                            $data[$j]['NO2']	= $value['T'];
		...
		public function LampiranPolis() {
            ...
            if(substr($arr["KDPRODUK"], 0, 4) == "JL4B") {
                $this->font_body();
                $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
                $this->Cell($this->pg_w-($this->num_space*2), $height, "I. Pilihan Jenis Dana Investasi Awal :", 0, 1, 'L');
                ...
                // $this->font_body('B');
                // $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
                // $this->Cell($this->pg_w-($this->num_space*2), $height, "Persentase Alokasi Dana Berdasarkan Tahun Polis", 0, 1, 'L');
                // $this->Ln(2);


                // /* Halaman Persentase Alokasi Investasi - 20052022 */
                // if($arr["KDPRODUK"] == 'JL4BPRO'){
                // 	$this->font_body();
	               //  $y = $this->GetY();
	               //  $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
	               //  $this->Cell(($this->pg_w-($this->num_space*2))*3/27, $height, "Tahun", 1, 0, 'C');

	               //  $x1 = $this->GetX();
	               //  $y1 = $this->GetY();
	               //  $this->MultiCell(($this->pg_w-($this->num_space*2))*12/27, $height, "Alokasi Biaya", 1, 'C');
	               //  $y2 = $this->GetY();
	               //  $yH = $y2 - $y1;

	               //  $this->SetXY($x1 + ($this->pg_w-($this->num_space*2))*12/27, $y2 - $yH);
	               //  $this->MultiCell(($this->pg_w-($this->num_space*2))*12/27, $height, "Alokasi Investasi", 1, 'C');

	               //  $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
	               //  $this->Cell(($this->pg_w-($this->num_space*2))*3/27, $height, "1", 1, 0, 'C');
	               //  $this->Cell(($this->pg_w-($this->num_space*2))*12/27, $height, "40%", 1, 0, 'C');
	               //  $this->Cell(($this->pg_w-($this->num_space*2))*12/27, $height, "60%", 1, 1, 'C');

	               //  $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
	               //  $this->Cell(($this->pg_w-($this->num_space*2))*3/27, $height, "2", 1, 0, 'C');
	               //  $this->Cell(($this->pg_w-($this->num_space*2))*12/27, $height, "40%", 1, 0, 'C');
	               //  $this->Cell(($this->pg_w-($this->num_space*2))*12/27, $height, "60%", 1, 1, 'C');

	               //  $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
	               //  $this->Cell(($this->pg_w-($this->num_space*2))*3/27, $height, "3 dst", 1, 0, 'C');
	               //  $this->Cell(($this->pg_w-($this->num_space*2))*12/27, $height, "0%", 1, 0, 'C');
	               //  $this->Cell(($this->pg_w-($this->num_space*2))*12/27, $height, "100%", 1, 1, 'C');

                // }else{
                // 	$this->font_body();
	               //  $y = $this->GetY();
	               //  $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
	               //  $this->Cell(($this->pg_w-($this->num_space*2))*3/27, $height*3, "Tahun", 1, 0, 'C');
	               //  $this->SetXY(($this->num_space*2)+(($this->pg_w-($this->num_space*2))*3/27), $y);

	               //  $x1 = $this->GetX();
	               //  $y1 = $this->GetY();
	               //  $this->MultiCell(($this->pg_w-($this->num_space*2))*12/27, $height, "Persentase Dana Investasi dari Premi Regular dan Top Up Regular", 1, 'C');
	               //  $y2 = $this->GetY();
	               //  $yH = $y2 - $y1;

	               //  $this->SetXY($x1 + ($this->pg_w-($this->num_space*2))*12/27, $y2 - $yH);
	               //  $this->MultiCell(($this->pg_w-($this->num_space*2))*12/27, $height, "Persentase Biaya Akuisisi dari Premi Regular dan Biaya Top Up dari Top Up Regular	", 1, 'C');
	               //  $this->SetX(($this->num_space*2)+(($this->pg_w-($this->num_space*2))*3/27));
	               //  $this->Cell(($this->pg_w-($this->num_space*2))*6/27, $height, "Premi Regular", 1, 0, 'C');
	               //  $this->Cell(($this->pg_w-($this->num_space*2))*6/27, $height, "Top Up Regular", 1, 0, 'C');
	               //  $this->Cell(($this->pg_w-($this->num_space*2))*6/27, $height, "Premi Regular", 1, 0, 'C');
	               //  $this->Cell(($this->pg_w-($this->num_space*2))*6/27, $height, "Top Up Regular", 1, 1, 'C');

	               //  $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
	               //  $this->Cell(($this->pg_w-($this->num_space*2))*3/27, $height, "1", 'LR', 0, 'C');
	               //  $this->Cell(($this->pg_w-($this->num_space*2))*6/27, $height, "10%", 'LR', 0, 'C');
	               //  $this->Cell(($this->pg_w-($this->num_space*2))*6/27, $height, "95%", 'LR', 0, 'C');
	               //  $this->Cell(($this->pg_w-($this->num_space*2))*6/27, $height, "90%", 'LR', 0, 'C');
	               //  $this->Cell(($this->pg_w-($this->num_space*2))*6/27, $height, "5%", 'LR', 1, 'C');

	               //  $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
	               //  $this->Cell(($this->pg_w-($this->num_space*2))*3/27, $height, "2", 'LR', 0, 'C');
	               //  $this->Cell(($this->pg_w-($this->num_space*2))*6/27, $height, "50%", 'LR', 0, 'C');
	               //  $this->Cell(($this->pg_w-($this->num_space*2))*6/27, $height, "95%", 'LR', 0, 'C');
	               //  $this->Cell(($this->pg_w-($this->num_space*2))*6/27, $height, "50%", 'LR', 0, 'C');
	               //  $this->Cell(($this->pg_w-($this->num_space*2))*6/27, $height, "5%", 'LR', 1, 'C');

	               //  $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
	               //  $this->Cell(($this->pg_w-($this->num_space*2))*3/27, $height, "3", 'LR', 0, 'C');
	               //  $this->Cell(($this->pg_w-($this->num_space*2))*6/27, $height, "60%", 'LR', 0, 'C');
	               //  $this->Cell(($this->pg_w-($this->num_space*2))*6/27, $height, "95%", 'LR', 0, 'C');
	               //  $this->Cell(($this->pg_w-($this->num_space*2))*6/27, $height, "40%", 'LR', 0, 'C');
	               //  $this->Cell(($this->pg_w-($this->num_space*2))*6/27, $height, "5%", 'LR', 1, 'C');

	               //  $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
	               //  $this->Cell(($this->pg_w-($this->num_space*2))*3/27, $height, "4", 'LR', 0, 'C');
	               //  $this->Cell(($this->pg_w-($this->num_space*2))*6/27, $height, "70%", 'LR', 0, 'C');
	               //  $this->Cell(($this->pg_w-($this->num_space*2))*6/27, $height, "95%", 'LR', 0, 'C');
	               //  $this->Cell(($this->pg_w-($this->num_space*2))*6/27, $height, "30%", 'LR', 0, 'C');
	               //  $this->Cell(($this->pg_w-($this->num_space*2))*6/27, $height, "5%", 'LR', 1, 'C');

	               //  $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
	               //  $this->Cell(($this->pg_w-($this->num_space*2))*3/27, $height, "5", 'LR', 0, 'C');
	               //  $this->Cell(($this->pg_w-($this->num_space*2))*6/27, $height, "90%", 'LR', 0, 'C');
	               //  $this->Cell(($this->pg_w-($this->num_space*2))*6/27, $height, "95%", 'LR', 0, 'C');
	               //  $this->Cell(($this->pg_w-($this->num_space*2))*6/27, $height, "10%", 'LR', 0, 'C');
	               //  $this->Cell(($this->pg_w-($this->num_space*2))*6/27, $height, "5%", 'LR', 1, 'C');
	               ...
	               //  $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
	               //  $this->Cell(($this->pg_w-($this->num_space*2))*3/27, $height, "6 dst", 'LRB', 0, 'C');
	               //  $this->Cell(($this->pg_w-($this->num_space*2))*6/27, $height, "100%", 'LRB', 0, 'C');
	               //  $this->Cell(($this->pg_w-($this->num_space*2))*6/27, $height, "95%", 'LRB', 0, 'C');
	               //  $this->Cell(($this->pg_w-($this->num_space*2))*6/27, $height, "0%", 'LRB', 0, 'C');
	               //  $this->Cell(($this->pg_w-($this->num_space*2))*6/27, $height, "5%", 'LRB', 1, 'C');
                // }


                $this->font_body();
                $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
                $this->Cell($this->pg_w-($this->num_space*2), $height, "II. Komposisi Premi Reguler dan Top-Up Premi Reguler :", 0, 1, 'L');
                $this->Ln(2);
                $this->font_body('B');
                $y = $this->GetY();
                $x1 = $this->GetX();
                $y1 = $this->GetY();
                $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
                $this->MultiCell(($this->pg_w-($this->num_space*2))*12/27, $height, "Persentase Premi Reguler dari Premi", 1, 'C');
                $y2 = $this->GetY();
                $yH = $y2 - $y1;
                $this->SetXY($x1 + ($this->pg_w-($this->num_space*2))*13.6/27, $y2 - $yH);
                $this->MultiCell(($this->pg_w-($this->num_space*2))*12/27, $height, "Persentase Top-Up Premi Reguler dari Premi", 1, 'C');

                $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
                $this->Cell(($this->pg_w-($this->num_space*2))*12/27, $height, "70%", 1, 0, 'C');
                $this->Cell(($this->pg_w-($this->num_space*2))*12/27, $height, "30%", 1, 1, 'C');
                 $this->Ln(10);


                $this->font_body();
                $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
                $this->Cell($this->pg_w-($this->num_space*2), $height, "III. Persentase alokasi Dana Investasi dan Biaya berdasarkan tahun Polis :", 0, 1, 'L');
                $this->Ln(2);
                $this->font_body('B');
            	$this->SetFillColor(255,255,255);
                $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
				// $x = $this->GetX();
				// $y = $this->GetY();
				// $this->Cell(($this->pg_w-($this->num_space*2))*5/25, $height*3, "Tahun", 1, 0, 'C', true);
				
				// $x1 = $this->GetX();
				// $y1 = $this->GetY();
				// $this->MultiCell(($this->pg_w-($this->num_space*2))*3/25, 5, "Persentase Dana Investasi dari Premi Reguler dan Top Up Premi Reguler", 1, 'C', true);
				$this->Cell(20, $height*4, "Tahun", 1, '0', "C", false);
				$x1 = $this->GetX();
				$y1 = $this->GetY();
	            $this->MultiCell(75, ($height*2)/2, "Persentase Dana Investasi dari Premi Reguler dan Top Up Premi Reguler", 1, '0', "C", true);
	            $y2 = $this->GetY();
				$yH = $y2 - $y1;
			
				$this->SetXY($x1 + ($this->pg_w-($this->num_space*2))*8.5/25, $y2 - $yH);
	            $this->MultiCell(75, ($height*2)/2, "Persentase Biaya dari Premi Reguler dan Top Up Premi Reguler", 1, '0', "C", true);
	            $this->Ln(0);
	            $this->Cell(30, $height, NULL, 0, 0, 'C');
	            $x1 = $this->GetX();
				$y1 = $this->GetY();
				$this->SetFont('Arial','B',7);
				$this->MultiCell(28, ($height*2), "Premi Reguler", 1, 'C', true);
				$y2 = $this->GetY();
				$yH = $y2 - $y1;
				
				$this->SetXY($x1 + ($this->pg_w-($this->num_space*2))*4/25, $y2 - $yH);
				$this->MultiCell(30.5, ($height*2), "Top Up Premi Reguler", 1, 'C', true);
				$this->SetXY($x1 + ($this->pg_w-($this->num_space*2))*8.5/25, $y2 - $yH);
				$this->MultiCell(37.2, ($height*2), "Premi Reguler", 1, 'C', true);
				$this->SetXY($x1 + ($this->pg_w-($this->num_space*2))*14/25, $y2 - $yH);
				$this->MultiCell(37.5, ($height*2), "Top Up Premi Reguler", 1, 'C', true);
				$this->Ln(0);
				$this->Cell(10, $height, NULL, 0, 0, 'C');
	            $x1 = $this->GetX();
				$y1 = $this->GetY();
				$this->SetFont('Arial','B',7);
				$this->MultiCell(20, ($height*2), "1", 1, 'C', true);
				$y2 = $this->GetY();
				$yH = $y2 - $y1;
				$this->SetXY($x1 + ($this->pg_w-($this->num_space*2))*2.9/25, $y2 - $yH);
				$this->MultiCell(27.5, ($height*2), "60%", 1, 'C', true);
				$this->SetXY($x1 + ($this->pg_w-($this->num_space*2))*7/25, $y2 - $yH);
				$this->MultiCell(30, ($height*2), "95%", 1, 'C', true);
				$this->SetXY($x1 + ($this->pg_w-($this->num_space*2))*11.4/25, $y2 - $yH);
				$this->MultiCell(37.3, ($height*2), "40%", 1, 'C', true);
				$this->SetXY($x1 + ($this->pg_w-($this->num_space*2))*16.9/25, $y2 - $yH);
				$this->MultiCell(37.6, ($height*2), "5%", 1, 'C', true);
				$this->Ln(0);
				$this->Cell(10, $height, NULL, 0, 0, 'C');
	            $x1 = $this->GetX();
				$y1 = $this->GetY();
				$this->SetFont('Arial','B',7);
				$this->MultiCell(20, ($height*2), "2", 1, 'C', true);
				$y2 = $this->GetY();
				$yH = $y2 - $y1;
				$this->SetXY($x1 + ($this->pg_w-($this->num_space*2))*2.9/25, $y2 - $yH);
				$this->MultiCell(27.5, ($height*2), "60%", 1, 'C', true);
				...
				$y1 = $this->GetY();
				$this->SetFont('Arial','B',7);
				$this->MultiCell(20, ($height*2), "3", 1, 'C', true);
				$y2 = $this->GetY();
				$yH = $y2 - $y1;
				$this->SetXY($x1 + ($this->pg_w-($this->num_space*2))*2.9/25, $y2 - $yH);
				$this->MultiCell(27.5, ($height*2), "60%", 1, 'C', true);
				...
				$y1 = $this->GetY();
				$this->SetFont('Arial','B',7);
				$this->MultiCell(20, ($height*2), "4", 1, 'C', true);
				$y2 = $this->GetY();
				$yH = $y2 - $y1;
				$this->SetXY($x1 + ($this->pg_w-($this->num_space*2))*2.9/25, $y2 - $yH);
				$this->MultiCell(27.5, ($height*2), "80%", 1, 'C', true);
				$this->SetXY($x1 + ($this->pg_w-($this->num_space*2))*7/25, $y2 - $yH);
				$this->MultiCell(30, ($height*2), "95%", 1, 'C', true);
				$this->SetXY($x1 + ($this->pg_w-($this->num_space*2))*11.4/25, $y2 - $yH);
				$this->MultiCell(37.3, ($height*2), "20%", 1, 'C', true);
				$this->SetXY($x1 + ($this->pg_w-($this->num_space*2))*16.9/25, $y2 - $yH);
				$this->MultiCell(37.6, ($height*2), "5%", 1, 'C', true);
				$this->Ln(0);
				$this->Cell(10, $height, NULL, 0, 0, 'C');
				...
				$y1 = $this->GetY();
				$this->SetFont('Arial','B',7);
				$this->MultiCell(20, ($height*2), "5", 1, 'C', true);
				$y2 = $this->GetY();
				$yH = $y2 - $y1;
				$this->SetXY($x1 + ($this->pg_w-($this->num_space*2))*2.9/25, $y2 - $yH);
				$this->MultiCell(27.5, ($height*2), "80%", 1, 'C', true);
				...
				$y1 = $this->GetY();
				$this->SetFont('Arial','B',7);
				$this->MultiCell(20, ($height*2), "6 dst", 1, 'C', true);
				$y2 = $this->GetY();
				$yH = $y2 - $y1;
				$this->SetXY($x1 + ($this->pg_w-($this->num_space*2))*2.9/25, $y2 - $yH);
				$this->MultiCell(27.5, ($height*2), "100%", 1, 'C', true);
				$this->SetXY($x1 + ($this->pg_w-($this->num_space*2))*7/25, $y2 - $yH);
				$this->MultiCell(30, ($height*2), "95%", 1, 'C', true);
				$this->SetXY($x1 + ($this->pg_w-($this->num_space*2))*11.4/25, $y2 - $yH);
				$this->MultiCell(37.3, ($height*2), "0%", 1, 'C', true);
				$this->SetXY($x1 + ($this->pg_w-($this->num_space*2))*16.9/25, $y2 - $yH);
				$this->MultiCell(37.6, ($height*2), "5%", 1, 'C', true);
                ...
                $this->custom_font(7,false);
                $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
                //$this->Cell($this->pg_w-($this->num_space*2), $height, "Biaya Pembelian Unit (Unit Purchase Cost) sebesar 5% (lima persen) dari Dana Investasi", 0, 1, 'L');
            }
			// JS Pro Idaman
			else {
                $this->font_body('B');
                ...
                $this->font_body('B');
                $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
                $this->Cell($this->pg_w-($this->num_space*2), $height, "Persentase Alokasi Dana", 0, 1, 'L');
                $this->Ln(2);
                ...
                $x1 = $this->GetX();
                $y1 = $this->GetY();
                $this->MultiCell(($this->pg_w-($this->num_space*2))*12/24, $height, "Persentase Dana Investasi dari Premi Single dan Top Up Single", 1, 'C');
                $y2 = $this->GetY();
                $yH = $y2 - $y1;
                $this->SetXY($x1 + ($this->pg_w-($this->num_space*2))*12/24, $y2 - $yH);
                $this->MultiCell(($this->pg_w-($this->num_space*2))*12/24, $height, "Persentase Biaya Akuisisi dari Premi Single dan Biaya Top Up dari Top Up Single", 1, 'C');

                $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
                $this->Cell(($this->pg_w-($this->num_space*2))*6/24, $height, "Premi Single", 'LRB', 0, 'C');
                $this->Cell(($this->pg_w-($this->num_space*2))*6/24, $height, "Top Up Single", 'LRB', 0, 'C');
                $this->Cell(($this->pg_w-($this->num_space*2))*6/24, $height, "Premi Single", 'LRB', 0, 'C');
                $this->Cell(($this->pg_w-($this->num_space*2))*6/24, $height, "Top Up Single", 'LRB', 1, 'C');

                $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
                $this->Cell(($this->pg_w-($this->num_space*2))*6/24, $height, "95%", 'LRB', 0, 'C');
                $this->Cell(($this->pg_w-($this->num_space*2))*6/24, $height, "95%", 'LRB', 0, 'C');
                $this->Cell(($this->pg_w-($this->num_space*2))*6/24, $height, "5%", 'LRB', 0, 'C');
                $this->Cell(($this->pg_w-($this->num_space*2))*6/24, $height, "5%", 'LRB', 1, 'C');
		...
		/*====================================
			...
			Date : 03-10-2022
			Noted : Tambahan SEOJK, JL4BPRO
		=====================================*/
		public function LampiranPolis2(){
			$DB	  	= new Database($this->user_id, $this->password, $this->db);
			$D2	  	= new Database($this->user_id, $this->password, $this->db);
			$D3	  	= new Database($this->user_id, $this->password, $this->db);
			$DBx 	= new Database($this->user_id, $this->password, $this->db);
			$noper 	= $this->prefix.$this->nopertanggungan;
			$this->AddPage();
            ...
            $this->font_body('B');
            $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
            $this->Cell(($this->pg_w-($this->num_space*2))*3/27, $height, "Usia", 1, 0, 'C');
            $this->Cell(($this->pg_w-($this->num_space*2))*6/24, $height, "Biaya Asuransi per Bulan", 1, 0, 'C');
            $this->Cell(($this->pg_w-($this->num_space*2))*6/24, $height, "", 0, 0, 'C');
            $this->Cell(($this->pg_w-($this->num_space*2))*3/27, $height, "Usia", 1, 0, 'C');
            $this->Cell(($this->pg_w-($this->num_space*2))*6/24, $height, "Biaya Asuransi per Bulan", 1, 0, 'C');

        	/**======================================== 
        		Untuk mencari biaya COI + rider rider
        		created by : rizal Jihadudin 0912022
        	===========================================**/
    		$sql = "SELECT A.PREFIXPERTANGGUNGAN,
					...
					A.KDPRODUK,
					(SELECT NAMAPRODUK FROM TABEL_202_PRODUK WHERE KDPRODUK = A.KDPRODUK) NAMA_PRODUK,
					A.KDSTATUSFILE,
					(SELECT NAMASTATUSFILE FROM TABEL_299_STATUS_FILE WHERE KDSTATUSFILE = A.KDSTATUSFILE) STATUS_POLIS,
					TO_CHAR(A.MULAS, 'DD/MM/YYYY') MULAS,
					A.JUAMAINPRODUK,
					A.KDCARABAYAR,
					(SELECT NAMACARABAYAR FROM TABEL_305_CARA_BAYAR WHERE KDCARABAYAR = A.KDCARABAYAR) CARABAYAR,
					A.PREMI1,
					A.PREMI2,
					A.PREMISTD,
					A.NOPEMEGANGPOLIS,
					...
					(SELECT NOTASI FROM TABEL_304_VALUTA WHERE KDVALUTA = A.KDVALUTA) NOTASI,
					A.USIA_TH, 
					A.USIA_BL,
					A.LAMAASURANSI_TH,
                	...
                	LEFT JOIN TABEL_100_KLIEN B ON B.NOKLIEN = A.NOPEMEGANGPOLIS
                	LEFT JOIN TABEL_100_KLIEN C ON C.NOKLIEN = A.NOTERTANGGUNG
					WHERE A.KDPRODUK LIKE 'JL4%'
					AND (A.NOPOLBARU = '$noper' OR A.PREFIXPERTANGGUNGAN||A.NOPERTANGGUNGAN = '$noper')
				...
				$produk1 = array('CP19100RWI', 'CP19200RWI', 'CP19300RWI', 'CP19400RWI', 'CP19500RWI', 'CP19600RWI', 'CP19700RWI', 'CP19800RWI', 'CP19900RWI', 'CP191000RWI');
				$produk2 = array('PBD', 'PBCI', 'PTPD', 'SPBD', 'SPBCI', 'SPTPD', 'PBTPD');

				while($usia <= 99)
				{

					foreach($arr_b as $row){
						if($row["KDBENEFIT"] == 'WPCI51'){
							$kdtarif = 'WPCI';
							$pembagi = 100;
							$filterjnsklm = "";
							$filterusia = $usia;

						}else if($row["KDBENEFIT"] == 'WPTPD'){
							$kdtarif = 'WAIVER';
							$pembagi = 100;
							$filterjnsklm = "";
							$filterusia = $usia;

						}else if(in_array($row["KDBENEFIT"], $produk1)){
							$kdtarif = str_replace('RWI', '', $row["KDBENEFIT"]);


							$filterusia = $usia;

						}else if(in_array($row["KDBENEFIT"], $produk2)){
							if($row["KDBENEFIT"] == 'PBTPD'){
								$kdtarif = 'PTPD';
							}else{
								$kdtarif = $row["KDBENEFIT"];
							}
						...
						$kenaikan_basiscoicor = (($biaya_dasar * 1000 * 12 / $row["NILAIBENEFIT"]) / 1000) * ($row["NILAIBENEFIT"] * ($row["NILAI_BASISCOICOR"] / 100)) / 12;
						$kenaikan_basisjua = $row["NILAIBENEFIT"] * $row["NILAI_BASISJUA"] / 100 / 12;
						$kenaikan_basishobi = $row["NILAIBENEFIT"] * $row["NILAI_BASISHOBI"] / 100 / 12;
						$total_nilai = $biaya_dasar + $kenaikan_basiscoicor + $kenaikan_basisjua + $kenaikan_basishobi;
						$allTotal += $total_nilai;
					}
		            ...
		            $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
		            $this->Cell(($this->pg_w-($this->num_space*2))*3/27, $height, $usia, 1, 0, 'C');
		            $this->Cell(($this->pg_w-($this->num_space*2))*6/24, $height, number_format($allTotal,2,',','.'), 1, 0, 'C');
		            $this->Cell(($this->pg_w-($this->num_space*2))*6/24, $height, "", 0, 0, 'C');
		            $allTotal = 0;
		            $usia++;
				}
