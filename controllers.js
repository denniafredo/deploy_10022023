		...
		$scope.validateThisFormOnPageAccept = function () {
			...
			try {
				...
				if($scope.data.rangePendapatanPempol != '0'){
					...
					if($scope.data.sumberPendapatanPempolLainnya == '' || $scope.data.sumberPendapatanPempolLainnya == null){
						$scope.messages.push({
...
.controller('produkDanManfaat12Ctrl', ['$state', '$scope', '$stateParams', 'spajProvider', 'dataFactory', '$store', '$ionicPopup',
	function ($state, $scope, $stateParams, spajProvider, dataFactory, $store, $ionicPopup) {
		...
		$scope.init_data = function () {
			...
			let masaBayarPremHitung = ((prospek.kd_produk == 'JL4BLN' || prospek.kd_produk == 'JL4BPRO' || prospek.kd_produk == 'JL4BLN_' || prospek.kd_produk == 'JL4XN' || prospek.kd_produk == 'JL4SMR') ? "99" : (prospek.kd_produk == 'JSDMPPN')? "10":"5") - prospek.usiactt;
			let jangkaWaktuAsuransiHitung = ((prospek.kd_produk == 'JL4BLN' || prospek.kd_produk == 'JL4BPRO' || prospek.kd_produk == 'JL4BLN_' || prospek.kd_produk == 'JL4XN' || prospek.kd_produk == 'JL4SMR') ? "99" : (prospek.kd_produk == 'JSDMPPN')? "10":"5") - prospek.usiactt;
...
.controller('lembarPersetujuanCtrl', ['$scope', '$stateParams', 'spajProvider','$ionicPopup', 'dataFactory', '$store', '$state',
	function ($scope, $stateParams, spajProvider, $ionicPopup, dataFactory, $store, $state) {
		...
		/** UAT : 08-02-2023 **/
		$scope.dataTextProduk = null;
		$scope.dataTextProduk1 = null;
		...
		if(prospek.kd_produk == 'JL4BPRO'){
			$scope.dataTextProduk  = 'IFG LIFE PROTECTION PLATINUM';
			$scope.dataTextProduk1 = 'IFG Life Protection Platinum';
			$scope.isProdukLpp = true;
		}else{
			$scope.dataTextProduk  = 'IFG LIFE PRIME PROTECTION DAN IFG ULTIMATE PROTECTION';
			$scope.dataTextProduk1 = 'IFG Life Prime Protection & IFG Life Ultimate Protection';
			$scope.isProdukLpp = false;
		}

		/*** tambahan rizal 10/11/2022 **/
		let wp = false;
		if(prospek.kd_produk == 'JL4BPRO'){
			if(prospek.IS_CI || prospek.ispci || prospek.IS_WAIVER_CI || prospek.IS_HCP_MURNI || prospek.issci == true){
				wp = true;
			}
		}
		/*** end tambahan rizal **/
		$scope.isWaitingPeriodeX = wp;
		
		$scope.data = {
			// 'isWaitingPeriodeX': wp,
		...
		// $scope.init_data = function () {
		// }
		console.log('scope data disini :', $scope.isWaitingPeriodeX)
...
.controller('tambahPenerimaManfaatCtrl', ['$state', '$scope', '$stateParams', 'dataFactory', 'spajProvider', '$store', '$ionicPopup',
	function ($state, $scope, $stateParams, dataFactory, spajProvider, $store, $ionicPopup) {
		...
		$scope.saveDataSpaj = function () {
			...
			let datex = $scope.formatDate($scope.data.tglLahirPenerimaManfaat);

			$scope.newPenerimaManfaat = {
				'namaPenerimaManfaat': $scope.data.namaPenerimaManfaat,
				'tglLahirPenerimaManfaat': new Date(datex).toISOString(),
...
.controller('sKKTertanggungCtrl', ['$state', '$scope', '$stateParams', '$store', 'spajProvider', 'dataFactory', '$ionicPopup',
	function ($state, $scope, $stateParams, $store, spajProvider, dataFactory, $ionicPopup) {
		...
		$scope.init_data = function () {
			...
			// console.log('test disini buat skk bang');
			// console.log('critical illnes 53', prospek.IS_CI);
			// console.log('Payor benefit critical illness 51', prospek.ispci);
			// console.log('IFG Spouse Payor Critical iLLness 51', false);
			// console.log('IFG Waiver Premium Critical illness 51', prospek.IS_WAIVER_CI);
			// console.log('IFG Hospital Cash Plan', prospek.IS_HCP_MURNI)

			let wp = false;

			/** tambahan 16022023 waiting periode LPP**/
			if(prospek.kd_produk == 'JL4BPRO'){
				if(prospek.IS_CI || prospek.ispci || prospek.IS_WAIVER_CI || prospek.IS_HCP_MURNI || prospek.issci == true){
					wp = true;
				}
			...
			if (response.data[0] === null) {
				...
				for (i = 0; i < response.data.length; i++) {
					...
					$tdata.push({
						...
						'total_dokumen' : response.data[i].TOTAL_DOKUMEN,
