...
// echo $sql;
 ...
 if ($_GET['action'] == 'hapus_spaj') {
 $sql = "update $DBUser.tabel_200_pertanggungan set kdstatusfile='X', ".
 			  "userupdated=user,tglupdated=sysdate ".
 			  "where prefixpertanggungan='$prefixpertanggungan' and nopertanggungan='$noper' ".
				"and kdstatusfile='1' and kdpertanggungan='1'";
// echo $sql;
 echo 'disini untuk ngapus SPAJ, nunggu Fahmi aja';
 //$DB->parse($sql);
 //$DB->execute();
...
echo "<p align=\"center\">";
echo "<font face=\"verdana\" size=\"3\"><b>NO. PROPOSAL : ".($PER->nopolbaru ? $PER->nopolbaru : $prefixpertanggungan." - ".$nopertanggungan)."</b></font>";
?>
<div align="center">
...
<table border="0" cellpadding="0" cellspacing="1" width="100%" class="arial10">
  <tr>
    <td class="tblhead" colspan="6" align="center">PROPOSAL <?=$PER->suspend200 ? '(TERTUNDA)' : null;?></td>
  </tr>
  <tr>
  ...
  <?php
    $sql = "SELECT BUILDID FROM $DBUser.TABEL_SPAJ_ONLINE WHERE NOSPAJ = '".$PER->nosp."'";
    $DB->parse($sql);
    $DB->execute();
    $arrb=$DB->nextrow();
  ?>
  <tr>
    <td class="verdana8blk">Nomor Build ID</td>
    <td class="verdana8blk">:  <? echo $arrb["BUILDID"]; ?></td>
  ...
  <tr>
    ...
    <td class="verdana8blk"><b>:  <? echo $PER->statusfile.($PER->suspend200 ? ' (TERTUNDA)' : null); ?>
	<?php
	if ($PER->statusfile == 'DELETE' && $_SESSION['kantor'] == 'KP'){
		echo "<a href='peliharaprop.php?nopertanggungan=002370237&prefixpertanggungan=MG&action=hapus_spaj'>Hapus SPAJ</a>";
	}
	?>
	</td>
    <td class="verdana8blk">Taltup</td>
    <td class="verdana8blk"><b>:  <? echo $PER->taltup; ?></td>
  </tr>
  <tr>
  	
    <!-- Tambahan untuk memunculkan status approval - Teguh (08/11/2019) -->
    <?php
      $sql="SELECT
            CASE
             ...
             WHEN KDJENISAPPROVAL = 'X' THEN 'Decline'
            END STATUSAPPROVAL
            FROM $DBUser.TABEL_214_APPROVAL_PROPOSAL
      ...
      $arvket=$DB->nextrow();
    ?>
    <td class="verdana8blk">Status Approval</td>
    <td class="verdana8blk">: <? echo $arvket["STATUSAPPROVAL"]; ?></td>
  ...
  <tr>
    ...
    <td class="verdana8blk">:  <? echo $PER->namatertanggung; ?></td>
	  ...
	  <? 
	$sql="SELECT a.namaklien1, 
          a.namaklien2, 
          b.namapekerjaan, 
          c.namahobby, 
          a.gelar,
          f.namaagama,
          NVL(NO_PONSEL, nvl(PHONETETAP02,nvl(PHONETAGIH02,nvl(PHONETETAP01,PHONETAGIH02)))) phone,
          to_char(a.tgllahir,'DD/MM/YYYY') tgllahir, 
          decode(a.jeniskelamin,'P','PEREMPUAN','L','LAKI-LAKI') jeniskelamin,
          a.kdid||'-'||a.noid identitas, 
          a.tinggibadan,
          a.beratbadan,
          alamattetap01,
          alamattetap02, 
          alamattagih01, 
          alamattagih02,
          (SELECT UPPER(NAMAKELURAHAN) FROM $DBUser.TABEL_111_KELURAHAN WHERE KDKELURAHAN = A.KDKELURAHANTAGIH)NAMAKELURAHAN_TAGIH,
          (SELECT 
                UPPER(NAMAKECAMATAN) 
            FROM  $DBUser.TABEL_110_KECAMATAN 
            WHERE KDKECAMATAN = (SELECT KDKECAMATAN FROM $DBUser.TABEL_111_KELURAHAN WHERE KDKELURAHAN = A.KDKELURAHANTAGIH)
          )NAMAKECAMATAN_TAGIH,
          (SELECT namakotamadya FROM $DBUser.tabel_109_kotamadya WHERE kdkotamadya = a.kdkotamadyatagih) namakotamadya_tagih,
          (SELECT namapropinsi FROM $DBUser.tabel_108_propinsi WHERE kdpropinsi = a.kdpropinsitagih) namapropinsi_tagih,
          a.kodepostagih,
          a.phonetetap01,
          a.phonetagih01,
          d.namakotamadya,
          e.namapropinsi,
          decode(a.meritalstatus,'L','LAJANG','K','KAWIN','J','JANDA','D','DUDA') merital 
        FROM $DBUser.tabel_100_klien a, 
          $DBUser.tabel_105_pekerjaan b, 
          $DBUser.tabel_114_hobby c, 
          $DBUser.tabel_109_kotamadya d, 
          $DBUser.tabel_108_propinsi e,
          $DBUser.tabel_102_agama f 
        WHERE a.kdpekerjaan=b.kdpekerjaan(+) 
          and a.kdhobby=c.kdhobby(+) 
          and a.kdagama=f.kdagama(+)
          and a.kdpropinsitetap=e.kdpropinsi(+) 
          and a.kdkotamadyatetap=d.kdkotamadya(+) 
          and a.noklien='$PER->notertanggung' ";
	//echo $sql;
	$DB->parse($sql);
	$DB->execute();
	$arv=$DB->nextrow();
  ...
  <tr>
    ...
    <td class="verdana8blk" colspan="3">:  <? echo $arv["ALAMATTAGIH02"]." ".$arv["NAMAKELURAHAN_TAGIH"]." ".$arv["NAMAKECAMATAN_TAGIH"]." ".$arv["NAMAKOTAMADYA_TAGIH"]." ".$arv["NAMAPROPINSI_TAGIH"]." ".$arv["KODEPOSTAGIH"]; ?></td>
  </tr> 
	<tr>
	...
	<tr>
    <td class="verdana8blk">No. HP</td>
    <td class="verdana8blk">:  <? echo $arv["PHONE"]; ?></td>
    <td class="verdana8blk"></td>
    <td class="verdana8blk"></td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td class="verdana8blk">:  <? echo $PER->produk; ?> <? echo $PER->namaproduk; ?></td>
	...
	$avc=$DB->nextrow(); 
     ...
     <td class="verdana8blk">Virtual Account BNI</td><? } ?>
    <td class="verdana8blk">:  
  ...
  <tr>
    <td class="verdana8blk">Alamat Email</td>
    <td class="verdana8blk">:  <? echo $PER->emailpemegangpolis; ?></td>
    <td class="verdana8blk"></td>
    <td class="verdana8blk"></td>
  </tr> 
    ...
    <td colspan="4" align="center">
		<? printf("<input type=\"button\" name=\"benefit\" value=\"Benefit\" onclick=\"NewWindow('showbenefit1.php?prefixpertanggungan=%s&nopertanggungan=%s&kdproduk=%s','popupbenefit',760,400,1);\">",$prefixpertanggungan,$nopertanggungan,$PER->produk); 
		   $nottemp=true;
			...
			/*
			*
			* Date		  : 21 Agustus 2014
			* Enhanced By : Iie Sumitra
			* Hal		  : Penambahan tombol HIST PREMI dan HIST RIDER supaya BO bisa check untuk pembayaran H2H 
			*
			*/
			printf("<input type=\"button\" name=\"tariftebus\" value=\"Hist Premi\" onclick=\"NewWindow('../akunting/kartupremi1.php?prefix=%s&noper=%s','popuptebus','800','600','yes');return false\">",$prefixpertanggungan,$nopertanggungan); 
			printf("<input type=\"button\" name=\"tariftebus\" value=\"Hist Rider\" onclick=\"NewWindow('../akunting/karturider1.php?prefix=%s&noper=%s','popuptebus','800','600','yes');return false\">",$prefixpertanggungan,$nopertanggungan); 

			/*
			*
			* End Of Enhanced Iie Sumitra | HIST PREMI & HIST RIDER
			*
			*/
		  ...
		  $polisbaru_smart="$prefixpertanggungan-$nopertanggungan"; 
			printf("<input type=\"button\" name=\"docpolis\" value=\"Dokumen\" onclick=\"NewWindow('https://sae-aws.ifg-life.id/smart_ifglife/list.php?no_polis1=".base64_encode(base64_encode($polisbaru_smart))."','popupkomisi','700','400','yes');return false\" style=\"font-size: 10pt\">");
			printf("<input type=\"button\" name=\"docList\" value=\"DOKUMEN PERNYATAAN\" onclick=\"NewWindow('documentpernyataan.php?no_polis=".base64_encode(base64_encode(@$PER->nopolbaru ? $PER->nopolbaru : $PER->nopol))."&nopertanggungan=".base64_encode(base64_encode($nopertanggungan))."&prefix=".base64_encode(base64_encode($prefixpertanggungan))."','popupkomisi','700','400','yes');return false\" style=\"font-size: 10pt\">");	
		...
		<?
         ...
         printf("<input type=\"button\" name=\"benefit\" value=\"Hist UW\" onclick=\"NewWindow('showhistunderwriting.php?prefixpertanggungan=%s&nopertanggungan=%s','popupbenefit',1200,400,1);\">",$prefixpertanggungan,$nopertanggungan);
		?>
		</td>
  </tr>
</table>
