...
<?
	if ($submit=="SUBMIT") {
		...
		//$DB->parse($sql);
		 ...
		 $sqlno="select max(nopkajagen) nopkajagen from $DBUser.tabel_400_pkaj_agen";
		 $DB->parse($sqlno);
		 $DB->execute();
		 $rowno=$DB->nextrow();
         $nomerx= $rowno["NOPKAJAGEN"];
		 
		 $sqlinsert="insert into $DBUser.HISTORY_LOG_PKAJ (ACTIVITY, USR, ID_USER, DOCUMENT_NO, DOCUMENT_TYPE, DATETIME, REMARK, STATUS)
		 VALUES ('CREATED DOCUMENT ePKAJ','$namusr','$noidagen',$nomerx,'PDF',localtimestamp,'TESTINGxx','Created')";
