<?
  ...
  include "./includes/formula44-lpp.php";
  include "./includes/klien-lpp.php";
	...
	function GetFormula($DBX,$kdrumus) {
	  $sql="select rumus from $DBUser.tabel_224_rumus where kdrumus='$kdrumus'"; 
	  //echo $sql."<br><br>";
   	$DBX->parse($sql);
    $DBX->execute();
    $arr=$DBX->nextrow();
            ...
            //		   		 "values ('$prefixpertanggungan','$nopertanggungan','$kdproduk','$kdbenefit',$periodebayar,$periodebenefit,'R')";	 	
						...
						if(($vbenefit>$maxuarider) && ($kdbenefit=="ADDB" || $kdbenefit=="TI" || $kdbenefit=="TPD" || $kdbenefit=="PBTPD" || $kdbenefit=="SPTPD" || $kdbenefit=="PBD" || $kdbenefit=="SPBD" || $kdbenefit=="CI53" || $kdbenefit=="PBCI" || $kdbenefit=="SPBCI" )){
							...
							if (in_array($kdbenefit, array('PBTPD','SPTPD','PBD','SPBD','WPCI51','WPTPD','CI53','TI','ADDB','TPD','ADB','PBCI','SPBCI'))) {
								...
								/** tambahan 22/02/2023 */
								if($kdproduk == 'JL4BPRO'){
									$filterusia = 'AND d.usia = '.$usia;
								}else{
									$filterusia = 'AND d.usia = floor(months_between(mulas,b.tgllahir)/12)';
								}
								/** end */

								// $cekkls="SELECT   noklien,to_char(b.tgllahir,'dd/mm/yyyy') tgllahir,floor(months_between(mulas,b.tgllahir)/12) usia_th, trunc(months_between(sysdate,b.tgllahir) - (trunc(months_between(sysdate,b.tgllahir) / 12) * 12)) as usia_month,b.merokok,((".$vbenefit." / 1000) * tarif) / 12 as premirider FROM   $DBUser.tabel_200_temp a,".
								...
								$cekkls="SELECT noklien,to_char(b.tgllahir,'dd/mm/yyyy') tgllahir,floor(months_between(mulas,b.tgllahir)/12) usia_th, trunc(months_between(sysdate,b.tgllahir) - (trunc(months_between(sysdate,b.tgllahir) / 12) * 12)) as usia_month,b.merokok,((".$vbenefit." / 1000) * tarif) / 12 as premirider FROM   $DBUser.tabel_200_temp a,".
											...
											$queryklien.
											" A.PREFIXPERTANGGUNGAN = '$prefixpertanggungan' AND a.nopertanggungan = '$nopertanggungan' AND d.kdproduk = a.kdproduk ".
											"AND d.usia = (CASE WHEN TRUNC ( MONTHS_BETWEEN (SYSDATE, b.tgllahir) - (TRUNC (MONTHS_BETWEEN (SYSDATE, b.tgllahir) / 12) * 12)) >= 6 THEN  FLOOR (MONTHS_BETWEEN (mulas, b.tgllahir) / 12) + 1 ELSE  FLOOR (MONTHS_BETWEEN (mulas, b.tgllahir) / 12) END) AND kdtarif='$kdbenefitnya' AND d.kdproduk = e.kdproduk  and e.kdbasispremi=d.kdbasis";
								/** END **/

								/** Perubahan 22/02/2023 */
								// $cekkls="SELECT   noklien,to_char(b.tgllahir,'dd/mm/yyyy') tgllahir,floor(months_between(mulas,b.tgllahir)/12) usia_th, trunc(months_between(sysdate,b.tgllahir) - (trunc(months_between(sysdate,b.tgllahir) / 12) * 12)) as usia_month,b.merokok,((".$vbenefit." / 1000) * tarif) / 12 as premirider FROM   $DBUser.tabel_200_temp a,".
								// 			//"$DBUser.tabel_100_klien b,$DBUser.tabel_205_tarip_premirider d WHERE ".
								// 			"$DBUser.tabel_100_klien b,$DBUser.tabel_205_tarip_premi d ,$DBUser.TABEL_246_PRODUK_BASIS e WHERE ".
								// 			//"a.nopemegangpolis = b.noklien ".
								// 			$queryklien.
								// 			" A.PREFIXPERTANGGUNGAN = '$prefixpertanggungan' AND a.nopertanggungan = '$nopertanggungan' AND d.kdproduk = a.kdproduk ".
								// 			$filterusia." AND kdtarif='$kdbenefitnya' AND d.kdproduk = e.kdproduk  and e.kdbasispremi=d.kdbasis";
							...
							// $cekkls="SELECT noklien,to_char(b.tgllahir,'dd/mm/yyyy') tgllahir,usia_th,  trunc(months_between(sysdate,b.tgllahir) -
    			// 								(trunc(months_between(sysdate,b.tgllahir) / 12) * 12)) as usia_month,b.merokok,((".$vbenefit." / 1000) * tarif) / 12 as premirider FROM   $DBUser.tabel_200_temp a,".
							// 			//"$DBUser.tabel_100_klien b,$DBUser.tabel_205_tarip_premirider d WHERE ".
							// 			"$DBUser.tabel_100_klien b,$DBUser.tabel_205_tarip_premi d ,$DBUser.TABEL_246_PRODUK_BASIS e WHERE ".
							...
							$cekkls="SELECT noklien,to_char(b.tgllahir,'dd/mm/yyyy') tgllahir,usia_th,  trunc(months_between(sysdate,b.tgllahir) -
										...
										"a.notertanggung = b.noklien ".
										"AND A.PREFIXPERTANGGUNGAN = '$prefixpertanggungan' AND a.nopertanggungan = '$nopertanggungan' AND d.kdproduk = a.kdproduk ".
										"AND d.usia = (CASE WHEN TRUNC ( MONTHS_BETWEEN (SYSDATE, b.tgllahir) - (TRUNC (MONTHS_BETWEEN (SYSDATE, b.tgllahir) / 12) * 12)) >= 6 THEN  FLOOR (MONTHS_BETWEEN (mulas, b.tgllahir) / 12) + 1 ELSE  FLOOR (MONTHS_BETWEEN (mulas, b.tgllahir) / 12) END) AND kdtarif='$kdbenefit' AND d.kdproduk = e.kdproduk  and e.kdbasispremi=d.kdbasis";
    			...
    			// 					(trunc(months_between(sysdate,b.tgllahir) / 12) * 12)) as usia_month,b.merokok,((".$vbenefit." / 1000) * tarif) / 12 as premirider FROM   $DBUser.tabel_200_temp a,".
							...
							// 			"a.notertanggung = b.noklien ".
							// 			"AND A.PREFIXPERTANGGUNGAN = '$prefixpertanggungan' AND a.nopertanggungan = '$nopertanggungan' AND d.kdproduk = a.kdproduk ".
							// 			$filterusia." AND kdtarif='$kdbenefit' AND d.kdproduk = e.kdproduk  and e.kdbasispremi=d.kdbasis";
							...
							// var_dump($cekkls);die;
								...
								/** UAT 20022023 **/
								// if($kdproduk == 'JL4BPRO'){
								// 	if((int)$arrcekkls["USIA_MONTH"] >= 6){
								// 		(int)$arrcekkls["USIA_TH"] += 1;
								// 	}
								// }
								/** END OF ERA */
							...
							if($kdbenefit=="PBTPD" || $kdbenefit=="SPTPD" || $kdbenefit=="PBD" || $kdbenefit=="SPBD" || $kdbenefit=="WPTPD" || $kdbenefit=="WPCI51" || $kdbenefit=="PBCI" || $kdbenefit=="SPBCI"){
								...
								/** UAT 23022023 **/
									...
									$cekkls="SELECT   usia_th,b.merokok,(((".$vbenefit." / 1000) * tarif) / $faktorcb) * extrapremium  as extra FROM   $DBUser.tabel_200_temp a,".
										...
										"b.kdpekerjaan = c.kdpekerjaan AND a.nopemegangpolis = b.noklien ".
										"AND A.PREFIXPERTANGGUNGAN = '$prefixpertanggungan' AND a.nopertanggungan = '$nopertanggungan' AND d.kdproduk = a.kdproduk ".
										"AND d.usia = (CASE WHEN a.USIA_BL >= 6 THEN  a.USIA_TH + 1 ELSE  a.USIA_TH END) AND kdtarif='$kdbenefit'";
								/** END **/
								...
								/** UAT 23022023 **/
									$cekkls="SELECT   usia_th,b.merokok,round((((".$vbenefit." / 1000) * tarif) * extrapremium )/ 12,2)  as extra FROM   $DBUser.tabel_200_temp a,".
										...
										"b.kdpekerjaan = c.kdpekerjaan AND a.notertanggung = b.noklien ".
										"AND A.PREFIXPERTANGGUNGAN = '$prefixpertanggungan' AND a.nopertanggungan = '$nopertanggungan' AND d.kdproduk = a.kdproduk ".
										"AND d.usia = (CASE WHEN a.USIA_BL >= 6 THEN  a.USIA_TH + 1 ELSE  a.USIA_TH END) AND kdtarif='$kdbenefit' and NVL(b.merokok,'0')";
								/** END **/
...
*/     
  	...
  	//============================================================================
		...
		switch ($premijua) {
		  case 'jua' : {
			  header("location:benefit1-lpp.php?vara=$vara&premijua=$premijua&prefixpertanggungan=$prefixpertanggungan&nopertanggungan=$nopertanggungan&noproposal=$noproposal&kdper=$kdper");//asli propmtc14
			} break;
			case 'premi' : {
			  ...
			  header("location:benefit2-lpp.php?vara=$vara&premijua=$premijua&prefixpertanggungan=$prefixpertanggungan&nopertanggungan=$nopertanggungan&noproposal=$noproposal&kdper=$kdper");//asli propmtc14
			} break;
		}				
		exit;
