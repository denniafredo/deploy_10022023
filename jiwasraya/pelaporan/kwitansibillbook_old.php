...
?>
  ...
  ?>
	...
	<!--
	<td width="20%"><font face="Verdana" size="2">Masukkan Nomor Penagih</font></td>
  <td width="20%"><input type="text" class="a" name="nopenagih" size="10" maxlength="10" readonly>
  ...
  <td>
	...
	$sqla="select ".
       ...
       "where ".
   	 			...
   	 			"(e.kdrayonpenagih in (select kdkantor from $DBUser.tabel_001_kantor start with kdkantor='IF' connect by prior kdkantor=kdkantorinduk)) AND  ".
...
<script language="javascript">
  ...
  var halcari=document.clntmtc.carihal.value;
	var xxx = parseInt(document.clntmtc.carihal.value);
	  nextpg = xxx + 10;
		if((xxx < 11)||(halcari =='')) {
		  window.location.replace('kwitansibillbook.php?action=list&maju=ok&lastpg=11&get='+halcari+'&thisperiode=$thisperiode');
    }else {
		  window.location.replace('kwitansibillbook.php?action=list&maju=ok&lastpg='+nextpg+'&get='+halcari+'&thisperiode=$thisperiode');
		}
	}
</script>
<table>
...
if ($mundur){
   $a = $xx + 10;
   $url = $PHP_SELF . "?action=list&mundur=ok&lastpg=$a&get=$i&users=$users&thisperiode=$thisperiode"; 
} else {
   $a = $total_pages;
   $url = $PHP_SELF . "?action=list&maju=ok&lastpg=$a&get=$i&users=$users&thisperiode=$thisperiode"; 
}
	if ($get==$i) {
...
$stop = $xx - 10;
//echo $stop."<br>";
$urln = $PHP_SELF . "?action=list&maju=ok&lastpg=$xx&get=$linkm&users=$users&thisperiode=$thisperiode";
if($stop > $pg){ 
	     ...
	     "where k='$get'";
			    ...
			    //echo $sql;
					...
					$DB->execute();
							...
							echo "<br><a class=\"verdana10blk\">Billing Booking bulan $bln $vthn Kantor $kantor</a>";
							echo "<table>";
					    echo "<tr>";
