 ...
 WHERE   a.nopenagih=p.nopenagih
		...
		//"AND a.kdvaluta='$kdvaluta' ".
		 ...
		 ";
			...
			// echo $sql."<br/>";
?><br>
<a class="myButton" href="#" onClick="window.open('./cetakjatuhtempoexpirasiall.php?tglcari=<?=$tglcari;?>&kdvaluta=<?=$kdvaluta;?>&carabayar=<?=$carabayar;?>&kdkantor=<?=$kdkantor;?>&prefix=<?=$arr["PREFIXPERTANGGUNGAN"]."-".$arr["NOPERTANGGUNGAN"];?>','','width=800,height=600,top=100,left=100,scrollbars=yes');">
		CETAK JATUH TEMPO EXPIRASI PREMI</a>&nbsp;<a class="myButton" href="#" onClick="window.open('./jatuh_tempo_expirasi_dl.php?tglcari=<?=$tglcari;?>&kdvaluta=<?=$kdvaluta;?>&carabayar=<?=$carabayar;?>&kdkantor=<?=$kdkantor;?>&prefix=<?=$arr["PREFIXPERTANGGUNGAN"]."-".$arr["NOPERTANGGUNGAN"];?>','','width=800,height=600,top=100,left=100,scrollbars=yes');">
		DOWNLOAD JATUH TEMPO EXPIRASI PREMI</a><br>
...
<table border="0" cellspacing="1" style="border-collapse: collapse" width="100%" id="table1" cellpadding="4" bordercolor="#B3CFFF">
	...
	?>
        ...
        <td><?=$row["TELP"];?></td>
        <td><?=$row["KOTA"];?></td>
        <td><?="<a href=\"javascript:NewWindow('../pelaporan/cetakjatuhtempo_expirasi.php?prefixpertanggungan=".$row["PREFIXPERTANGGUNGAN"]."&nopertanggungan=".$row["NOPERTANGGUNGAN"]."&tglexp=".$row["TGLEXP"]."','polisinfo',815,600,'yes');\">cetak</a>";?> |
			<?="<a href=\"javascript:NewWindow('../pelaporan/cetak_pdf_jatuh_tempo_benefit.php?prefixpertanggungan=".$row["PREFIXPERTANGGUNGAN"]."&nopertanggungan=".$row["NOPERTANGGUNGAN"]."&kdkantor=".$kdkantor."','polisinfo',815,600,'yes');\">CETAK BARU</a>";?>
		</td>
