...
// Transaksi Entry Proposal
  /*$sqx = "select prefixpertanggungan as POLIS from $DBUser.tabel_200_pertanggungan where kdproduk like 'JL%' and (kdpertanggungan='1' or kdpertanggungan='2') and ".
	...
	$sqx="select a.nopertanggungan as KAMPRET,a.juamainproduk, a.premi1,a.kdcarabayar as CARA,a.kdproduk,substr(a.kdproduk,-1) as kdprodgroup,a.mulas,a.tglcetak, ".
		...
		"and a.tglcetak is not null and a.prefixpertanggungan='".substr($kdktr,0,2)."' and a.kdstatusfile in ('1','4') ".
...
//		$sql = "select distinct namaproduk from $DBUser.tabel_202_produk where (substr(kdproduk,1,3)='JL2' AND STATUS IS NULL) or (substr(kdproduk,1,5)='JL2XS' AND STATUS='S')";
		//$sql = "select distinct substr(kdproduk,1,3) kdproduk, namaproduk from $DBUser.tabel_202_produk where substr(kdproduk,1,3) in ('JL2','JL3') AND STATUS !='S'";// AND STATUS IS NULL";
		$sql="select distinct (DECODE(STATUS,'S','JL2',(substr(kdproduk,1,3)))) kdproduk, REPLACE(namaproduk,'(S)','') namaproduk,STATUS 
from $DBUser.tabel_202_produk where substr(kdproduk,1,3) in ('JL2','JL3') AND NVL(STATUS,'0') IN ('0','S')";
		//	echo $sqa;
		$DB->parse($sql);
		$DB->execute();	
		$i=0;				 
		while($row_produk=$DB->nextrow()) {
			...
			$kdprod=substr($row_produk['KDPRODUK'],0,3)."%B";
			}
			elseif(substr($row_produk['NAMAPRODUK'],8,1)=="E") {
			$kdprod=substr($row_produk['KDPRODUK'],0,3)."%E";
			}
			else  {
			$kdprod=substr($row_produk['KDPRODUK'],0,3)."%F";
			}		
																	
			echo	'<option value="'.$kdprod.'"'.(($kdprod==$_POST['filter_produk']) ? ' selected':'').'>'.
						'('. $row_produk['KDPRODUK'] .') '. $row_produk['NAMAPRODUK'] .
					'</option>';
		}
echo			"</select>";
echo "      </td>";
