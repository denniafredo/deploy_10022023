...
<table>
        ...
        <td>Nama Bank
	<select name="kdbank">
		<option value="VBN" <? if ($kdbank=="VBNI") {echo "selected";} else { echo "";}?>>VIRTUAL ACCOUNT BNI</option>
		<option value="BCN" <? if ($kdbank=="BCN") {echo "selected";} else { echo "";}?>>VIRTUAL ACCOUNT CIMB NIAGA</option>
	</select></td>
    <td>
	Tanggal Download<?=DateSelector("d");?> 
    <!--input type="submit" disabled=disabled name="upjur" value="UPLOAD JURNAL" class="but"-->
    <input type="button" onclick="window.location.href='../mutasi/flagpelunasanva.php'" value="FLAG JURNAL" />
  </td>
    </tr>
...
<?
	if ($kdbank=='MDR') {
		...
		$namabank='VIRTUAL ACCOUNT BANK NEGARA INDONESIA (BNI)';
		$namalike='BNI';
	}
	else if ($kdbank=='CBN') {
...
if($upjur)
{
   ...
   $sql="BEGIN $DBUser.JURNAL_UTILITY.proses_jurnal_va('$tglupload'); END;";
   //echo $sql;
   //die;
	$DB->parse($sql);
    $DB->execute();
...
<b>HASIL <?=$namabank;?> <br />BULAN <?=$tglcari;?></b>
  ...
  $sql = "select * from $DBUser.TABEL_315_UPLD_AUTODEBET where to_char(tglproses,'dd/mm/yyyy')='$tglcari' and kdbank='$kdbank' and status='0'";
  $kantorcetak=$kantor1;
