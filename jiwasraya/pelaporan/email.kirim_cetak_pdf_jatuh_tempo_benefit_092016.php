...
//echo 'masuk ddn'; die;
	...
	//$db = new ConnectionJL();
	//$conn = $db->Open();	
	$queryxx = "SELECT * FROM $DBUser.TABLE_KIRIM_EMAIL WHERE EMAIL IS NOT NULL and jenis = 'E' order by PREFIXPERTANGGUNGAN,NOPERTANGGUNGAN,RAYONPENAGIHAN";
	//echo $query;
	//$dbxx->Parse($queryxx);
	//$dbxx->Execute();
	$DBx->parse($queryxx);
	...
	//while ($rowxx = oci_fetch_array($dbxx->get_statement())) {
		...
		echo 'Kirim email ke '.$prefixpertanggungan.$nopertanggungan.$kdkantor.'<br>';
			...
			$pdf->Cell(5,4,':',0,0,'C');
			$pdf->SetFont('Arial','B',8);
			$pdf->Cell(70,4,'PEMBERITAHUAN JATUH TEMPO POLIS NO : '.$rowx["PREFIXPERTANGGUNGAN"].'-'.$rowx["NOPERTANGGUNGAN"].' / '.$rowx["NOPOL"].' KE-1',0,0,'L');
			$pdf->Ln();
			$pdf->SetFont('Arial','',8);
			...
			$path_name = 'pdf_email/JT-102016-'.$rowx["PREFIXPERTANGGUNGAN"].'-'.$rowx["NOPERTANGGUNGAN"].'.pdf';
		...
		try {		
			...
			$mail->Subject  = " Pemberitahuan Jatuh Tempo Ekspirasi Polis ".$rowx["PREFIXPERTANGGUNGAN"].'-'.$rowx["NOPERTANGGUNGAN"]. " atas nama tertanggung ".$rowx["TERTANGGUNG"];
			...
			$isipesan = file_get_contents('http://192.168.2.23/jiwasraya/pelaporan/email.get_cetakjatuhtempo_expirasi.php?prefixpertanggungan='.$rowx["PREFIXPERTANGGUNGAN"].'&nopertanggungan='.$rowx["NOPERTANGGUNGAN"].'&tglexp='.$rowx["EXPIRASI"].'&kantor='.$kdkantor);
			...
			$query = "update $DBUser.TABLE_KIRIM_EMAIL set status = 'X' WHERE PREFIXPERTANGGUNGAN = '".$rowx["PREFIXPERTANGGUNGAN"]."' AND NOPERTANGGUNGAN = '".$rowx["NOPERTANGGUNGAN"]."' AND JENIS = 'A' ";
			echo $query;
			$DB->parse($query);
			$DB->execute();
			$row=$DB->nextrow();
