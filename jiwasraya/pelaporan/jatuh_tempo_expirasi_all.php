...
$sql = "SELECT   nopol,prefixpertanggungan,nopertanggungan,
 ...
 WHERE   a.nopenagih=p.nopenagih
		...
		//"AND a.kdvaluta='$kdvaluta' ".
		 ...
		 AND expirasi <= TO_date ('".$tglcari."', 'YYYYMM')
		 ";
...
<table border="0" cellspacing="1" style="border-collapse: collapse" width="100%" id="table1" cellpadding="4" bordercolor="#B3CFFF">
	...
	?>
        ...
        <td><?=$row["TELP"];?></td>
        <td><?=$row["KOTA"];?></td>
        <td><?="<a href=\"javascript:NewWindow('../pelaporan/cetakjatuhtempo_expirasi.php?prefixpertanggungan=".$row["PREFIXPERTANGGUNGAN"]."&nopertanggungan=".$row["NOPERTANGGUNGAN"]."&tglexp=".$row["TGLEXP"]."','polisinfo',815,600,'yes');\">cetak</a>";?></td>
