...
Pilih Kantor : <select name="kantornya" size="1" onChange="GantiCari(document.getpremi)">
  ...
  ?><input type="submit" name="submit" value="Cari">					
	...
	$sql="SELECT   nopemegangpolis, (SELECT   namaklien1
			   ...
			   WHERE       A.KDPRODUK = B.KDPRODUK
					   AND b.point > 0
					   AND TO_CHAR (MULAS, 'YYYYmm') >= '201306'
  ...
  ?>
	...
	<? }?>											
                                                ...
                                                <? //echo "<a onclick='topWindow()' href=daftar_cadangan.php?idx=".$arr["ID"]."&kantornya=".$kantornya."&sho=$i> Tambah Data</a>";?><a href="daftar_point_reward.php?idx=<?=$arr["ID"];?>&kantornya=<?=$kantornya;?>&sho=<?=$i;?>" onClick="NewWindow('./daftar_pointx.php?id=<?=$arr["NOPEMEGANGPOLIS"];?>','popuppage','420','500','yes')"><img src="./img/error.png" height="15" width="15" border="0" alt="Tambah Data">&nbsp;Redeem Point</a>
                                                </td></tr>
                                                </TABLE>
