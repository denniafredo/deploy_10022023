...
<table>
    ...
    <input type="submit" disabled=disabled name="upjur" value="UPLOAD JURNAL" class="but">
  </td>
    </tr>
...
if($upjur)
{
   ...
   $tglupload = str_replace("/", "", $tglcari);
if ($radio=='R') {
   $sql="BEGIN $DBUser.JURNAL_UTILITY.proses_jurnal_autodebet_r('$tglupload', '$kdbank'); END;";
} else {   //echo $sql;
	$sql="BEGIN $DBUser.JURNAL_UTILITY.proses_jurnal_autodebet('$tglupload', '$kdbank'); END;";
	}
   //die;
	$DB->parse($sql);
    $DB->execute();
...
if($updet)
{	if ($radio=='R') {
	$sql="DECLARE RetVal VARCHAR2(32767); BEGIN  RetVal := $DBUser.UPD_KARTU_RIDER ( '$tglcari', '$kdbank', '$userid','$radio' ); COMMIT; END;";
} else {
	$sql="DECLARE RetVal VARCHAR2(32767); BEGIN  RetVal := $DBUser.UPD_KARTU_PREMI ( '$tglcari', '$kdbank', '$userid','$radio' ); COMMIT; END;";}
   echo $sql;
   //die;
	$DB->parse($sql);
    $DB->execute();
...
if($apply)
{	$sql="delete FROM $DBUser.TABEL_315_UPLD_AUTODEBET_RJ a where to_char(tglproses,'dd/mm/yyyy')='$tglcari' and kdbank='$kdbank'";
     //echo $sql;
	 $DB->parse($sql);
     $DB->execute();
    ...
    if ($ffilename!="none"){
            ...
            $my_uploader = new uploader;
				...
				//echo $_FILES[ffilename][name];
				  ...
				  $sqa="select max(id)+1 mid from $DBUser.TABEL_315_UPLD_AUTODEBET_RJ where kdbank='$kdbank'";
				  //echo $sqa;
				  $DB->parse($sqa);
      			  $DB->execute();
				  $ar=$DB->nextrow();	
				  ...
				  for($i=0; $i<sizeof($fcontents); $i++) { 
					  ...
					  if (("'x".implode("','", str_replace("'","`",$arr)) ."x'")=="'xx'") {$sql="";} else {
					  	 $sql = "insert into $DBUser.TABEL_315_UPLD_AUTODEBET_RJ (kdbank,id,status,tglproses,peserta_text) values ('$kdbank','".($ar['MID']+$i)."','0',TO_DATE('".$tglcari."','DD/MM/YYYY'),'". 
...
<b>HASIL <?=$namabank;?> <br />BULAN <?=$tglcari;?></b>
  <? 
  // di edit oleh fendy sesuai permintaan atar tanggal 10 februari 2016 /* XXXXXX */
  $sql = "SELECT   substr(peserta_text,88,6)BOOKED,
substr(peserta_text,76,11) NOPOLIS,
         SUBSTR (a.peserta_text, 54, 17)/100 JUMLAHTAGIHAN,
         b.nopolbaru,
         /*NVL (NO_PONSEL, PHONETETAP02) PHONE,*/
		 NVL(NVL(NVL(NVL (NO_PONSEL, PHONETAGIH02), PHONETAGIH01), PHONETETAP02), PHONETETAP01) PHONE,
         'Pemegang Polis Yth, kami tidak berhasil melakukan pendebetan polis Anda no. '|| NVL(B.NOPOLBARU, B.PREFIXPERTANGGUNGAN||B.NOPERTANGGUNGAN) ||' sebesar Rp '||SUBSTR (a.peserta_text, 54, 17)/100||'  tgl '||to_char(A.tglproses,'dd/mm/yyyy')||'. Untuk info hub. 1500176' sms
  FROM   $DBUser.TABEL_315_UPLD_AUTODEBET_RJ A,
         $DBUser.TABEL_200_PERTANGGUNGAN B,
         $DBUser.TABEL_100_KLIEN C
 WHERE       substr(peserta_text,76,11) = B.PREFIXPERTANGGUNGAN || B.NOPERTANGGUNGAN
         AND NOPEMEGANGPOLIS = NOKLIEN
         AND to_char(tglproses,'dd/mm/yyyy')='$tglcari' and a.kdbank='$kdbank' and a.status='0'";
  $kantorcetak=$kantor1;
 ...
 if ($kirim) {
  // di edit oleh fendy sesuai permintaan atar tanggal 10 februari 2016 /* XXXXXX */
  $sql = "SELECT   substr(peserta_text,88,6)BOOKED,
substr(peserta_text,76,11) NOPOLIS,PREFIXPERTANGGUNGAN, NOPERTANGGUNGAN,
         SUBSTR (a.peserta_text, 54, 17)/100 JUMLAHTAGIHAN,
         /*NVL (NO_PONSEL, PHONETETAP02) PHONE,*/
		 NVL(NVL(NVL(NVL (NO_PONSEL, PHONETAGIH02), PHONETAGIH01), PHONETETAP02), PHONETETAP01) PHONE,
         'Pemegang Polis Yth, kami tidak berhasil melakukan pendebetan polis Anda no. '|| NVL(B.NOPOLBARU, B.PREFIXPERTANGGUNGAN||B.NOPERTANGGUNGAN) ||' sebesar Rp '||SUBSTR (a.peserta_text, 54, 17)/100||'  tgl '||to_char(A.tglproses,'dd/mm/yyyy')||'. Untuk info hub. 1500176' sms
 ...
 WHERE       substr(peserta_text,76,11) = B.PREFIXPERTANGGUNGAN || B.NOPERTANGGUNGAN
  ...
  $kantorcetak=$kantor1;
	...
	while ($arr1=$DB1->nextrow()) {
		...
		$mysqlins="insert into smsjiwasraya (PHONE, MESSAGE,JENIS_SMS,KODE_KANTOR,NAMA_DIVISI,NO_POLIS) VALUES('".$arr1["PHONE"]."','".$arr1["SMS"]."','NOTIFIKASI GAGAL DEBET','".$arr1["PREFIXPERTANGGUNGAN"]."','PENJUALAN','".$arr1["NOPOLIS"]."')";
		//echo $mysqlins;
