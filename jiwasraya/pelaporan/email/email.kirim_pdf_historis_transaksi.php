...
//echo 'masuk ddn'; die;
	...
	$queryxx = "SELECT c.*,to_char(TGLBOOKED,'MM/YYYY') periode,to_char(TGLBOOKED,'YYYYMM') tglcari,
					   (select NAMAKLIEN1
						...
						to_char(tglseatled,'YYYYMM') tglseatledX		
				...
				/*and rayonpenagihan in ('DK','G E','B G')*/
	...
	//$conn = $db->Open();	
		...
		$path_name = 'pdf/'.$tglcari.'_HISTORIS_TRANSAKSI_'.$prefixpertanggungan.'-'.$nopertanggungan.'.pdf';		
		try {	
			...
			//$mail->From       = "PT. Asuransi Jiwa IFG <postmaster@jiwasraya.co.id>";
			...
			$mail->Subject  = "Pemberitahuan Historis Transaksi Unitlink Polis ".$prefixpertanggungan.'-'.$nopertanggungan. " atas nama tertanggung ".$namaklien;
			...
			$isipesan = file_get_contents('http://192.168.2.23/jiwasraya/pelaporan/email/email.get_cetakhistoristransaksi_premi_simple.php?prefixpertanggungan='.$prefixpertanggungan.'&nopertanggungan='.$nopertanggungan.'&tglexp='.$periode.'&kantor='.$kdkantor);
			...
			$mail->Send();
				...
				$query = "update $DBUser.PENERIMA_EMAIL_TES set status = 'X',tgl_kirim=sysdate,PESAN_EROR='".$e->getMessage()."' WHERE PREFIXPERTANGGUNGAN = '".$prefixpertanggungan."' AND NOPERTANGGUNGAN = '".$nopertanggungan."' AND JENIS = '$jenis' and ID_BLAST = '".$_GET['idblast']."'";
	...
	//header("Location: detail_daftar_email_sekper.php?idblast=".$_GET['idblast']."&jenis_blast=EMAIL_HISTORIS_JL4&gruping=INDIVIDU");
?>