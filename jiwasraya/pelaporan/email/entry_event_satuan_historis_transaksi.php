...
if ($billing) {
	$sql = "begin $DBUser.GEN_EMAIL_BLAST_1_HIST_TRANS ( '".$_POST['jnsblast']."', '".$_POST['periode']."', '".$_POST['prefix']."', '".$_POST['noper']."' );end;";
	$DB->parse($sql);
	$DB->execute();
	//echo $sql;
	//echo '<br><br>';
	...
	$queryxx = "SELECT c.*,to_char(TGLBOOKED,'MM/YYYY') periode,to_char(SYSDATE,'YYYYMM') tglcari,
			   (select NAMAKLIEN1
	...
	include 'function_gen_pdf_ftp_historis_transaksi.php';		
	//die;
	$DBx->parse($queryxx);
	$DBx->execute();
	while ($rowxx=$DBx->nextrow()) {
		...
		gen_pdf($prefixpertanggungan,$nopertanggungan,$tglcari,$kdkantor); //die;
		$nama_file = $tglcari.'_HISTORIS_TRANSAKSI_'.$prefixpertanggungan.'-'.$nopertanggungan.'.pdf';
		echo $nama_file;
	...
	//echo $queryxx;
	header("Location: daftar_email_sekper.php");
} else {
      ...
      <tr>
        ...
        <td>: 
  				...
  				/*switch($kdklaim)
  				{
  				  ...
  				  echo "<option value='EMAIL_HISTORIS_JL4'>EMAIL_HISTORIS_JL4</option>";
  					echo "<option value='EMAIL_HISTORIS_JL3'>EMAIL_HISTORIS_JL3</option>";
...
<table width="800">
  <tr>
    <td width="50%" class="arial10" align="left"><a href="daftar_email_sekper.php">Kembali</a></td>
