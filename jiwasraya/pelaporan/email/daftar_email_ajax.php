...
$sqlx="select a.*,(select count(*) from $DBUser.penerima_email where id_blast = a.id_blast and status = '1') jml_berhasil,
	...
	<table border="1" style="border-collapse: collapse" width="100%" id="table2" cellpadding="4" bordercolor="#B3CFFF">
		...
		<tr>
			<td bgcolor="#CEE7FF">
				...
				<a href="entry_event_satuan.php" class="verdana8blu">Create Event Blast Satuan</a>
			</td>
		</tr>
	</table>
...
<table border="0" cellspacing="1" style="border-collapse: collapse" width="100%" id="table1" cellpadding="4" bordercolor="#B3CFFF">
	...
	?>
		...
		if($row["JML_VALID_BLM_DIKIRIM"]>0){
			if($row["JENIS_BLAST"]=='EMAIL_JATUH_PREMI_BERKALA'){?>
		...
		?>		
		<br>
		<a href="detail_daftar_email.php?idblast=<?=$row["ID_BLAST"];?>&jenis_blast=<?=$row["JENIS_BLAST"];?>">Detail</a><br><br>
		<?php 
        ...
        <td>
			Total : <?=number_format($row["TOTAL_EMAIL"],2,",",".");?><br><br>
			Valid : <?=number_format($row["EMAIL_VALID"],2,",",".");?><br><br>
			Invalid : <?=number_format($row["EMAIL_JML_INVALID"],2,",",".");?><br><br>
			Terkirim : <?=number_format($row["JML_KIRIM_VALID_BERHASIL"],2,",",".");?><br><br>
			Blm Terkirim : <?=number_format($row["JML_VALID_BLM_DIKIRIM"],2,",",".");?><br><br>			
			Gagal Terkirim : <?=number_format($row["JML_VALID_GAGAL_DIKIRIM"],2,",",".");?>			
		</td>        		       
        <td>
			Total : <?=number_format($row["JML_SMS"],2,",",".");?><br><br>			
			Terkirim : <?=number_format($row["JML_DIKIRIM_SMS"],2,",",".");?><br><br>
			Belum Terkirim : <?=number_format($row["JML_BLM_DIKIRIM_SMS"],2,",",".");?>
		</td>        		       
        <td>			
			Valid : <?=number_format($row["JML_URL_VALID"],2,",",".");?><br><br>
			Invalid : <?=number_format($row["JML_URL_INVALID"],2,",",".");?>			
		</td>
	</tr>
	<? 
	$i++;
