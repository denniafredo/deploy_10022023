...
$queryzz = "insert into  $DBUser.LOG_QUERY values (to_char(sysdate,'dd/mm/yyyy hh:mi:ss'),'mulai eksekusi melalui cron sebelum looping')";
		$DB->parse($queryzz);
		$DB->execute();
...
while ($rowxx=$DBx->nextrow()) {
	...
	if($res->status){
		...
		$url_pendek = 'invalid';
		$queryzz = "insert into  $DBUser.LOG_QUERY values (to_char(sysdate,'dd/mm/yyyy hh:mi:ss'),'eksekusi akan berakhir memelalui cron karena url sudah invalid')";
		$DB->parse($queryzz);
		$DB->execute();
	}
	...
	if ($url_pendek == 'invalid') {
		...
		$queryzz = "insert into  $DBUser.LOG_QUERY values (to_char(sysdate,'dd/mm/yyyy hh:mi:ss'),' proses berhenti dieksekusi memelalui cron karena udh invalid paling bawah')";
		$DB->parse($queryzz);
		$DB->execute();
		break;    
	}
