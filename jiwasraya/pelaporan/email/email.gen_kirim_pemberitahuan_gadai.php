<?php
	...
	if($_GET['delete'] == '1'){
		...
		while ($rowxx=$DBx->nextrow()) {
			...
			$path_name = 'pdf_gadai/'.date("Y").'/'.date("m").date("Y").'_SURAT_PEMBERITAHUAN_GADAI_'.$prefixpertanggungan.$nopertanggungan.'.pdf';
				...
				"
					<table border='0' cellpadding='0' cellspacing='0' style='border-collapse: collapse' bordercolor='#111111' width='782' id='AutoNumber1'>
						<tr>
	    					...
	    					<p align='justify'>Bersama ini kami sampaikan posisi gadai polis ".$prefixpertanggungan.$nopertanggungan." periode bulan ".$tglcari." </p>
							<p align='justify'>Untuk menghindari Kapitaslisasi Bunga menjadi Pokok mohon melakukan pembayaran minimal sebesar Tagihan Bunga Gadai. Pembayaran Gadai polis saat ini melalui Virtual Account (VA) BNI : <b><i>".$rowxx['NOACCOUNT']."</i></b> </p>
							<table border='0' cellspacing='1' style='border-collapse: collapse' width='100%' id='table1' cellpadding='4' bordercolor='#B3CFFF'>
			  					<tr>
			...
			try {
				...
				$mail->Subject  = "Pemberitahuan Posisi Pinjaman Polis ".$prefixpertanggungan.'-'.$nopertanggungan. " atas nama pemegang polis ".$namaklien;
				$mail->WordWrap   = 80;
