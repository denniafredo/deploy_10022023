<?
	...
	function gen_pdf($prefixpertanggungan,$nopertanggungan,$tglcari,$kdkantor,$image_promo){
		...
		$DEBEPD = New database($userid, $passwd, $DBName);	
		...
		$sql = "SELECT ptg.prefixpertanggungan PREFIXPERTANGGUNGAN, 
				...
				TO_CHAR(add_months(cek_bulan_valid_akhir_V2 ('".$tglcari.$arr["TMULAS"]."'),(SELECT GRACEPERIODE  
				...
				TO_CHAR(cek_bulan_valid_akhir_V2('".$tglcari.$arr["TMULAS"]."')+(SELECT   GRACEPERIODE*30
				FROM   $DBUser.TABEL_241_GRACE_PERIODE 
				...
				CASE
				WHEN (MONTHS_BETWEEN (cek_bulan_valid_akhir('$tglexp'), MULAS) / 12) >=5
				THEN
				PREMI2
				ELSE
				PREMI1
				...
				from $DBUser.tabel_300_historis_rider where prefixpertanggungan=ptg.prefixpertanggungan and nopertanggungan=ptg.nopertanggungan AND tglBOOKED BETWEEN to_date('".$tglcari."','YYYYMM') AND LAST_DAY(to_date('".$tglcari."','YYYYMM'))),0)
				PREMI,
				(SELECT STATUS
		...
		$pdf->ln();
			if($row["PRODUK"] == 'JS PROMAPAN'){
				$queryhp="SELECT   TO_CHAR (min(tglbooked), 'dd/mm/yyyy') booked,
								   ...
								   sum(PREMITAGIHAN)   +
								   NVL(SUM((select PREMITAGIHAN 
								   from $DBUser.tabel_300_historis_rider where prefixpertanggungan=A.prefixpertanggungan and nopertanggungan=A.nopertanggungan AND tglseatled IS NULL)),0) PREMITAGIHAN
							FROM   $DBUser.tabel_300_historis_premi A
		...
		$pdf->ln();
		$pdf->SetFont('Arial','',8);
		$pdf->Cell(190,4,'email : customer_service@jiwasraya.co.id, dengan melampirkan bukti pelunasan premi yang sah.',0,0,'L');
		$pdf->ln(7);
		$pdf->SetFont('Arial','',8);
		...
		// OUTPUT
		$pdf->Output('pdf/'.$tglcari.'_JATUH_TEMPO_PREMI_'.$row["PREFIXPERTANGGUNGAN"].'-'.$row["NOPERTANGGUNGAN"].'.pdf');
		//$pdf->Output();
		...
		$file = 'pdf/'.$tglcari.'_JATUH_TEMPO_PREMI_'.$row["PREFIXPERTANGGUNGAN"].'-'.$row["NOPERTANGGUNGAN"].'.pdf';
		$namafile = 'DOKUMEN_NOTIFIKASI_EMAIL/'.$tglcari.'_JATUH_TEMPO_PREMI_'.$row["PREFIXPERTANGGUNGAN"].'-'.$row["NOPERTANGGUNGAN"].'.pdf';
		// upload file
