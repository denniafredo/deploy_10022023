...
<html>
	...
	<body topmargin="20">
		...
		<table border="0" style="border-collapse: collapse" width="100%" id="table2" cellpadding="4">
			...
			<tr>
				<td><a href="detail_blast_system_download.php?idblast=<?=$row['ID_BLAST']?>">Download</a></td>
			</tr>
		</table></br>
			<?
				}
			...
			<tr align="center">
				...
				<?php
					if($row["STATUS"] == ''){
						...
						echo "<td><img src=../../img/success.png align=absmiddle border='0'></td><td>".$row["TGL_KIRIM"]."</td>";
					}

					if($row["STATUS2"] == ''){
						echo "<td>-</td><td>-</td>";
					}else{
						echo "<td><img src=../../img/success.png align=absmiddle border='0'></td><td>".$row["TGL_KIRIM2"]."</td>";
					}

					if($row["TGL_KIRIM_SMS"] == ''){
						echo "<td>-</td><td>-</td>";
					}else{
						echo "<td><img src=../../img/success.png align=absmiddle border='0'></td><td>".$row["TGL_KIRIM_SMS"]."</td>";
					}

					if($row["TGL_CETAK_SURAT"] == ''){
						echo "<td>-</td><td>-</td>";
					}else{
						echo "<td><img src=../../img/success.png align=absmiddle border='0'></td><td>".$row["TGL_CETAK_SURAT"]."</td>";
					}
				...
				<td>
					<?php
						if($jenisblast == 'PEMBERITAHUAN_POSISI_GADAI'){
					?>
							<a href="pdf_gadai/<?=$tahun?>/<?=$bulan.$tahun;?>_SURAT_PEMBERITAHUAN_GADAI_<?=$row["PREFIXPERTANGGUNGAN"];?><?=$row["NOPERTANGGUNGAN"];?>.pdf" target="_blank"><img src=../../img/cetak.gif align=absmiddle border=0> Cetak </a>
					<?php
						}else if($jenisblast == 'PEMBERITAHUAN_POLIS_BPO'){
					?>
							<a href="pdf_bpo/<?=$tahun?>/<?=$bulan.$tahun;?>_SURAT_PEMBERITAHUAN_BPO_<?=$row["PREFIXPERTANGGUNGAN"];?><?=$row["NOPERTANGGUNGAN"];?>.pdf" target="_blank"><img src=../../img/cetak.gif align=absmiddle border=0> Cetak </a>
					<?php
						}
					?>
		...
		<table>
			<tr><td><a href="blast_system.php">Back</a></td></tr>
		</table>
	</body>
</html>
