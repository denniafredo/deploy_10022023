...
?>
	...
	<a class="verdana10blk\"><b>DAFTAR PROPOSAL MASUK (LOG PROSES)</b></a>
	<hr size=1>
	<table>
				  ...
				  $sql = "select (SELECT namaklien1 FROM $DBUser.tabel_100_klien kli WHERE kli.noklien=a.nopemegangpolis) as PEMPOL , ".
						   ...
						   "(SELECT   to_char(tglunderwriting,'dd/mm/yyyy')
							  ...
							  WHERE   prefixpertanggungan = a.prefixpertanggungan
								 ...
								 "a.userupdated, (SELECT   userupdated
              FROM   $DBUser.TABEL_214_ACCEPTANCE_DOKUMEN
             WHERE   prefixpertanggungan = a.prefixpertanggungan
                     AND nopertanggungan = a.nopertanggungan)
              as USERAKSEP, (SELECT   to_char(TGLACCEPTANCE,'dd/mm/yyyy')
							   FROM   $DBUser.TABEL_214_ACCEPTANCE_DOKUMEN
							  WHERE   prefixpertanggungan = a.prefixpertanggungan
									  AND nopertanggungan = a.nopertanggungan) as TGLAKSEP, ".
								 ...
								 "(SELECT   to_char(TGLSEATLED,'dd/mm/yyyy')
							   FROM   $DBUser.TABEL_300_HISTORIS_PREMI
							  WHERE   prefixpertanggungan = a.prefixpertanggungan
									  AND nopertanggungan = a.nopertanggungan
									  AND KDKUITANSI='BP3') tgltransfer, a.jmltransfer ".
...
//					break;
					...
					$DB->execute();
					 ...
					 echo "<table width=100% class=tblisi cellpadding=0 cellspacing=0>";
						...
						//	echo("<td align=center>Nopol</td>");
							...
							echo("<td align=center>Jml Transfer</td>");
              ...
              echo("<td><font face=\"Verdana\" size=\"1\">".$i."</font></td>");
				  ...
				  echo("<td align=right><font face=\"Verdana\" size=\"1\">".number_format($arr["JMLTRANSFER"],2,",",".")."</font></td>");
