...
//echo 'masuk ddn'; die;
	...
	//$db = new ConnectionJL();
	//$conn = $db->Open();	
	$queryxx = "SELECT * FROM $DBUser.TABLE_KIRIM_EMAIL_MOHONMAAF WHERE EMAIL IS NOT NULL and status is null and jenis in ('M','N','X','R') order by PREFIXPERTANGGUNGAN,NOPERTANGGUNGAN,RAYONPENAGIHAN";
	//echo $query;
	//$dbxx->Parse($queryxx);
	//$dbxx->Execute();
	$DBx->parse($queryxx);
	...
	//$conn = $db->Open();	
		...
		*/
		...
		try {	
			...
			$mail->Subject  = 'Permohonan maaf terkait pengiriman email jatuh tempo tanggal 01/09/2016';
			...
			$isipesan = file_get_contents('http://192.168.2.23/jiwasraya/pelaporan/get_mintamaaf.php');
			...
			$mail->Send();
				$query = "update $DBUser.TABLE_KIRIM_EMAIL_MOHONMAAF set status = '1' WHERE PREFIXPERTANGGUNGAN = '".$rowx["PREFIXPERTANGGUNGAN"]."' AND NOPERTANGGUNGAN = '".$rowx["NOPERTANGGUNGAN"]."' AND JENIS = '$jenis' ";
				echo $query;
				$DB->parse($query);
				$DB->execute();
				$row=$DB->nextrow();
		} catch (phpmailerException $e) {
				$query = "update $DBUser.TABLE_KIRIM_EMAIL_MOHONMAAF set status = 'X' WHERE PREFIXPERTANGGUNGAN = '".$rowx["PREFIXPERTANGGUNGAN"]."' AND NOPERTANGGUNGAN = '".$rowx["NOPERTANGGUNGAN"]."' AND JENIS = '$jenis' ";
				echo $query;
				$DB->parse($query);
				$DB->execute();
				$row=$DB->nextrow();
