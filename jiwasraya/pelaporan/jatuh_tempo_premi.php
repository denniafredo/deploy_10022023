...
<body topmargin="20">
<b><font size="3">JATUH TEMPO PREMI</font></b>
...
$sql = "SELECT   nopol,prefixpertanggungan,nopertanggungan,
		...
		(SELECT   namaklien1
         ...
         premi2,
		 CASE
            WHEN (MONTHS_BETWEEN (TO_DATE (TO_DATE ('".$tglcari."01', 'YYYYMMDD'), 'DD/MM/YYYY'), MULAS) / 12) >=5
            THEN
               PREMI2
            ELSE
               PREMI1
         ...
         (SELECT   nilaibenefit
           ...
           WHERE       prefixpertanggungan = a.prefixpertanggungan
            ...
            benefitexp,TO_CHAR(tglakhirpremi,'DD/MM/YYYY') tglakhirpremi,
 ...
 WHERE   a.nopenagih=p.nopenagih
         ...
         AND a.kdcarabayar <> 'X'
		 and to_char(a.tglakhirpremi,'yyyymm') > $tglcari
		 AND SUBSTR(a.kdproduk,1,3) not in ('JL2','JL3')
        ...
        (SELECT 'x'
        FROM   $DBUser.tabel_223_transaksi_produk x,
                $DBUser.tabel_207_kode_benefit y
        WHERE x.kdbenefit = y.kdbenefit
        AND   y.kdkelompokbenefit ='T'
        AND   nvl(x.nilaibenefit,0) > 0 
        AND   x.prefixpertanggungan=a.prefixpertanggungan AND x.nopertanggungan=a.nopertanggungan
        AND   TRUNC(TO_DATE ('".$tglcari."', 'yyyymm')) BETWEEN TRUNC(x.expirasi) AND ADD_MONTHS(TRUNC(x.expirasi),11)
        AND   exists (select 'x' from $DBUser.tabel_206_produk_benefit c 
                      where c.kdproduk = a.kdproduk
                      and   c.kdjenisbenefit ='G'))
		AND substr(kdproduk,0,2) <> 'JL'  AND NOT EXISTS (
					SELECT   'X'
					  FROM   $DBUser.TABEL_UL_BENEFIT_KLAIM
					 WHERE       PREFIXPERTANGGUNGAN = a.PREFIXPERTANGGUNGAN
							 AND NOPERTANGGUNGAN = a.NOPERTANGGUNGAN
							 AND decode($DBUser.F_DATE (TO_CHAR (SYSDATE, 'DD') || '/".$month."/".$year."','DD/MM/YYYY'),0,
              last_day(TO_DATE ( '/".$month."/".$year."','/MM/YYYY')),
              TO_DATE (TO_CHAR (SYSDATE, 'DD') || '/".$month."/".$year."','DD/MM/YYYY'))
...
<table border="0" cellspacing="1" style="border-collapse: collapse" width="100%" id="table1" cellpadding="4" bordercolor="#B3CFFF">
	...
	?>
		...
		<td><?=number_format($row["PREMI"],2,",",".");?></td>
        <td><?=$row["TGLEXP"];?></td>
        ...
        <td><?=$row["VALUTA"];?></td>
        <!--td><?="<a href=\"javascript:NewWindow('../pelaporan/cetakjatuhtempo.php?prefixpertanggungan=".$row["PREFIXPERTANGGUNGAN"]."&nopertanggungan=".$row["NOPERTANGGUNGAN"]."&tglexp=".$row["TGLEXP"]."','polisinfo',815,600,'yes');\">cetak</a>";?></td>
        <td><?="<a href=\"javascript:NewWindow('../pelaporan/cetakinfocb.php?prefixpertanggungan=".$row["PREFIXPERTANGGUNGAN"]."&nopertanggungan=".$row["NOPERTANGGUNGAN"]."&tglexp=".$row["TGLEXP"]."','polisinfo',815,600,'yes');\">Info CB</a>";?></td-->
