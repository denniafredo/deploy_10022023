...
<br />
<div align="center">
<b>PEMBUKUAN PREMI CITIBANK 1BILL/ BNI CREDIT CARD <br />BULAN <?=$namabulan." ".$year;?> <?=$KTR->namakantor;?> </b>
</div>
<br />
<hr size="1">
<!--------------  start Rupiah --------------->
  	...
  	$sql = "select * from (select ".
           ...
           "from ".
              ...
              "b.nopertanggungan=d.nopertanggungan and ".
							"substr(b.kdproduk,1,3) NOT IN ('JL2', 'JL3') and ".
...
where namaakun is not null";//,e.rekening2";
...
<? 
  ...
  <!---------- summary  -------->
	  <?
		/*$sql = "select (select nama from $DBUser.tabel_802_kodeakun where akun='272010') namaakunperantara,".
              "(select nama from $DBUser.tabel_802_kodeakun where akun='553000') namaakunpotongan,".
              "(select nama from $DBUser.tabel_802_kodeakun where akun='604000') namaakunmatre,".
		...
		$sql="SELECT * FROM (
			select (select nama from $DBUser.tabel_802_kodeakun where akun='272010') namaakunperantara,
			(select nama from $DBUser.tabel_802_kodeakun where akun='553000') namaakunpotongan,
			(select nama from $DBUser.tabel_802_kodeakun where akun='604000') namaakunmatre,
			(select nama from $DBUser.tabel_802_kodeakun where akun='931000') selisihbedaindek, 'CTB' kdbank
			from $DBUser.tabel_802_kodeakun where akun in('272010')
			UNION
			select (select nama from $DBUser.tabel_802_kodeakun where akun='148481000') namaakunperantara,
			(select nama from $DBUser.tabel_802_kodeakun where akun='553000') namaakunpotongan,
			(select nama from $DBUser.tabel_802_kodeakun where akun='604000') namaakunmatre,
			(select nama from $DBUser.tabel_802_kodeakun where akun='931000') selisihbedaindek, 'CBN' kdbank 
			from $DBUser.tabel_802_kodeakun where akun in('272010'))
			WHERE KDBANK='$kdbank'";
		$DB->parse($sql);
	  $DB->execute();
	  ...
	  <? 
  	 ...
  	 <td align="right"><?=number_format(($jmlpremi+$jmlpremi_di+$jmlpremi_va)-($jmlmatre+$jmlmatre_di+$jmlmatre_va),2,",",".");?></td>
  	 <td align="right"></td>
  	</tr>
<!--
		<tr>
...
<? 
		...
		echo("<font face=Verdana size=1><a href=\"#\" onclick=\"window.open('../akunting/cetakslip_pindahbuku_1bill.php?month=".$month."&year=".$year."&kdbank=".$kdbank."','','width=800,height=600,top=100,left=100,scrollbars=yes');\"><img src=../img/cetak.gif align=absmiddle border=0> Cetak SLIP</a></font>");					 
?>
</body>
</html>