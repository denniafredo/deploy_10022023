...
<table border="0" cellpadding="2" cellspacing="1" style="border-collapse: collapse" bordercolor="#CCCCCC" width="100%" id="AutoNumber1">
  <tr>
	...
	<? 
	if ($metodebayar=="CTB" or $produk=="JL4"){
  ...
  $materenye="(select nilaimeterai from $DBUser.tabel_999_batas_materai where (to_number(a.jumlahtagihan)/100)-6000 between batasbawahpremi and batasataspremi ) as matere, ";
  }
	...
	if($metodebayar=="MDR" || $metodebayar=="BNI"  || $metodebayar=="BRI" || $metodebayar=="CBN" || $metodebayar=="CTB" || $metodebayar=="POS"){
		 $sql = "select ".
            ...
            "a.nopolis,to_char(a.tglbooked,'MM/YYYY') as blntagihan,".
			 ...
			 "(select premitagihan from $DBUser.tabel_300_historis_rider where 
                     ...
                     AND nopertanggungan = b.nopertanggungan
                     and to_char(tglbooked,'ddmmyyyy')=to_char(a.tglbooked,'ddmmyyyy') ) rider ".
          "from ".
		 ...
		 $sql = "SELECT   c.kdrayonpenagih,
			   ...
			   b.premistd+(NVL((SELECT   premi
				 ...
				 WHERE   prefixpertanggungan = b.prefixpertanggungan
				 AND nopertanggungan=b.nopertanggungan
				 AND kdbenefit='BNFTOPUP'),0)) AS premitagihan,
			   d.kdrekeninglawan,
...
-->
    ...
    echo number_format($arr["JMLPREMI"]/100,2,",",".");
    }
...
//	$jmltotaldebet += $arr["PREMITAGIHAN"]+$arr["MATERE"]-$discount;
 ...
 if($metodebayar=="CTB" ){  // payment statement tidak menggunakan materai
 $jmlpremi += ($arr["JMLPREMI"])-$arr["RIDER"]-$arr["MATERE"]-$discount;
 }else{
 $jmlpremi += ($arr["JMLPREMI"]/100)-$arr["RIDER"]-$arr["MATERE"]-$discount;
 }
   ...
   echo number_format($jmltotaldebet/100,2,",",".");
   }
   ?></td>
...
$kom=$jmltotaldebet;
//$nilai1=$jmlpremi;
$nilai1=$jmlpremi+$jmlpremir-$jmltotalselisih;
$nilai2=$jmldiscount;
$nilai3=$jmlmatre;
...
echo("<font face=Verdana size=1><a href=\"#\" onclick=\"window.open('../akunting/cetakslip_autodebet.php?kdbank=UL".$metodebayar."&tgl=".$tglbayar."&kom=".$kom."&nilai1=".$nilai1."&nilai2=".$nilai2."&nilai3=".$nilai3."&nilai4=".$nilai4."','','width=800,height=600,top=100,left=100,scrollbars=yes');\"><img src=../img/cetak.gif align=absmiddle border=0> Cetak SLIP</a></font>");
echo("<font face=Verdana size=1>&nbsp;&nbsp;|&nbsp;&nbsp;</font>");
