...
//echo 'masuk ddn'; die;
	...
	//$db = new ConnectionJL();
	//$conn = $db->Open();	
	$queryxx = "SELECT * FROM $DBUser.TABLE_KIRIM_EMAIL_tes WHERE EMAIL IS NOT NULL and jenis in ('M','R','N') order by PREFIXPERTANGGUNGAN,NOPERTANGGUNGAN,RAYONPENAGIHAN";
	//echo $query;
	//$dbxx->Parse($queryxx);
	//$dbxx->Execute();
	$DBx->parse($queryxx);
	...
	//$conn = $db->Open();	
		...
		$pdf->Ln();
		$pdf->Ln();
		$pdf->MultiCell(170,4,'Mohon untuk dapat melakukan pembayaran premi tersebut diatas setelah tanggal jatuh tempo. Untuk menjaga kesinambungan manfaat Polis Ibu agar pembayaran premi dilakukan tidak melebihi masa kelonggaran pembayaran premi (grace period) yaitu '.$rowx["GP"].' hari setelah tanggal jatuh tempo premi.');
		$pdf->Ln();	
		...
		$pdf->Ln();	
		$pdf->MultiCell(170,4,'Pastikan saldo rekening autodebet anda mencukupi.');
		$pdf->Ln();	
		...
		$mail->Subject  = " <Ujicoba kirim dari Server JLINDO> Pemberitahuan Jatuh Tempo Premi Polis ".$rowx["PREFIXPERTANGGUNGAN"].'-'.$rowx["NOPERTANGGUNGAN"]. " atas nama tertanggung ".$rowx["TERTANGGUNG"];
