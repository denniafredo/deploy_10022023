...
$sqlx="
  SELECT   SD.kdrayonpenagih,           
    ...
    FROM   (  SELECT   kdrayonpenagih,                       
                       NAMACARABAYAR,
                       SUM (CASE WHEN KDKUITANSI = 'NB' THEN nilairp ELSE 0 END)
                          TAG_NB,
                       SUM (
                          CASE WHEN KDKUITANSI = 'OB' THEN (nilairp) ELSE 0 END
                       )
                          TAG_OB,
                ...
                FROM   (  SELECT   kdrayonpenagih,
                           ...
                           WHERE   KDRAYONPENAGIH IN
                                         (SELECT   KDKANTOR
                                           ...
                                           WHERE   KDKANTORINDUK = '".$wilayah."')                                   
								   AND TGLBOOKED between to_date('2000','yyyy') and
  ...
  SELECT   SD.NAMACARABAYAR,
   ...
   ORDER BY   SD.NAMACARABAYAR";
	...
	?>
        ...
        <td align="right"><strong><a onclick="popitup('detail_tagihan_bln_ini.php?jenis=TM_TAG&kd_kuitansi=NB&kd_carabayar=<?=$row["NAMACARABAYAR"];?>&kd_wilayah=<?=$wilayah;?>&bln=<?=$tglcari;?>')" href="#"><?=number_format($row["TM_TAG_NB"],2,",",".");?></a></strong></td>
