...
//echo 'masuk ddn'; die;
	...
	//$db = new ConnectionJL();
	//$conn = $db->Open();	
	$queryxx = "SELECT * FROM $DBUser.TABLE_KIRIM_EMAIL WHERE EMAIL IS NOT NULL and jenis in ('M','R','B','E') order by PREFIXPERTANGGUNGAN,NOPERTANGGUNGAN,RAYONPENAGIHAN";
	//echo $query;
	//$dbxx->Parse($queryxx);
	//$dbxx->Execute();
	$DBx->parse($queryxx);
	...
	//$conn = $db->Open();	
		...
		$pdf->SetFont('Arial','',9);	
		$pdf->Ln();	
		$pdf->MultiCell(170,4,'Informasi lebih lanjut dapat menghubungi Kantor Cabang PT Asuransi Jiwa IFG terdekat atau Call Center PT Asuransi Jiwa IFG 021-1500151 atau email customer_service@ifg-life.co.id atau kunjungi www.jiwasraya.co.id');	
		$pdf->Ln();	
