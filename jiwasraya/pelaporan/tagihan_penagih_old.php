...
?>			
  		...
  		$sql =	"SELECT  a.prefixpertanggungan, 
              ...
              WHERE     a.prefixpertanggungan = c.prefixpertanggungan AND 
                  ...
                  a.nopenagih = '$nopenagih' AND a.tglbooked <= TRUNC (SYSDATE) 
                  $filtertgl 
									$filtergrp 
									$filterkdk 
									$filterstt
...
/*
					 if(substr($arr["KDREKENING"],-1)==1)
					 {
  					 $premirpiob = round($arr["PREMITAGIHAN"],2);
  					 $jmlpremirpiob += $premirpiob;
						 $jrpiob ++;
					 }
					 elseif(substr($arr["KDREKENING"],-1)==2)
					 {
...
*/
      ...
      ?>				
			...
			?>
  			<hr size="1">
        <?="<a href=# onclick=\"NewWindow('tagihan_penagih.php?mode=print&cari=1&tglprocess=".$tglprocess."&nopenagih=".$nopenagih."&kdgroup=".$kdgroup."&kdstatus=".$kdstatus."&kdkuitansi=".$kdkuitansi."','',900,500,1)\"><img src=../img/cetak.gif align=absmiddle border=0> Cetak</a>";?>
