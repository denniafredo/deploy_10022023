<? 
	...
	if ($submit) {
	 	...
	 	$sql = "SELECT DISTINCT(X.JABATAN_TTD) AS REGIONAL,
                ...
                (
                    ...
                    WHERE A.NOPENAGIH = B.NOPENAGIH
                        ...
                        AND A.KDSTATUSEMAIL = '1' 
                        ".$sendemail."
                        AND A.PREFIXPERTANGGUNGAN||A.NOPERTANGGUNGAN NOT IN (
                            SELECT X.PREFIXPERTANGGUNGAN||X.NOPERTANGGUNGAN 
                            FROM $DBUser.TABEL_200_PERTANGGUNGAN X
                            WHERE X.KDPERTANGGUNGAN = '1' 
                                AND X.KDSTATUSFILE = '7' 
                                AND X.PREFIXPERTANGGUNGAN||X.NOPERTANGGUNGAN NOT IN (
                                    (SELECT PREFIXPERTANGGUNGAN||NOPERTANGGUNGAN FROM $DBUser.TABEL_214_APPROVAL_PROPOSAL)
                                )
                        )
                ) PROPOSAL,
                (
		...
		while ($arr_l=$DB->nextrow()){
			...
			$proposal_approve_final 	= $proposal_approve_final + $proposal_approve + $proposal_bayar;

			$proposal_bayar_final 		= $proposal_bayar_final + $proposal_bayar;
			$proposal_belum_bayar 		= $proposal_approve_final - $proposal_bayar_final;
		...
		<div class="header">
			...
			<div class="kolom kolom4">
				...
				<table style="text-align: left; width: 100%; font-size: 12px; font-style: italic;" >
                    <tr>
                        <td width="15%">Lunas</td>
                        <td width="25%">: <a target="_blank" href="http://192.168.2.23/jiwasraya/pelaporan/report/detail_dashboard.php?rayon_cari=<?=$rayon_cari;?>&bulan_cari=<?=$bulan_cari;?>&tahun_cari=<?=$tahun_cari;?>&jenis_cari=LUNAS"><?=$proposal_bayar_final;?></a></td>
                        <td></td>
                        <td width="20%">Belum Lunas</td>
                        <td width="25%">: <a target="_blank" href="http://192.168.2.23/jiwasraya/pelaporan/report/detail_dashboard.php?rayon_cari=<?=$rayon_cari;?>&bulan_cari=<?=$bulan_cari;?>&tahun_cari=<?=$tahun_cari;?>&jenis_cari=BELUM_LUNAS"><?=$proposal_belum_bayar; ?></td>
                    </tr>
                    <tr>
                        ...
                        <td style="border-bottom-style: solid;">
                            : <?php echo "Rp. ".(number_format($premi_lunas_perrayon_final,0,',','.')); ?>
                        </td>
                        <td></td>
                        <td style="border-bottom-style: solid;">
                            Premi
                        ...
                        <td style="border-bottom-style: solid;">
                            : <?php echo "Rp. ".(number_format(($potensi_premi_perrayon_final-$premi_lunas_perrayon_final),0,',','.')); ?>
                        </td>
                    </tr>
                    <tr>
                        <td>ANP</td>
                        <td>: <?php echo "Rp. ".(number_format($anp_lunas_perrayon_final,0,',','.')); ?></td>
                        <td></td>
                        <td>ANP</td>
					...
					$sql_d = "SELECT  
				                  ...
				                  (
				                      ...
				                      WHERE PREFIXPERTANGGUNGAN = A.KDKANTOR 
				                          AND KDPERTANGGUNGAN IN ('1', '2') 
				                          AND KDSTATUSEMAIL = '1' 
				                          AND TO_CHAR(TGLSENDEMAIL, 'MM/YYYY') = '$bulan_cari$tahun_cari'
				                          AND KDSTATUSFILE != '7'
				                  ) PROPOSAL,
				                  (
				                      SELECT 
				                  ...
				                  (
				                      ...
				                      WHERE X.PREFIXPERTANGGUNGAN = A.KDKANTOR
				                          ...
				                          AND TO_CHAR(X.TGLSENDEMAIL, 'MM/YYYY') = '$bulan_cari$tahun_cari'
				                          AND X.KDSTATUSFILE != '7'
				                  ) APPROVE,
				                  (
				                      SELECT 
