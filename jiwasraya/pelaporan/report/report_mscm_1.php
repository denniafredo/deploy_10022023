...
<html>
  ...
  <body>
      ...
      <tr bgcolor="#89acd8" align="center">
        <td rowspan="4">NO</td>    
        <td rowspan="4">RAYON</td>
        <td rowspan="4">NAMA SAM</td>
        <td rowspan="4">KANTOR</td>
        <td colspan="12">MONTH TO DATE</td>
        <td colspan="12">YEAR TO DATE</td>
      </tr>
      <tr bgcolor="#89acd8" align="center">
        <td colspan="4">PRO MAPAN</td>
        <td colspan="4">PRO IDAMAN</td>
      ...
      <tr bgcolor="#89acd8" align="center">
        <td colspan="3">REGULER PREMIUM</td>
        <td>SINGLE TOP UP</td>
        <td colspan="3">REGULER PREMIUM</td>
        <td>SINGLE TOP UP</td>
        <td colspan="3">ALL REGULER PREMIUM</td>
        <td>ALL SINGLE TOP UP</td>
        <td colspan="3">REGULER PREMIUM</td>
        <td>SINGLE TOP UP</td>
        <td colspan="3">REGULER PREMIUM</td>
        <td>SINGLE TOP UP</td>
  ...
  ?>
    <tr align="center">
      ...
      <?php
        ...
        foreach ($urutan as $x) {
          if($x == '1'){
            $periode = "AND TO_CHAR(X.MULAS, 'MM/YYYY') = TO_CHAR(SYSDATE, 'MM/YYYY')";
            $periode_topupx = "AND TO_CHAR(W.TGLUPDATED, 'MM/YYYY') = TO_CHAR(SYSDATE, 'MM/YYYY')";
            $periode_topupx_jl23 = "and year(trx_date) = $tahun and month(trx_date) = $bulan";
          }else{
            $periode = "AND TO_CHAR(X.MULAS, 'YYYY') = TO_CHAR(SYSDATE, 'YYYY')";
          ...
          foreach ($produks as $produk) {
            if($produk == '1'){
              $cariproduk = "AND X.KDPRODUK LIKE 'JL4B%'";
            }else if($produk == '2'){
              $cariproduk = "AND X.KDPRODUK LIKE 'JL4X%'";
            }else{
              $cariproduk = "AND X.KDPRODUK LIKE 'JL4%'";
            }
            $sql_l = "SELECT 
                        COUNT(X.NOPOL) POLIS,
                        SUM(
