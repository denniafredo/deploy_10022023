<?  
  ...
  include "../../includes/session.php"; 
	...
	$DBN=new Database("JSADM","JSADM",$DBInfo);	
	?>
  ...
  if(parseInt(navigator.appVersion) >= 4){win.window.focus();}
	...
	echo "<a class=\"verdana10blk\"><b>POLIS KONVERSI KANTOR $kantor (TIDAK ADA DI MASTER)</b></a>";
  echo "<hr size=1>";
	
	//echo "<a class=\"verdana8blk\">Klik </a><a class=verdana8blk href=\"updatepenagihkonversi.php?lihat=$lihat&start=1&end=200&page=1\">disini</a> <a class=\"verdana8blk\">untuk update data per halaman</a>";
	//echo "<a class=\"verdana8blk\">&nbsp;atau klik </a><a class=verdana8blk href=\"updatekonversisatuan.php\">disini</a> <a class=\"verdana8blk\">untuk update data per polis.</a>";
	echo "<br>";
	echo "<div align=center>";
  
	$sqla = "select count(a.nopol) jml from $DBUser.tabel_200_pertanggungan_lost a, ".
...
<script language="javascript">
  ...
  var halcari=document.clntmtc.carihal.value;
	var xxx = parseInt(document.clntmtc.carihal.value);
	  nextpg = xxx + 10;
		if((xxx < 11)||(halcari =='')) {
		  window.location.replace('portopoliokantor_lost.php?action=list&maju=ok&lastpg=11&get='+halcari+'');
    }else {
		  window.location.replace('portopoliokantor_lost.php?action=list&maju=ok&lastpg='+nextpg+'&get='+halcari+'');
		}
	}
</script>
<table>
...
#------------------------------ END NAVIGASI --------------------------------- 
					...
					$sql = "select s.*,x.namaklien1 as namapenagih ".
					       "from ".
								 "$DBUser.tabel_100_klien x,".
					       "(select floor((rownum-1)/40) + 1 k,".
                 "a.nopol,a.prefix,a.nopenagih ".
                 "from ".
                 "$DBUser.tabel_200_pertanggungan_lost a,".
                 ...
                 "a.nopenagih=c.nopenagih) s ".
					...
					$DBN->execute();
					    ...
					    echo("<tr bgcolor=#97b3b9 >");
							...
							echo("<td align=center class=\"verdana7blk\"><b>Prefix</b></td>");
              ...
              //echo("<td class=\"verdana7blk\" align=center><input type=\"submit\" value=\"UPDATE\" name=\"update\" onclick=\"NewWindow(".
							...
							echo("<td class=\"verdana7blk\" align=center>".$arr["NOPOL"]."</td>");
							echo("<td class=\"verdana7blk\">".$prefix."</td>"); 
						  /*
