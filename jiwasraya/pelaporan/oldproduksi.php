...
#------------------------------
  ...
  $DB=new Database($userid, $passwd, $DBName);
	...
	$query = "select to_char(sysdate,'DD/MM/YYYY  HH24:MI:SS') tanggal,".
           ...
           //echo $thisperiode."<br>";
     $qry = "select a.kdkantor,a.namakantor,a.kdjeniskantor,b.proposalprod,c.polisprod,d.proposalprodm,".
		        "e.proposalprodn,f.kliennormal,g.agen,h.penagih ".
						...
						"a.kdkantor = h.kdkantor(+) order by a.kdkantor";
				 //echo "<br>".$qry."<br>";	
	       $DB->parse($qry);
	       $DB->execute();
...
<table border="0" bgcolor="#FFFFFF" cellspacing="1" cellpadding="2">
  ...
  <tr>
	...
	while($ars=$DB->nextrow()){
	 ...
	 $propmed = ($ars["KDJENISKANTOR"] <> '2') ? '' : "<b><a href=\"#\" onclick=\"window.open('detilprodmedical.php?kdkantor=$kdkantor&periode=$thisperiode','updclnt','scrollbars=yes,width=800,height=400,top=100,left=100')\">".udebelon($ars["PROPOSALPRODM"])."</a>";
	 $propnmed = ($ars["KDJENISKANTOR"] <> '2') ? '' : "<b><a href=\"#\" onclick=\"window.open('detilprodnonmedical.php?kdkantor=$kdkantor&periode=$thisperiode','updclnt','scrollbars=yes,width=800,height=400,top=100,left=100')\">".udebelon($ars["PROPOSALPRODN"])."</a>";
