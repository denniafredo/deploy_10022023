...
<table cellpadding="1" cellspacing="2">
 <tr>
	...
	<tr>		
		...
		<td valign="top"><!--<select name="kdstatusfile">-->
			...
			while ($arx=$DB->nextrow()) {
				  echo "<label><input type='checkbox' ";
					if ($arx["KDSTATUSFILE"]==$cb1 || $arx["KDSTATUSFILE"]==$cb2 || $arx["KDSTATUSFILE"]==$cb3 || $arx["KDSTATUSFILE"]==$cb4 || $arx["KDSTATUSFILE"]==$cb5 || $arx["KDSTATUSFILE"]==$cb6 || $arx["KDSTATUSFILE"]==$cb7 || $arx["KDSTATUSFILE"]==$cb8 || $arx["KDSTATUSFILE"]==$cbX){ echo " checked "; }
        ...
        <td></td>
        <? $incb=" in ('".$cb1."','".$cb2."','".$cb3."','".$cb4."','".$cb5."','".$cb6."','".$cb7."','".$cb8."','".$cbX."')";
	...
	<tr>
		...
		<td> </td><td> </td><td> </td>
		<td> </td><td> </td><td> </td>
		<td align="right"> <? if ($kantor=='KP' && $userid=='GUSTIA') { echo "<input name='proses' value='PROSES' type='submit'>";} else {} ?>
		</td>
	</tr>
	<? //$kdjenisgadai=0; ?>
...
//		case 'ALL' : $kdakun = "160000','160001','160002"; $kdvaluta="0','1','2','3"; $kdvaluta2="0','1','2','3"; break;
	...
	if($cv2=='2') {$kdakun = '161001/117140000 & 423.001/125.212.700 & 851.001/361.515.000'; $nmakun = '117140000';}
...
<table border="1" cellpadding="2" style="border-collapse: collapse" width="100%" bordercolor="#09b0ce">
	<tr>
      ...
      <td rowspan="3" bgcolor="#7dc2d9" align="center">ACTION</td>
  </tr>
	<tr>
	...
	$sql ="select sum(MBI_BNG_DR),sum(MSBI_BNG_DR),sum(MSBI_BNG_KR),TGLBILL,STS,sum(SP_AWL),sum(SB_AWL),sum(MBI_PK_D),sum(MBI_BNG_D),sum(MBI_PK_K),sum(MBI_BNG_K),sum(MSBI_PK_D),sum(MSBI_BNG_D),sum(MSBI_PK_K),sum(MSBI_BNG_K),KDVALUTA,STSGADAI,KDKANTOR,sum(SAK_P,SAK_B),sum(NAMASTS) from (select to_char(tglgadai,'dd/mm/yyyy') tgadai, round(MBI_BNG_D,decode(kdvaluta,'3',2,0)) MBI_BNG_DR, round(MSBI_BNG_D,decode(kdvaluta,'3',2,0)) MSBI_BNG_DR, round(MSBI_BNG_K, decode(kdvaluta,'3',2,0)) MSBI_BNG_KR,(select nopol from $DBUser.tabel_200_pertanggungan where prefixpertanggungan=l.prefixpertanggungan and nopertanggungan=l.nopertanggungan) nopol,L.*, (NVL(SP_AWL,0)+NVL(MSBI_PK_D,0)-NVL(MSBI_PK_K,0)) SAK_P, ROUND(NVL(SB_AWL,0),decode(kdvaluta,'3',2,0))+ROUND(NVL(MSBI_BNG_D,0),decode(kdvaluta,'3',2,0))-ROUND(NVL(MSBI_BNG_K,0),decode(kdvaluta,'3',2,0)) SAK_B, (select namastatusfile from $DBUser.TABEL_299_STATUS_FILE where kdstatusfile=l.sts) namasts from $DBUser.TABEL_700_LAMNER L where tglbill=TO_DATE ('$year$month', 'YYYYMM') AND KDKANTOR='$kdkantor' and kdvaluta ".$incv." and stsgadai ".$incs." and sts ".$incb." union ".
	"select to_char(tglgadai,'dd/mm/yyyy') tgadai, round(MBI_BNG_D,decode(kdvaluta,'3',2,0)) MBI_BNG_DR, round(MSBI_BNG_D,decode(kdvaluta,'3',2,0)) MSBI_BNG_DR, round(MSBI_BNG_K, decode(kdvaluta,'3',2,0)) MSBI_BNG_KR,".
	"(select nopol from $DBUser.tabel_200_pertanggungan where prefixpertanggungan=x.prefixpertanggungan and nopertanggungan=x.nopertanggungan) nopol,x.*, (NVL(SP_AWL,0)+NVL(MSBI_PK_D,0)-NVL(MSBI_PK_K,0)) SAK_P, ROUND(NVL(SB_AWL,0),decode(kdvaluta,'3',2,0))+ROUND(NVL(MSBI_BNG_D,0),decode(kdvaluta,'3',2,0))-ROUND(NVL(MSBI_BNG_K,0),decode(kdvaluta,'3',2,0)) SAK_B, (select namastatusfile from $DBUser.TABEL_299_STATUS_FILE where kdstatusfile=x.sts) namasts from $DBUser.tabel_700_lamner x where kdkantor='$kdkantor' and sts IN ('1','3','4') and kdvaluta ".$incv." and stsgadai ".$incs."
			 ...
			 WHERE   x.kdvaluta = $cv3
					 AND x.tglkursberlaku =
						   (SELECT   MAX (tglkursberlaku)
							 ...
							 WHERE   x.kdvaluta = $cv3
									 AND y.tglkursberlaku <= to_date('$year$month','YYYYMM'))";	
	//echo $sql;
	//echo $kdvaluta; 
	$DB->parse($sql);
	$DB->execute();
