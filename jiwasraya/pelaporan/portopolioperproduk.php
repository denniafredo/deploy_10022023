<?  
  ...
  include "../../includes/session.php"; 
	...
	echo "<title>DAFTAR POLIS PER PRODUK</title>";
	?>
  ...
  if(parseInt(navigator.appVersion) >= 4){win.window.focus();}
	...
	<?
	if ($vbln=="0"){
		$vthn=$vthn ? date("m/Y"): date("Y") ;
		$periode="to_char(a.mulas,'YYYY')='$vthn' and ";
	} else {	
		$periode="to_char(a.mulas,'MMYYYY')='".substr("0".$vbln,-2)."$vthn' and ";
		//$periode="to_char(a.mulas,'MMYYYY')='".date("mY")."' and ";
	}

  switch($vbln)
  {
  	case '0' : $namabulan = $vthn; break ;
    case '1' : $namabulan ="JANUARI ".$vthn; break ;
    case '2' : $namabulan ="FEBRUARI ".$vthn; break ;
    case '3' : $namabulan ="MARET ".$vthn; break ;
    case '4' : $namabulan ="APRIL ".$vthn; break ;
    case '5' : $namabulan ="MEI ".$vthn; break ;
    case '6' : $namabulan ="JUNI ".$vthn; break ;
    case '7' : $namabulan ="JULI ".$vthn; break ;
    case '8' : $namabulan ="AGUSTUS ".$vthn; break ;
    case '9' : $namabulan ="SEPTEMBER ".$vthn; break ;
    case '10' : $namabulan ="OKTOBER ".$vthn; break ;
    case '11' : $namabulan ="NOVEMBER ".$vthn; break ;
    case '12' : $namabulan ="DESEMBER ".$vthn; break ;
	}
	
	echo "<link href=\"../jws.css\" rel=\"stylesheet\" type=\"text/css\">";
	echo "<body bgcolor=#cee0ff>";
	echo "<a class=\"verdana10blk\"><b>INFORMASI POLIS PRODUK $kdproduk KANTOR $kdkantor PERIODE $namabulan</b></a>";
  echo "<hr size=1>";
	...
	$sqla = "select count(a.kdvaluta) jml from $DBUser.tabel_200_pertanggungan a, ".
	        ...
	        "where a.nopenagih=b.nopenagih ".
					"and a.kdpertanggungan='2' and a.kdstatusfile='1' ".
...
<script language="javascript">
  ...
  var halcari=document.clntmtc.carihal.value;
	var kdproduk=document.clntmtc.kdproduk.value;
	var kdkantor=document.clntmtc.kdkantor.value;
	var vbln=document.clntmtc.vbln.value;
	var vthn=document.clntmtc.vthn.value;
	var xxx = parseInt(document.clntmtc.carihal.value);
	  nextpg = xxx + 10;
		if((xxx < 11)||(halcari =='')) {
		  window.location.replace('portopolioperproduk.php?action=list&maju=ok&lastpg=11&get='+halcari+'&kdproduk='+kdproduk+'&kdkantor='+kdkantor+'&vbln='+vbln+'&vthn='+vthn+'');
    }else {
		  window.location.replace('portopolioperproduk.php?action=list&maju=ok&lastpg='+nextpg+'&get='+halcari+'&kdproduk='+kdproduk+'&kdkantor='+kdkantor+'&vbln='+vbln+'&vthn='+vthn+'');
		}
	}
</script>
<table>
...
if($get<11){
 ...
 $urlp = $PHP_SELF . "?action=list&mundur=ok&lastpg=$xx&get=$linkp&users=$users&kdproduk=$kdproduk&kdkantor=$kdkantor&vbln=$vbln&vthn=$vthn"; 
}
...
if ($mundur){
   $a = $xx + 10;
   $url = $PHP_SELF . "?action=list&mundur=ok&lastpg=$a&get=$i&users=$users&kdproduk=$kdproduk&kdkantor=$kdkantor&vbln=$vbln&vthn=$vthn"; 
} else {
   $a = $total_pages;
   $url = $PHP_SELF . "?action=list&maju=ok&lastpg=$a&get=$i&users=$users&kdproduk=$kdproduk&kdkantor=$kdkantor&vbln=$vbln&vthn=$vthn"; 
}
	if ($get==$i) {
...
$stop = $xx - 10;
//echo $stop."<br>";
$urln = $PHP_SELF . "?action=list&maju=ok&lastpg=$xx&get=$linkm&users=$users&kdproduk=$kdproduk&kdkantor=$kdkantor&vbln=$vbln&vthn=$vthn";
if($stop > $pg){ 
...
<td>
 ...
 <input type="hidden" name="vthn" value="<? echo $vthn; ?>">
</td>
<td class=verdana10blk>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
<? echo "Halaman : ".$get; ?>
</td>
...
#------------------------------ END NAVIGASI ---------------------------------
		  ...
		  $sql="select ".
           ...
           "from ".
                 "(select floor((rownum-1)/10000000) + 1 k,".
