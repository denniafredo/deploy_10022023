<?
  ...
  include "../../includes/database.php";
	...
	if ($kdkantor=='all') {
	$wkantor=" ";
	} else {$wkantor=" and KDKANTOR='$kdkantor' ";}
	//echo $wkantor;
    ...
    <td>Kantor</td>
		<td><select name="kdkantor">
  			...
  			$sqa="select kdkantor,namakantor from $DBUser.tabel_001_kantor where kdjeniskantor='2' ".
  					...
  					$DB->parse($sqa);
  					$DB->execute();	
					echo "<option value='all'>SELURUH KANTOR</option>";
...
<table border="1" cellpadding="2" style="border-collapse: collapse" width="100%" bordercolor="#09b0ce">
    ...
    WHERE   prefixpertanggungan = XX.prefixpertanggungan
				...
				TGLBILL,
										  ...
										  KDVALUTA,
										  STSGADAI,
										  KDKANTOR, (NVL(SP_AWL,0)+NVL(MSBI_PK_D,0)-NVL(MSBI_PK_K,0)) SAK_P, ROUND(NVL(SB_AWL,0),decode(kdvaluta,'3',2,0))+ROUND(NVL(MSBI_BNG_D,0),decode(kdvaluta,'3',2,0))-ROUND(NVL(MSBI_BNG_K,0),decode(kdvaluta,'3',2,0)) SAK_B, (select namastatusfile from $DBUser.TABEL_299_STATUS_FILE where kdstatusfile=x.sts) namasts from $DBUser.tabel_700_lamner x where sts IN ('1','3','4','8','L') $wkantor and kdvaluta ".$incv." and stsgadai ".$incs."
