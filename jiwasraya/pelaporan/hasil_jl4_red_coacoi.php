...
$KTR=new Kantor('nadm','JSADMOKE',$kantor);
*/
...
<form name="getRedCOACOI" action="<?=$PHP_SELF;?>" method="post">
	<table>
		...
		<tr>
			<td>S.D TANGGAL</td><td><?=DateSelector("d"); ?></td><!--<td>Sampai</td><td><?=DateSelector("s"); ?></td>-->
...
?>
	...
	<tr>
		...
		<?php		
			...
			/*if($kodeKantor == "ALL")
			{ */
			/**	$sql = "SELECT   KET, AKUN, DEBET, KREDIT
    FROM   tabel_802_trvouc@gllink
   WHERE   TO_CHAR (TGL_TRANS, 'dd/mm/yyyy')  = '" .$tglawalInd. "' and (KET LIKE '%COI%' OR KET LIKE '%COA%' OR KET LIKE '%COR%')
ORDER BY   TGL_TRANS DESC";*/
				$sql = "SELECT   KET, AKUN, DEBET, KREDIT
    FROM   tabel_802_trvouc@gllink
   WHERE   TO_CHAR (TGL_TRANS, 'dd/mm/yyyy')  = '" .$tglawalInd. "' and (KET LIKE '%IFG Life Prime Protection%' or KET LIKE '%IFG Ultimate Protection%')
ORDER BY   KET";
			/*	
			}
			else
			...
			while($arr = $DB->nextrow())
			{
				...
				echo "<td>".$arr["KET"]."</td>";
				echo "<td>".$arr["AKUN"]."</td>";
				echo '<td align="right">'.number_format($arr["DEBET"],2,",",".")."</td>";
				echo '<td align="right">'.number_format($arr["KREDIT"],2,",",".")."</td>";
