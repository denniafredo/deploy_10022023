	...
	$sqla = "select to_char((last_day(sysdate) + 5),'DD/MM/YYYY') lastbill from dual";
	   ...
	   $qry = "select a.kdkantor,a.namakantor,a.kdjeniskantor,".
		        ...
		        "e.proposalprodn,".
						...
						"where  to_char(v.mulas,'yyyy')='$thisperiode' ".
...
echo "<font face=verdana size=2><b>Transaksi Penjualan Polis NBPP Tahun $vthn</b><br><br>";//Tanggal $beginbilling s/d $lastbilling</b></font>";
	while($ars=$DB->nextrow()){
	 ...
	 $propmed = ($ars["KDJENISKANTOR"] <> '2') ? '' : "<b><a href=\"#\" onclick=\"window.open('propstatusmedical.php?kdkantor=$kdkantor&periodeth=$thisperiode&sttmed=M','updclnt','scrollbars=yes,width=800,height=400,top=100,left=100')\">".udebelon($ars["PROPOSALPRODM"])."</a>";
	 $propnmed = ($ars["KDJENISKANTOR"] <> '2') ? '' : "<b><a href=\"#\" onclick=\"window.open('propstatusmedical.php?kdkantor=$kdkantor&periodeth=$thisperiode&sttmed=N','updclnt','scrollbars=yes,width=800,height=400,top=100,left=100')\">".udebelon($ars["PROPOSALPRODN"])."</a>";
	 $proptolak = ($ars["KDJENISKANTOR"] <> '2') ? '' : "<b><a href=\"#\" onclick=\"window.open('propstatusaccept.php?kdkantor=$kdkantor&periodeth=$thisperiode&sttacc=tolak','updclnt','scrollbars=yes,width=800,height=400,top=100,left=100')\">".udebelon($ars["PROPOSALTOLAK"])."</a>";
	 $proptunda = ($ars["KDJENISKANTOR"] <> '2') ? '' : "<b><a href=\"#\" onclick=\"window.open('propstatusaccept.php?kdkantor=$kdkantor&periodeth=$thisperiode&sttacc=tunda','updclnt','scrollbars=yes,width=800,height=400,top=100,left=100')\">".udebelon($ars["PROPOSALTUNDA"])."</a>";
