 ...
 //echo $periodenya;
  ...
  echo "<hr size=1>";
	echo "<div align=center>";
  				 echo "<font color=\"003399\" face=verdana size=2><b>NOTA DEBET  KANTOR $kantor PERIODE ".strtoupper($blnn)." $vthn KUITANSI $kdkuitansi</b></font><br><br>";
...
sum(decode(a.kdvaluta,'0',a.premitagihan/b.indexawal,'1',a.premitagihan,'3',a.premitagihan)) jmlpremi
 ...
 WHERE       a.prefixpertanggungan = b.prefixpertanggungan
		 ...
		 if($kantor=="KP"){
			 $sql="select blnbooked,kdkuitansi,valuta,kdrekeningpremi,sum(jmlkwt) jmlkwt ,cabas,sum(jmlpremi) jmlpremi,sum(JMLNILAIRP)JMLNILAIRP,statusbilling,
			 nvl((select kurs from $DBUser.tabel_999_kurs where kdvaluta=a.kdvaluta and trunc(tglkursberlaku,'month')=trunc(blnbooked,'month')),1) kurs_standar from $DBUser.TABEL_300_NOTADEBET a where substr(kdkuitansi,1,2) = '$kdkuitansi'
         AND TO_CHAR (blnbooked, 'mmyyyy') = '$periodenya' and statusbilling ='NOTADEBET'
		 group by blnbooked,KDKUITANSI,KDREKENINGLAWAN,KDREKENINGPREMI,KDKUITANSI,VALUTA,CABAS,statusbilling,kurs_standar,kdvaluta";
		 }else{
			 $sql="select * from $DBUser.TABEL_300_NOTADEBET where kdkantor = '$kantor' AND substr(kdkuitansi,1,2) = '$kdkuitansi'
         AND TO_CHAR (blnbooked, 'mmyyyy') = '$periodenya' and statusbilling ='NOTADEBET'";
		 }
		 ...
		 $i=1;	 
			...
			echo "<td align=right>".number_format($arr["JMLNILAIRP"],2)."</td>";
			echo "<td align=right>";
			if($arr["KDVALUTA"]=="3"){
				 $kursnya=$kursdolar;
			...
			echo number_format($arr["KURS_STANDAR"],2)."</td>";
			echo "<td align=right>".number_format($arr["JMLNILAIRP"] *$arr["KURS_STANDAR"],2,",",".")."</td>";
			echo "</tr>";
			$i++;
