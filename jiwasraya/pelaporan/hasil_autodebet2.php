...
<table border="0" cellpadding="2" cellspacing="1" style="border-collapse: collapse" bordercolor="#CCCCCC" width="100%" id="AutoNumber1">
  ...
  $sql = "select ".
          ...
          "where ".
            ...
            "to_char(a.tglupdated,'MMYYYY')='$bulancari' and ".
						//to_char(a.tglupdated,'DDMMYYYY') between '28112005' and  '31112005' and 
  ...
  foreach ($arr as $foo => $row ) {
    ...
    <td style="border-left-width: 1; border-right-width: 1; border-top: 1px solid #D8E1FA; border-bottom: 1px solid #D8E1FA"><?=substr($arr["NAMAKLIEN"],6,100);?></td>
	...
	<tr bgcolor="#f5d79c">
	 ...
	 <td><?=number_format(${"totkomisith1".$cabas},2,",",".");?></td>
	 <td><?=number_format(${"totkomisith2".$cabas},2,",",".");?></td>
	 <td><?=number_format(${"totkomisith3".$cabas},2,",",".");?></td>
	 <td></td>
	</tr>
	<tr bgcolor="#f5d79c">
	 <td></td>
...
<table border="0" cellpadding="2" cellspacing="1" style="border-collapse: collapse" bordercolor="#CCCCCC" width="100%" id="AutoNumber1">
  ...
  foreach ($arr as $foo => $row ) {
            ...
            "(select nilaimeterai from $DBUser.tabel_999_batas_materai ".
             ...
             ") as matere, ".
      	       ...
      	       "(select max(tglkursberlaku) from $DBUser.tabel_999_kurs_transaksi ".
      	     	 "where tglkursberlaku<=a.tglupdated and kdvaluta='0') ".
      	     	 "and kdvaluta='0' ) kurs ".
          "from ".
            "$DBUser.tabel_300_historis_premi d,".
            "$DBUser.tabel_200_pertanggungan b,".
  	  ...
  	  $kdkui_di = substr($arr["KDKUITANSI"],0,2);
  		if($kdkui_di=="NB")
  		{
  		  $preminb_di = $arr["PREMITAGIHAN"];
  			...
  			$discountnb_di = (($arr["PREMITAGIHAN"]/$arr["INDEXAWAL"]) * 0.01 * $arr["KURS"]);
  			$discountob_di = 0;
  		...
  		elseif($kdkui_di=="OB")
  		{
  		  $premiob_di = $arr["PREMITAGIHAN"];
  			...
  			//$discountob_di = ($arr["PREMITAGIHAN"]* 0.01);
				$discountob_di = (($arr["PREMITAGIHAN"]/$arr["INDEXAWAL"]) * 0.01 * $arr["KURS"]);
  			$discountnb_di = 0;
    ...
    <? 
		...
		?>
		<td style="border-left-width: 1; border-right-width: 1; border-top: 1px solid #D8E1FA; border-bottom: 1px solid #D8E1FA" align="right"><?=//number_format($arr["PREMITAGIHAN"],2,",",".");?><?=number_format($premihitung_di,2,",",".");?></td>
		  ...
		  <?
			...
			else
			{
			  //$discount_di = ($arr["PREMITAGIHAN"]* 0.01);
				...
				$discount_di = $premihitung_di * 0.01 * $arr["KURS"];
			}
	...
	<tr bgcolor="#f5d79c">
	 ...
	 <td><?=number_format(${"totkomisith1_di".$cabas},2,",",".");?></td>
	 <td><?=number_format(${"totkomisith2_di".$cabas},2,",",".");?></td>
	 <td><?=number_format(${"totkomisith3_di".$cabas},2,",",".");?></td>
	 <td></td>
	</tr>
	<tr bgcolor="#f5d79c">
	 <td></td>
  ...
  foreach ($arr as $foo => $row ) {
	...
	$sql = "select ".
            ...
            "(select nilaimeterai from $DBUser.tabel_999_batas_materai ".
             ...
             ") as matere, ".
						"(select kurs from $DBUser.tabel_999_kurs_transaksi where tglkursberlaku=".
      	       "(select max(tglkursberlaku) from $DBUser.tabel_999_kurs_transaksi ".
      	     	 "where tglkursberlaku<=a.tglupdated and kdvaluta='3') ".
      	     	 "and kdvaluta='3' ) kurs ".
          "from ".
            "$DBUser.tabel_300_historis_premi d,".
            "$DBUser.tabel_200_pertanggungan b,".
	...
	*/
	...
	<tr bgcolor="#f5d79c">
	 ...
	 <td><?=number_format(${"totkomisith1_a".$cabas},2,",",".");?></td>
	 <td><?=number_format(${"totkomisith2_a".$cabas},2,",",".");?></td>
	 <td><?=number_format(${"totkomisith3_a".$cabas},2,",",".");?></td>
	 <td></td>
	</tr>
	<tr bgcolor="#f5d79c">
	 <td></td>
