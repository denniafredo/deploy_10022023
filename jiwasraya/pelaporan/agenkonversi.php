...
$fieldname=(!$b) ? "noagen" : $b;	
  ...
  $sql = "select a.noagen, b.namaklien1 from $DBUser.tabel_100_klien b,$DBUser.tabel_400_agen a ".
	       "where b.noklien=a.noagen and a.prefixagen='".$kantor."' order by b.namaklien1";
	//echo $sql;
	$DB->parse($sql);
	$DB->execute();
?>
...
<?
	...
	while($arr=$DB->nextrow()) {
	  include "../../includes/belang.php";
		...
		$namaagen=str_replace("'","`",$namaagen);
		$a="<td class=arial10ungu><a href=\"#\" onclick=\"javascript:".
		"window.opener.document.updatekonversi.noagenbaru.value='".$arr["NOAGEN"]."';".
		"window.opener.document.updatekonversi.namaagenbaru.value='".$namaagen."';".
