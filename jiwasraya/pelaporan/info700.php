...
$kim = ($bl=='13') ? "to_char(a.tglrekam,'YYYY')='".$th."' and " : "to_char(a.tglrekam,'MMYYYY')='".$bl.$th."' and ";
  ...
  $sql = "select ".
    			 	 "a.nomorsip,a.prefixpertanggungan,a.nopertanggungan,a.userrekam,b.namastatus,a.status, '".strtoupper($jns)."' kdklaim, ".
      		 	 "to_char(a.tglmohon,'DD/MM/YYYY') tglmohon,to_char(a.".$tglhitung.",'DD/MM/YYYY') tglhitung,nvl(a.metodebayar,'TRANSFER') metodebayar,nvl(approvekasi,'0') approvekasi,".
	...
	$sqa =  "select DISTINCT e.kdproduk,f.jenispengajuan,a.approveho,".
      		 		"a.nomorsip,a.prefixpertanggungan,a.nopertanggungan,a.userrekam,c.namastatus,nvl(a.status,'0') status, nvl(a.metodebayar,'TRANSFER') metodebayar,nvl(approvekasi,'0') approvekasi,".
      		    "a.kdklaim, ".
				"b.namaklaim,to_char(a.tglpengajuan,'DD/MM/YYYY') tglmohon,to_char(a.tglpengajuan,'yyyymmdd') tglmulai, ".
  				...
  				"where ".
    				  $kim.
      				"a.kdklaim=b.kdklaim ".
      				"and a.prefixpertanggungan=e.prefixpertanggungan and a.nopertanggungan=e.nopertanggungan and nvl(a.klaimgroup,'0') not in ('1','2') ".
      			  "and e.nopenagih=d.nopenagih ".
				  //"and a.klaimgroup not in ('2') ".
  				...
  				"order by status,a.prefixpertanggungan,a.nopertanggungan ";
...
while ($arr=$DB->nextrow()) {
	...
	$lanjut = ""; // untuk clear cache
		...
		$tglotorisasi = (!$arr["TGLOTORISASI"]=='') ? $arr["TGLOTORISASI"] : "<font color=red>-</font>";
			...
			case 'tebus':
			 switch ($status) {
			  ...
			  case '1':
			     ...
			     //echo $arr["APPROVEKASI"];
				 if (($arr['APPROVEHO'] != '1' && ($arr['KDPRODUK'] != 'JSR1' && $arr['KDPRODUK'] != 'JSR2' && $arr['KDPRODUK'] != 'JSR3' && $arr['KDPRODUK'] != 'JSR4')) || ($arr['APPROVEHO'] == '1' && ($arr['KDPRODUK'] == 'JSR1' || $arr['KDPRODUK'] == 'JSR2' || $arr['KDPRODUK'] == 'JSR3' || $arr['KDPRODUK'] != 'JSR4'))){
					$include_list=array("AC","AE","AF","BI","MG","QB","QC","QD","QE");
					$include_klaim=array("ANUITAS","TAHAPAN","EXPIRASI","BEASISWA", "EXPIRASIRB","MENINGGAL","MENINGGALX","TEBUS","OTHERS");
					$exclude_produk=array("ATP","DGI");
					
					if($arr["METODEBAYAR"]=="TRANSFER"){
						if(((in_array($kdklaim,array("ANUITAS","EXPIRASI")))) || $kantor == 'KN') {
							...
							if($arr["APPROVEKASI"]=="1" && $modul=='2AL'){
								$lanjut = "<a href=\"#\" onclick=\"NewWindow('../akunting/verifikasi_sip_klaim_cabang.php?kdsip=$kdklaim&pertanggungan=".$prefix."-".$noper."&nomorsip=".$arr["NOMORSIP"]."&tglsip=".$arr["TGLSIP"]."&tglpengajuan=".$arr["TGLMOHON"]."','',700,400,1)\">VERIFIKASI PEMBAYARAN</a>";
							}elseif($arr["APPROVEKASI"]=="0" && $modul!='2AL'){
					...
					//}elseif(in_array($kantor, $include_list) && in_array($kdklaim,$include_klaim) && $arr["KDVALUTA"]=='1' && !in_array($PER->produk,$exclude_produk) && $arr["METODEBAYAR"]=="TRANSFER"){
					/*}elseif(in_array($kdklaim,$include_klaim) && $arr["KDVALUTA"]=='1' && !in_array($PER->produk,$exclude_produk) && $arr["METODEBAYAR"]=="TRANSFER"){
							if($arr["APPROVEKASI"]=="1" && $modul=='2AL'){
								$lanjut = "<a href=\"#\" onclick=\"NewWindow('../akunting/verifikasi_sip_klaim_cabang.php?kdsip=$kdklaim&pertanggungan=".$prefix."-".$noper."&nomorsip=".$arr["NOMORSIP"]."&tglsip=".$arr["TGLSIP"]."&tglpengajuan=".$arr["TGLMOHON"]."','',700,400,1)\">VERIFIKASI PEMBAYARAN</a>";
							}elseif($arr["APPROVEKASI"]=="0" && $modul!='2AL'){
								$lanjut = "<a href=\"#\" onclick=\"NewWindow('../akunting/validasi_sip_klaim_cabang.php?kdsip=$kdklaim&pertanggungan=".$arr[PREFIXPERTANGGUNGAN]."-".$arr[NOPERTANGGUNGAN]."&nomorsip=".$arr["NOMORSIP"]."&tglsip=".$arr["TGLSIP"]."&tglpengajuan=".$arr["TGLMOHON"]."','',700,400,1)\">VALIDASI PEMBAYARAN</a>";
			  ...
			  case '2':
			   ...
			   //$cetaksip =  "<a href=# onclick=NewWindow('../polis/cetaksip_tebus.php?kdsip=$jenis&pertanggungan=".$arr["PREFIXPERTANGGUNGAN"]."-".$arr["NOPERTANGGUNGAN"]."&nomorsip=".$arr["NOMORSIP"]."&tglsip=".$arr["TGLSIP"]."','',700,400,1)>CETAK SIP</a>";
				$exclude_produk=array("ATcP","DsGI");
				if($arr["METODEBAYAR"]=="TRANSFER" && !in_array($PER->produk,$exclude_produk)){				   	   
						  $lanjut="";					   
				   }else{
			 ...
			 switch ($status) {
			  ...
			  case '1':
				 if(substr($PER->produk,0,2)=="PAs")
				 {
				   ...
				   if (($arr['APPROVEHO'] != '1' && ($arr['KDPRODUK'] != 'JSR1' && $arr['KDPRODUK'] != 'JSR2' && $arr['KDPRODUK'] != 'JSR3' && $arr['KDPRODUK'] != 'JSR4')) || ($arr['APPROVEHO'] == '1' && ($arr['KDPRODUK'] == 'JSR1' || $arr['KDPRODUK'] == 'JSR2' || $arr['KDPRODUK'] == 'JSR3' || $arr['KDPRODUK'] != 'JSR4'))){
					$include_list=array("AC","AE","AF","BI","MG","QB","QC","QD","QE");
					$include_klaim=array("ANUITAS","TAHAPAN","EXPIRASI","BEASISWA", "EXPIRASIRB","MENINGGAL","MENINGGALX",'TEBUS','RAWATINAP',"OTHERS");
					$exclude_produk=array("ATdP","DGsI");
					...
					if(in_array($kdklaim,$include_klaim) && $arr["METODEBAYAR"]=="TRANSFER"){
						//if(((in_array($kdklaim,array("ANUITAS","EXPIRASI")))) || $kantor == 'KN') {
						if((in_array($PER->produk,$exclude_produk) && (in_array($kdklaim,array("ANUITAS","EXPIRASI")))) || $kantor == 'KN') {
							$lanjut = "<a href=\"#\" onclick=\"window.location.replace('../polis/pengajuanklaimFF.php?prefix=$prefix&noper=$noper&kdklaim=$kdklaim&tglpengajuan=".$arr["TGLMOHON"]."')\">KLIK DISINI</a>";
						}else{
							if($arr["APPROVEKASI"]=="1" && $modul=='2AL'){
								$lanjut = "<a href=\"#\" onclick=\"NewWindow('../akunting/verifikasi_sip_klaim_cabang.php?kdsip=$kdklaim&pertanggungan=".$prefix."-".$noper."&nomorsip=".$arr["NOMORSIP"]."&tglsip=".$arr["TGLSIP"]."&tglpengajuan=".$arr["TGLMOHON"]."','',700,400,1)\">VERIFIKASI PEMBAYARAN</a>";
				...
				case '2':
				$include_list=array("AC","AE","AF","BI","MG","QB","QC","QD","QE");
					$include_klaim=array("ANUITAS","TAHAPAN","EXPIRASI","BEASISWA", "EXPIRASIRB","MENINGGAL","MENINGGALX");
					$exclude_produk=array("AdTP","DGsI");
					$include_klaim_nas=array("TAHAPAN","EXPIRASI","BEASISWA", "EXPIRASIRB","MENINGGAL","MENINGGALX");
				...
				{				   
				   if(in_array($kdklaim,$include_klaim) && $arr["METODEBAYAR"]=="TRANSFER"){
				   	   if(($arr["TGLMULAI"]<="20171031") || in_array($PER->produk,$exclude_produk) || $kantor == 'KN'){
					   $lanjut = "<a href=\"#\" onclick=\"window.location.replace('../akunting/bayar.php?kdklaim=".$arr["KDKLAIM"]."&prefix=$prefix&noper=$noper')\">KLIK DISINI</a>";
						}else{
							  $lanjut="";
					   }
				...
				// }else{
				   ...
				   //}else
					//if(in_array($kantor, $include_list) && in_array($kdklaim,$include_klaim) && $arr["KDVALUTA"]=='1' && !in_array($PER->produk,$exclude_produk) && $arr["METODEBAYAR"]=="TRANSFER"){
					if(in_array($kdklaim,$include_klaim) && $arr["METODEBAYAR"]=="TRANSFER"){
				   	  if(in_array($PER->produk,$exclude_produk) || $kantor == 'KN'){
			 ...
			 switch ($status) {
			  ...
			  case '1':
				 if(substr($PER->produk,0,2)=="PAs")
				 {
				   ...
				   if (($arr['APPROVEHO'] != '1' && ($arr['KDPRODUK'] != 'JSR1' && $arr['KDPRODUK'] != 'JSR2' && $arr['KDPRODUK'] != 'JSR3' && $arr['KDPRODUK'] != 'JSR4')) || ($arr['APPROVEHO'] == '1' && ($arr['KDPRODUK'] == 'JSR1' || $arr['KDPRODUK'] == 'JSR2' || $arr['KDPRODUK'] == 'JSR3' || $arr['KDPRODUK'] != 'JSR4'))){
					$include_list=array("AC","AE","AF","BI","MG","QB","QC","QD","QE");
					$include_klaim=array("ANUITAS","TAHAPAN","EXPIRASI","BEASISWA", "EXPIRASIRB","MENINGGAL","MENINGGALX",'TEBUS','RAWATINAP',"OTHERS","CASHPLAN");
					$exclude_produk=array("ATdP","DGsI");
					//if(in_array($kantor, $include_list) && in_array($kdklaim,$include_klaim) && $arr["KDVALUTA"]=='1' && !in_array($PER->produk,$exclude_produk) && $arr["METODEBAYAR"]=="TRANSFER"){
					//if(in_array($kantor, $include_list) && in_array($kdklaim,$include_klaim) && $arr["TGLMULAI"]>="20170306" && $arr["KDVALUTA"]=='1' && !in_array($PER->produk,$exclude_produk) && $arr["METODEBAYAR"]=="TRANSFER"){
					if(in_array($kdklaim,$include_klaim) && $arr["METODEBAYAR"]=="TRANSFER"){
		  ...
		  if (($arr["KDKLAIM"]=='TAHAPAN'||$arr["KDKLAIM"]=='EXPIRASI' || $jns=='tebus' ||$arr["KDKLAIM"]=='EXPIRASIRB' ||$arr["KDKLAIM"]=='ANUITAS' || $arr["KDKLAIM"]=='CASHPLAN' ||$arr["KDKLAIM"]=='BEASISWA' ||substr($arr["KDKLAIM"],0,9)=='MENINGGAL'||$arr['KDKLAIM']=='RAWATINAP') && $jmlrow>0){
		  ...
		  else if (($arr["KDKLAIM"]=='ANUITAS') && $jmlrow>0) {
		  print( "	<td class=verdana8blu align=\"left\"><a href=\"#\" onclick=\"NewWindow('../akunting/memo_klaim_n.php?kdklaim=".$arr["KDKLAIM"]."&prefix=".$arr["PREFIXPERTANGGUNGAN"]."&noper=".$arr["NOPERTANGGUNGAN"]."&tglgadai=".$arr["TGLHITUNG"]."&tglmohon=".$arr["TGLMOHON"]."','',750,300,1);\">CETAK MEMORIAL</a></td>\n" );		  
		  }else if ($jns != 'gadai') {
		  print( "	<td class=verdana8blu align=\"left\"><a href=\"#\" onclick=\"NewWindow('../akunting/create_memoklaim_n.php?prefix=".$arr["PREFIXPERTANGGUNGAN"]."&noper=".$arr["NOPERTANGGUNGAN"]."&nomorsip=".$arr["NOMORSIP"]."&kdklaim=".$arr["KDKLAIM"]."&tglgadai=".$arr["TGLHITUNG"]."&tglmohon=".$arr["TGLMOHON"]."&kdklaim=".$arr["KDKLAIM"]."','',750,300,1);window.location.reload();\">PROSES MEMORIAL</a></td>\n" );
			...
			print( " </tr>" ); //echo $jns;
			//echo "$noper#".$arr["KDKLAIM"]."# Jumlah row -".$jmlrow."<br>";
		 $i++;
