<?  
	...
	<a class="verdana10blk\"><b>KONTES "REWARD 156" (Katagori Polis)</b></a><br />
    <?php
	/* edit by fendy untuk tidak menampilkan di menu jaim 01/10/2015 */
    if ($_GET['dari_jaim'] == 'betul') {
		echo "<a class=verdana10blk href=\"info156premi.php?dari_jaim=betul\">Katagori Premi klik disini</a>";
	}
	else {
		echo "<a class=verdana10blk href=\"info156premi.php\">Katagori Premi klik disini</a>";
	}
	/* akhir dari edit by fendy */
	?>
	<hr size=1>
		...
		/*		  $sql = "select a.prefixpertanggungan,a.nopertanggungan,a.kdproduk,".
         ...
         (SELECT   namaareaoffice
           ...
           WHERE   kdkantor = x.kdkantor AND kdareaoffice = x.kdareaoffice)
            area,
         (SELECT   namaunitproduksi
            FROM   $DBUser.TABEL_410_KODE_UNIT_PRODUKSI
           WHERE       kdkantor = x.kdkantor
                   AND kdareaoffice = x.kdareaoffice
                   AND kdunitproduksi = x.kdunitproduksi)
            unit,
         polis,
         premi
  FROM   (  SELECT   b.kdkantor,
                     b.kdareaoffice,
                     b.kdunitproduksi,
                     COUNT ( * ) polis,
                     SUM (premi1) premi
             ...
             WHERE mulas between to_date('01/10/2015', 'dd/mm/yyyy') and to_date('31/12/2015', 'dd/mm/yyyy')
                     ...
                     AND kdjabatanagen IN ('00', '05', '09')
          GROUP BY   b.kdkantor, b.kdareaoffice, b.kdunitproduksi
          ORDER BY   COUNT ( * ) DESC, SUM(premi1) DESC) x";       // Permintaan Ica 1 Oktober 2014 Oleh Dedi
						 //echo "ORDER BY   prefixpertanggungan, nopertanggungan) order by tglpengajuan desc";
					
					//echo $sql;
					$DB->parse($sql);
					$DB->execute();
					 echo "<font color=\"003399\" face=verdana size=2><b>Daftar Polis Kontes Reward 156 Katagori Polis</b></font><br>";
		...
		//$found=$DB2->nextrow();
		   ...
		   echo("<tr><td colspan=4><font face=\"Verdana\" size=\"1\">TOTAL</font></td>
