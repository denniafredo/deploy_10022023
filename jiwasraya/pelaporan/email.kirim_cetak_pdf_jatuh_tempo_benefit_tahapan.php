...
//echo 'masuk ddn'; die;
	...
	//while ($rowxx = oci_fetch_array($dbxx->get_statement())) {
		...
		echo 'Kirim email ke '.$prefixpertanggungan.$nopertanggungan.$kdkantor.'<br>';
			...
			$pdf->Cell(5,4,':',0,0,'C');
			$pdf->SetFont('Arial','B',8);
			$pdf->Cell(70,4,'PEMBERITAHUAN JATUH TEMPO '.substr($rowx["NAMABENEFIT"],10,strlen($rowx["NAMABENEFIT"])-10).' POLIS NO : '.$rowx["PREFIXPERTANGGUNGAN"].'-'.$rowx["NOPERTANGGUNGAN"].' / '.$rowx["NOPOL"],0,0,'L');
			$pdf->Ln();
			$pdf->SetFont('Arial','',8);
			...
			$mail->Subject  = " <Ujicoba kirim dari Server JLINDO> Pemberitahuan Jatuh Tempo ".substr($rowx["NAMABENEFIT"],10,strlen($rowx["NAMABENEFIT"])-10)." Polis ".$rowx["PREFIXPERTANGGUNGAN"].'-'.$rowx["NOPERTANGGUNGAN"]. " atas nama tertanggung ".$rowx["TERTANGGUNG"];
