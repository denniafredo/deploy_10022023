...
//echo 'masuk ddn'; die;
	...
	$queryxx = "SELECT * 
				...
				and status is null 
				and jenis in ('M','N','X','R') 
				and to_char(tglrekam,'mm/yyyy') = to_char(sysdate,'mm/yyyy')
	...
	//$conn = $db->Open();	
		...
		$pdf->Ln();
		$pdf->SetFont('Arial','',7);
		$pdf->MultiCell(170,4,'Informasi lebih lanjut dapat menghubungi Kantor Cabang PT Asuransi Jiwasraya IFG terdekat atau Call Center PT Asuransi Jiwa IFG 021-1500151 atau email customer_service@ifg-life.co.id atau kunjungi www.ifg-life.co.id',1);			
		$pdf->Ln();
		$pdf->Ln();
		$pdf->SetFont('Arial','',5);
		...
		try {	
			...
			$isipesan = file_get_contents('http://192.168.2.23/jiwasraya/pelaporan/email.get_cetakjatuhtempo_premi.php?prefixpertanggungan='.$rowx["PREFIXPERTANGGUNGAN"].'&nopertanggungan='.$rowx["NOPERTANGGUNGAN"].'&tglexp='.$rowx["EXPIRASI"].'&kantor='.$kdkantor);
