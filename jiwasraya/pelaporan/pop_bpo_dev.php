...
$sql = "SELECT   c.kdrayonpenagih,
   ...
   WHERE   b.nopenagih = c.nopenagih
           ...
           AND b.prefixpertanggungan = e.prefixpertanggungan AND b.nopertanggungan = e.nopertanggungan
           AND e.tgl_kirim IS NULL
           AND e.tgl_kirim_sms IS NULL
           AND TO_CHAR(e.tglbooked, 'MM/YYYY') = '".substr($tglcari,3,7)."'
           AND e.id_blast = (SELECT MAX(id_blast) FROM $DBUser.EMAIL_BLAST_EVENT_TESTING WHERE jenis_blast = 'PEMBERITAHUAN_POLIS_BPO' AND TO_CHAR(TGL_EVENT, 'MM/YYYY') = '".substr($tglcari,3,7)."')
