  ...
  if(parseInt(navigator.appVersion) >= 4){win.window.focus();}
	...
	<table>
	  <form action="<?=$PHP_SELF;?>" method="post">
		<tr>
		  <td class="verdana10blk">INFORMASI POLIS KONVERSI <b>AKTIF</b> KANTOR <b><?=$kantor;?></b></td>
...
<script language="javascript">
  ...
  var halcari=document.clntmtc.carihal.value;
	...
	var xxx = parseInt(document.clntmtc.carihal.value);
	  nextpg = xxx + 10;
		if((xxx < 11)||(halcari =='')) {
		  window.location.replace('portopoliokantoraktif.php?action=list&maju=ok&lastpg=11&get='+halcari+'&kdao='+getao+'');
    }else {
		  window.location.replace('portopoliokantoraktif.php?action=list&maju=ok&lastpg='+nextpg+'&get='+halcari+'&kdao='+getao+'');
		}
	}
</script>
<table>
...
if ($mundur){
   $a = $xx + 10;
   $url = $PHP_SELF . "?action=list&mundur=ok&lastpg=$a&get=$i&users=$users&kdao=$kdao"; 
} else {
   $a = $total_pages;
   $url = $PHP_SELF . "?action=list&maju=ok&lastpg=$a&get=$i&users=$users&kdao=$kdao"; 
}
	if ($get==$i) {
...
$stop = $xx - 10;
//echo $stop."<br>";
$urln = $PHP_SELF . "?action=list&maju=ok&lastpg=$xx&get=$linkm&users=$users&kdao=$kdao";
if($stop > $pg){ 
