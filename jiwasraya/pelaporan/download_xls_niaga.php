...
<b>HASIL <?=$namabank;?> <br />BULAN <?=$month."/".$year;?></b>
  ...
  $sql = "select yy.*,TO_CHAR (
                        ADD_MONTHS (tglbooked,
                                    (SELECT   GRACEPERIODE
                                       FROM   $DBUser.TABEL_241_GRACE_PERIODE
                                      WHERE   KDPRODUK =  (yy.kdproduk))),
                        'ddmmyy'
                     )
                        EXP,yy.premitot,DECODE(KDPRODUK,(SELECT KDPRODUK FROM $DBUser.TABEL_202_PRODUK_MATERAI WHERE KDPRODUK=YY.KDPRODUK),0,z.nilaimeterai) nilaimeterai,
         yy.premitot + DECODE(KDPRODUK,(SELECT KDPRODUK FROM $DBUser.TABEL_202_PRODUK_MATERAI WHERE KDPRODUK=YY.KDPRODUK),0,z.nilaimeterai) premitotal
  			...
  			SELECT   PHONETETAP01,
							 ...
							 (select namacarabayar from $DBUser.TABEL_305_CARA_BAYAR where kdcarabayar=b.kdcarabayar) carabayar, b.kdproduk,NULL norekening,
                               (select noaccount from $DBUser.tabel_100_klien_account WHERE prefixpertanggungan =
                                                                 ...
                                                                 AND nopertanggungan =
                                                                        b.
                                                                         nopertanggungan
                                                                         AND kdbank='BCN') norekeningdebet,
					 ...
					 and nopertanggungan=yy.nopertanggungan and tglseatled is null ) and yy.premitot  BETWEEN z.batasbawahpremi AND  z.batasataspremi 
					  AND NOT EXISTS (
					SELECT   'X'
  ...
  $kantorcetak=$kantor1;
    ...
    <td bgcolor="#89acd8" align="center">NO. VA</td>
    ...
    <td bgcolor="#89acd8" align="center">BPO</td>
    ...
    <td align="center" style="border-left-width: 1; border-right-width: 1; border-top: 1px solid #D8E1FA; border-bottom: 1px solid #D8E1FA">&nbsp;<?=$arr["BOOKEDNEXT"];?></td>
