...
<table border="0" class="tblborder" cellspacing="1" cellpadding="1" width="100%" align="center">
          ...
          // Kondisi Produk
          	if ($_GET["whereproduk"]==''){
          		 ...
          		 $whereproduk="substr(kdproduk,1,3)='JL2' and ";	
            ...
            $sqa = "select * ".
                   "from $DBUser.tabel_200_pertanggungan ".
            			 ...
            			 "to_char(".$fieldtgl.",'YYYYMMDD')>='".$_GET["tglDariCari"]."' and to_char(".$fieldtgl.",'YYYYMMDD')<='".$_GET["tglSampaiCari"]."' ".
									 "and kdpertanggungan='2' and kdstatusfile in('1',4')";
            */
						$sqa = "SELECT b.prefixpertanggungan,b.nopertanggungan,".
						  	 	 		 "b.kdvaluta,b.juamainproduk,b.premi1,".
                       "a.namaklien1, TO_CHAR (a.tgllahir, 'YYYY-MM-DD'), a.alamattetap01,".
                       "a.alamattetap02, (SELECT c.namakotamadya ".
                                           "FROM $DBUser.tabel_109_kotamadya c ".
                                          "WHERE c.kdkotamadya = a.kdkotamadyatetap),".
                       "(SELECT d.namapropinsi ".
                          "FROM $DBUser.tabel_108_propinsi d ".
                         "WHERE d.kdpropinsi = a.kdpropinsitetap), a.kodepostetap,".
                       "a.phonetetap01, a.emailtetap, (SELECT f.namaid ".
                                                        "FROM $DBUser.tabel_104_kartu_identitas f ".
                                                       "WHERE a.kdid = f.kdid), a.noid,".
                       "TO_CHAR (b.tglrekam, 'YYYY-MM-DD'), b.juamainproduk, b.premi1,".
                       "b.kdproduk, e.namaproduk, TO_CHAR (b.mulas, 'YYYY-MM-DD'),".
                       "TO_CHAR (b.expirasi, 'YYYY-MM-DD'), b.kdcarabayar,". 
                       "(SELECT c.premi ".
                          "FROM $DBUser.tabel_223_transaksi_produk c ".
                         "WHERE b.prefixpertanggungan = c.prefixpertanggungan ".
                           "AND b.nopertanggungan = c.nopertanggungan ".
                           "AND c.kdbenefit = 'BNFTOPUP') premitopup,".
                       "(SELECT c.premi ".
                          "FROM $DBUser.tabel_223_transaksi_produk c ".
                         "WHERE b.prefixpertanggungan = c.prefixpertanggungan ".
                           "AND b.nopertanggungan = c.nopertanggungan ".
                           "AND c.kdbenefit = 'BNFTOPUPSK') premisekaligus,".
                       "(SELECT SUM (c.premi) ".
                          "FROM $DBUser.tabel_223_transaksi_produk c ".
                         "WHERE b.prefixpertanggungan = c.prefixpertanggungan ".
                           "AND b.nopertanggungan = c.nopertanggungan ".
                           "AND c.kdbenefit LIKE 'CP%') premicp ".
                 "FROM ".
								 			 "$DBUser.tabel_100_klien a, ".
											 "$DBUser.tabel_200_pertanggungan b, ".
											 "$DBUser.tabel_500_penagih c,".
											 "$DBUser.tabel_202_produk e ".
                 "WHERE a.noklien = b.notertanggung ".
								   "and b.nopenagih=c.nopenagih ".
									 "and c.kdrayonpenagih='".$wherektr."' ".
                   "and e.kdproduk = b.kdproduk ".
                   "and b.kdproduk LIKE 'JL2%' and b.kdstatusfile='1'".
                   //"AND TO_CHAR (b.tglrekam, 'YYYYMMDD') >= '".$_GET["tglDariCari"]."' ".
                   //"AND TO_CHAR (b.tglrekam, 'YYYYMMDD') <= '".$_GET["tglSampaiCari"]."' ".
