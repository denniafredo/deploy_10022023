<?
  ...
  $DB = new Database($userid, $passwd, $DBName);
	...
	echo "<a class=verdana10blu><b>REKAPITULASI PREMI DAN UANG ASURANSI PERTANGGUNGAN PERORANGAN (NON LINK)</b></a>";
	echo "<hr size=1>";
...
if($masa=="pilih"){
  ...
  echo "</div>";
	...
	for ($a=0; $a<count($kdvaluta); $a++) {
		echo "<td valign=\"top\">";
	  $sql="select 	sum(a.polis) polis,".
								 "sum(a.pempol) pempol,".
								 "sum(a.jua) jua,".
								 "sum(a.premi) premi, ".
// Tambahan oleh Ari 04/02/2008
					 		 	 "sum(a.komisibp3) komisibp3, ".
								 "sum(a.komisi1) komisi1, ".
								 "sum(a.premilink) premilink ". 
// End of Tambahan oleh Ari 04/02/2008
              ...
              "WHERE x.nopenagih = y.nopenagih and ".
               ...
               //AND x.kdstatusfile = '1'
				 ...
				 "where ".
				     "p.kdproduk=a.kdproduk(+) and p.kdproduk not like 'JL2%'";
	...
	if($a=="0"){
	    ...
	    $produktitle = "<td class=verdana8blk align=center bgcolor=#82d7ff><b>POLIS</b><td class=verdana8blk align=center bgcolor=#82d7ff><b>PEMPOL</b></td><td class=verdana8blk align=center bgcolor=#9ce29a><b>JUA</b></td><td class=verdana8blk align=center bgcolor=#fee498><b>PREMI</b></td>";
  } else {
	 		$valutatitle = "<td class=verdana8blk align=center colspan=6 bgcolor=#b4d7e0><b>".$namavaluta[$a]."</b></td>";
  ...
  echo "<tr>";
	...
	while($ars=$DB->nextrow()){
	 ...
	 if(substr($kode[$a],0,3)=="JL2") {
	    echo $kolomproduk."<td class=verdana8blk align=right bgcolor=#e1f5ff>".substr($kode[$a],0,3).$polis[$a]."</td><td class=verdana8blk align=right bgcolor=#e1f5ff>".$pempol[$a]."</td><td class=verdana8blk align=right bgcolor=#defdd9>".number_format($jua[$a],2,",",".")."</td><td class=verdana8blk align=right bgcolor=#fff5d7>".number_format($premilink[$a],2,",",".")."</td><td class=verdana8blk align=right bgcolor=#ffccff>".number_format($komisibp3[$a],2,",",".")."</td><td class=verdana8blk align=right bgcolor=#ffccff>".number_format($komisi1[$a],2,",",".")."</td>";
	 }
	 else {
				...
				// <td class=verdana8blk align=right bgcolor=#ffccff>".number_format($komisibp3[$a],2,",",".")."</td><td class=verdana8blk align=right bgcolor=#ffccff>".number_format($komisi1[$a],2,",",".")."</td>";
	 }
	/* echo "</tr>";
