         ...
         yy.premitot + DECODE(KDPRODUK,(SELECT KDPRODUK FROM $DBUser.TABEL_202_PRODUK_DISKON WHERE KDPRODUK=YY.KDPRODUK),0,z.nilaimeterai) premitotal
					...
					(select premitagihan from $DBUser.tabel_300_historis_rider where 
					 ...
					 AND JENIS='CC' AND KDBANK='BNI') EXP
					  FROM (
  			SELECT    TO_CHAR(B.MULAS,'DD/MM/YYYY') MULAS, KDPRODUK, null norekening,
                       ...
                       WHERE   (c.prefixpertanggungan = b.prefixpertanggungan)
                               ...
                               " AND (b.kdstatusfile = '1') AND B.KDVALUTA='1'
                      UNION
                      SELECT   TO_CHAR(B.MULAS,'DD/MM/YYYY') MULAS, KDPRODUK, NULL norekening,
                               (select noaccount from $DBUser.tabel_100_klien_account WHERE prefixpertanggungan =
                                                                    b.
...
<a href="#" class="verdana8blu" onClick="NewWindow('download_xls_niaga.php?month=<?=$month;?>&year=<?=$year;?>&kdbank=<?=$kdbank;?>&kantornya=<?=$kantornya;?>','Downloadexcel',600,250,1);">Dowload ke Excel</a>
