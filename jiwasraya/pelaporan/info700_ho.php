...
$kim = ($bl=='13') ? "to_char(a.tglrekam,'YYYY')='".$th."' and " : "to_char(a.tglrekam,'MMYYYY')='".$bl.$th."' and ";
  ...
  $sql = "select ".
	...
	$sqa =  "select e.kdproduk,f.jenispengajuan,a.approveho, ".
      		 		"a.nomorsip,a.prefixpertanggungan,a.nopertanggungan,a.userrekam,c.namastatus,nvl(a.status,'0') status, ".
  				...
  				"where ".
      			  ...
      			  "and e.nopenagih=d.nopenagih ".
      			 	...
      			 	"and nvl(a.status,'0')=c.kdstatus and c.jenisstatus='KLAIM' ".
							...
							"/*and e.kdproduk in ('JSR1','JSR2','JSR3')*/ ".
							"and a.kdkantorproses = 'KP' ".
							//"and a.status='".$status."' ".
...
$tglotorisasi = (!$arr["TGLOTORISASI"]=='') ? $arr["TGLOTORISASI"] : "<font color=red>-</font>";
	...
	case 'pulih':
	 ...
	 switch ($status) {
	  ...
	  case '1':
		 ...
		 {
		   //$lanjut = "PROSES MSH DIPERTANGGUNGAN";
		   if ($arr['APPROVEHO'] == '1') //&& ($arr['KDPRODUK'] == 'JSR1' || $arr['KDPRODUK'] == 'JSR2' || $arr['KDPRODUK'] == 'JSR3'))
  ...
  if (($arr["KDKLAIM"]=='TAHAPAN'||$arr["KDKLAIM"]=='EXPIRASI' || $jns=='tebus' ||$arr["KDKLAIM"]=='EXPIRASI' ||$arr["KDKLAIM"]=='ANUITAS' ||$arr["KDKLAIM"]=='BEASISWA' ||substr($arr["KDKLAIM"],0,9)=='MENINGGAL') && $jmlrow>0){
  print( "	<td class=verdana8blu align=\"left\"><a href=\"#\" onclick=\"NewWindow('../akunting/memo_klaim.php?prefix=".$arr["PREFIXPERTANGGUNGAN"]."&noper=".$arr["NOPERTANGGUNGAN"]."&tglgadai=".$arr["TGLHITUNG"]."&tglmohon=".$arr["TGLMOHON"]."','',750,300,1);\">CETAK MEMORIAL</a></td>\n" );
  } 
  else { 
  print( "	<td class=verdana8blu align=\"left\"><a href=\"#\" onclick=\"NewWindow('../akunting/create_memoklaim.php?prefix=".$arr["PREFIXPERTANGGUNGAN"]."&noper=".$arr["NOPERTANGGUNGAN"]."&nomorsip=".$arr["NOMORSIP"]."&kdklaim=".$arr["KDKLAIM"]."&tglgadai=".$arr["TGLHITUNG"]."&tglmohon=".$arr["TGLMOHON"]."&kdklaim=".$arr["KDKLAIM"]."','',750,300,1);window.location.reload();\">PROSES MEMORIAL</a></td>\n" ); 
  }
