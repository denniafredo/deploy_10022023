...
<table border="1" cellpadding="2" style="border-collapse: collapse" width="100%" bordercolor="#09b0ce">
	...
	$sql ="select --SAB_X,TGADAI,
			...
			NOPERTANGGUNGAN,
			PEMPOL,
			to_char(TGLGADAI,'dd/mm/yyyy') TGADAI,
			SP_AWL,
			SB_AWL,
			MBI_PK_D,
			MBI_BNG_D,
			...
			KDKANTOR,
			SAK_P,
			--DECODE(NVL(SAB_X,0),0,0,DECODE(SAK_B,1,0,SAK_B)) SAK_BX,
			NAMASTS, SAK_P, SAK_B
			 from (select (select nopol from $DBUser.tabel_200_pertanggungan where prefixpertanggungan=l.prefixpertanggungan and nopertanggungan=l.nopertanggungan) nopol,L.*, (select namastatusfile from $DBUser.TABEL_299_STATUS_FILE where kdstatusfile=l.sts) namasts from $DBUser.TABEL_700_LAMNER_BUX L where tglbill=TO_DATE ('$year$month', 'YYYYMM') AND KDKANTOR='$kdkantor' and kdvaluta ".$incv." and stsgadai ".$incs." and sts ".$incb.")";
