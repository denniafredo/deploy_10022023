...
//echo 'masuk ddn'; die;
	...
	//$db = new ConnectionJL();
	//$conn = $db->Open();	
	//$queryxx = "SELECT * FROM $DBUser.TABLE_KIRIM_EMAIL_CAMPAIN_tes WHERE STATUS IS NULL and jenis = 'Y'";
	$queryxx = "SELECT * FROM $DBUser.TABLE_KIRIM_EMAIL_CAMPAIN WHERE STATUS IS NULL and jenis = 'Y'";
	//echo $query;
	//$dbxx->Parse($queryxx);
	//$dbxx->Execute();
	$DBx->parse($queryxx);
	...
	//while ($rowxx = oci_fetch_array($dbxx->get_statement())) {
		...
		$email = $rowxx['EMAIL'];
		//$path_name = 'attachment_pkt/pkt_02_2017/'.$rowxx['NO_ID'].'.pdf';
		$path_name = 'attachment_pkt/pkt_02_2018/'.$rowxx['NO_ID'].'.pdf';
		...
		try{		
			...
			$mail->Subject  = " Laporan Akumulasi Dana Kepesertaan pada Program THT (produk pendanaan saving plan) ";
			...
			$isipesan = file_get_contents('http://192.168.2.23/jiwasraya/pelaporan/email/email.get_contentcampaign_pkt.php');
			...
			$mail->Send();
			$query = "update $DBUser.TABLE_KIRIM_EMAIL_CAMPAIN set status = '1',tglkirim=sysdate WHERE email ='$email' and jenis = 'Y'";
			//$query = "update $DBUser.TABLE_KIRIM_EMAIL_CAMPAIN_tes set status = '1',tglkirim=sysdate WHERE email ='$email'  and jenis = 'Y'";
			echo $query;
			$DB->parse($query);
			$DB->execute();
			$row=$DB->nextrow();
				...
				$query = "update $DBUser.TABLE_KIRIM_EMAIL_CAMPAIN set status = 'X',tglkirim=sysdate WHERE email ='$email' and jenis = 'Y' ";
				//$query = "update $DBUser.TABLE_KIRIM_EMAIL_CAMPAIN_tes set status = 'X',tglkirim=sysdate WHERE email ='$email' and jenis = 'Y'";
				echo $e.'<br>';
				echo $query;
				$DB->parse($query);
				$DB->execute();
