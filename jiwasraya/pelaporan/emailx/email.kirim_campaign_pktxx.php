...
//echo 'masuk ddn'; die;
	...
	$queryxx = "SELECT * FROM $DBUser.TABLE_KIRIM_EMAIL_CAMPAIN WHERE STATUS IS NULL";
	//echo $query;
	//$dbxx->Parse($queryxx);
	//$dbxx->Execute();
	$DBx->parse($queryxx);
	...
	//while ($rowxx = oci_fetch_array($dbxx->get_statement())) {
		...
		try{		
			...
			//$query = "update $DBUser.TABLE_KIRIM_EMAIL_CAMPAIN_tes set status = '1',tglkirim=sysdate WHERE email ='$email' ";
			echo $query;
			$DB->parse($query);
			$DB->execute();
			$row=$DB->nextrow();
