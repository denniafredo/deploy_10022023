...
//echo 'masuk ddn'; die;
	...
	//$conn = $db->Open();	
		...
		//**** akhir isi		
		//** akhir isi pengajuan
		$path_name = '../pdf_email/PRM-102016-'.$rowx["PREFIXPERTANGGUNGAN"].'-'.$rowx["NOPERTANGGUNGAN"].'.pdf';
		...
		try {	
			...
			$isipesan = file_get_contents('http://192.168.2.23/jiwasraya/pelaporan/email/email.get_cetakjatuhtempo_premi.php?prefixpertanggungan='.$rowx["PREFIXPERTANGGUNGAN"].'&nopertanggungan='.$rowx["NOPERTANGGUNGAN"].'&tglexp='.$rowx["EXPIRASI"].'&kantor='.$kdkantor);
			...
			$mail->Send();
				...
				//$query = "update $DBUser.PENERIMA_EMAIL_TES set status = '1' WHERE PREFIXPERTANGGUNGAN = '".$rowx["PREFIXPERTANGGUNGAN"]."' AND NOPERTANGGUNGAN = '".$rowx["NOPERTANGGUNGAN"]."' AND JENIS = '$jenis' ";
				...
				//$query = "update $DBUser.PENERIMA_EMAIL_TES set status = 'X' WHERE PREFIXPERTANGGUNGAN = '".$prefixpertanggungan."' AND NOPERTANGGUNGAN = '".$nopertanggungan."' AND JENIS = '$jenis' ";
