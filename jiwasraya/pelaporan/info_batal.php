...
$sqled = "update $DBUser.TABEL_603_MUTASI_PERT a set status='1' where ".
      			 "a.prefixpertanggungan='$pfx' and a.nopertanggungan='$noptg' and TO_CHAR(tglmutasi,'DDMMYYYY')='$idel' and kdmutasi='14'";
 ...
 <tr>
	...
	<?
	  //var_dump($bulan);
			...
			$jnsval = array(1=>'batal');
			$jnslab = array(1=>'Pembatalan Polis');
	?>
  <tr>
  ...
  <tr>
    ...
    <td class="tblisi" align="center">
      ...
      $kim = ($bl=='13') ? "to_char(a.tglmutasi,'YYYY')='".$th."' and " : "to_char(a.tglmutasi,'MMYYYY')='".$bl.$th."' and ";
		...
		$sql="select a.*, NVL(STATUS,0) AS STATUS, TO_CHAR(tglmutasi,'DD/MM/YYYY') AS TGLMUTASI,". 			"TO_CHAR(tglmutasi,'DDMMYYYY') AS TGLMUTASIDETIL, c.premi1,TO_CHAR(c.mulas,'DD/MM/YYYY') AS mulas ".
      			 "from ".
      			 "$DBUser.tabel_200_pertanggungan c, $DBUser.TABEL_603_MUTASI_PERT a,".
      			 "$DBUser.tabel_500_penagih d ".
      			 "where ".
				 $kim.
      			 "a.prefixpertanggungan=c.prefixpertanggungan and a.nopertanggungan=c.nopertanggungan ".
      			 "and c.nopenagih=d.nopenagih and d.kdrayonpenagih='$kantor' and kdmutasi='14' and kdstatusfile='1' ".
     ...
     /* $sqa =  "select ".
      ...
      $tglotorisasi = (!$arr["TGLOTORISASI"]=='') ? $arr["TGLOTORISASI"] : "<font color=red>-</font>";
        ...
        print( "	<td class=verdana8blu align=\"center\">".number_format($arr["PREMI1"],2)."</td>\n" );
		print( "	<td class=verdana8blu align=\"center\">".$arr["MULAS"]."</td>\n" );
        if ($arr["STATUS"]==0){
		$sts='SUBMIT';
...
//		$sts='TELAH DIAKSEPTASI OLEH HO';
		...
		if ($arr["STATUS"]!='0'){
  					 ...
  					 echo "<td align='center'><a href=info_batal.php?pfx=".$arr["PREFIXPERTANGGUNGAN"]."&noptg=".$arr["NOPERTANGGUNGAN"]."&idel=".$arr["TGLMUTASIDETIL"]."&jns=".$jns."><img border='0' src='../images/edit.gif' width='16' height='16' alt='Approve'></a></td>";
				}
      	print( " </tr>" );
       $i++;
      }			 
