<?  
	...
	?>
		...
		<font face=verdana size=2><b>DETAIL CETAK POLIS SENTRALISASI</b><br>
			Kantor  : <?echo $kantor;?>
		</font>
		<hr size=1>
	<?
...
<html>
	<body>
		<form name="getpremi" action="<?=$PHP_SELF;?>" action="post">
			<a class="verdana10blk">Pilih Periode Cetak Polis</a>
		...
		<? if ($submit){ ?>
			...
			<div align="center">
				...
				<table width=100% class=tblisi cellpadding=2 cellspacing=0>
					<? 
				  		$sql = "SELECT a.prefixpertanggungan, a.nopertanggungan, b.nopol, b.nopemegangpolis, b.notertanggung, 
							d.namaklien1, b.kdproduk, to_char(b.mulas,'dd/mm/yyyy') as MULAS, to_char(a.tglcetakpolis,'dd/mm/yyyy') as TGLCETAKPOLIS, to_char(a.tglverifikasi,'dd/mm/yyyy') as TGLVERIFIKASI, a.USERCETAKPOLIS, a.USERVERIFIKASI, to_char(a.tglkirim,'dd/mm/yyyy') as TGLKIRIM, a.noresi, b.nopolbaru,
							b.premi1,
							b.premi2,
							b.premistd,
							(SELECT premi 
							...
							LEFT JOIN $DBUser.TABEL_200_PERTANGGUNGAN B 
							ON a.NOPERTANGGUNGAN=B.NOPERTANGGUNGAN and a.prefixpertanggungan = B.PREFIXPERTANGGUNGAN 
							LEFT JOIN $DBUser.TABEL_214_ACCEPTANCE_DOKUMEN C 
							ON A.NOPERTANGGUNGAN=C.NOPERTANGGUNGAN and A.prefixpertanggungan = C.PREFIXPERTANGGUNGAN 
							LEFT JOIN $DBUser.TABEL_100_KLIEN d 
							ON b.nopemegangpolis = d.noklien 
							where a.prefixpertanggungan is not null and A.NOPERTANGGUNGAN is not null and b.kdpertanggungan = '2' 
							and to_date(a.TGLCETAKPOLIS) >= to_date('".$tglawaldari."', 'dd/mm/yyyy') 
							and to_date(a.TGLCETAKPOLIS) <= to_date('".$tglakhirsampai."', 'dd/mm/yyyy') 
						...
						$DB->execute();
				  		...
				  		echo("<tr class=hijao>");
						    ...
						    echo("<td align=center><font size=2>Premi Std.</font></b></td>");
						    echo("<td align=center><font size=2>TopUp Ber.</font></b></td>");
						    echo("<td align=center><font size=2>Topup Skg.</font></b></td>");
						    ...
						    echo("<td align=center><font size=2>Tanggal Verifikasi</font></b></td>");
						    echo("<td align=center><font size=2>Diverifikasi Oleh</font></b></td>");
						...
						while ($arr=$DB->nextrow()) {
							...
							include "../../includes/belang.php";	 
								...
								echo("<td align=center><font face=Verdana size=2><a href=\"#\" onclick=\"NewWindow('../polis/polis.php?j=1&noper=$nopertanggungan&prefix=$prefix','updclnt',800,height=600,1);\">".$arr["NOPOLBARU"]."</a></td>");
						 		...
						 		echo("<td align=center><font face=\"Verdana\" size=\"2\">".number_format($arr["PREMISTD"],2,',','.')."</font></td>");
						 		echo("<td align=center><font face=\"Verdana\" size=\"2\">".number_format($arr["TOPUP_BERKALA"],2,',','.')."</font></td>");
						 		echo("<td align=center><font face=\"Verdana\" size=\"2\">".number_format($arr["TOPUP_SEKALIGUS"],2,',','.')."</font></td>");
								...
								echo("<td align=center><font face=\"Verdana\" size=\"2\">".$arr["TGLVERIFIKASI"]."</font></td>");
								echo("<td align=center><font face=\"Verdana\" size=\"2\">".$arr["USERVERIFIKASI"]."</font></td>");
