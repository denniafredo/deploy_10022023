...
//echo 'masuk ddn'; die;
	...
	//$db = new ConnectionJL();
	//$conn = $db->Open();	
	$queryxx = "SELECT * FROM $DBUser.TABLE_KIRIM_EMAIL WHERE EMAIL IS NOT NULL and jenis = 'J' and status is null order by PREFIXPERTANGGUNGAN,NOPERTANGGUNGAN,RAYONPENAGIHAN";
	//echo $query;
	//$dbxx->Parse($queryxx);
	//$dbxx->Execute();
	$DBxx->parse($queryxx);
	...
	//while ($rowxx = oci_fetch_array($dbxx->get_statement())) {
		...
		echo 'Kirim email ke '.$prefixpertanggungan.$nopertanggungan.$kdkantor.'<br>';
			...
			$pdf->Ln();
			$pdf->Cell(10,4,chr(127),0,0,'R');
			$pdf->MultiCell(150,3,'Keterangan lebih lanjut tentang pelunasan premi hubungi Call Center PT Asuransi Jiwa IFG di nomor (021) 1500151, atau kirimkan surat ke : Customer Service, Jl. Ir. H. Juanda No. 34 Jakarta, E-mail : asuransi@jiwasraya.co.id',0,'L');
			$pdf->Ln();
			//** akhir isi pengajuan
			//$pdf->Output();
