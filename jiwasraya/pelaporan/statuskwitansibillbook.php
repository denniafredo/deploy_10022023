...
echo " 	function CariNama() {";
echo "  var halcari=document.clntmtc.carihal.value;";
echo "	var xxx = parseInt(document.clntmtc.carihal.value);";
echo "	  nextpg = xxx + 10;";
echo "		if((xxx < 11)||(halcari =='')) {";
echo "		  window.location.replace('statuskwitansibillbook.php?action=list&maju=ok&lastpg=11&get='+halcari+'&thisperiode=$thisperiode&nopenagih=$nopenagih&status=$status&vbln=$vbln&vthn=$vthn');";
echo "    }else {";
echo "		  window.location.replace('statuskwitansibillbook.php?action=list&maju=ok&lastpg='+nextpg+'&get='+halcari+'&thisperiode=$thisperiode&nopenagih=$nopenagih&status=$status&vbln=$vbln&vthn=$vthn');";
echo "		}";
echo "	}";
echo "</script>";
?>
...
if($get<11){
 ...
 $urlp = $PHP_SELF . "?action=list&mundur=ok&lastpg=$xx&get=$linkp&users=$users&thisperiode=$thisperiode&nopenagih=$nopenagih&status=$status&vbln=$vbln&vthn=$vthn"; 
}
...
if ($mundur){
   $a = $xx + 10;
   $url = $PHP_SELF . "?action=list&mundur=ok&lastpg=$a&get=$i&users=$users&thisperiode=$thisperiode&nopenagih=$nopenagih&status=$status&vbln=$vbln&vthn=$vthn"; 
} else {
   $a = $total_pages;
   $url = $PHP_SELF . "?action=list&maju=ok&lastpg=$a&get=$i&users=$users&thisperiode=$thisperiode&nopenagih=$nopenagih&status=$status&vbln=$vbln&vthn=$vthn"; 
}
	if ($get==$i) {
...
$stop = $xx - 10;
//echo $stop."<br>";
$urln = $PHP_SELF . "?action=list&maju=ok&lastpg=$xx&get=$linkm&users=$users&thisperiode=$thisperiode&nopenagih=$nopenagih&status=$status&vbln=$vbln&vthn=$vthn";
if($stop > $pg){ 
