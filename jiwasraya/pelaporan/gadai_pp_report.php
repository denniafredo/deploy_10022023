...
<table border="0" cellpadding="2" style="border-collapse: collapse" width="100%" bordercolor="#C0C0C0">
	...
	<tr>
		<td colspan="2">
		<p align="center"><b>LAPORAN PINJAMAN POLIS PERTANGGUNGAN PERORANGAN</b></td>
	</tr>
	<? 
	switch($kdvaluta)
	{
	  case '1' : $kdakun = 160001; break;
		case '0' : $kdakun = 160002; break;
		case '3' : $kdakun = 161000; break;
		default : $kdakun = 160001; break;
	}
...
<table border="1" cellpadding="2" style="border-collapse: collapse" width="100%" bordercolor="#09b0ce">
	...
	$sql = "select 
              ...
              to_char(b.tglgadai,'DD/MM/YYYY') as tglgadai,b.nilaigadai,b.sisagadai,b.kdvaluta,b.status,
	...
	while ($row=$DB->nextrow()) {
	  $angsuran   = $row["ANGSURANPOKOK"]=="" ? 0 : $row["ANGSURANPOKOK"];
		$bungabayar = $row["ANGSURANPOKOK"]=="" ? 0 : $row["BUNGA"];
		$kapitalisasi = $row["KAPITALISASI"]=="" ? 0 : $row["KAPITALISASI"];
	  if($i%2){ echo "<tr>"; } else { echo "<tr bgcolor=#c4e1f2>"; } 
	  ?>
		<td valign="top"><?=$i;?></td>
		<td valign="top"><?=$row["NOPOL"];?></td>
		<td valign="top"><?="<a href=\"javascript:NewWindow('../polis/polis.php?prefix=".$row["PREFIXPERTANGGUNGAN"]."&noper=".$row["NOPERTANGGUNGAN"]."','polisinfo',800,600,'yes');\">".$row["PREFIXPERTANGGUNGAN"]."-".$row["NOPERTANGGUNGAN"]."</a>";?></td>
		<td valign="top"><?=$row["NAMAPEMEGANGPOLIS"];?></td>
		<td valign="top"><?=$row["TGLGADAI"];?></td>
		
		<td valign="top" align="right"><?=numberFormat($row["SALDOBULANLALU"]);?></td>
		<td valign="top" align="right"><?=numberFormat($row["BUNGABULANLALU"]);?></td>
		
		<td valign="top" align="right"><?=numberFormat($row["BUNGA"]);?></td>
		...
		<td valign="top" align="right"><?=numberFormat($row["TOTALANGSURAN"]);?></td>
		<td valign="top" align="right"><?=numberFormat($row["BUNGA"]);?></td>
	</tr>
	<?
	$i++;
	...
	$jmlsaldopinjaman += $row["SALDOPINJAMAN"];
	$jmltotalangsuran += $row["TOTALANGSURAN"];
	}
	?>
	<tr bgcolor="#afd6ed">
		<td><?=$i-1;?></td>
...
<?="<a href=# onclick=NewWindow('gadai_pp_report.php?action=cetak&year=".$year."&month=".$month."&kdkantor=".$kdkantor."&kdvaluta=".$kdvaluta."','',900,400,1)><img src=../img/cetak.gif border=0 align=absmiddle> Cetak</a>";?>
<? 
}
?>
</body>
