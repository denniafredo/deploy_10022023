...
<table cellpadding="1" cellspacing="2">
 <tr>
		...
		<td valign="top"><!--<select name="kdstatusfile">-->
  			<?
				...
				// if ($kantor=='KP' && $userid=='GUSTIA'){
				 	$sqa="select kdstatusfile,namastatusfile from $DBUser.tabel_299_status_file WHERE kdstatusfile in ('1','4','3','5','8','6') order by namastatusfile";//}
				 //else {
	...
	<tr>
		...
		<td> </td><td> </td><td> </td>
		<td> </td><td> </td><td> </td>
		<td align="right"> <? if ($kantor=='YKP') { echo "<input name='proses' value='PROSES' type='submit'>";} else {} ?>
		</td>
	</tr>
	<? //$kdjenisgadai=0; ?>
...
<table border="1" cellpadding="2" style="border-collapse: collapse" width="100%" bordercolor="#09b0ce">
    ...
    WHERE   prefixpertanggungan = XX.prefixpertanggungan
				...
				TGLBILL,
										  ...
										  SB_AWL,
										  MBI_PK_D,
										  DECODE(sts,'4',0,MBI_BNG_D) MBI_BNG_D,
										  MBI_PK_K,
										  MBI_BNG_K,
										  MSBI_PK_D,
										  MSBI_BNG_D,
										  ...
										  KDVALUTA,
										  STSGADAI,
										  KDKANTOR, (NVL(SP_AWL,0)+NVL(MSBI_PK_D,0)-NVL(MSBI_PK_K,0)) SAK_P, ROUND(NVL(SB_AWL,0),decode(kdvaluta,'3',2,0))+ROUND(NVL(MSBI_BNG_D,0),decode(kdvaluta,'3',2,0))-ROUND(NVL(MSBI_BNG_K,0),decode(kdvaluta,'3',2,0)) SAK_B, (select namastatusfile from $DBUser.TABEL_299_STATUS_FILE where kdstatusfile=x.sts) namasts from $DBUser.tabel_700_lamner_bu x where kdkantor='$kdkantor' and sts IN ('1','3','4') and kdvaluta ".$incv." and stsgadai ".$incs."
			and tglbill=(select max(tglbill) from $DBUser.tabel_700_lamner_bu where prefixpertanggungan=x.prefixpertanggungan 
			and nopertanggungan=x.nopertanggungan and tglbill < to_date('$year$month','YYYYMM')) AND NOT EXISTS (SELECT 'X' FROM $DBUser.tabel_700_lamner_bu
