...
<table border="0" cellpadding="2" cellspacing="1" style="border-collapse: collapse" bordercolor="#CCCCCC" width="100%" id="AutoNumber1">
  <tr>
	...
	if($kantor=="KP"){
		$querykantor=" ";
		/*$querykantor=" AND c.kdrayonpenagih IN
									  (SELECT   DISTINCT c.kdrayonpenagih AS kdkantor
										...
										WHERE   b.prefixpertanggungan =
												   SUBSTR (a.nopolis, 1, 2)
												AND b.nopertanggungan =
													  SUBSTR (a.nopolis, 3, 9)
												...
												AND TO_CHAR (a.tglrekam, 'YYYYMMDD') between '$tglawal' and  '$tglakhir'
												AND b.kdproduk = e.kdproduk
												AND a.kdbank = '$metodebayar') "; */
	}else{
		$querykantor=" AND c.kdrayonpenagih='$kantor' ";
	}
		  ...
		  FROM   (  SELECT   c.kdrayonpenagih AS kodekantor,
							 ...
							 (SELECT   phonetetap02
							   ...
							   WHERE   noklien = b.nopembayarpremi)
								AS telp2,
							 null beritakredit,
							 a.tglrekam PROCCESS_DATE,
							 TO_CHAR (d.tglbayar, 'DD/MM/YYYY') AS tglbayar,
							 a.tglrekam tglupdated,
							 TO_CHAR (d.tglbayar, 'MM/YYYY') AS blnbayar,
							 TO_CHAR (d.tglbayar, 'YYYYMM') AS BLNBAYARNYA,
							 b.nopenagih,
							 b.noagen,
							 ...
							 (SELECT   komisiagencb
							   ...
							   WHERE   prefixpertanggungan = b.prefixpertanggungan
									   AND nopertanggungan = b.nopertanggungan
									   AND thnkomisi =
											 CEIL(MONTHS_BETWEEN (a.tglrekam,
																  b.mulas)
												  / 12)
									   AND kdkomisiagen = '01')
								komisiagen,
							 ...
							 DECODE (
								...
								* DECODE (
								...
								disco,  TO_CHAR(a.tglrekam,'DDMMYYYY') z_entry_time, TO_CHAR(d.tglbooked,'MMYYYY') z_tglbuk
					  FROM   $DBUser.tabel_300_historis_premi d,
							 $DBUser.tabel_200_pertanggungan b,
							 $DBUser.tabel_315_pelunasan_va a,
							 $DBUser.tabel_500_penagih c,
		...
		<?
			...
			else
			{
				...
				//else {
					...
					$matere=$arr["MATERE"];
					if($metodebayar=="BMRI" || $metodebayar=="BBRI" || $metodebayar=="BIMA" || $metodebayar=="PPOS" || $metodebayar=="FINN" || $metodebayar=="VBNd" || $metodebayar=="BTN" || $metodebayar=="POS"){
						$materenb=0;
						$discountnb=0;
						$matereob=0;
						$discountob=0;
