  ...
  <a class="verdana10blk">Nama Bank</a>
	<select name="kdbank">
	  <option value="MDR" <?=$selbank1;?>>MANDIRI</option>
        ...
        <option value="BBNI" <?=$selbank8;?>>H2H BNI</option>
        <option value="BIMA" <?=$selbank9;?>>H2H BIMASAKTI</option>
...
<table width="600">
<? 
if($kdbank=="BMRI" || $kdbank=="BBRI" || $kdbank=="BBNI" || $kdbank=="BIMA" ) {
...
if($kdbank=="BMRI" || $kdbank=="BBRI" || $kdbank=="BBNI" || $kdbank=="BIMA" ) {
	$sql = "SELECT d.kdrekeninglawan,
         ...
         (SELECT kurs
           ...
           WHERE     tglkursberlaku =
                        (SELECT MAX (tglkursberlaku)
                           FROM $DBUser.tabel_999_kurs_transaksi
                          WHERE tglkursberlaku <= a.ENTRY_TIME AND kdvaluta = '0')
                 AND kdvaluta = '0')
            kurs,
         (SELECT kurs
           ...
           WHERE     tglkursberlaku =
            ...
            kursstd,
         e.kdcabas,
         SUM (a.BILL_AMOUNT) AS premitagihan,
         SUM ( (B.premiSTD / b.indexawal)) AS premihitungan,
         SUM (TO_NUMBER (B.PREMISTD) / 100) AS jmlpremi,
         SUM (
            (SELECT nilaimeterai
         ...
         SUM (
            DECODE (
               TO_CHAR (d.tglbayar, 'MM/YYYY'),
               TO_CHAR (D.tglbooked, 'MM/YYYY'),   ROUND (
                                                      (  d.premitagihan
                                                       / b.indexawal),
                                                      2)
                                                 * 0.01
   ...
   WHERE     b.prefixpertanggungan = SUBSTR (a.no_polis, 1, 2)
         ...
         AND a.void = '0'
         AND b.kdvaluta = '0'
          AND TO_CHAR (a.entry_time, 'YYYYMMdd') BETWEEN '$tglawaldari'
                                                    AND '$tglakhirsampai'
         AND c.kdrayonpenagih = '$kantor'
         AND b.kdproduk = e.kdproduk
         AND a.company_code = '$kdbank'
         GROUP BY d.kdrekeninglawan, e.kdcabas,a.entry_time";}
else {
  	$sql = "select ".
              "d.kdrekeninglawan,".
//			  "b.indexawal,".
...
//			  "b.indexawal,".
  	...
  	while ($arr=$DB->nextrow()) {
			...
			if($kdbank=="BMRI" || $kdbank=="BBRI" || $kdbank=="BBNI" || $kdbank=="BIMA" ) {
				$premitagihan_di = round($arr["PREMITAGIHAN"],2);
			}
			//round(1.95583, 2)
  ...
  <!---------- summary  -------->
	  ...
	  $DB->execute();
  	 ...
  	 <td align="right"><?=$nilaiselisihK;?></td>
		...
		if($kdbank=="BMRI" || $kdbank=="BBRI" || $kdbank=="BBNI" || $kdbank=="BIMA" ) {
				$nilaiselisihD_di=0;$nilaiselisihK_di=0;$nsK_di=0;
		}
...
echo("<font face=Verdana size=1><a href=\"#\" onclick=\"window.open('../akunting/cetakslip_pindahbuku_autodebet.php?tglawaldari=".$tglawaldari."&tglakhirsampai=".$tglakhirsampai."&kdbank=".$kdbank."&periode=".$periode."&month=".$month."&year=".$year."','','width=800,height=600,top=100,left=100,scrollbars=yes');\"><img src=../img/cetak.gif align=absmiddle border=0> Cetak SLIP</a></font>");
?>
