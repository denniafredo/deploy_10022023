...
$sql="select *
		...
		(SELECT rownum rn,PREFIXPERTANGGUNGAN,NOPERTANGGUNGAN,NAMA,NOAGEN,NAMA_AGEN,KDRAYONPENAGIH,TGLBOOKED,
				...
				PENAGIH,KDSTATUSFILE,JMLNILAIRP-LUNAS tunggakan,kdrekeningpremi,kdrekeninglawan
			  FROM $DBUser.tagihan_vs_pelunasan a
			  WHERE KDRAYONPENAGIH IN (select kdkantor
										  from $DBUser.tabel_001_kantor
										  start with kdkantor = '$kdwilayah'
										  connect by prior kdkantor = kdkantorINDUK)
			  /*AND TO_CHAR (TGLBOOKED, 'MM/YYYY') <= '$tglbooked'*/
			  AND TGLBOOKED <= LAST_DAY (TO_DATE ('$tglbooked', 'MM/YYYY'))
			  and kdstatusfile = '1'
			  /*and lunas = 0*/
		...
		/*where tunggakan > 0	  
		where rn between nvl('$batasbawah','1') and nvl('$batasatas','1000')     
		order by rn*/";		
			 echo $sql."<br/><br/>";
			 //die;
?>
...
echo "</table><table>";	
	$sql="SELECT   trunc(count(*)/1000) jml
                FROM   $DBUser.rekap_tagihan_vs_pelunasan a
               WHERE       KDRAYONPENAGIH IN (SELECT   KDKANTOR
                                                FROM   $DBUser.TABEL_001_KANTOR
                                               WHERE   KDKANTORINDUK = '$kdwilayah')
                       AND TO_CHAR (TGLBOOKED, 'MM/YYYY') <= '$tglbooked'
					   and lunas <> jmlnilairp
                       AND KDCARABAYAR IN (SELECT   KDCARABAYAR
                                             FROM   $DBUser.TABEL_305_CARA_BAYAR
                                            WHERE   NAMACARABAYAR = '$carabayar')
                       AND KDKUITANSI = '$kdkuitansi'";
		//echo 'dsdsd';
			 //echo $sql."<br/><br/>";
			 //die;
	...
	echo '<tr>';	
		for ($x = 1; $x <= $jmlpage; $x++) {
			$batasbawah = 1 + ($x * 1000) - 1000;
			$batasatas = $x * 1000;
			echo "<td>
					<a href='detail_tagihan_sd_bln_ini.php?jenis=$jenis&kd_kuitansi=$kdkuitansi&kd_carabayar=$carabayar&kd_wilayah=$kdwilayah&bln=$tglbooked&batasbawah=$batasbawah&batasatas=$batasatas'>$x</a>		
				  </td>";
		} 
	echo '</tr>';	
	?>
