...
<form name="getpremi" action="<?=$PHP_SELF;?>" method="post">
  ...
  <td>Pemb.</td>
	<td>
	  <? 
		switch($metodebayar)
		{
		  ...
		  case 'BRI': $sr = "selected"; break;
			...
			case 'PAMP': $spamp = "selected"; break;
	  ...
	  <option value="PAMP" <?=$spamp;?>>H2H Pampasy</option>
...
<table border="0" cellpadding="2" cellspacing="1" style="border-collapse: collapse" bordercolor="#CCCCCC" width="100%" id="AutoNumber1">
  <tr>
	...
	//echo $metodebayar;
	  if($metodebayar=="BMRI" || $metodebayar=="BBRI" || $metodebayar=="BIMA" || $metodebayar=="BBNI" || $metodebayar=="PPOS" || $metodebayar=="FINN" || $metodebayar=="PAMP"){
	  ...
	  else if($metodebayar=="VBN" || $metodebayar=="BCN"){
	   if($kantor=="KP"){
     		$filterbo="";
     	}else{
     		$filterbo=" and c.kdrayonpenagih='".$kantor."'";
     	}
     ...
     $sql = "select KODEKANTOR KDRAYONPENAGIH,PREFIXPERTANGGUNGAN,NOPERTANGGUNGAN,KDVALUTA,
      ...
      NAMAAGEN,KDCABAS,
      decode(kdkuitansi,'BP3',0,(select nilaimeterai
      from $DBUser.tabel_999_batas_materai
      where ".
	  /* Opsi 1*/	  
	"(select
...
"		-   
	  ...
	  (select premitagihan from $DBUser.tabel_300_historis_rider where 
            prefixpertanggungan = zz.prefixpertanggungan
                               AND nopertanggungan = zz.nopertanggungan
                               and to_char(tglbooked,'ddmmyyyy')=to_char(zz.tglbuk,'ddmmyyyy') and buktisetor=replace(zz.buktistr,'MANDIRI','') )				  				 		,NULL,'0',	  (select premitagihan from $DBUser.tabel_300_historis_rider where 
            prefixpertanggungan = zz.prefixpertanggungan
                               AND nopertanggungan = zz.nopertanggungan
                               and to_char(tglbooked,'ddmmyyyy')=to_char(zz.tglbuk,'ddmmyyyy') and buktisetor=replace(zz.buktistr,'MANDIRI','') )				  				 	)	
		   ".
      //"jmlpremi-disco ".
      "between batasbawahpremi
	  ...
	  (select premitagihan from $DBUser.tabel_300_historis_rider where 
            prefixpertanggungan = zz.prefixpertanggungan
                               AND nopertanggungan = zz.nopertanggungan
                               and to_char(tglbooked,'ddmmyyyy')=to_char(zz.tglbuk,'ddmmyyyy') and buktisetor=replace(zz.buktistr,'MANDIRI','') ) rider,
							   selisih
      from (
      ...
      .nokontrol,(a.norekdebet) as norekdebet,a.norekkredit,a.
      ...
      to_char(a.tglrekam,'MM/YYYY'),DECODE (b.KDPRODUK,
										(SELECT KDPRODUK FROM $DBUser.TABEL_202_PRODUK_DISKON WHERE KDPRODUK=b.KDPRODUK), 0,
										1)* decode(b.kdvaluta,'1',d.premitagihan,'0', ROUND(d.
      ...
      FROM $DBUser.tabel_999_kurs_transaksi y
      WHERE x.kdvaluta = y.kdvaluta
      AND y.tglkursberlaku <= d.tglbayar))) /100,'0') disco, nvl(selisih,0) selisih
      from $DBUser.tabel_300_historis_premi d,$DBUser.tabel_200_pertanggungan b,
      $DBUser.tabel_315_pelunasan_va a,$DBUser.tabel_500_penagih c, $DBUser.
      tabel_202_produk e
      ...
      and b.nopenagih=c.nopenagih ".$filterbo."
      and substr(b.kdproduk,1,2)<>'JL'
      and a.statuspembayaran='2'
      and ".$filterperiode.
      //" and b.kdproduk=e.kdproduk    //aslinya ini bro!!!! 
      ...
      NOPOLIS,BLNTAGIHAN,NOKONTROL,NOREKDEBET,NOREKKREDIT,STATUSPEMBAYARAN,
      BERITAKREDIT,TGLREKAM,TGLBAYAR,TGLUPDATED,BLNBAYAR,
      BLNBAYARNYA,NOPENAGIH,NOAGEN,NOREKENINGDEBET,NAMAKLIEN,THNKOMISI,						
	  KOMISIAGEN,
      NAMAAGEN,KDCABAS,DISCO,
...
"		-   
	  ...
	  (select premitagihan from $DBUser.tabel_300_historis_rider where 
            prefixpertanggungan = zz.prefixpertanggungan
                               ...
                               and to_char(tglbooked,'ddmmyyyy')=to_char(zz.tglbuk,'ddmmyyyy') and buktisetor=replace(zz.buktistr,'MANDIRI','') ) rider
      from (
      select B.KDPRODUK, c.kdrayonpenagih as kodekantor,(
      select namakantor
      from $DBUser.tabel_001_kantor
      where kdkantor=c.kdrayonpenagih) as namakantor,b.prefixpertanggungan,b.
      nopertanggungan,b.kdvaluta,b.indexawal,d.tglbooked as tglbuk,d.buktisetor as buktistr,d.
      ...
      to_char(d.tglbayar,'YYYYMM') as BLNBAYARNYA,b.nopenagih,b.noagen,b.norekeningdebet,
      ceil(months_between(a.tglupdated,b.mulas)/12) as thnkomisi, (
      select round(komisiagencb)
      from $DBUser.tabel_404_temp
      where prefixpertanggungan=b.prefixpertanggungan
      and nopertanggungan=b.nopertanggungan
      and thnkomisi= ceil(months_between(a.tglupdated,b.mulas)/12)
      and kdkomisiagen=DECODE ('$metodebayar', 'BTN', '36', '01')) komisiagen, (
      select kurs
      from $DBUser.tabel_999_kurs_transaksi
      where tglkursberlaku=(
      select max(tglkursberlaku)
      ...
      from $DBUser.tabel_999_batas_materai
      where to_number(a.jumlahtagihan)/100 between batasbawahpremi
      and batasataspremi ) as materex, 
	  decode((SELECT COUNT(*) FROM $DBUser.TABEL_300_HISTORIS_PREMI WHERE prefixpertanggungan =
                                                                b.prefixpertanggungan
                                                             AND nopertanggungan = b.nopertanggungan 
	  AND to_char(tglseatled,'YYYYMMDD') between '$tglawal' and  '$tglakhir' 
															 and tglbooked < D.TGLBOOKED),0,
	  decode(to_char(a.tglbooked,'MM/YYYY'),
   ...
   WHERE       b.prefixpertanggungan = SUBSTR (a.nopolis, 1, 2)
           ...
           AND a.kdbank = '$metodebayar') ". */
      //" and b.kdproduk=e.kdproduk    //aslinya ini bro!!!! 
      " b.kdproduk=e.kdproduk ".$filterjenispremi.
      " and a.kdbank='$metodebayar'
      order by d.kdkuitansi, d.prefixpertanggungan, d.nopertanggungan) zz";
		...
		<?
			...
			else
			{
				...
				//else {
					...
					$matere=$arr["MATERE"];
					if(($metodebayar=="BMRI" || $metodebayar=="BBRI" || $metodebayar=="BIMA" || $metodebayar=="PPOS" || $metodebayar=="FINN" || $metodebayar=="PAMP" || $metodebayar=="VBN" || $metodebayar=="BTN" || $metodebayar=="POS") && $arr["NOPERTANGGUNGAN"] != '002010619'){
						$materenb=0;
						$discountnb=0;
						$matereob=0;
						$discountob=0;
