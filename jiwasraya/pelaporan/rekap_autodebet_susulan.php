...
SELECT c.kdrayonpenagih,  (TO_NUMBER (jumlahtagihan) / 100) jmlrider,  (
      ...
      AND TO_CHAR (a.tglrekam, 'YYYYMMDD') between '$tglawaldari' and  '$tglakhirsampai' and 
		   c.kdrayonpenagih='$kodekantor' and 
		   b.kdproduk=e.kdproduk and a.kdbank='".$kdbank."'
      and a.kdbank='MDR') GROUP BY kdrayonpenagih";
	$DBA->parse($sqlr);
	$DBA->execute();
	$rarr=$DBA->nextrow();
	?>	
    ...
    <td style="border-left-width: 1; border-right-width: 1; border-top: 1px solid #D8E1FA; border-bottom: 1px solid #D8E1FA" align="right"><?=number_format($jmlmatrenb+$jmlmatreob+$rarr["X"],2,",",".");?></td>
    <td style="border-left-width: 1; border-right-width: 1; border-top: 1px solid #D8E1FA; border-bottom: 1px solid #D8E1FA" align="right"><?
	if($kdbank=='BMRI' || $kdbank=='BBRI'){echo number_format($jmlpreminb+$jmlpremiob,2,",",".");} else {echo number_format($jmltotaldebetnb+$jmltotaldebetob,2,",",".");};?></td>
	</tr>
	<? 
	
  	$total=$total+$arr["JMLPREMI"];
	$totalr=$totalr+$rarr["JMLRIDER"];
	...
	<tr bgcolor="#f5d79c">
	 ...
	 <td align="right"><?=number_format($totaltotaldebetnb+$totaltotaldebetob+$totalr+$totalmr,2,",",".");?></td>
     <? }?>
	</tr>
</table>
