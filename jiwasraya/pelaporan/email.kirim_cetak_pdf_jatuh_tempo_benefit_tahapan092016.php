	...
	//while ($rowxx = oci_fetch_array($dbxx->get_statement())) {
		...
		echo 'Kirim email ke '.$prefixpertanggungan.$nopertanggungan.$kdkantor.'<br>';
			...
			try{	
				...
				$isipesan = file_get_contents('http://192.168.2.23/jiwasraya/pelaporan/email.get_cetakjatuhtempo_tahapan.php?prefixpertanggungan='.$rowx["PREFIXPERTANGGUNGAN"].'&nopertanggungan='.$rowx["NOPERTANGGUNGAN"].'&tglexp='.$rowx["EXPIRASI"].'&kantor='.$kdkantor.'&tglexp='.$tglexp);
