...
<table>
<?
	 $filter = $kdbank=='BPK' ? " and kdproduk NOT like 'JL%' " : null;
	 
	 $sql="SELECT   LPAD (ROWNUM, 5, '0') || satu||LTRIM(REPLACE (
                                 TO_CHAR (ROUND (premitot+decode(kdproduk,'JL4B',0,  /* REQ ARIEF YUSUF TGL 02/05/2014*/
								 										  'JL4X',0,
                                      'JL4BLN',0,
								 										  'JL4BL',0,
                                      'JL4BF',0,nilaimeterai), 0),
                                          '099999999999999'),
                                 '.',
                                 ''
                              ))||dua
					 ...
					 WHERE       PREFIXPERTANGGUNGAN = b.PREFIXPERTANGGUNGAN
                      ...
                      SELECT   d.norekening,
                       ...
                       WHERE   (c.prefixpertanggungan = b.prefixpertanggungan)
                               ...
                               AND (b.nopenagih = f.nopenagih)
                               AND (d.kdbank = b.kdbank)
							   AND (b.kdproduk not in ('JL4B','JL4X','JL4BL', 'JL4BLN'))                               
                               AND (c.tglbooked <= TO_DATE (to_char(sysdate,'DD')||'/".$month."/".$year."', 'DD/MM/YYYY'))".
          ...
          GROUP BY   norekening,
                     ...
                     where yy.premitot  BETWEEN z.batasbawahpremi AND  z.batasataspremi $filter";
	 //echo $sql;
	 //die;
	 $DB->parse($sql);
	 $DB->execute();
