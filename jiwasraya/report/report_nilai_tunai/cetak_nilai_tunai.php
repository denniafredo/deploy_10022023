	...
	function createPDF($nopertanggungan){
		...
		while ($arr2=$DB2->nextrow()){
		    ...
		    foreach($aru as $key){
				...
				$pdf->Cell(25,($line * $cellHeight),number_format(str_replace(',', '.', $key["UNIT"])*-1,4,",","."),0,0,'C', true);
	    ...
	    $pdf->Ln();
	    $pdf->Cell(30,4,'4.	Biaya Pengalihan Dana ',0,0,'FJ');
	    $pdf->SetFont('Arial','I',7);
	    ...
	    $pdf->Ln();
	    $pdf->Cell(6,4,'5.	Biaya Pembentukan unit 5% (lima persen) dari Dana Investasi dan Biaya Top Up (Penambahan Dana) dikenakan sebesar 5 % (lima persen) dari Premi Top Up.',0,0,'FJ');
	    $pdf->Ln();
	    $pdf->Cell(61,4,'6.	Pembatalan Polis oleh Pemegang Polis dalam masa ',0,0,'FJ');
	    $pdf->SetFont('Arial','I',7);
	    ...
	    /** Redaksi Informasi untuk Cara Bayar selain 'Sekaligus' **/
	    ...
	    $pdf->SetFont('Arial','',7);
	    $pdf->Ln();
	    $pdf->Cell(6,4,'Apabila membutuhkan informasi dan penjelasan lebih lanjut mengenai polis tersebut, dapat menghubungi Call Center 1500176, email ke customer_care@ifg-life.id,',0,0,'FJ');
	    $pdf->Ln();
