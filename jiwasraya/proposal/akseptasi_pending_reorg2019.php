...
/*if($kantor=='KP') {
	$sql= "select a.suspend, a.keterangan,a.suspend,a.kdstatusemail, ".
...
if($kantor=="KP"){
	...
	if($kantor=="KP"){
		$sql  = "select ".
      				  "a.prefixpertanggungan,a.suspend,a.nopertanggungan,a.kdstatusemail,a.keterangan,".
		...
		elseif($arr["KDSTATUSEMAIL"]=="1" && $kantor!="KP" && $arr["STATUSAPPROVAL"]=="1" && $arr["STATUSAKSEPTASI"]=="0")
			$cebox = "Approved"; 
				...
				if($kantor=="KP"){
					...
					if($arr["STATUSAKSEPTASI"]=="1"){
						$cebox = "<font color=green><b>Accepted <input type=\"button\" value=\"cetakdok\" onClick=\"NewWindow('../polis/cetakdokumen.php?prefix=$arr[PREFIXPERTANGGUNGAN]&noper=$arr[NOPERTANGGUNGAN]','popuppage','620','500','yes')\" style=\"padding:2px;margin:0px;font-size:12px;font-face:verdana;\" /></b></font>"; 						
					}elseif($arr["STATUSAPPROVAL"]=="1"){
    ...
    <td width="4%" bgcolor="#FFFFFF" align="center">
		...
		<?php } else {
			...
			if ($arr['SUSPEND'] && $kantor!="KP") { ?>
					...
					<input type="submit" name="kirim" value="Lanjut Proposal Pending" style="padding:0px;margin:0px;" />
