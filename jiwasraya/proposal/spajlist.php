...
?>	
  ...
  if (!$nama) {
   ...
   $dannama = "and namapempol like '".strtoupper($nama)."%'"; 
  }
	$ad = ($o==1) ?  'desc' : 'asc';
	$f = (!$f) ? '2' : $f;
...
?>
  ...
  <tr bgcolor="#f89aa4">
    ...
    $o = (int)!((boolean)$o);
		...
		echo "<td align=\"center\"><b>No. SPAJ</b></td>";
...
<?
	...
	while($arr=$DB->nextrow()) {
	  ...
	  //include "./includes/belang.php"; 
		...
		$a="<td align=center><a href=\"#\" onclick=\"javascript:window.opener.document.".$formname.".nosp.value='".$arr["NOSP"]."';javascript:window.opener.document.".$formname.".noagen.value='".$arr["NOAGEN"]."';javascript:window.opener.document.".$formname.".taltup.value='".$arr["TALTUP"]."';window.close();\" >".$arr["NOSP"]."</a></td><td align=center>".$arr["NAMAPEMPOL"]."</td>".
