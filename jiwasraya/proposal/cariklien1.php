<?  
	...
	$DB->execute();
	$r = $DB->nextrow();
	$namaklien1 = trim(strtoupper(strtolower(empty($namaklien1)) && !$submit ? trim($r['NAMAKLIEN1']) : trim($namaklien1)));
	$tgllahir = empty($tgllahir) && !$submit ? $r['TGLLAHIR'] : $tgllahir;
?>
<html>
	<head>
	...
	<body topmargin=0>
		<div align=center>
			...
			<tr>
				<td class="verdana8" colspan="7" style="padding:3px 0;">
				<br/>
				<p>
				Tips Mencari Klien Lama :<br>
				1. Yang menjadi kunci untuk pencarian klien lama adalah <b>Nama Lengkap</b> dan <b>Tanggal Lahir</b><br>
				2. Silahkan pastikan di (a) Nama Klien dan (b) Tgl Lahir pada kolom yang tersedia sesuai dengan data kliennya.<br>
				3. kolom (a) Nama klien dan (b) Tgl Lahir dapat di edit/diubah hanya untuk kebutuhan pencarian/pencocokan nama klien dan tgl lahir <i>(perubahan difield (a) dan (b) untuk pencarian/pencocokan klien tidak merubah data master klien)</i>
			...
			<tr class="tblisi">
				<td class="verdana8" colspan="7">
					...
					(a) Nama Klien : <input type="text" name="namaklien1" size="20" class="c" onFocus="highlight(event)" value="<?=$namaklien1?>">
					(b) Tgl Lahir : <input type="text" name="tgllahir" size="8" class="c" onFocus="highlight(event)" value="<?=$tgllahir?>">
