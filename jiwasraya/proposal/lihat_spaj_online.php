...
// include "../../includes/klien_online.php";
 ...
 $sql="select pathalias,NOSPAJ,NAMATERTANGGUNG,NOID,idspaj,TGLEXPIRED_ID,TEMPATLAHIR, tgllahir,decode(JENISKELAMIN,'L','LAKI-LAKI','P','PEREMPUAN') jeniskelamin,".
	 ...
	 "tglnikah from DIMAS.TABEL_MAPPING_KLIEN a where idspaj='".$_GET['id']."'";
$DB->parse($sql);
$DB->execute();
//echo $sql;
$no=1;	
$arr=$DB->nextrow();
echo file_get_contents("http://jaim.jiwasraya.co.id/spaj/spaj/14102015/000000004");
die;
...
<?php 
$sql="select NOSPAJ,NAMATERTANGGUNG,NOID,idspaj,TGLEXPIRED_ID,TEMPATLAHIR, tgllahir,decode(JENISKELAMIN,'L','LAKI-LAKI','P','PEREMPUAN') jeniskelamin,".
     "(select namaagama from $DBUser.tabel_102_agama where kdagama=a.kdagama) AGAMA,TINGGIBADAN,BERATBADAN,NAMAIBUKAND,DIKAKHIR,NPWP,MERITALSTATUS,".
...
<tr>
	...
	<?php } ?>
	<a href="?action=no&nosp=<?php echo $arr["NOSPAJ"]; ?>&id=<?php echo $arr["NOKLIEN"]; ?>">Reject</a>|<a href="#" onclick="window.open('lihat_spaj_online.php?spaj=<?php echo $arr["NOSPAJ"]; ?>&id=<?=$arr["IDSPAJ"]?>','popuppage','scrollbars=yes,width=780,height=600,top=100,left=50')">Lihat SPAJ</a></font></td></tr>
<?php
