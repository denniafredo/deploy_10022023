...
if($kantor!="KP")
echo "Untuk Akseptasi sudah dilakukan secara otomatis pada saat pelunasan premi!!";
die;
/*--------------------------------------------------
Author : Iie Sumitra
Date   : 19 Jul 2013
...
if($_POST['sbmt'] == '1')
{
	...
	for($i=0; $i<sizeof($cheklist); $i++)
	{
		...
		$sqlUpd = "INSERT INTO $DBUser.TABEL_214_ACCEPTANCE_DOKUMEN (PREFIXPERTANGGUNGAN,
                                          ...
                                          KDACCEPTANCE,
                                          TGLACCEPTANCE,
                                          USERUPDATED,ALASANACCEPTANCE)
				  VALUES   ('".substr(".$NoPolis.",1,2)."',
							'".substr(".$NoPolis.",3,9)."',
							1,
							sysdate,
							'$userid','VIA WEB : akseptasi.php')";
		$DB->parse($sqlUpd);
		$DB->execute();
		//echo $DB->error;
		//echo $sqlUpd;
		//$conn = ocilogon($userid, $passwd, "JSDEPLOY");
  ...
  //ociexecute($sql);
		...
		$sms="SELECT   A.PREFIXPERTANGGUNGAN || A.NOPERTANGGUNGAN NOPOLIS,A.PREFIXPERTANGGUNGAN,
		 ...
		 WHERE       a.prefixpertanggungan = b.prefixpertanggungan
				 ...
				 AND b.kdbank = 'BNI'
				 AND c.KDPRODUK NOT IN ('JSSPHANA')";
		//echo $sms;
		$DSMS->parse($sms);
		$DSMS->execute();
		$arsms=$DSMS->nextrow();
...
<?=$kantor;?>
  <tr>
    ...
    <td bgcolor="#C2CAED" rowspan="2" align="center"><font face="Verdana" size="1"><b>NO.
      POLIS</b></font></td>
    <td bgcolor="#C2CAED" colspan="3" align="center">
...
if($kantor=='KP') {
	$sql= "select ".
    	      ...
    	      "a.nopertanggungan,a.kdproduk,to_char(a.mulas,'DD/MM/YYYY') mulas,".
				...
				"where ".
					...
					//" and exists (select 'x' from $DBUser.TABEL_214_UNDERWRITING
    	      ...
    	      "a.nopertanggungan,a.kdproduk,to_char(a.mulas,'DD/MM/YYYY') mulas,".
				...
				"where ".
    				//"a.kdstatusemail='1' ".
					" (SUBSTR (a.kdproduk, 1, 3) NOT IN ('JL3', 'JL4'))".//and SUBSTR (a.kdproduk, -3) NOT IN ('BTN')) ".
    				"AND a.nopenagih is not null ".
    				"AND a.premi1 != '0' ".
    				"AND a.prefixpertanggungan = '$kantor' ".
    				//"AND a.kdstatusmedical in ('N') ".
    				"AND a.kdpertanggungan = '1' ".
    				"AND a.kdstatusfile = '1' ".
					" and exists (select 'x' from $DBUser.TABEL_214_UNDERWRITING
						where prefixpertanggungan=a.prefixpertanggungan ".
                 "and nopertanggungan=a.nopertanggungan ) ".
				 " and exists (select 'x' from $DBUser.TABEL_300_HISTORIS_PREMI
						where prefixpertanggungan=a.prefixpertanggungan ".
                 "and nopertanggungan=a.nopertanggungan and kdkuitansi='BP3') ".
					" and not exists (select 'x' from $DBUser.TABEL_214_ACCEPTANCE_DOKUMEN
						where prefixpertanggungan=a.prefixpertanggungan ".
                 "and nopertanggungan=a.nopertanggungan ) ".
				 //ini ditambahin salman tgl 01/04/2016, request nya fahmi
				 "UNION select ".
    				"a.prefixpertanggungan,a.kdstatusmedical,a.juamainproduk,".
    	      "a.nopertanggungan,a.kdproduk,to_char(a.mulas,'DD/MM/YYYY') mulas,".
    				"a.lamaasuransi_th,a.premi1,".
	...
	#----------------------------[ START CEK PELUNASAN PREMI ]----------------------------
	 ...
	 $sqlx = "select NVL(TO_CHAR(tglseatled,'DD/MM/YYYY'),'X') TGLSEATLED ".
	   		 	 "from $DBUser.tabel_300_historis_premi ".
	  		 	 "where prefixpertanggungan='".$arr["PREFIXPERTANGGUNGAN"]."' and nopertanggungan='".$arr["NOPERTANGGUNGAN"]."' ".
	  			 "and  kdkuitansi in ('BP3') order by tglbayar ";
	 //echo $sqlx;
	 $DC->parse($sqlx);
	 $DC->execute();
	 $arx=$DC->nextrow();
...
//	echo "Ari ".$nilairp." - ".$PERT->premi1." - ".$TR->Kurs($PERT->valuta,$tglbp3);
  ...
  <tr>
    ...
    <td bgcolor="#FFFFFF"><font face="Verdana" size="1"><?=$PERT->namaagen;?></font></td>
...
//-------------------------------------------------- start medical --------------------------------
	$sql  = "select ".
          ...
          "where ".
                ...
                "and a.prefixpertanggungan='$kantor' and a.kdstatusemail ='1' ".
      					"and a.nopenagih is not null ".
      					"and a.kdpertanggungan='1' ".
                "and a.kdstatusfile = '1' ".
				"AND substr(a.kdproduk,1,3) in ('JL4') ".
      					"and a.kdstatusmedical='XM' and not exists (select 'x' from $DBUser.TABEL_214_ACCEPTANCE_DOKUMEN
						where prefixpertanggungan=a.prefixpertanggungan ".
