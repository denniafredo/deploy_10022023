...
$tabel=($nottemp) ? "from $DBUser.tabel_200_pertanggungan a,$DBUser.tabel_247_pertanggungan_basis b " : "from $DBUser.tabel_200_temp a,$DBUser.tabel_247_temp b ";
	...
	if ($kdproduk=='AJSMBPA' || $kdproduk=='AJSMBPB' || $kdproduk=='AJSMBPC') {
		...
		$DB->parse($sql);
		$DB->execute();
		$arr_dnawl=$DB->nextrow();
		$danaawal = $arr_dnawl['NILAIBENEFIT'];
	 }
	...
	if($kdproduk=="AJSPP") {
		$sql=//"select 1 x, t, tarif from ".$tabeltebus." where ".
		     ...
		     "(kdproduk,masa,cara,usia,kdvaluta,kdbasis)=".
				 ...
				 "and kdtarif='TEBUS' ".
				 "order by x, t";
	} else if ($kdproduk == 'JSMANTAP') {
		$sql = "SELECT '-' AS t, nilai tarif
				FROM $DBUser.tabel_223_historis_transaksi
				WHERE prefixpertanggungan = '$pref'
					AND nopertanggungan = '$noper'
					AND nourut = (
						SELECT MAX(nourut) 
		...
		if ($res["TARIF"] <> 0){
			...
			if ($kdvaluta=='1') {
				if (substr($kdproduk,0,4)=="JSSP" || substr($kdproduk,0,4)=="JSSH") {
					...
					print( "<td align=\"right\" class=verdana9>".number_format($prm/1000*$res["TARIF"],0)."</td>" );
				}elseif ($kdproduk=='AJSMBPA' || $kdproduk=='AJSMBPB' || $kdproduk=='AJSMBPC') {
					print( "<td align=\"right\" class=verdana9>".number_format($danaawal/100*$res["TARIF"],0)."</td>" );
				} elseif ($kdproduk=='JSMANTAP') {
					print( "<td align=\"right\" class=verdana9>".number_format(round($res["TARIF"],0),0)."</td>" );
				} else {
					//echo 'disnin'.$jua;
