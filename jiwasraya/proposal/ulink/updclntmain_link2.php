...
ob_start();
  ...
  $DB->parse($sql);
   ...
   //echo $sql;
	 ...
	 $DB->execute();
	 $DB->commit();
	 if($_POST['pempre']=="pempre" && $_POST['tert']=="tert"){
	 header("location:http://$HTTP_HOST/$KAMP/proposal/ulink/ntryprop_ulink1.php?noklien=$noklien&pempre=$noklien&pempol=$nopempol");
   $sql1="insert into $DBUser.tabel_113_temp (noklieninsurable,kdhubungan,notertanggung) values ('$noklien','X','$noklien')";
   $DB->parse($sql1);
   $DB->execute();
   }elseif($_POST['tert']=="tert" && $_POST['pempre']==""){
	 $sql1="insert into $DBUser.tabel_113_temp (noklieninsurable,kdhubungan,notertanggung) values ('$noklien','X','$noklien')";
   $DB->parse($sql1);
   $DB->execute();
   header("location:http://$HTTP_HOST/$KAMP/proposal/ulink/editclntmain_link3.php?noklien=$noklien&pempol=$nopempol");
	 }elseif($_POST['pempre']=="pempre" && $_POST['tert']==""){
	 $sql1="insert into $DBUser.tabel_113_temp (noklieninsurable,kdhubungan,notertanggung) values ('$noklien','".$_POST['kdhubungan']."','2')";
   $DB->parse($sql1);
   $DB->execute();
   header("location:http://$HTTP_HOST/$KAMP/proposal/ulink/editclntmain_link3.php?pempre=$noklien&pempol=$nopempol");
   }else{
   echo "<br>Proses Gagal<br><br>";
	 echo "<a href=editclntmain_link1.php>Kembali</a>";
	 }
