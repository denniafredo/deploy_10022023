...
if($_POST['sbmt'] == '1')
{
	...
	for($i=0; $i<sizeof($cheklist); $i++)
	{
		...
		$sql = "SELECT nopolbaru FROM $DBUser.tabel_200_pertanggungan WHERE prefixpertanggungan||nopertanggungan = '$NoPolis'";
		$DB->parse($sql);
		$DB->execute();
		$r = $DB->nextrow();
		echo $x.". ".$r['NOPOLBARU']."<br>";
		//echo $x.". ".$NoPolis."<br>";

		//Menambahkan menu insert ke tabel_216_mutasi_pending untuk mencatat kapan proposal dikirim email ke HO pada masing-masing nomor proposal. (Teguh 08/11/2019)
	    $sqlip = "INSERT INTO $DBUser.tabel_216_historis_mutasi_uw (prefixpertanggungan, nopertanggungan, status, kategoripending, sumberpending, keterangan, tglrekam, userrekam)
	                    VALUES ('".substr(".$NoPolis.",1,2)."', '".substr(".$NoPolis.",3,9)."', 'Proposal Baru Dikirim',  '-', '-', '-', sysdate, '$userid')";
	    $DB->parse($sqlip);
	    $DB->execute();
...
<font face="Verdana" size="2"><b>PEMBERITAHUAN KELENGKAPAN DOKUMEN</b><br>
Kantor : <?echo $kantor; ?>
<hr size="1">
<a class="verdana8blu">
<font color="red"><b>Perhatikan, Agar dibaca dengan seksama ! </b></font><br>
<ol type="1">
<li>Sekali proposal dikirim ke Head Office berarti Anda telah memeriksa 
kebenaran isi SPAJ sesuai dengan ketentuan underwriting yang berlaku dan telah 
melaksanakan SAE untuk dokumen SPAJ tersebut.</li>
<li>Setelah proposal dikirim maka terhadap data SPAJ dimaksud tidak dapat dikoreksi.</li>
<li>Untuk kasus terjadi kesalahan entry atau pengiriman, sejauh belum dilakukan akseptasi oleh Kepala Bagian Underwriting PP Head Office, Branch Office harus membuat berita acara untuk koreksi SPAJ dimaksud dengan ditanda-tangani oleh Branch Manager, Kasie Pertanggungan dan Kasie Operasional.</li>
<!--
...
<form name="xxx" method="POST" action="<?=$_SERVER['PHP_SELF']?>">
  <tr>
    ...
    <td bgcolor="#C2CAED" colspan="6" align="center">
      <p align="center"><font face="Verdana" size="1"><b>JENIS FUND</b></font></td>
...
*/
	...
	$sql= "select ".
    				"a.nopolbaru, a.prefixpertanggungan,a.kdstatusmedical,a.juamainproduk,a.notertanggung,".
    	      "a.nopertanggungan,a.kdproduk,substr(a.kdproduk,0,4) pr4,to_char(a.mulas,'DD/MM/YYYY') mulas,".
           ...
           (SELECT PREMI FROM $DBUser.TABEL_223_TRANSAKSI_PRODUK WHERE prefixpertanggungan=a.prefixpertanggungan and
             ...
             (select namacarabayar from $DBUser.tabel_305_cara_bayar where kdcarabayar = a.KDCARABAYAR) cara_bayar ".
				...
				"where ".
					...
					"AND substr(a.kdproduk,1,3) in ('JL3','JL4') ".
	...
	//echo "Ari ".$nilairp." - ".$PERT->premi1." - ".$TR->Kurs($PERT->valuta,$tglbp3)." - $tglbp3";
	//var_dump($TR);
    ...
    $numrowsDocSae = 0;
	 ...
	 ?><!--</font></td> -->
		<? 
			 ...
			 //echo 'topupbkl '.$arr["TOPUPBKL"];
			//if($arr["TOPUPBKL"]==0 || $arr["TOPUPBKL"]==''){
			  if($arr["TOPUPBKL"]==null){
	 ...
	 //echo $sqlx;
		...
		if (($arr["FF"]+ $arr["BF"]+ $arr["EF"]+ $arr["MM"]+ $arr["IG"]+ $arr["IH"])!=100){
			$catatan = "<font color=red>JENIS FUND TIDAK LENGKAP</font>";
		} 
		//elseif ($arr["PR4"]=='JL4B' && $catatanx <>''){ $catatan = $catatanx; }
		elseif ($arr["PR4"]=='JL4B' && $catatanx <>''){ $catatan = $catatan; }
		elseif ($arr["PR4"]=='JL4B'){ $catatan = $catatan; }
...
//-------------------------------------------------- start medical --------------------------------
	$sql  = "select ".
      				  "a.prefixpertanggungan,a.nopertanggungan,a.nopolbaru,a.notertanggung,".
          ...
          "where ".
                ...
                "and a.prefixpertanggungan='$kantor' and NVL(a.kdstatusemail,0) = 0 ".
      					"and a.nopenagih is not null ".
      					"and a.kdpertanggungan='1' ".
                "and a.kdstatusfile = '1' ".
				"AND substr(a.kdproduk,1,3) in ('JL3','JL4') ".
      					"and a.kdstatusmedical='M' ORDER BY a.prefixpertanggungan, a.nopertanggungan ";
  //echo $sql;
	$DB->parse($sql);
	$DB->execute();
  ...
  <tr>
    ...
    <td width="10%" bgcolor="#FFFFFF" align="center"><font face="Verdana" size="1"><? echo "<a href=\"#\" onclick=\"NewWindow('./polis.php?j=1&prefix=".$arr["PREFIXPERTANGGUNGAN"]."&noper=".$arr["NOPERTANGGUNGAN"]."','',800,500,1)\">"./*$arr["PREFIXPERTANGGUNGAN"]."-".$arr["NOPERTANGGUNGAN"]*/$arr['NOPOLBARU']."</a></font></td>";?>
	 ...
	 */
	 ...
	 ?><!--</font></td> -->
		<?php
			 ...
			 if (($arr["FF"]+ $arr["BF"]+ $arr["EF"]+ $arr["MM"]+ $arr["IG"]+ $arr["IH"])!=100){
				$catatan = "<font color=red>JENIS FUND TIDAK LENGKAPP</font>";
			} else {  
				$catatan = $catatan;}
		?>
