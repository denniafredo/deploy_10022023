 ...
 $sql = "select kdvaluta,namavaluta from $DBUser.tabel_304_valuta b ".
   ...
   $teks.="window.opener.document.ntryprop.kdvaluta.options[".$i."].text = '".$arr ["NAMAVALUTA"]."'; " ;
	...
	if ($cb=='X') {
	 ...
	 $sql = "select nopenagih from $DBUser.tabel_100_klien b,$DBUser.tabel_500_penagih a ".
			 	 "where a.nopenagih=b.noklien and a.kdstatuspenagih='03' and a.kdrayonpenagih='$kantor' and ".
				 "nvl(a.penagihskg,'0')='0' and b.namaklien1 like 'PENAGIH KHUSUS%' and b.namaklien1 not like '%AUTODEBET%' order by 1 desc";
	 //echo $sql;
	 //die;
	 $DB->parse($sql);
	 $DB->execute();
