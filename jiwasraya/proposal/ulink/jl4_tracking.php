...
if ($kantor=='KP') {$ktrnya="";} else {$ktrnya="AND a.prefixpertanggungan = '$kantor' ";}
	$sql= "select ".
    				"a.prefixpertanggungan,a.kdstatusmedical,a.juamainproduk,a.notertanggung,a.nopolbaru,".
		...
		DECODE (
         ...
         DECODE (
            ...
            AKSEP,
         DECODE (kdcetakpolis, NULL, 'N', 'Y') cetakpolis, DECODE (a.kdstatusemail, NULL, 'N', 'Y') MAIL ".
				"from ".
				    "$DBUser.tabel_200_pertanggungan a ,$DBUser.TABEL_214_ACCEPTANCE_DOKUMEN b ".
				"where ".    				
...
//	echo "Ari ".$nilairp." - ".$PERT->premi1." - ".$TR->Kurs($PERT->valuta,$tglbp3);
  ...
  <tr>
    ...
    <td bgcolor="#FFFFFF" align="center"><font face="Verdana" size="1"><? echo "<a href=\"#\" onclick=\"NewWindow('./polis.php?j=1&prefix=".$arr["PREFIXPERTANGGUNGAN"]."&noper=".$arr["NOPERTANGGUNGAN"]."','',800,500,1)\">".$arr["NOPOLBARU"]."</a></font></td>";?>
...
/*ga jadi karena berat loading nya : 23 jul 2013/ iie*/
	...
	echo $catatan; 
	if($arr["MAIL"]=="Y" && $arr["SKK"]=="Y" && $arr["APP"]=="Y" && $arr["BP3"]=="Y" && $arr["AKSEP"]=="Y" && $arr["CETAKPOLIS"]=="N"){
		echo "Dok dicetak pusat";
		//echo "<input type=\"button\" value=\"cetakdok\" onClick=\"NewWindow('../../polis/cetakdokumen.php?prefix=$arr[PREFIXPERTANGGUNGAN]&noper=$arr[NOPERTANGGUNGAN]','popuppage','620','500','yes')\" style=\"padding:2px;margin:0px;font-size:12px;font-face:verdana;\" />";		
	}
	?></font></td>
</tr>
	<? 
