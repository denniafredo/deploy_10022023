...
if ($jns=='topup'){
$sqled = "delete $DBUser.tabel_ul_transaksi_topup a where ".
      			 "a.prefixpertanggungan='$pfx' and a.nopertanggungan='$noptg' and TO_CHAR(TGLMUTASI,'DDMMYYYYHHMMSS')='$idel' ";
} else {
$sqled = "delete $DBUser.tabel_ul_transaksi_switchfund a where ".
  ...
  <tr>
    ...
    <td class="tblisi" align="center">
      ...
      $sql = "select a.*, NVL(STATUS,0) AS STATUS, TO_CHAR(TGLMUTASI,'DDMMYYYYHHMMSS') AS TGLMUTASIDETIL ".
      			 "from ".
      			 "$DBUser.tabel_200_pertanggungan c, $DBUser.tabel_ul_transaksi_topup a,".
		...
		$sql="select a.*, NVL(STATUS,0) AS STATUS, TO_CHAR(TGLPENGAJUAN,'DD/MM/YYYY') AS TGLMUTASI,". 			"TO_CHAR(TGLPENGAJUAN,'DDMMYYYY') AS TGLMUTASIDETIL ".
      			 "from ".
      			 "$DBUser.tabel_200_pertanggungan c, $DBUser.TABEL_ul_TRANSAKSI_SWITCHFUND a,".
      			 "$DBUser.tabel_500_penagih d ".
      			 "where ".
				 $kam.
      			 "a.prefixpertanggungan=c.prefixpertanggungan and a.nopertanggungan=c.nopertanggungan ".
...
//		$sts='TELAH DIAKSEPTASI OLEH HO';
		...
		if ($arr["STATUS"]!='0'){
  					 ...
  					 echo "<td align='center'><a href=info_tupup.php?pfx=".$arr["PREFIXPERTANGGUNGAN"]."&noptg=".$arr["NOPERTANGGUNGAN"]."&idel=".$arr["TGLMUTASIDETIL"]."&jns=".$jns."><img border='0' src='images/delete.gif' width='16' height='16' alt='Delete'></a></td>";
				}
      	print( " </tr>" );
       $i++;
      }			 
...
<table width="100%">
  <tr>
    <td width="50%" align="left"><a href="../../submenu.php?mnuinduk=150"><font face="Verdana" size="2">Menu New Unit Link</font></a></td>
