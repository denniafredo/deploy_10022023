<?php
	...
	class CetakSertifikat extends FPDF {
		...
		public function Sertifikat() {
			...
			$this->Image('./images/ttd_dir.png', 124, $this->getY()+4, 70);
			...
			$this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
			$this->Cell(62, $height, $nama_pejabat, 0, 0, 'C');
			$this->Cell($this->pg_w - ($this->num_space * 2) - 124, $height, '', 0, 0, 'C');
            $startnamadir = strpos($arr['NAMADIREKTUR'], '.');
            $endnamadir = strpos($arr['NAMADIREKTUR'], ',');
            $namadirektur = $startnamadir && $endnamadir ? substr($arr['NAMADIREKTUR'], 0, $startnamadir).substr(ucwords(strtolower($arr['NAMADIREKTUR'])), $startnamadir, $endnamadir - $startnamadir).substr($arr['NAMADIREKTUR'], $endnamadir) : ucwords(strtolower($arr['NAMADIREKTUR']));
			$this->Cell(62, $height, $namadirektur, 0, 1, 'L');
			$this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
			$this->Cell(62, $height, $nama_jabatan, 0, 0, 'C');
			$this->Cell($this->pg_w - ($this->num_space * 2) - 124, $height, '', 0, 0, 'C');
			$this->Cell(62, $height, ucwords(strtolower($arr['JABATANDIREKTUR'])), 0, 1, 'L');
		}
