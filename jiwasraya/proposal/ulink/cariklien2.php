...
<?
		...
		if ($no) {
			$sql="select noklien, namaklien1,gelar,tinggibadan,".
			     "beratbadan,to_char(tgllahir,'DD/MM/YYYY') tgllahir ".
			     "from $DBUser.tabel_100_klien where noklien='$no' and ".
					 "(status <> '9' or status is null)";
		} else if ($nama) {
		  $nama = strtoupper($nama);
		  $sql="select noklien, namaklien1,gelar,tinggibadan,beratbadan,".
				   "to_char(tgllahir,'DD/MM/YYYY') tgllahir ".
				   "from $DBUser.tabel_100_klien  where ".
					 "namaklien1 like '$nama%' and ".
					 "(status <> '9' or status is null)";
		} else if ($hi) {
			$sql="select noklien, namaklien1,gelar,tinggibadan,".
			     "beratbadan,to_char(tgllahir,'DD/MM/YYYY') tgllahir ".
					 "from $DBUser.tabel_100_klien ".
					 "where trunc(tglrekam)=trunc(sysdate) and userrekam='$userid' and ".
					 "(status <> '9' or status is null) ";
		}
					 //echo $sql;
  $DB->parse($sql);
