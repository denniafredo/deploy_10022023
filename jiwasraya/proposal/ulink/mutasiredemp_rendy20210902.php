...
<script language="JavaScript" type="text/javascript" src="../../includes/js/jquery-3.3.1.slim.min.js"></script>
...
function loadcek(theForm){
	...
	if (!polis =='') { 
		...
		if(cekjua.kdredemption.value=='U'){
			if((jmlawal-jml)<=1000) { var konfirmasi=confirm('Sisa Jumlah Unit kurang dari 1000, Lanjutkan dengan tebus?')
				if (konfirmasi){
...
if ($Submit) {
 ...
 else 
 {
     ...
     //echo $sql;
	   ...
	   echo 'cek'.$PER->namatertanggung; echo '</br>';
	   echo 'nopertanggungan'.$nopertanggungan;die;
        ...
        echo "<form name=\"cekjua\" id=\"cekjua\" action=\"$PHP_SELF\" method=\"post\" >";//onSubmit=\"return checkbox_checker();\"
        ?>
          <table border="0" width="600" bgcolor="#ED1C24" cellspacing="1" cellpadding="6">
            <tr>
              <td align="center" bgcolor="#ED1C24" class="verdana9wht"><b><?echo "NO. POLIS : ".$prefixpertanggungan." - ".$nopertanggungan; ?></b></td>
            </tr>
            <tr>
              <td align="center" bgcolor="#ED1C24" class="verdana9wht"><input type="text" name="prefixpertanggungan" class="c" size="2" maxlength="2" onFocus="highlight(event);document.porm.nopol.value=''" value="<?echo strtoupper($prefixpertanggungan);?>" onChange="javascript:this.value=this.value.toUpperCase();">
            ...
            <tr>
              <td align="center" bgcolor="#ED1C24" class="verdana9wht"><? echo "<input type=\"submit\" value=\"Lihat Data Polis\" name=\"insert\">";?></td>
            </tr>
              ...
              <td width="100%" bgcolor="#e5e5e5">
                ...
                echo "    </tr>";
				 ...
				 while ($arr=$DB->nextrow()) {
					...
					echo "<td class=verdana9blk>: ".$arr["KDFUND"]." <input type=\"text\" size=\"20\" id='btn".$arr["KDFUND"]."' name=".$arr["KDFUND"]." onfocus=\"highlight(event)\" onblur=\"return loadcek(document.cekjua,this.form.".$arr["KDFUND"].")\"></td>";
					//echo "<td class=verdana9blk align=center>".$arr["PORSI"]."</td>";
					echo "</tr>";
            ...
            <? 
				...
				?>
				<tr>
              <td align="center" bgcolor="#E4E4E4" class="verdana10redb"><?php 
			   if($PER->statusfile!="AKTIF"){
					echo 'MUTASI TIDAK DAPAT DIPROSES! POLIS SEDANG TIDAK AKTIF.</br>'; 
				} else if (substr($PER->produk,0,3)!="JL4"){
						echo 'MUTASI TIDAK DAPAT DIPROSES! PRODUK BUKAN JENIS UNIT LINK </br>';
				} else if($arrcek["ADA"]>=1){
					echo 'MUTASI TIDAK DAPAT DIPROSES! MASIH TERDAPAT TRANSAKSI YANG BELUM GOODFUND </br>';
				} 
			  else {
				  ?>
				<script>
					...
					echo "<input id='btSubmit' type=\"button\" value=\"SUBMIT\" name=\"Submit\" style=\"font-size: 8pt; font-family: Verdana\">";
					//echo "<button type=\"submit\" name =\"submit\" onclick=\"alert('Ajukan redemption?')\">Submit Pengajuan Redemption</button>";
				}
