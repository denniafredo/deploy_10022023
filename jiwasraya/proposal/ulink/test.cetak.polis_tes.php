<?php
	...
	class CetakPolis extends FPDF {
		...
		public function Init() {
			...
			/*===== asuransi tambahan =====*/
			$sqlx = "SELECT MIN (nilaibenefit) nilaibenefit,
						kdbnfx,
			...
			$sql = "SELECT juamainproduk AS jua, $DBUser.terbilang(juamainproduk) AS juaterbilang, 
						TO_CHAR(mulas,'DD/MM/YYYY') AS mulas, TO_CHAR(mulas + INTERVAL '1' YEAR, 'DD/MM/YYYY') AS mulas_1year, 
		...
		public function SPAJ() {
			...
			$this->font_title('B');
			$this->Cell($this->pg_w, $height, 'PT ASURANSI JIWASRAYA (PERSERO)', 0, 1, 'C');
			$this->custom_font(8,'');
			...
			$image->save($filename);
			/*end of QRCode */
			echo 'cek based '.base64_encode($this->prefix.$this->nopertanggungan); die;
		...
		public function DataPolis() {
			...
			/* add tambahan by salman 08/12/2015 */
			...
			if (!empty($this->art)) {
				...
				foreach($this->art as $i => $value) {
					//$kdbnfx = substr($kd_produk, 0, 3) == "JL3" ? "KK ".$value['KDBNFX'] : $value['KDBNFX']; --> change by salman by req bu Liza 08/12/2015
					$kdbnfx = "KK ".$value['KDBNFX'];
					//$mulas = substr($kd_produk, 0, 3) == "JL3" ? $tgl_mulas : trim($value["ATAS"]); --> change by salman by req bu Liza 08/12/2015
					$mulas = $tgl_mulas;
		...
		public function KetentuanPolis() {
			...
			foreach($this->arrk as $i => $value) {
				// cek header atau footer paragraph redaksi jika kdparagraph null atau = T, maka header
				if (!$value['KDPARAGRAPH'] || $value['KDPARAGRAPH'] == 'T') {
					$judul = str_replace("{N_NMPRODUK}", $this->arrp['NAMAPRODUK'], $value['JUDUL']);
					...
					$text  = str_replace("{N_NMPRODUK}", $this->arrp['NAMAPRODUK'], $text);
					...
					$this->SetFont('Arial','',9);
					if (!empty($text)) {
						$arr_text = explode("{NUMBER=", $text);
						foreach($arr_text as $j => $values) {
							if (strpos($values, "/}") !== false && !empty($values)) {
								$no = substr($values, 0, strpos($values, "/}"));
								$this->Cell($this->num_space+5, $height, NULL, 0, 0, 'C');
								$this->Cell(6, $height, $no, 0, 0, 'L');
								$values = substr($values, strpos($values, "/}")+2);
								$this->MultiCell($this->pg_w-($this->num_space*2+5)-6, 5, $values, 0, 'J');
							}
							else if (!empty($values)) {
								$values = str_replace("}", NULL, $values);
								$this->Cell($this->num_space+5, $height, NULL, 0, 0, 'C');
								$this->MultiCell($this->pg_w-($this->num_space*2+5), 5, $values, 0, 'J');
							}
						}
			...
			// cek redaksi jika kdparagraph = M
			foreach($this->arrk as $i => $value) {
	...
	// cek apakah produk yang ingin dicetak adalah produk asuransi jiwa kredit, jika ya redirect ke cetak kredit
		...
		header("Location: http://$_SERVER[SERVER_ADDR]/jiwasraya/proposal/ulink/test.cetak.polis.kredit.php?prefix=$data[prefix]&nopertanggungan=$data[nopertanggungan]");
	}
	else {
