...
function Cari(theForm) {
    NewWindow('./cariklien1.php', 'caripage', 650, 400, 1);
...
var maxBeneficary = 8;
var currBeneficary = 1;

function Beneficiari() {
	if($("input[name='notertanggung']").val() == "") {
		alert('Silahkan pilih no SPAJ!');
		return false;
	}
	if(currBeneficary <= 8){
		 //var a = document.ntryprop.notertanggung.value;
		a= $("input[name='notertanggung']").val();
		//var b = Number(document.ntryprop.demit.value);
		b= parseInt($("input[name='demit']").val());
		//document.ntryprop.demit.value = b + 1;
		b= parseInt($("input[name='demit']").val(b + 1));
		...
		isi = isi + "<td ><input onClick= 'kurangiBeneficary(this);' type=\"button\" name=\"del" + i + "\"  class=\"del\" value=\"X\" ></input>  </td>";
		isi = isi + "</tr>";

		$(isi).appendTo($('#tblbeneficaryisi'));
		currBeneficary++;
	}
...
function kurangiBeneficary(x){
	$(x).closest('tr').remove();
    currBeneficary--;
    return false;
}

function BeneficiariDel(thi) {
    var isi = '';
...
function Klear(theForm) {
    ...
    for (i = 0; i < theForm.length; i++) {
        if (theForm.elements[i].type == 'text') {
            if (theForm.elements[i].name == 'tglbp3' || theForm.elements[i].name == 'tglsp' || theForm.elements[i].name == 'noagen') {} else {
