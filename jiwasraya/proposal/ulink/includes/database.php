...
class Database {
    ...
    var $errorstring="";  // komplete html error
		var $numcols;
	// Start Tambahan from Farhan (11072022)
	var $user="NADM", 
		$pass="ifg#dbs#nadm#2020", 
		$server="IFGDB";
	// End Tambahan from Farhan (11072022)

    //var $fied;

    function Database($User = "", $Pass = "", $DB = "", $Server = 0) {
		// Perubahan from Farhan (11072022)
		// Perubahan nilai untuk koneksi ke database
        $this->DBUser = $this->user;
        $this->DBPass = $this->pass;
        $this->DBName = $this->server;
