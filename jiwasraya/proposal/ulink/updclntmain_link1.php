...
ob_start();
    ...
    $sql = "insert into $DBUser.tabel_100_klien(kdklien,noklien,namaklien1,gelar,jeniskelamin,tempatlahir, ".
	         "tgllahir,kdagama,meritalstatus,tinggibadan,beratbadan,kdid,noid,kdpekerjaan,kdhobby,pendapatan, ".
					 ...
					 //tambahan oleh dedi z untuk memenuhi permintaan PMN dari Div. URC 19/07/2011
						"npwp,warganegara,dik_akhir,tglexpired_id,namaibukand,alamattetap01,alamattetap02,status_tinggal,kdpropinsitetap,KDKOTAMADYATETAP,".
            "phonetetap01,fax_rmh,emailtetap,kodepostetap,alamattagih01,alamattagih02,kdpropinsitagih,KDKOTAMADYATAGIH,kodepostagih,no_ponsel2,".
  ...
  $DB->parse($sql);
   ...
   //echo $sql;
	 ...
	 header("location:http://$HTTP_HOST/$KAMP/proposal/ulink/ntryprop_ulink1.php?noklien=$noklien&pempre=$noklien&pempol=$noklien");
	 }elseif($_POST['pempol']=="pempol" && $_POST['pempre']=="pempre" && $_POST['tert']==""){
   $sql1="insert into $DBUser.tabel_113_temp (noklieninsurable,kdhubungan,notertanggung) values ('$noklien','".$_POST['kdhubungan']."','1')";
   $DB->parse($sql1);
   $DB->execute();
   $sql2="insert into $DBUser.tabel_113_temp (noklieninsurable,kdhubungan,notertanggung) values ('$noklien','".$_POST['kdhubungan']."','2')";
   $DB->parse($sql2);
   $DB->execute();
   //echo $sql1.'</br>'.$sql2;
	 header("location:http://$HTTP_HOST/$KAMP/proposal/ulink/editclntmain_link3.php?pempre=$noklien&pempol=$noklien");
	 }elseif($_POST['pempol']=="pempol" && $_POST['tert']=="tert" && $_POST['pempre']==""){
	 $sql1="insert into $DBUser.tabel_113_temp (noklieninsurable,kdhubungan,notertanggung) values ('$noklien','".$_POST['kdhubungan']."','1')";
   $DB->parse($sql1);
   $DB->execute();
   $sql2="insert into $DBUser.tabel_113_temp (noklieninsurable,kdhubungan,notertanggung) values ('$noklien','X','$noklien')";
   $DB->parse($sql2);
   $DB->execute();
