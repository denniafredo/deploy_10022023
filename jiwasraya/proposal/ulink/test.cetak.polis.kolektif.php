...
$pdf->Open();
	...
	$sqlk="select * from $DBUser.tabel_200_pertanggungan where PREFIXPERTANGGUNGAN='KN'
AND NOPERTANGGUNGAN IN ('002118452','002115683')";
	$DC->parse($sqlk);
	$DC->execute();

while($arr=$DC->nextrow()){


//$prefix				= trim($_GET['prefix']);
//$nopertanggungan	= trim($_GET['nopertanggungan']);	

$prefix				= trim($arr["PREFIXPERTANGGUNGAN"]);
$nopertanggungan	= trim($arr['NOPERTANGGUNGAN']);	
$tglCetak			= date("Ymd");
$jamCetak			= date("His");
$premi1				= 0;
$premi2				= 0;
