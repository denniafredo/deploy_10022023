	...
	class CetakPolis extends FPDF {
		...
		public function Init() {
			...
			$sql = "SELECT a.PREFIXPERTANGGUNGAN, a.TGLSP, TO_CHAR(a.MULAS, 'DD/MM/YYYY') AS MULAS, a.USIA_TH, a.NOPERTANGGUNGAN, 
						...
						/*CASE WHEN a.KDPRODUK = 'JSSHTBU' THEN (SELECT NAMAPEJABAT FROM $DBUser.TABEL_002_PEJABAT WHERE KDKANTOR = '$this->prefix' AND KDORGANISASI = '160') 
						...
						(SELECT pj.NAMAJABATAN FROM $DBUser.TABEL_001_KANTOR kn, $DBUser.TABEL_400_AGEN ag, $DBUser.TABEL_002_PEJABAT pj WHERE kn.KDKANTOR = ag.KDKANTOR AND ag.NOAGEN = a.NOAGEN AND kn.KDKANTOR = pj.KDKANTOR AND pj.KDORGANISASI = '160' ) AS NAMAJABATAN,
						...
						i.KDMAPPING, TO_CHAR(a.TGLCETAK, 'ddmmyyyy') TGLCETAKQR, a.NOREKENINGDEBET,
		...
		public function SPAJ() {
			...
			$this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
			$this->Cell(($this->pg_w-($this->num_space*2))/3, $height, 'Pejabat yang berwenang', 0, 0, 'C');
			$this->Cell(($this->pg_w-($this->num_space*2))/3, $height, NULL, 0, 0, 'C');
		...
		public function DataPolis() {
			...
			foreach($this->arrp2 as $i => $value2) {
				if ($value2['KDBENEFIT'] == 'RESTPREHT' && $arr['KDPRODUK'] == 'JSAEP') {
					$akhirasuransi = $value2['EXPIRASI']; 
					break;
				} else {
					$akhirasuransi = '-';
				}
			...
			if ($kd_cara_bayar == "X" /*|| $kd_cara_bayar == "E" || $kd_cara_bayar == "J"*/ || $kd_sts_medical == "M" || $kd_ht == "HT" || $lama_premi < 5) {
				$this->Cell(($this->pg_w-($this->num_space*2))*17/25, $height, "Rp.".number_format($premi1,2)." DIBAYAR SECARA ".$arr["NAMACARABAYAR"], 0, 1, 'L');
			}
			else if ($premi2 > 0) {
			...
			$this->Cell(($this->pg_w-($this->num_space*2))*4/25, $h*2, $arr['MULAS'], 0, 0, 'C');
			$this->Cell(($this->pg_w-($this->num_space*2))*4/25, $h*2, '-', 0, 0, 'C');
			$this->Cell(($this->pg_w-($this->num_space*2))*3/25, $h*2, $akhirasuransi, 0, 1, 'C');
			...
			if (!empty($this->art)) {
				...
				foreach($this->art as $i => $value) {
					...
					$y = $this->GetY();
					$h = $y2 == $this->GetY ? 4 : $y2 - $y;
					$this->Cell(($this->pg_w-($this->num_space*2))*3/25, $h, trim($value["KDBNFX"]), 0, 0, 'L');
					$this->Cell(($this->pg_w-($this->num_space*2))*4/25, $h, number_format($value["NILAIBENEFIT"], 2, ",", "."), 0, 0, 'L');
					$this->Cell(($this->pg_w-($this->num_space*2))*4/25, $h, trim($value["ATAS"]), 0, 0, 'C');
