...
class CetakPolis extends FPDF {
    ...
    public function Init() {
        ...
        $sql = "SELECT a.PREFIXPERTANGGUNGAN, a.TGLSP, TO_CHAR(a.MULAS, 'DD/MM/YYYY') AS MULAS, a.USIA_TH, a.NOPERTANGGUNGAN, 
					...
					j.KDMAPPING, TO_CHAR(SYSDATE, 'ddmmyyyy') TGLCETAKQR, a.NOREKENINGDEBET, d.MERITALSTATUS,
					...
					(SELECT 'D$this->prefix' || TO_CHAR(SYSDATE, 'yyyy') || LPAD(COUNT(*)+1, 2, '0') FROM $DBUser.TABEL_248_BLANKO_CETAK WHERE PREFIXPERTANGGUNGAN = a.PREFIXPERTANGGUNGAN AND NOPERTANGGUNGAN = a.NOPERTANGGUNGAN) NOBLANKO, a.NOPOL,
					...
					CASE WHEN TO_DATE('01/01/2015', 'dd/mm/yyyy') > a.mulas THEN 1 ELSE 0 END polislama
    ...
    public function DataPolis() {
        ...
        if ($kd_cara_bayar == "X" || $kd_cara_bayar == "E" || $kd_cara_bayar == "J" || $kd_cara_bayar == "HT" || $lama_premi < 5) {
            ...
            $this->Cell(($this->pg_w-($this->num_space*2))/25, $height, ":", 0, 0, 'L');
            $this->SetFont('Arial','',9);
            $this->Cell(($this->pg_w-($this->num_space*2))*17/25, $height, $arr['NOTASI'].". ".number_format($arr['PREMISTD'],2,",","."), 0, 1, 'L'); // $premi_standard -> $premi1
            //$this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
        ...
        // add kondisi by salman 10/12/2015
    ...
    public function LampiranPolis() {
        ...
        if(substr($arr["KDPRODUK"], 0, 4) != "JL4B") {
            ...
            if ($lampiranjl4b) {
                ...
                $this->font_body();
                $this->Cell($this->num_space+6, $height, NULL, 0, 0, 'C');
                $this->MultiCell($this->pg_w-($this->num_space*2)-6, 5, "No Claim Bonus atau potongan premi (diskon) sebesar 3 (tiga) bulan premi diberikan kepada Pemegang polis yang melakukan perpanjangan Asuransi Tambahan JS Hospital Care, dengan ketentuan apabila selama masa Asuransi Tambahan JS Hospital Care sebelumnya, Penanggung tidak pernah melakukan pembayaran klaim kepada Tertanggung. Besar manfaat no claim bonus yang menjadi hak Pemegang Polis adalah 3 (tiga) bulan premi dari premi Asuransi Tambahan JS Hospital Care dan akan diperhitungkan pada pembayaran premi untuk Asuransi Tambahan JS Hospital Care tahun berikutnya.", 0, 'J');
            }
        }
    }
