	   ...
	   $PER	= New Pertanggungan($userid,$passwd,$prefix,$nopertanggungan);
		// echo "<br />Prefix = ".$prefix;
		 ...
		 /*$qunit = "SELECT a.NOMOR_POLIS,b.kode_fund,sum(UNIT* ".
				...
				$qunit="select nomor_polis,sum(unit * case trx_type when 'S' then 1 when 'T' then 1 when 'R' then -1 when 'C' then -1 end)
                           saldo,substr(kode_fund, 5, 2)fund,(select nab_jual from $DBUser.tabel_ul_nab where kode_fund=substr(b.kode_fund, 5, 2) ".
          ...
          <table border="0" width="600" bgcolor="#006699" cellspacing="1" cellpadding="6">
            ...
            <tr>
              ...
              <td width="100%" bgcolor="#DAE2EF">
                ...
                echo "      <td class=\"verdana9blk\">: ".$aru["NAB"]."</td>";
                echo "    </tr>";
				echo "    <tr>";
                echo "      <td class=\"verdana9blk\">Dalam Rupiah</td>";
                echo "      <td class=\"verdana9blk\">: ".number_format(($aru["NAB"]*$jmlunit),2)."</td>";
                echo "    </tr>";
                echo "    <tr>";
