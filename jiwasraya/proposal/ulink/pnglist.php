  ...
  if (!$nama) {
   ...
   $dannama = "and b.namaklien1 like '".strtoupper($nama)."%' "; 
	...
	if($modeshow=="all") {
		...
		{
			if (substr($prd,0,3)=='JL4' || substr($prd,0,3)=='JL3') {
	...
	$sql="select kdkantor from $DBUser.tabel_888_userid where userid='".$userid."' and decrypt(password)='".$passwd."'";
	$DA->parse($sql);
	$DA->execute();
	$arx=$DA->nextrow();
	$tes=$arr["KDKANTOR"];
	//$con=ocilogon("JSADM","x","JSDB");
