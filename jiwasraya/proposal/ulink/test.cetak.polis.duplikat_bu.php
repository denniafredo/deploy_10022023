<?php
			...
			foreach($this->arrk as $i => $value) {
				if ($value['KDPARAGRAPH'] == 'M') {
					...
					foreach($this->arrp2 as $i => $value2) {
						...
						$text = str_replace("{T_".$value2['KDBENEFIT']."}", $expirasi, $text);
						$text = str_replace("{B_".$value2['KDBENEFIT']."}", $value2['EXPIRASIB'], $text);
						$text = str_replace("{A_".$value2['KDBENEFIT']."}", $value2['AKHPMB'], $text);
					}
	...
	if (in_array($produk['KDPRODUK'], $anuitas)) {
		//echo "belum dibuat produk anuitas duplikat";
	...
	// cek apakah produk yang ingin dicetak adalah produk jl4 (js promapan), jika ya redirect ke cetak newlink
        ...
        header("Location: test.cetak.polis.duplikat.linknew_bu.php?prefix=$data[prefix]&nopertanggungan=$data[nopertanggungan]");
	}
	else if (in_array($produk['KDPRODUK'], $restru)) {
        header("Location: test.cetak.polis.duplikat.restru_bu.php?prefix=$data[prefix]&nopertanggungan=$data[nopertanggungan]");
	}
	else {
