...
if($_POST['sbmt'] == '1')
{
	...
	for($i=0; $i<sizeof($cheklist); $i++)
	{
		...
		//echo $sqlUpd;
		//================SMS SENDING================
		$sms="SELECT   A.PREFIXPERTANGGUNGAN || A.NOPERTANGGUNGAN NOPOLIS,c.NOSP,to_char(c.tglsp,'yyyy-mm-dd') tglsp,
				 NOACCOUNT,
				 (SELECT   MAX (NILAIRP)
				 ...
				 + (SELECT   SUM (NILAI)
					 ...
					 WHERE       prefixpertanggungan = b.prefixpertanggungan
							 ...
							 AND JENIS = 'B')
					PREMI,
				 (SELECT   decode(c.kdproduk,'JL4BA',TO_CHAR (EXPIREDDATE, 'YYYY-MM-DD HH24:MI'),TO_CHAR (EXPIREDDATE, 'DD/MM/YYYY')) EXPDATE
					FROM   $DBUser.TABEL_300_TAGIHAN_PERTAMA
				   WHERE       prefixpertanggungan = b.prefixpertanggungan
						   AND nopertanggungan = b.nopertanggungan
						   AND JENIS = 'B')
					EXPDATE,
				 REGEXP_REPLACE((SELECT   PHONETETAP01
				 ...
				 (SELECT   NAMAKLIEN1
				   ...
				   WHERE   NOKLIEN = NOPEMEGANGPOLIS)
					NAMA,c.kdproduk,
					(select kdmapping from $DBUser.TABEL_001_KANTOR where kdkantor=c.prefixpertanggungan )||c.nopertanggungan h2h,substr(a.alasanunderwriting,1,49) alasanuw
...
//[Nama_Tertanggung]
//[No_Host_to_Host]
//[No_Virtual_Account_Mandiri]
//[No_Polis]
//[No_HP]
//--------end of contoh format--------

		//$mysqlins="insert into smsjiwasraya (PHONE, MESSAGE) VALUES('081212524247','".$msg."')";
		//$mysqlins2="insert into smsjiwasraya (PHONE, MESSAGE) VALUES('085883160055','".$msg."')";
		mysql_query($mysqlins2);
		}else{
		//if(date("Ym")=="201709"){
		...
		/*}else{
			$msg="Yth. Bpk/Ibu ".$arsms["NAMA"].", harap lakukan pembayaran premi melalui transfer ke Virtual Acc BNI ".
			 $arsms["NOACCOUNT"]." sebesar ".$arsms["PREMI"]." paling lambat ".$arsms["EXPDATE"]." Utk info hub 021-1500151";			 
		}*/	 
		...
		// ===================== CREATE VA BNI (DEV) ===========================//
			...
			// $DL = new Database('AAAA','AAAA',"JSDB");
			$sql_seq = "select SEQ_ECOLL_T1.nextval@JLINDO_JL NEXT from DUAL";
			$DB->parse($sql_seq);
			$DB->execute();
			$seq=$DB->nextrow();
			//var_dump($seq);exit;
			...
			$client_id = '00106';
			$p_host = "https://192.168.1.26:4433/api_bni_va/inquiry.php";
			$thepost = FALSE;

			$api_url = 'dev';//'prod'
			$arsms['NOACCOUNT'] = '988' . $client_id . substr($arsms['NOPERTANGGUNGAN'], -8);
			$arsms['KETERANGAN'] = @date('yymm').'-FIRST PAYMENT-'.$arsms['NOPERTANGGUNGAN'].'-'.$arsms["NAMA"];
			$arsms['URUTAN'] = 0;
			if($seq > 0){
				$thepost['$data'][] = array(
					'type' => 'createbilling', 
			...
			if(is_array($dat) && count($dat) > 0){
				...
				if(isset($dat['status']) &&  $dat['status'] == "102"){
					
				}else{
					$sql_ins = "
						insert into TABEL_315_PELUNASAN_VA_ECOLL@JLINDO_JL  (
							...
							,NOHP ,NOACCOUNT ,NAMAKLIEN ,KETERANGAN ,KDBANK ,JENIS ,CABANGBANK ,AMOUNT
							,TRXID,URUTAN
						) values (
							'"."ECOLSYS"."','".'31'."','07','".'2018'."',sysdate,to_date('"."2018-08-31"." 23:59:59','yyyy-mm-dd hh24:mi:ss'),'".'0'."',
							'".$arsms['PREFIXPERTANGGUNGAN']."','".$arsms['NOPERTANGGUNGAN']."','".''."','".''."',
							'".$arsms['PHONE']."','".$arsms['NOACCOUNT']."','".$arsms['NAMA']."','".$arsms['KETERANGAN']."','".'BNI'."','".'VA'."','".'BNI PUSAT'."','".$arsms['PREMI']."'
							,".$seq.",".$arsms['URUTAN']."
						) ";
						...
						echo ' Nomor VA BNI: '.$arsms['NOACCOUNT'] . ' berhasil terbentuk!<br />';
						}
...
<font face="Verdana" size="2"><b>DAFTAR UNDERWRITING POLIS</b><br>
Kantor : <?echo $kantor; ?>
	...
	$sql  = "select ".
          ...
          "where ".
				...
				"AND substr(a.kdproduk,1,3) not in ('JL3','JL4') ".
      					"and a.kdstatusmedical='M' and not exists (select 'x' from $DBUser.TABEL_214_UNDERWRITING
						where prefixpertanggungan=a.prefixpertanggungan ".
