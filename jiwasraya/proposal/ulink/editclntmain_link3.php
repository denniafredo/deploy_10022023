...
<!--
function CekSetuju1(theForm) {
 if ( theForm.pempre.checked) {
...
function CekSetuju2(theForm) {
 if (theForm.tert.checked) {
  theForm.Submit.disabled=false
 } else {
  theForm.Submit.disabled=true
 }
...
<form name="klien" action="updclntmain_link3.php" method="POST" onSubmit="return OnSumbit(document.klien)">
  <tr>
    <td width="100%">
    <table width="100%" border="0" cellpadding="4" cellspacing="0" class="tblisi1">
      <tr>
        ...
        <td colspan="2"><font color="yellow"><? echo $namaklien; ?></td>
				<td align="right"><b><font color="red"><? $a = ($_GET["pempre"]) ? 'ENTRY TERTANGGUNG' : 'ENTRY PEMBAYAR PREMI';echo $a; ?></td>
      </tr>
      <tr>
        ...
        <p align="right">Status : <b>Aktif</td>
      </tr>
    </table>
    </td>
  </tr>
  <tr>
    <td width="100%">
    <table width="100%" border="0" cellpadding="2"  class="tblisi" cellspacing="4">
      <tr>
	 ...
	 echo $pddk;
        ...
        $query = "select kdpropinsi,namapropinsi from $DBUser.tabel_108_propinsi order by namapropinsi";
	      ...
	      ?>
            ...
            <td class="verdana10blk">Kode Pos</td>
            <td class="verdana10blk"><input type="text" class="c"  onFocus="highlight(event)" name="kdposK																																	P" size="20" maxlength="20" value=""></td>
          </tr>
          <tr>
            <td colspan="4" class="verdana10blk"><hr></td>
          </tr>
...
$sbrLain=array(0=> "1/Bonus","2/Komisi","3/Penjualan Aset","4/Hasil Investasi","5/Hibah","6/Warisan","7/Lainnya","8/Tidak ada Penghasilan");
  ...
  $disabled="disabled";
    ...
    <!--<input type="checkbox" name="pempol" value="pempol" onClick="CekSetuju(document.klien)" checked="checked" disabled> Pemegang Polis <br/ >-->
    <input type="hidden" name="pempol" value="<?php echo $_GET["pempol"]; ?>">
    <?php if($_GET["tert"]!=""){ ?>
    <input type="hidden" name="tert" value="<?php echo $_GET["noklien"]; ?>">
    <input type="checkbox" name="pempre" value="pempre" onClick="CekSetuju1(document.klien)"> Pembayar Premi
    <?php }else{ ?>
    <input type="hidden" name="pempre" value="<?php echo $_GET["pempre"]; ?>">
    <input type="checkbox" name="tert" value="tert" onClick="CekSetuju2(document.klien)"> Tertanggung
    <?php } ?>
    <div align="left">
    Hubungan Dengan Tertanggung : <select class="a" size="1" name="kdhubungan" onFocus="highlight(event)">
	<?php
