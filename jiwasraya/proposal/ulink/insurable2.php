  ...
  $sql ="select b.noklieninsurable, namahubungan, c.namaklien1 ".
	...
	$adan=ereg_replace("'","`",$arr["NAMAKLIEN1"]);
	  include "./includes/belang.php";
		...
		"window.opener.document.$porm.namatertanggung2.value='%s'; ".
		"window.opener.document.$porm.notertanggung2.value='%s'; ".
		//"window.opener.document.$porm.$hubungan".$n.".value='%s'; ".
		"window.close();\">%s</a></td><td class=verdana8>%s</td><td class=verdana8>%s</td>",$adan,$arr["NOKLIENINSURABLE"].
		"",$arr["NOKLIENINSURABLE"],$arr["NAMAKLIEN1"],$arr["NAMAHUBUNGAN"]);
		$i++;
	}
?>
</table>
