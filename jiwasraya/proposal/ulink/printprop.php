              ...
              <?
               $sql = "select juamainproduk,premistd,nosp,to_char(tglsp,'DD/MM/YYYY') tglsp,nobp3,to_char(tglbp3,'DD/MM/YYYY') tglbp3,".
               			  "kdstatusmedical,to_char(mulas,'DD/MM/YYYY') mulas,to_char(expirasi,'DD/MM/YYYY') expirasi,".
              				...
              				"decode (a.gadaiotomatis,'1','SETUJU','TIDAK SETUJU') gpo, a.kdproduk,".
...
?>
		 	 	...
		 	 	<tr>
			 		<td width="100%" colspan="6"><b>Porsi Fund</td>
				</tr>
                <tr>
			 		<td colspan="3">
                    <table width="100%" cellpadding="1" cellspacing="1" class="sans8">
                    <td width="30%" align="center">Jenis Fund</td>
					<td width="20%">Porsi Fund</td>
                    <?php
					$sql="select b.namafund,porsi from $DBUser.tabel_ul_opsi_fund_temp a,$DBUser.tabel_ul_kode_fund b where".
						 " prefixpertanggungan='$prefixpertanggungan' and nopertanggungan='$noproposal' and a.kdfund=b.kdfund ";
						 
						 //echo $sql;
						 $DB->parse($sql);
						 $DB->execute();
						 while($arrfund=$DB->nextrow()){
						 echo "<tr><td align=left>".$arrfund["NAMAFUND"]."</td><td align center>".$arrfund["PORSI"]."%</td></tr>";
						 }
					?>
...
/*$sql = "select a.nourut, a.noklien, b.namahubungan ".
		   "from ".$t219." a, $DBUser.tabel_218_kode_hubungan b ".
			 ...
			 $sql = "select a.nourut, a.noklien, b.namahubungan,c.namaklien1,c.namaklien2 ".
		   "from $DBUser.tabel_219_pemegang_polis_baw a, $DBUser.tabel_218_kode_hubungan b,$DBUser.tabel_100_klien c ".
			 "where a.prefixpertanggungan='$prefixpertanggungan' and a.nopertanggungan='$noproposal' and a.noklien=c.noklien ".
			 "and a.notertanggung='$ttg' and a.kdinsurable=b.kdhubungan order by a.nourut";
	//echo $sql;
	$DB->parse($sql);
	$DB->execute();
	...
	while ($ara=$DB->nextrow()) {
  	...
  	echo "<td>".$ara["NAMAHUBUNGAN"]."</td>";
		echo "<td></td>";
  	//echo "<td>".$nama."</td>";
	echo "<td>".$ara["NAMAKLIEN1"].$ara["NAMAKLIEN2"]."</td>";
  	echo "</tr>";
		$i++;
  }	
