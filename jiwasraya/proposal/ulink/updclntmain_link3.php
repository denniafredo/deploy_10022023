...
ob_start();
  ...
  $DB->parse($sql);
   ...
   //echo $sql;
	 ...
	 $DB->execute();
	 $DB->commit();
	 if($_POST['pempre']=="pempre"){
	 $nopempol=$_POST['pempol'];
   $tert=$_POST['tert'];
   $sql1="insert into $DBUser.tabel_113_temp (noklieninsurable,kdhubungan,notertanggung) values ('$noklien','".$_POST['kdhubungan']."','2')";
   $DB->parse($sql1);
   $DB->execute();
   header("location:http://$HTTP_HOST/$KAMP/proposal/ulink/ntryprop_ulink1.php?noklien=$tert&pempre=$noklien&pempol=$nopempol");
   }elseif($_POST['tert']=="tert"){
   $nopempol=$_POST['pempol'];
   $pempre=$_POST['pempre'];
   $sql1="insert into $DBUser.tabel_113_temp (noklieninsurable,kdhubungan,notertanggung) values ('$noklien','X','$noklien')";
   $DB->parse($sql1);
   $DB->execute();
	 header("location:http://$HTTP_HOST/$KAMP/proposal/ulink/ntryprop_ulink1.php?noklien=$noklien&pempol=$nopempol&pempre=$pempre");
	 }else{
