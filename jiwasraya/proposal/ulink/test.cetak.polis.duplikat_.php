<?php
	...
	class CetakPolis extends FPDF {
		...
		public function Init() {
			...
			$sql = "SELECT a.PREFIXPERTANGGUNGAN, a.TGLSP, TO_CHAR(a.MULAS, 'DD/MM/YYYY') AS MULAS, a.USIA_TH, a.NOPERTANGGUNGAN, 
						...
						j.KDMAPPING, TO_CHAR(SYSDATE, 'ddmmyyyy') TGLCETAKQR, a.NOPOL,
						(SELECT COUNT(*) FROM $DBUser.TABEL_223_TRANSAKSI_RIDER WHERE PREFIXPERTANGGUNGAN = a.PREFIXPERTANGGUNGAN AND NOPERTANGGUNGAN = a.NOPERTANGGUNGAN) JMLRIDER,
						...
						a.INDEXAWAL, CASE WHEN TO_DATE(SYSDATE, 'dd/mm/yyyy') <= ADD_MONTHS(a.MULAS, 12) THEN 1 ELSE 0 END AS TAMPILAN_II_KP, a.NOREKENINGDEBET,
						...
						CASE WHEN TO_DATE('01/01/2003', 'dd/mm/yyyy') > a.mulas THEN 1 ELSE 0 END polislama
			...
			$sql = "SELECT a.NOTERTANGGUNG, a.NOURUT, a.KDINSURABLE, a.NOKLIEN,
						CASE WHEN b.kdhubungan in ('2T', 'A2', 'G2', '04', 'K2', 'P', 'O2', 'PP', 'PM', 'T2', 'TA', 'TB', 'TC', 'TI', 'TS') THEN b.namahubungan ELSE b.namahubungan || ' TERTANGGUNG' END NAMAHUBUNGAN,
		...
		public function SPAJ() {
			...
			$height			= $this->height;
			$tcduplikat		= "DENGAN DITERBITKANNYA POLIS DUPLIKAT INI, MAKA POLIS DENGAN NOMOR : $arr[NOPOL] "
							. "YANG DITERBITKAN SEBELUMNYA DINYATAKAN TIDAK BERLAKU.";
			
			$this->SetFont('Arial','B',14);
			$this->Cell($this->pg_w, $height, 'POLIS ASURANSI JIWA', 0, 1, 'C');
			$this->SetFont('Arial','B',10);
			$this->Cell($this->pg_w, $height, "NOMOR POLIS : $arr[NOPOL]", 0, 0, 'C');
			...
			$x1 = $this->GetX();
			$y1 = $this->GetY();
			$this->Cell(($this->pg_w-($this->num_space*2))/3, 5, null , 0, 1, 'C');
			$this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
			$this->Cell(($this->pg_w-($this->num_space*2))/3, 4, null, 0, 0, 'C');
			...
			$image->save($filename);
			/*end of QRCode */
			$this->Image($filename, 158, 175, 30, 30);
		...
		public function DataPolis() {
			...
			$lamapembpremi  = $kd_cara_bayar == 'E'
							? '5 TAHUN'
							: ($kd_cara_bayar == 'J' 
							? '10 TAHUN'
							: ($arr['PERIODE_BULAN'] > 0 
							? "$arr[LAMAPEMBPREMI_TH] TAHUN $arr[PERIODE_BULAN] BULAN"
							: "$arr[LAMAPEMBPREMI_TH] TAHUN"));
			
			$this->font_title('B');
			$this->Cell($this->pg_w, $height, 'DATA POLIS', 0, 1, 'C');
			$this->Cell($this->pg_w, $height, "NOMOR : $arr[NOPOL]", 0, 0, 'C');
			...
			$this->MultiCell(($this->pg_w-($this->num_space*2))*17/25, 5, $arr["ALAMATPEMPOL"], 0, 'L');
			//
			$this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
			...
			/* tambahan untuk index awal kdvaluta = 0 oleh fendy */
			if ($arr['KDVALUTA'] == '0') {
				$this->SetFont('Arial','B',9);
				$this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
				$this->Cell(($this->pg_w-($this->num_space*2))*7/25, $height, "Index Awal", 0, 0, 'L');
				$this->Cell(($this->pg_w-($this->num_space*2))/25, $height, ":", 0, 0, 'L');
				$this->SetFont('Arial','',9);
				$this->Cell(($this->pg_w-($this->num_space*2))*17/25, $height, "$arr[INDEXAWAL]", 0, 1, 'L');
			}
			...
			// jika memiliki rider tampilkan
			if ($arr['JMLRIDER'] > 0) {
				$this->SetFont('Arial','B',9);
				$this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
				$this->Cell(($this->pg_w-($this->num_space*2))*7/25, $height, "Premi Jaminan Tambahan", 0, 0, 'L');
				$this->Cell(($this->pg_w-($this->num_space*2))/25, $height, ":", 0, 0, 'L');
				$this->SetFont('Arial','',9);
				$this->Cell(($this->pg_w-($this->num_space*2))*17/25, $height, "SESUAI LEMBAR REKAPITULASI PREMI", 0, 1, 'L');
			}
			//end add tambahan jika memiliki rider
			
			/* tambahan untuk produk JSSPKN & JSSPBTN & JSR1 & JSR2 &JSR3 topup disembunyikan oleh fendy */
			if (substr($kd_produk, 0, 2) == 'JL') {
				...
				if($arr["KDPRODUK"] == 'JL4B' && $arrs["PREMITUP"] > 0) {
					$this->SetFont('Arial','B',9);
					$this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
			...
			if($arr["NAMACARABAYAR"]<>'SEKALIGUS'){
				...
				$this->Cell(($this->pg_w-($this->num_space*2))/25, $height, ":", 0, 0, 'L');
				$this->SetFont('Arial','',9);
				$this->Cell(($this->pg_w-($this->num_space*2))*17/25, $height, $lamapembpremi, 0, 1, 'L');
			}
			...
			$this->SetFont('Arial','B',9);
			$this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
			$this->Cell($this->pg_w-($this->num_space*2), $height, "Penerima Manfaat Asuransi ".($arr['POLISLAMA'] ? 'Menurut Urutan' : null), 0, 1, 'L');
			$this->SetFont('Arial','',9);
			foreach($this->ars as $value) {
				...
				$this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
				$this->Cell($this->pg_w-($this->num_space*2), $height, trim($value["NOURUT"]).". ".trim($hub).", ".trim($nama), 0, 1, 'L');
			}
			$this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
			$this->Cell($this->pg_w-($this->num_space*2), $height, ($arr['POLISLAMA'] ? "AHLIWARIS TERTANGGUNG BERSAMA-SAMA" : null), 0, 1, 'L');
			...
			$this->Cell(($this->pg_w-($this->num_space*2))*3/25, $h*2, $arr["EXPIRASI"], 0, 1, 'R');
			...
			if (!empty($this->art)) {
				...
				foreach($this->art as $i => $value) {
					...
					$this->Cell(($this->pg_w-($this->num_space*2))*4/25, $h, $value["TGLAKHIRPREMI"], 0, 1, 'R');
					$this->Ln(1);
				}
			}
		}
		...
		public function KetentuanPolis() {
			...
			$this->Cell($this->pg_w, $height, "POLIS NOMOR : $arr[NOPOL]", 0, 0, 'C');
    ...
    $sql_u="UPDATE $DBUser.TABEL_700_DUPLIKAT SET TGLCETAK = SYSDATE, USERCETAK = '$userid', STATUS = '3', TGLUPDATED = SYSDATE, USERUPDATED = '$userid'
            WHERE prefixpertanggungan = '$prefix' AND nopertanggungan = '$nopertanggungan'
	...
	$anuitas = array('AI0', 'AI0B', 'AI0BNI', 'ASI', 'ASIBNI', 'ASIB', 'ASP', 'ASPBNI', 'ASPB', 'JSAEP', 'AI0N', 'AI0NB');
	$pa = array('PAA', 'PAB', 'PAAC');
	$kredit = array('AKM','AKL');
	$restru = array('JSTAMPAN','JSTAMPANX', 'JSMANTAP', 'AJSMBPA', 'AJSMBPB', 'AJSMBPC', 'JSANUITAS');
	...
	if (in_array($produk['KDPRODUK'], $anuitas)) {
		header("Location: test.cetak.polis.duplikat.anuitas.php?prefix=$data[prefix]&nopertanggungan=$data[nopertanggungan]");
	}
	// cek apakah produk yang ingin dicetak adalah produk pa, jika ya redirect ke cetak pa
	else if (in_array($produk['KDPRODUK'], $pa)) {
		header("Location: test.cetak.sertifikat.duplikat.akdp.php?prefix=$data[prefix]&nopertanggungan=$data[nopertanggungan]");
	}
	// cek apakah produk yang ingin dicetak adalah produk jl4 (js promapan), jika ya redirect ke cetak newlink
		...
		header("Location: test.cetak.polis.duplikat.linknew.php?prefix=$data[prefix]&nopertanggungan=$data[nopertanggungan]");
	}
	else if (in_array($produk['KDPRODUK'], $restru)) {
        header("Location: test.cetak.polis.duplikat.restru.php?prefix=$data[prefix]&nopertanggungan=$data[nopertanggungan]");
	}
	else {
