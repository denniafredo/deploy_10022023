<?php
    ...
    $data['paragraph0']      = "Berdasarkan ketentuan Pasal 12 POJK Nomor 1 Tahun 2013 tentang Perlindungan Konsumen Sektor Jasa "
                             . "Keuangan mengenai kewajiban menginformasikan kepada Konsumen setiap perubahan manfaat, biaya, "
                             . "risiko, syarat, dan ketentuan yang tercantum dalam dokumen dan/atau perjanjian mengenai produk "
                             . "dan/atau layanan Pelaku Usaha Jasa Keuangan, maka untuk memenuhi ketentuan tersebut Penanggung telah mengirimkan serta menyampaikan:";
    ...
    class CetakPolis extends FPDF {
        ...
        public function __construct($data) {
            ...
            $this->paragraph0      = $data['paragraph0'];
        ...
        public function Init() {
            ...
            $sql = "SELECT a.PREFIXPERTANGGUNGAN, a.TGLSP, TO_CHAR(a.MULAS, 'DD/MM/YYYY') AS MULAS, a.USIA_TH, a.NOPERTANGGUNGAN, 
                            ...
                            j.KDMAPPING, TO_CHAR(SYSDATE, 'ddmmyyyy') TGLCETAKQR, a.NOREKENINGDEBET, d.MERITALSTATUS, a.NOPOLSWITCH,
                            ...
                            (SELECT NILAIBENEFIT FROM $DBUser.TABEL_223_TRANSAKSI_PRODUK WHERE PREFIXPERTANGGUNGAN = '$this->prefix' AND NOPERTANGGUNGAN = '$this->nopertanggungan' AND KDBENEFIT = 'DNAWL') AS DANAAWAL,
                            /*
                            CASE
                                WHEN A.KDPRODUK = 'JSANUITAS' THEN (SELECT PREMI FROM $DBUser.TABEL_223_TRANSAKSI_PRODUK WHERE PREFIXPERTANGGUNGAN = '$this->prefix' AND NOPERTANGGUNGAN = '$this->nopertanggungan' AND KDBENEFIT = 'BNFPHT')
                                ELSE (SELECT NILAIBENEFIT FROM $DBUser.TABEL_223_TRANSAKSI_PRODUK WHERE PREFIXPERTANGGUNGAN = '$this->prefix' AND NOPERTANGGUNGAN = '$this->nopertanggungan' AND KDBENEFIT = 'DNAWL')
                            END DANAAWAL,
                            */
                            CASE
                                WHEN A.KDPRODUK IN ('AJSMBPA', 'AJSMBPB', 'AJSMBPC') THEN
                                    (SELECT TO_CHAR(TGL_SURAT, 'DD/MM/YYYY') FROM $DBUser.DATA_RESTRU_BANCASS WHERE PREFIXPERTANGGUNGAN||NOPERTANGGUNGAN = A.PREFIXPERTANGGUNGAN||A.NOPERTANGGUNGAN)
                                ELSE (SELECT TO_CHAR(TGLSURATPENAWARAN, 'DD/MM/YYYY') FROM $DBUser.TABEL_700_RESTRU WHERE NOPROPOSAL = A.PREFIXPERTANGGUNGAN||A.NOPERTANGGUNGAN)
                            END TGLSURAT,
                            CASE
                                WHEN A.KDPRODUK IN ('AJSMBPA', 'AJSMBPB', 'AJSMBPC') THEN
                                    (SELECT TO_CHAR(TGL_PERSETUJUAN, 'DD/MM/YYYY') FROM $DBUser.DATA_RESTRU_BANCASS WHERE PREFIXPERTANGGUNGAN||NOPERTANGGUNGAN = A.PREFIXPERTANGGUNGAN||A.NOPERTANGGUNGAN)
                                ELSE (SELECT TO_CHAR(TGLPERSETUJUANFORM, 'DD/MM/YYYY') FROM $DBUser.TABEL_700_RESTRU WHERE NOPROPOSAL = A.PREFIXPERTANGGUNGAN||A.NOPERTANGGUNGAN)
                            END TGLPERSETUJUAN,
                            CASE
                                WHEN A.KDPRODUK IN ('AJSMBPA', 'AJSMBPB', 'AJSMBPC') THEN 'SPAJ'
                                ELSE (SELECT JENISPERSETUJUAN FROM $DBUser.TABEL_700_RESTRU WHERE NOPROPOSAL = A.PREFIXPERTANGGUNGAN||A.NOPERTANGGUNGAN)
                            END JENISPERSETUJUAN,
                            (SELECT 'D$this->prefix' || TO_CHAR(SYSDATE, 'yyyy') || LPAD(COUNT(*)+1, 2, '0') FROM $DBUser.TABEL_248_BLANKO_CETAK WHERE PREFIXPERTANGGUNGAN = a.PREFIXPERTANGGUNGAN AND NOPERTANGGUNGAN = a.NOPERTANGGUNGAN) NOBLANKO
            ...
            $sql = "SELECT a.kdproduk, a.kdvaluta, 
                            ...
                            b.kdbasistebus, a.premi1,
                            CASE
                                WHEN A.KDPRODUK = 'JSANUITAS' THEN (SELECT PREMI FROM $DBUser.TABEL_223_TRANSAKSI_PRODUK WHERE PREFIXPERTANGGUNGAN = '$this->prefix' AND NOPERTANGGUNGAN = '$this->nopertanggungan' AND KDBENEFIT = 'BNFPHT')
                                ELSE (SELECT NILAIBENEFIT FROM $DBUser.TABEL_223_TRANSAKSI_PRODUK WHERE PREFIXPERTANGGUNGAN = a.PREFIXPERTANGGUNGAN AND NOPERTANGGUNGAN = a.NOPERTANGGUNGAN AND KDBENEFIT = 'DNAWL')
                            END DANAAWAL
                        FROM $DBUser.TABEL_200_PERTANGGUNGAN a
        ...
        public function SPAJ() {
            ...
            $this->Cell($this->pg_w-$this->num_space-100, $height, "Dengan ini mengadakan perjanjian dengan : ", 0);
            // $this->font_body('B');
            // $x1 = $this->GetX();
            // $this->Write(5, $tgl_restru);
            // $x2 = $this->GetX();
            // $y1 = $this->GetY();
            // $this->font_body();
            ...
            $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
            $this->MultiCell($this->pg_w-($this->num_space*2), $height, $this->paragraph0, 0);
            $this->Ln(5);

            // $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
            // $this->Cell(5, $height, '1.', 'LTRB', 1, "L");
            // $this->Cell(100, $height, 'Surat Penanggung kepada Pemegang Polis/Tertanggung pada tanggal', 0, 1, "L");
            $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
            $this->Cell($this->num_space/2, $height, "1.", 0, 0, 'C');
            $this->MultiCell($this->pg_w-($this->num_space*2), $height, "Surat Penanggung kepada Pemegang Polis/Tertanggung pada tanggal ".$this->tgl_indonesia($arr["TGLSURAT"], 2).",", 0);
            $this->Ln(0);

            if($arr["KDPRODUK"] == 'AJSMBPA' || $arr["KDPRODUK"] == 'AJSMBPB' || $arr["KDPRODUK"] == 'AJSMBPC') {
                $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
                $this->Cell($this->num_space/2, $height, "2.", 0, 0, 'C');
                // $this->MultiCell($this->pg_w-($this->num_space*2), $height, "Surat Permintaan Asuransi Jiwa tanggal ".$this->tgl_indonesia($arr["TGLPERSETUJUAN"], 2).",", 0);
                $this->MultiCell($this->pg_w-($this->num_space*2), $height, "Surat Permintaan Asuransi Jiwa tanggal ".$this->tgl_indonesia($arr["TGLSURAT"], 2).",", 0);
                $this->Ln(0);
            }else{
                $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
                $this->Cell($this->num_space/2, $height, "2.", 0, 0, 'C');

                if($arr["JENISPERSETUJUAN"] == 'FORM'){
                    $this->MultiCell($this->pg_w-($this->num_space*2), $height, "Formulir Persetujuan Restrukturisasi Polis tanggal ".$this->tgl_indonesia($arr["TGLSURAT"], 2).",", 0);
                }else{
                    $this->MultiCell($this->pg_w-($this->num_space*2), $height, "Surat Permintaan Asuransi Jiwa tanggal ".$this->tgl_indonesia($arr["TGLSURAT"], 2).",", 0);
                }
            ...
            $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
            $this->Cell($this->num_space/2, $height, "3.", 0, 0, 'C');
            $this->MultiCell($this->pg_w-($this->num_space*2)-5, $height, "Pemberitahuan Restrukturisasi Polis Penanggung melalui Website Resmi Penanggung yang dipublikasikan pada Tanggal 11 Desember 2020,", 0,'J','');
            $this->Ln(0);

            $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
            $this->Cell($this->num_space/2, $height, "4.", 0, 0, 'C');
            $this->MultiCell($this->pg_w-($this->num_space*2), $height, "Pemberitahuan Restrukturisasi Polis melalui Media Massa pada tanggal 11 Desember 2020, ", 0);
            $this->Ln(5);
            ...
            $this->Image('./images/tandatangan.png', 130, $this->GetY(), 60);

            $this->Ln(20);
            //Menghilangkan nama dan jabatan kantor cabang
            $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
            $x1 = $this->GetX();
            $y1 = $this->GetY();
            $this->Cell(($this->pg_w-($this->num_space*2))/3, 5, '', 0, 1, 'C');
            //$this->Cell(($this->pg_w-($this->num_space*2))/3, 5, $nama_pejabat , 0, 1, 'C');
            $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
            $this->Cell(($this->pg_w-($this->num_space*2))/3, 4, '', 0, 0, 'C');
            ...
            $this->Image($filename, 158, 230, 30, 30);
           
            $tcduplikat     = "DENGAN DITERBITKANNYA POLIS DUPLIKAT INI, MAKA POLIS DENGAN NOMOR : $arr[PREFIXPERTANGGUNGAN]$arr[NOPERTANGGUNGAN] "
                            . "YANG DITERBITKAN SEBELUMNYA DINYATAKAN TIDAK BERLAKU.";

            $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
            $this->font_body('B');
            $this->MultiCell(($this->pg_w/2)-($this->num_space), $height, $tcduplikat, 0, 'J');
            ...
            $this->Cell(26, $height, $arr['NOBLANKO'], 0, 0, 'C');
        ...
        public function DataPolis() {
            ...
            $this->font_title('B');
            if($kd_produk == 'AJSMBPA' || $kd_produk == 'AJSMBPB' || $kd_produk == 'AJSMBPC') {
                $this->Cell($this->pg_w, $height, 'DATA POLIS', 0, 1, 'C');    
            }else{
                $this->Cell($this->pg_w, $height, 'DATA POLIS RESTRUKTURISASI', 0, 1, 'C');    
            }
            ...
            $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
            $this->Cell(($this->pg_w-($this->num_space*2))*10/25, $height, "Nama Pemegang Polis", 0, 0, 'L');
            ...
            $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
            $this->Cell(($this->pg_w-($this->num_space*2))*10/25, $height, "Nomor Induk Kependudukan", 0, 0, 'L');
            $this->Cell(($this->pg_w-($this->num_space*2))/25, $height, ":", 0, 0, 'L');
            $this->Cell(($this->pg_w-($this->num_space*2))*17/25, $height, $arr["NOID"], 0, 1, 'L');
            //
            $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
            $this->Cell(($this->pg_w-($this->num_space*2))*10/25, $height, "Nomor Polis yang direstrukturisasi", 0, 0, 'L');
            $this->Cell(($this->pg_w-($this->num_space*2))/25, $height, ":", 0, 0, 'L');
            $this->Cell(($this->pg_w-($this->num_space*2))*17/25, $height, $arr["NOPOLSWITCH"], 0, 1, 'L');
            //
            $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
            $this->Cell(($this->pg_w-($this->num_space*2))*10/25, $height, "Nomor Induk Kependudukan", 0, 0, 'L');
            $this->Cell(($this->pg_w-($this->num_space*2))/25, $height, ":", 0, 0, 'L');
            ...
            //
            $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
            $this->Cell(($this->pg_w-($this->num_space*2))*10/25, $height, "Alamat Pemegang Polis", 0, 0, 'L');
            $this->Cell(($this->pg_w-($this->num_space*2))/25, $height, ":", 0, 0, 'L');
            //$this->Cell(($this->pg_w-($this->num_space*2))*17/25, $height, $arr["ALAMATPEMPOL"], 1, 1, 'L');
            $this->MultiCell(($this->pg_w-($this->num_space*2))*17/27, 5, $arr["ALAMATPEMPOL"], 0, 'L', '');
            
            $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
            $this->Cell(($this->pg_w-($this->num_space*2))*10/25, $height, "Macam Asuransi", 0, 0, 'L');
            $this->Cell(($this->pg_w-($this->num_space*2))/25, $height, ":", 0, 0, 'L');

            if($kd_produk == 'JSMANTAP'){
                $this->Cell(($this->pg_w-($this->num_space*2))*17/25, $height, $arr["NAMAPRODUK"]."", 0, 1, 'L');

                $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
                $this->Cell(($this->pg_w-($this->num_space*2))*10/25, $height, "", 0, 0, 'L');
                $this->Cell(($this->pg_w-($this->num_space*2))/25, $height, "", 0, 0, 'L');
                $this->Cell(($this->pg_w-($this->num_space*2))*17/25, $height, "(ASURANSI JS MANTAP)", 0, 1, 'L');
            }else if (in_array($kd_produk, array('JSTAMPAN', 'JSTAMPANX'))) {
                $this->Cell(($this->pg_w-($this->num_space*2))*17/25, $height, $arr["NAMAPRODUK"]."", 0, 1, 'L');

                $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
                $this->Cell(($this->pg_w-($this->num_space*2))*10/25, $height, "", 0, 0, 'L');
                $this->Cell(($this->pg_w-($this->num_space*2))/25, $height, "", 0, 0, 'L');
                $this->Cell(($this->pg_w-($this->num_space*2))*17/25, $height, "(ASURANSI JS TAMPAN)", 0, 1, 'L');
            }else if (in_array($kd_produk, array('AJSMBPA'))) {
                $this->Cell(($this->pg_w-($this->num_space*2))*17/25, $height, $arr["NAMAPRODUK"], 0, 1, 'L');
                
                $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
                $this->Cell(($this->pg_w-($this->num_space*2))*10/25, $height, "", 0, 0, 'L');
                $this->Cell(($this->pg_w-($this->num_space*2))/25, $height, "", 0, 0, 'L');
                $this->Cell(($this->pg_w-($this->num_space*2))*17/25, $height, "(ASURANSI JS MANTAP PLUS PLAN A)", 0, 1, 'L');
            }else if (in_array($kd_produk, array('AJSMBPB'))) {
                $this->Cell(($this->pg_w-($this->num_space*2))*17/25, $height, $arr["NAMAPRODUK"], 0, 1, 'L');
                
                $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
                $this->Cell(($this->pg_w-($this->num_space*2))*10/25, $height, "", 0, 0, 'L');
                $this->Cell(($this->pg_w-($this->num_space*2))/25, $height, "", 0, 0, 'L');
                $this->Cell(($this->pg_w-($this->num_space*2))*17/25, $height, "(ASURANSI JS MANTAP PLUS PLAN B)", 0, 1, 'L');
            }else if (in_array($kd_produk, array('AJSMBPC'))) {
                $this->Cell(($this->pg_w-($this->num_space*2))*17/25, $height, $arr["NAMAPRODUK"], 0, 1, 'L');
                
                $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
                $this->Cell(($this->pg_w-($this->num_space*2))*10/25, $height, "", 0, 0, 'L');
                $this->Cell(($this->pg_w-($this->num_space*2))/25, $height, "", 0, 0, 'L');
                $this->Cell(($this->pg_w-($this->num_space*2))*17/25, $height, "(ASURANSI JS MANTAP PLUS PLAN C)", 0, 1, 'L');
            }else{
                $this->Cell(($this->pg_w-($this->num_space*2))*17/25, $height, $arr["NAMAPRODUK"], 0, 1, 'L');
            }
            ...
            if (in_array($kd_produk, array('JSAA', 'JSHAA', 'JSANUITAS'))) {
                $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
                $this->Cell(($this->pg_w-($this->num_space*2))*10/25, $height, "Anuitas Sebulan", 0, 0, 'L');
                $this->Cell(($this->pg_w-($this->num_space*2))/25, $height, ":", 0, 0, 'L');
                $this->SetFont('Arial','',9);
            ...
            // jika produknya JSPNSB atau JSPNSK
                ...
                $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
                $this->Cell(($this->pg_w-($this->num_space*2))*10/25, $height, "Uang Asuransi", 0, 0, 'L');
                $this->Cell(($this->pg_w-($this->num_space*2))/25, $height, ":", 0, 0, 'L');
                $this->SetFont('Arial','',9);
                $this->Cell(($this->pg_w-($this->num_space*2))*17/25, $height, $arr['NOTASI'].".".number_format($arr["JUAMAINPRODUK"],2,",","."), 0, 1, 'L');
                //
                $this->SetFont('Arial','B',9);
                $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
                $this->Cell(($this->pg_w-($this->num_space*2))*10/25, $height, "Masa Asuransi", 0, 0, 'L');
                $this->Cell(($this->pg_w-($this->num_space*2))/25, $height, ":", 0, 0, 'L');
                $this->SetFont('Arial','',9);
            ...
            else if ($kd_produk == 'JSIPA' || $kd_produk == 'JSMANTAP' || $kd_produk == 'AJSMBPA' || $kd_produk == 'AJSMBPB' || $kd_produk == 'AJSMBPC') {
                $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
                $this->Cell(($this->pg_w-($this->num_space*2))*10/25, $height, "Uang Asuransi Kecelakaan", 0, 0, 'L');
                $this->Cell(($this->pg_w-($this->num_space*2))/25, $height, ":", 0, 0, 'L');
                $this->SetFont('Arial','',9);
                $this->Cell(($this->pg_w-($this->num_space*2))*17/25, $height, $arr['NOTASI'].".".number_format($arr["JUAMAINPRODUK"],2,",","."), 0, 1, 'L');
                //
            ...
            // jika produknya bukan JS Heritage Annuity Assurance
                ...
                $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
                $this->Cell(($this->pg_w-($this->num_space*2))*10/25, $height, "Uang Asuransi Dasar", 0, 0, 'L');
                $this->Cell(($this->pg_w-($this->num_space*2))/25, $height, ":", 0, 0, 'L');
                $this->SetFont('Arial','',9);
                $this->Cell(($this->pg_w-($this->num_space*2))*17/25, $height, $arr['NOTASI'].".".number_format($arr["JUAMAINPRODUK"],2,",","."), 0, 1, 'L');
                //
            ...
            $this->SetFont('Arial','B',9);
            $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
            $this->Cell(($this->pg_w-($this->num_space*2))*10/25, $height, "Valuta", 0, 0, 'L');
            $this->Cell(($this->pg_w-($this->num_space*2))/25, $height, ":", 0, 0, 'L');
            $this->SetFont('Arial','',9);
            ...
            if($kd_produk == 'AJSMBPA' || $kd_produk == 'AJSMBPB' || $kd_produk == 'AJSMBPC') {
                $this->SetFont('Arial','B',9);
                $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
                $this->Cell(($this->pg_w-($this->num_space*2))*10/25, $height, "Dana Awal", 0, 0, 'L');
                $this->Cell(($this->pg_w-($this->num_space*2))/25, $height, ":", 0, 0, 'L');
                $this->Cell(($this->pg_w-($this->num_space*2))*17/25, $height, $arr['NOTASI'].".".number_format($arr['DANAAWAL'],2,",","."), 0, 1, 'L');
            }else if($kd_produk == 'JSANUITAS') {
                $this->SetFont('Arial','B',9);
                $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
                $this->Cell(($this->pg_w-($this->num_space*2))*10/25, $height, "Dana Awal", 0, 0, 'L');
                $this->Cell(($this->pg_w-($this->num_space*2))/25, $height, ":", 0, 0, 'L');
                $this->Cell(($this->pg_w-($this->num_space*2))*17/25, $height, $arr['NOTASI'].".".number_format($arr['DANAAWAL'],2,",",".").' Secara Sekaligus', 0, 1, 'L');
            }else{
                // $this->SetFont('Arial','B',9);
                // $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
                // $this->Cell(($this->pg_w-($this->num_space*2))*10/25, $height, "Dana Awal", 0, 0, 'L');
                // $this->Cell(($this->pg_w-($this->num_space*2))/25, $height, "", 0, 0, 'L');
                // $this->Cell(($this->pg_w-($this->num_space*2))*17/25, $height, "", 0, 1, 'L');
            ...
            if ($kd_produk == "JSSPKN") {
                $this->SetFont('Arial','B',9);
                $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
                $this->Cell(($this->pg_w-($this->num_space*2))*10/25, $height, "Periode Investasi", 0, 0, 'L');
                $this->Cell(($this->pg_w-($this->num_space*2))/25, $height, ":", 0, 0, 'L');
                $this->SetFont('Arial','',9);
            ...
            if ($kd_produk == "JSSPBTN") {
                ...
                $this->SetFont('Arial','B',9);
                $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
                $this->Cell(($this->pg_w-($this->num_space*2))*10/25, $height, "Premi", 0, 0, 'L');
                $this->Cell(($this->pg_w-($this->num_space*2))/25, $height, ":", 0, 0, 'L');
                $this->SetFont('Arial','',9);
                ...
                $this->SetFont('Arial','B',9);
                $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
                $this->Cell(($this->pg_w-($this->num_space*2))*10/25, $height, "Nomor Rekening", 0, 0, 'L');
                $this->Cell(($this->pg_w-($this->num_space*2))/25, $height, ":", 0, 0, 'L');
                $this->SetFont('Arial','',9);
            ...
            /* tambahan untuk produk JSREMAJA oleh fendy */
                ...
                $this->SetFont('Arial','B',9);
                $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
                $this->Cell(($this->pg_w-($this->num_space*2))*10/25, $height, "Premi Asuransi", 0, 0, 'L');
                $this->Cell(($this->pg_w-($this->num_space*2))/25, $height, ":", 0, 0, 'L');
                $this->SetFont('Arial','',9);
                $this->Cell(($this->pg_w-($this->num_space*2))*17/25, $height, $arr['NOTASI'].".".number_format($premi1,2), 0, 1, 'L');
                //
                $this->SetFont('Arial','B',9);
                $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
                $this->Cell(($this->pg_w-($this->num_space*2))*10/25, $height, "Nomor Host to Host", 0, 0, 'L');
                $this->Cell(($this->pg_w-($this->num_space*2))/25, $height, ":", 0, 0, 'L');
                $this->SetFont('Arial','',9);
            ...
            // jika produknya JS Index Plan Assurance
            ...
            else if ($kd_produk == 'JSMANTAP' || $kd_produk == 'JSTAMPANX' || $kd_produk == 'AJSMBPA' || $kd_produk == 'AJSMBPB' || $kd_produk == 'AJSMBPC' || $kd_produk == 'JSANUITAS') {
                ...
                if (!in_array($kd_produk, array('JSAA', 'JSRIA', 'JSPNSK', 'JSPNSB', 'JSHAA'))) {
                    $this->Cell(($this->pg_w - ($this->num_space * 2)) * 10 / 25, $height, 'Premi Asuransi Dasar', 0, 0, 'L');
                } else {
                    $this->Cell(($this->pg_w - ($this->num_space * 2)) * 10 / 25, $height, 'Premi', 0, 0, 'L');
                }
                $this->Cell(($this->pg_w - ($this->num_space * 2)) / 25, $height, ":", 0, 0, 'L');
                //
                $this->SetFont('Arial','',9);
                if(substr($kd_produk,0,8)=="JSTAMPAN"){
                      $this->Cell(($this->pg_w-($this->num_space*2))*17/25, $height, $arr['NOTASI'].".".number_format($premi1,2), 0, 1, 'L');
                }else{
                ...
                //     $this->SetFont('Arial','B',9);
                //     $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
                //     $this->Cell(($this->pg_w-($this->num_space*2))*10/25, $height, "Premi Asuransi Tambahan", 0, 0, 'L');
                //     $this->Cell(($this->pg_w-($this->num_space*2))/25, $height, ":", 0, 0, 'L');
                //     $this->SetFont('Arial','',9);
            ...
            if (!in_array($kd_produk, array('JSSPKN', 'JSSPBTN', 'JSR1', 'JSR2', 'JSR3', 'JSR4', 'JSAA', 'JSRIA', 'JSPNSK', 'JSPNSB', 'JSIPA', 'JSHAA', 'JSTAMPAN', 'JSTAMPANX', 'JSMANTAP', 'AJSMBPA', 'AJSMBPB', 'AJSMBPC', 'JSANUITAS'))) {
                /*Tambahan untuk perubahan Topup agar jenis topup dimasukkan ada dua Single dan Reguler oleh Dedi 19/03/2014*/
                $this->SetFont('Arial','B',9);
                $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
                $this->Cell(($this->pg_w-($this->num_space*2))*10/25, $height, "Top Up Berkala", 0, 0, 'L');
                $this->Cell(($this->pg_w-($this->num_space*2))/25, $height, ":", 0, 0, 'L');
                ...
                if($arr["KDPRODUK"] == 'JL4B') {
                    $this->SetFont('Arial','B',9);
                    $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
                    $this->Cell(($this->pg_w-($this->num_space*2))*10/25, $height, "Top Up Reguler", 0, 0, 'L');
                    $this->Cell(($this->pg_w-($this->num_space*2))/25, $height, ":", 0, 0, 'L');
            ...
            if ($kd_produk == 'JSMANTAP' || $kd_produk == 'JSTAMPANX' || $kd_produk == 'AJSMBPA' || $kd_produk == 'AJSMBPB' || $kd_produk == 'AJSMBPC' || $kd_produk == 'JSANUITAS') {

            }else{
                 //
                $this->SetFont('Arial','B',9);
                $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
                $this->Cell(($this->pg_w-($this->num_space*2))*10/25, $height, "Cara Bayar Premi", 0, 0, 'L');
                $this->Cell(($this->pg_w-($this->num_space*2))/25, $height, ":", 0, 0, 'L');
                //$this->SetFont('Arial','',9);
            ...
            if($arr["NAMACARABAYAR"]<>'SEKALIGUS' && substr($kd_produk,0,8) != 'JSTAMPAN' ){
                $this->SetFont('Arial','B',9);
                $this->Cell($this->num_space, $height, NULL, 0, 0, 'C');
                $this->Cell(($this->pg_w-($this->num_space*2))*10/25, $height, "Lama Pembayaran", 0, 0, 'L');
                $this->Cell(($this->pg_w-($this->num_space*2))/25, $height, ":", 0, 0, 'L');
                $this->SetFont('Arial','',9);
            ...
            // jika produknya JS Index Plan Assurance
            if ($kd_produk == 'JSMANTAP' || $kd_produk == 'AJSMBPA' || $kd_produk == 'AJSMBPB' || $kd_produk == 'AJSMBPC') {
                $x1 = $this->GetX();
                $y1 = $this->GetY();
                ...
                $this->Cell(($this->pg_w-($this->num_space*2))*4/25, $height*3, "Anuitas Sebulan", 0, 0, 'C', true);
            } else {
            ...
            if($arr["KDPRODUK"] == 'JSMANTAP'){
                $this->MultiCell(($this->pg_w-($this->num_space*2))*7/25, 4, $arr["NAMAPRODUK"]." (ASURANSI JS MANTAP)", 0, 'L');
                $y2 = $this->GetY();
                $yH = $y2 - $y1;
                ...
                $x1 = $this->GetX();
                $y1 = $this->GetY();
                $this->MultiCell(($this->pg_w-($this->num_space*2))*3/25, $h, "SUP JSM", 0, 'C');
            }else if($arr["KDPRODUK"] == 'AJSMBPA'){
                $this->MultiCell(($this->pg_w-($this->num_space*2))*7/25, 4, $arr["NAMAPRODUK"]." (ASURANSI JS MANTAP PLUS PLAN A)", 0, 'L');
                $y2 = $this->GetY();
                $yH = $y2 - $y1;
                ...
                $x1 = $this->GetX();
                $y1 = $this->GetY();
                $this->MultiCell(($this->pg_w-($this->num_space*2))*3/25, $h, "SUP AJSMP", 0, 'C');
            }else if($arr["KDPRODUK"] == 'AJSMBPB'){
                $this->MultiCell(($this->pg_w-($this->num_space*2))*7/25, 4, $arr["NAMAPRODUK"]." (ASURANSI JS MANTAP PLUS PLAN B)", 0, 'L');
                $y2 = $this->GetY();
                $yH = $y2 - $y1;
                ...
                $y1 = $this->GetY();
                $this->MultiCell(($this->pg_w-($this->num_space*2))*3/25, $h, "SUP AJSMP", 0, 'C');
            }else if($arr["KDPRODUK"] == 'AJSMBPC'){
                $this->MultiCell(($this->pg_w-($this->num_space*2))*7/25, 4, $arr["NAMAPRODUK"]." (ASURANSI JS MANTAP PLUS PLAN C)", 0, 'L');
                $y2 = $this->GetY();
                $yH = $y2 - $y1;
                ...
                $y1 = $this->GetY();
                $this->MultiCell(($this->pg_w-($this->num_space*2))*3/25, $h, "SUP AJSMP", 0, 'C');
            }else if($arr["KDPRODUK"] == 'JSANUITAS'){
                $this->MultiCell(($this->pg_w-($this->num_space*2))*7/25, 4, $arr["NAMAPRODUK"], 0, 'L');
                $y2 = $this->GetY();
                $yH = $y2 - $y1;
                ...
                $x1 = $this->GetX();
                $y1 = $this->GetY();
                $this->MultiCell(($this->pg_w-($this->num_space*2))*3/25, $h, "SUP KK", 0, 'C');
            }else{
                $this->MultiCell(($this->pg_w-($this->num_space*2))*7/25, 4, "ASURANSI JS TATA MASA DEPAN\n(ASURANSI JS TAMPAN)", 0, 'L');
                $y2 = $this->GetY();
                $yH = $y2 - $y1;
                ...
                $x1 = $this->GetX();
                $y1 = $this->GetY();
                $this->MultiCell(($this->pg_w-($this->num_space*2))*3/25, $h, "SUP JST", 0, 'C');
            }
            ...
            $this->Cell(($this->pg_w-($this->num_space*2))*4/25, $h, number_format($kd_produk=='JSIPA' ? $arr["PREMI1"] : $arr["JUAMAINPRODUK"], 2, ",", "."), 0, 0, 'C');
            $this->Cell(($this->pg_w-($this->num_space*2))*4/25, $h, $tgl_mulas, 0, 0, 'C');
            ...
            } else if (in_array($kd_produk, array('JSAA', 'JSHAA', 'JSANUITAS'))) {
                $this->Cell(($this->pg_w-($this->num_space*2))*4/25, $h, "-", 0, 0, 'C');
            } else {
                $this->Cell(($this->pg_w-($this->num_space*2))*4/25, $h, $tgl_mulas, 0, 0, 'C');
            }

            if (in_array($kd_produk, array('JSHAA', 'JSANUITAS'))) {
                $this->Cell(($this->pg_w-($this->num_space*2))*3/25, $h, "-", 0, 1, 'C');
            } else {
            ...
            if (!empty($this->art)) {
                ...
                foreach($this->art as $i => $value) {
                    ...
                    $y = $this->GetY();
                    $h = $y2 == $this->GetY ? 4 : $y2 - $y;
                    $this->Cell(($this->pg_w-($this->num_space*2))*3/25, $h, trim($kdbnfx), 0, 0, 'C');
        ...
        public function KetentuanPolis() {
            ...
            foreach($this->arrk as $i => $value) {
                if (!$value['KDPARAGRAPH'] || $value['KDPARAGRAPH'] == 'T') {
                    ...
                    $text  = str_replace("{N_NOPOLLAMA}", substr($this->arr["NOPOLSWITCH"], 0, 2)."-".substr($this->arr["NOPOLSWITCH"], 2), $text);
                    $text  = str_replace("{N_NOPOLBARU}", $this->prefix .'-'. $this->nopertanggungan, $text);
            ...
            if($this->arrt['KDPRODUK'] == 'AJSMBPA'){
                ...
                $this->font_body('B');
                $this->Cell(4, $height, '', 0, 0, 'L');
                $this->Cell($this->num_space+1, $height, 'III.', 0, 0, 'C');
                $this->Cell(1, $height, '', 0, 0, 'L');
                $this->MultiCell($this->pg_w-($this->num_space*2), 5, 'Nilai Tunai : ASURANSI JS MANFAAT BERTAHAP PLUS', 0, 'J');
                $this->Ln(2);

                $this->SetFont('Arial','',9);
                $this->CreateTableTitle();
            }else if($this->arrt['KDPRODUK'] == 'AJSMBPB' || $this->arrt['KDPRODUK'] == 'AJSMBPC'){
                $this->font_body('B');
                $this->Cell(4, $height, '', 0, 0, 'L');
                $this->Cell($this->num_space+1, $height, 'III.', 0, 0, 'R');
                $this->Cell(1, $height, '', 0, 0, 'L');
                $this->MultiCell($this->pg_w-($this->num_space*2), 5, 'Nilai Tunai : ASURANSI JS MANFAAT BERTAHAP PLUS', 0, 'J');
                $this->Ln(2);
            ...
            foreach($this->arrt2 as $i => $value) {
                if ($i < $k && $this->arr['KDPRODUK'] != 'JSKEL') {
                    ...
                    else if (in_array($this->arrt['KDPRODUK'],array('AJSMBPA','AJSMBPB','AJSMBPC'))) {
                        $data[$i]['NO']    = $value['T'];
                         // $data[$i]['NILAI'] = round(0.01 * $value['TARIF'] * $this->arrt['PREMI1']);
                        $data[$i]['NILAI'] = round(0.01 * $value['TARIF'] * $this->arrt['DANAAWAL']);//Update 19/01/2021 sesuai email Rizky - Teguh
                    }
                    else {
                        $data[$i]['NO']    = $value['T'];
                    ...
                    if (in_array($this->arrt['KDPRODUK'], array("JSSPO1","JSSPO2","JSSPO7A","JSSPO8","JSSPO9","JSKPD"))) {
                        ...
                        $data[$j]['NO2']    = $value['T'];
                        // $data[$j]['NILAI2'] = round(0.01 * $value['TARIF'] * $this->arrt['PREMI1']);
                        $data[$j]['NILAI2'] = round(0.01 * $value['TARIF'] * $this->arrt['DANAAWAL']); //Update 19/01/2021 sesuai email Rizky - Teguh
                    }
                    else {
                        $data[$j]['NO2']    = $value['T'];
            ...
            if($this->arrt['KDPRODUK'] == 'AJSMBPA'){
                $this->font_body('B');
                $this->Cell(4, $height, '', 0, 0, 'L');
                $this->Cell($this->num_space+1, $height, 'IV.', 0, 0, 'C');
                $this->Cell(1, $height, '', 0, 0, 'L');
                $this->MultiCell($this->pg_w-($this->num_space*2), 5, 'Ketentuan Penebusan Polis', 0, 'J');
                $this->Ln(0);

                $this->Cell(6, $height, '', 0, 0, 'L');
                $this->Cell($this->num_space, $height, '', 0, 0, 'C');
                $this->MultiCell($this->pg_w-($this->num_space*2), 5, 'Polis ini tidak dapat dilakukan penebusan', 0, 'J');
            }
            
            if($this->arrt['KDPRODUK'] == 'AJSMBPB' || $this->arrt['KDPRODUK'] == 'AJSMBPC'){
                $this->font_body('B');
                $this->Cell(4, $height, '', 0, 0, 'L');
                $this->Cell($this->num_space+1, $height, 'IV.', 0, 0, 'R');
                $this->Cell(1, $height, '', 0, 0, 'L');
                $this->MultiCell($this->pg_w-($this->num_space*2), 5, 'Ketentuan Penebusan Polis', 0, 'J');
                $this->Ln(0);
        ...
        public function CreateTableTitle() {
            if ($this->arrt['KDPRODUK'] == "HTB" || $this->arrt['KDPRODUK'] == "HTK" || $this->arrt['KDPRODUK'] == "HTS" || $this->arrt['KDPRODUK'] == "HTT" && count($this->arrt2) > 0) {
                ...
                $this->Cell(140/3, 8, "NILAI TEBUS", 1, 0, 'C');
                $this->Cell(25, 8, NULL, 0, 1, 'L');
            }else if($this->arrt['KDPRODUK'] == 'AJSMBPA' || $this->arrt['KDPRODUK'] == 'AJSMBPB' || $this->arrt['KDPRODUK'] == 'AJSMBPC'){
                $this->Cell(25, 8, NULL, 0, 0, 'L');
                $x1 = $this->GetX();
                $y1 = $this->GetY();
                $this->MultiCell(140/6, 4, "AWAL TAHUN KE", 1, 'C');
                $y2 = $this->GetY();
                $yH = $y2 - $y1;

                $this->SetXY($x1 + 140/6, $y2 - $yH);

                $this->Cell(140/3, 8, "NILAI TUNAI", 1, 0, 'C');
    ...
    // $sql = "UPDATE $DBUser.tabel_200_pertanggungan SET tglcetak = sysdate, lockmutasi = null
    ...
    // // masukan ke dalam historis mutasi
    // //$sqlinshist = "insert into $DBUser.tabel_600_historis_mutasi_pert values(SYSDATE, '$prefix','$nopertanggungan','28','CETAK POLIS DUPLIKAT','$userid',NULL,NULL)";
    // $sqlinshist = "INSERT INTO $DBUser.tabel_600_historis_mutasi_pert 
    //                select sysdate, '$prefix', '$nopertanggungan', '28', 'CETAK POLIS DUPLIKAT ' || to_char(MAX(tglmutasi), 'dd/mm/yyyy'), '$userid', NULL, NULL
    //                FROM $DBUser.tabel_603_mutasi_pert
    //                WHERE prefixpertanggungan = '$prefix' AND nopertanggungan = '$nopertanggungan' 
    //                    AND kdmutasi = 41";
    // // echo $sqlinshist;
    // // die();
    // $DA->parse($sqlinshist);
    ...
    // // masukan ke dalam historis cetak polis    
    // $sql = "SELECT 'D'|| $prefix || TO_CHAR(SYSDATE, 'yyyy') || LPAD(COUNT(*)+1, 2, '0') NOBLANKO
    ...
    // $sql = "INSERT INTO $DBUser.TABEL_248_BLANKO_CETAK (KDKANTOR, PREFIX, NOBLANKO, PREFIXPERTANGGUNGAN, NOPERTANGGUNGAN, KDSTATUS,
    //             KETERANGAN, USERREKAM, TGLREKAM, USERUPDATED, TGLUPDATED, KDBLANKO)
    //         SELECT '$prefix', ' ', MAX(NOBLANKO)+1, '$prefix', '$nopertanggungan',
    //             1, 'BERHASIL', '$userid', SYSDATE, NULL, NULL, '$noblanko[NOBLANKO]'
    //         FROM $DBUser.TABEL_248_BLANKO_CETAK";
    // $DA->parse($sql);
    // $DA->execute();


    // // UPDATE tanggal cetak di TABEL_999_DUPLIKAT
    // $sql_u="UPDATE $DBUser.TABEL_700_DUPLIKAT SET TGLCETAK = SYSDATE, USERCETAK = '$userid'
    //         WHERE prefixpertanggungan = '$prefix' AND nopertanggungan = '$nopertanggungan'
    //             AND TGLCETAK IS NULL
    //             AND STATUS = '2'";
    // // echo $sqlinshist;
    // // die();
    // $DA->parse($sql_u);
