...
if ($Submit) {
 #----------------------------------------------------------------------- sendmail mutasi jua
		...
		for ($i=1; $i<=$jmlf; $i++)
		{
		 ...
		 Lihat detail polis 
		 <?
		 echo "<a href=\"#\" onclick=\"window.open('jiwasraya/polis/polis.php??prefix=".$prefix."&noper=".$noper."','','width=800,height=600,top=100,left=100,scrollbars=yes');\">".$prefix."-".$noper."</a>";
          ...
          <table border="0" width="600" bgcolor="#006699" cellspacing="1" cellpadding="6">
            ...
            <? 
			  ...
			  if($PER->statusfile!="AKTIF"||substr($PER->produk,0,3)!="JL4"){
					echo 'MUTASI TIDAK DAPAT DIPROSES. POLIS SEDANG TIDAK AKTIF \n';
				} else {}
				if(substr($PER->produk,0,3)!="JL4"){
						echo 'PRODUK BUKAN JENIS UNIT LINK';
				} else {}
				if($arrcek["ADA"]>=1){
					echo 'MASIH TERDAPAT TRANSAKSI YANG BELUM GOODFUND';
				} else {
					echo "<input type=\"submit\" value=\"SUBMIT\" name=\"Submit\" style=\"font-size: 8pt; font-family: Verdana\">";
				}
