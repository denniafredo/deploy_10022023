...
?>
	...
	function limitMax(el) {
		...
		if (total>100) {
			alert('Pilih Maksimal ' + total + ' Jenis Fund saja!');
			//el.checked = false;
		    el.value=0;
			el.focus();
    ...
    print( "<!--\n" );
     ...
     print( "	 alert('Nilai JUA Nol, Kemungkinan Belum Ada Tarifnya')\n" );
	   echo "window.opener.document.ntryprop.submit.disabled=true;";
		...
		if(substr($kdproduk,0,3)=="JL3" || substr($kdproduk,0,3)=="JL4")
		{
...
?>
	...
	<td align=right><? echo number_format(($jmlpremi*$faktor)+$premitopup+$premitopupskg,2);	?></td>
	<td colspan=2></td>
	</tr>
	<tr class="tblisi1">
	<td colspan="4" align=left>Premi Standar Tahunan</td>
	<td align=right><? echo number_format($premistandar+$premitopup+$premitopupskg,2); ?></td>
	<td colspan=2></td>
	</tr>
	</table>
...
<tr class="tblisi1">
 ...
 while ($arr=$DB->nextrow()) {
	...
	echo "</tr>";
	$porsi=$porsi+$arr["PORSI"];
	$i++;
	}
	if ($i>3) { echo  '<script type="text/javascript">alert("Opsi Fund '.$i.' Tidak Sesuai !...");</script>';
	$sql = "delete from $DBUser.TABEL_UL_OPSI_FUND_TEMP where nopertanggungan='$nopertanggungan'";
    //echo $sql;
	$DA->parse($sql);
	$DA->execute();
...
<tr class=hijao align=center>
<td colspan="4" align="center"><font size="2"><b>Entry Porsi Fund</b></font></td>
</tr>
<tr class=hijao align=center>
<td  colspan="2" class=verdana8><font size="2">Nama Fund</font></td><td class=verdana8><font size="2">Porsi (%)</font></td>
</tr>
<?
...
echo " name='box1[]' value=".$arc["KDFUND"].$nopertanggungan." onPropertyChange='showHide(x".$arc["KDFUND"].");'></td>";
echo "<td class=verdana8>".$arc["NAMAFUND"]."</td><td class=verdana8><input style='visibility: hidden;' onBlur='limitMax(this);' id='x".$arc["KDFUND"]."' size='2' type='text' value='0'";
...
<?
	$sql="select * from $DBUser.TABEL_UL_FAKTOR_UA where usia=".$FM->usia;
	$DB->parse($sql);
	$DB->execute();
	$arfua=$DB->nextrow();
...
<tr><td></td><td></td><td align="center"><font size="2"><input type="submit" name="sporsi" id="sporsi" value="SIMPAN" /></font></td></tr>
</table>
 </td>
 </tr>
 </table>
