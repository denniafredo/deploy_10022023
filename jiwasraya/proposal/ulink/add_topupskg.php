...
<body>
<a class="verdana10blk"><b>MUTASI PENAMBAHAN TOP UP SEKALIGUS</b></a>
<hr size=1>
		...
		for ($i=1; $i<=$jmlf; $i++)
		{
			 $sql = "insert into $DBUser.TABEL_UL_TRANSAKSI_TOPUP (".
							...
							"values (".
								...
								${$kd[$i-1]}.",'TS',sysdate,user,to_date('$tgltransfer','DD/MM/YYYY'),'".$kd[$i-1]."')";
			 //echo ${$kd[$i-1]}.'</br>';
			 //echo ${'FF'}.'</br>';
			 echo $sql.'</BR>';
			 //die;
		 ...
		 echo "<br />";
		 echo "Lihat <a href='info_tupup.php'>Status Transaksi Top-Up Sekaligus</a>";
 ...
 else 
 {
		 $sql = "select a.prefixpertanggungan,a.nopertanggungan,b.kdrayonpenagih,c.namaklien1,d.namaklien1 tertanggung,b.nopenagih ".
			       ...
			       "where b.kdrayonpenagih='$prefixpertanggungan' and a.nopertanggungan='$nopertanggungan' and a.kdpertanggungan='2' ".
				     "and c.noklien=b.nopenagih and a.kdstatusfile in ('1') and a.nopenagih=b.nopenagih and a.notertanggung=d.noklien order by a.nopertanggungan";
     //echo $sql;
		 $DB->parse($sql);
	   $DB->execute();
	   $arx=$DB->nextrow();
     $prefix = $arx["PREFIXPERTANGGUNGAN"];
		 $PER	= New Pertanggungan($userid,$passwd,$arx["PREFIXPERTANGGUNGAN"],$arx["NOPERTANGGUNGAN"]);;
		 //echo "<br />Prefix = ".$prefix.$nopertanggungan;
		 if ($prefix == "X") { 
          ...
          <table border="0" width="600" bgcolor="#006699" cellspacing="1" cellpadding="6">
            ...
            <tr>
              <td width="100%" bgcolor="#DAE2EF">
                ...
                echo "    </tr>";
                echo "    <tr>";
                echo "      <td class=\"verdana9blk\">Top-Up Sekaligus yang diinginkan (9999.99)</td>";
