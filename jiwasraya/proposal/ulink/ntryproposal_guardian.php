...
/*----- End of Tambahan oleh Ari 24/06/2008 sesuai Nota Intern Divisi URC tgl 23/06/2008 Perihal Izin Entry Proposal ----------*/
	  	  ...
	  	  $Query= "select ".
								...
								"c.lama_min,c.lama_max, c.keterangan ".
	          "from ".
...
function BeneficiariGuard() {
	...
	isi = isi + "&nbsp;<td ><input type=\"text\" name=\"no"+i+"\" maxlength=\"1\" size=\"2\" readonly class=\"a\"></td>";
	isi = isi + "<td ><input type=\"text\" name=\"nama"+i+"\" size=\"33\" readonly class=\"a\"></td>";
	isi = isi + "<td ><input type=\"text\" name=\"klienno"+i+"\" maxlength=\"6\" size=\"10\" readonly class=\"a\"></td>";
	isi = isi + "<td ><input type=\"text\" name=\"hubungan"+i+"\" size=\"25\" readonly class=\"a\"></td>";
	isi = isi + "</tr>";
	
	//alert(a + " | " + document.ntryprop.demit.value + "  |  " + document.ntryprop.length + "  |  i = " + i + " | isi: " + isi);
...
function PempolGuardian(i){
var	 ttg1 = $("input[name='notertanggung']").val();
if (!ttg1==''){	 
	if(i == 1)
		NewWindow('../pempolpempre.php?n=1&c='+ttg1+'','page',600,400,1);
	else if(i == 2)
		NewWindow('../pempolpempre.php?n=2&c='+ttg1+'','page',600,400,1);
}
...
function cekbentukdataklien(nospajol) {
	alert(nospajol);
	//$("#pesan").text('');
	$.ajax({
		type : 'post',
		url : "../ajaxbentukdataklien.php",
		...
		success : function (data) {
			...
			$.ajax({
				type : 'post',
				url : "../ajaxgettertanggung_guardian.php",
				data : "nospaj="+nospajol,
				beforeSend: function() {
					$('#loaderttg').show();
				},
				success : function (data) {
					...
					var a = data.split(';');
					//alert(data);
					$("input[name='notertanggung']").val(a[0]);
					$('#kodeProduk').prop('disabled', false);
					$('#kodeProduk').val(a[1]).change();
					$('#caraBayar').val(a[2]).change();
					$('#lamapembpremi_th').val(a[3]);
					$('#mulas').val(a[5]);
					$('#lamaasuransi_th').val(a[3]);
					$('#nilai').val(a[4]);
					
					var mulas = a[5];
					var tglSplit = mulas.split("/");
					
					var tahunExp = tglSplit[0] + "/" + tglSplit[1] + "/" + (parseInt(tglSplit[2]) + parseInt(a[3])) ;
					
					$('#akhirpremi').val(tahunExp);
					$('#expirasi').val(tahunExp);
...
<table border="0" cellpadding="1" cellspacing="1" width="700" class="tblborder">
  ...
  <form name="ntryprop" method="post" action="insprop.php?<? echo $SID; ?>" onSubmit="return OnSumbit(document.ntryprop);disableForm(this);">
	...
	<tr bgcolor="#ffff91">
   	 ...
   	 <td class="arial10" colspan="5"> 
		 <!--input type="radio" id="taltupn" name="taltup" value="0" checked> NB
    ...
    <tr>
		...
		&nbsp;<input type="button" value="No.SPAJ"  onclick="NewWindow('spajol_guardian.php','popuppage',600,400,1);" class="buton">
  ...
  <tr>
     ...
     $disabled="disabled";
		 ...
		 <!--<input type="button" value="Cari Klien" name="cari" class="buton" <?php echo $disabled; ?> onClick="Klear(document.ntryprop);Cari(this.form);" onBlur="javascript:TanggalMulas()" >-->
		 <img id='loaderttg' src="../../includes/loader.gif" alt="Loading" height="16" width="16" style="display:none;"> 
		 </td>
  ...
  <tr>
   	 <td width="15%" class="arial10">Kode produk</td>
   	 <td width="2%" class="arial10">:</td>
   	 <td width="25%" class="arial10">
	 <select id="kodeProduk" name="kodeProduk" style="width:150px;" disabled="disabled">
		<option value='' selected></option>
		<option value='JL4XGIH'>JS GUARDIAN 85</option>
		<option value='JL4XGIG'>JS GUARDIAN 75</option>
	 </select>
  </tr>
  <tr>
   	 <td width="15%" class="arial10">Cara Bayar</td>
   	 <td width="2%" class="arial10">:</td>
   	 <td width="25%" class="arial10">
	 <select size="1" id="caraBayar" name="caraBayar" disabled="disabled">
		<option value='0' style="width:125px;" selected></option>
		<option value='X' style="width:125px;">SEKALIGUS</option>
	 </select>
  </tr>
  <tr>
  ...
  <tr>
   	 ...
   	 <td width="2%" class="arial10">:</td>
   	 <td width="25%" class="arial10"><input type="text" id="mulas" name="mulas" size="10" maxlength="10" onBlur="javascript:convert_date(mulas);HitungUsia(document.ntryprop);CekAwalMulas(document.ntryprop);" class="c" onFocus="highlight(event)" value="<? echo $mulas; ?>"></td>
  ...
  <tr>
   	 ...
   	 <td width="2%" class="arial10">:</td>
   	 <td width="25%" class="ver8ungu"><input type="text" id="lamapembpremi_th" name="lamapembpremi_th" size="2" maxlength="2"  onfocus="highlight(event)" class="c" onBlur="return cek_masapremi()"  value="<? echo $lprth; ?>" >tahun
   	 ...
   	 <td width="2%" class="arial10">:</td>
   	 <td width="20%" class="arial10"><input type="text" id="akhirpremi" name="akhirpremi" size="10" maxlength="10" class="a" onBlur="javascript:convert_date(akhirpremi);" readonly value="<? echo $akhpr; ?>"></td>
		 <td width="16%" class="arial10"></td>
	</tr>
  <tr>
   	 ...
   	 <td width="25%" class="ver8ungu">
		  <input type="text" id="lamaasuransi_th" name="lamaasuransi_th" size="2" maxlength="2" title="Lama Asuransi (Tahun)" class="a" readonly value="<? echo $lamth; ?>">tahun
   	 ...
   	 <td width="2%" class="arial10">:</td>
   	 <td width="20%" class="arial10"><input type="text" id="expirasi" name="expirasi" size="10" maxlength="10" class="a" onBlur="javascript:convert_date(expirasi);" readonly value="<? echo $expir; ?>"></td>
		 <td width="16%" class="arial10"></td>
	</tr>
...
-->
  ...
  <tr class="tblisi">
            ...
            <table border="0" cellpadding="1" cellspacing="1" width="100%" class="tblisi">
              ...
              <tr>
      				 <td align="center"><input type="button"  value="..." class="buton" onClick="PempolGuardian(1);"></td>
              ...
              <tr>
      				 <td align="center"><input type="button"  value="..." class="buton" onClick="PempolGuardian(2);"></td>
             ...
             <tr>
    					 <td width="100%" colspan="5" align="center" class="arial10">
	   					 <input type="button" class="buton" name="kurang" value="Kurangi Data Beneficiary" onClick="javascript:BeneficiariDel();">
	 	 					 <input type="button" class="buton" name="tambah" value="Tambah Data Beneficiary" onClick="javascript:BeneficiariGuard();"></td>
							</tr>	
              ...
              <tr class="tblhead1">
      				 ...
      				 <td width="32%" align="center">Hubungan</td>
							...
							<tr>
    					 <td width="100%" colspan="5" ><span id=pret1 style="position:relative;"></span></td>
							</tr>	
							<tr>
    					 <td width="100%" colspan="5"><span id=pret2 style="position:relative;"></span></td>
							</tr>	
							<tr>
    					 <td width="100%" colspan="5"><span id=pret3 style="position:relative;"></span></td>
							</tr>	
							<tr>
    					 <td width="100%" colspan="5"><span id=pret4 style="position:relative;"></span></td>
							</tr>	
							<tr>
    					 <td width="100%" colspan="5"><span id=pret5 style="position:relative;"></span></td>
							</tr>	
							<tr>
    					 <td width="100%" colspan="5"><span id=pret6 style="position:relative;"></span></td>
							</tr>	
							<tr>
    					 <td width="100%" colspan="5"><span id=pret7 style="position:relative;"></span></td>
							</tr>	
							<tr>
    					 <td width="100%" colspan="5"><span id=pret8 style="position:relative;"></span></td>
							</tr>	
	...
	<td>
	 ...
	 <tr>
   	 ...
   	 <td width="2%" class="arial10">:</td>
   	 <td width="25%" class="arial10"><input type="text" id="nilai" name="nilai" size="15" maxlength="15" onBlur="javascript:JuaPremi();MaxJuaPremi();"  class="c"></td>
