...
<?
    ...
    include "../../includes/belang.php";
		...
		/* Sementara ditutup dulu untuk meningkatkan performance database : Udi 26/11/2009 */
...
//echo $kdproduk;
	...
	$sql="select resikoawal from $DBUser.tabel_226_batas_resiko ".
			 "where $usia>=batasbawah and $usia<=batasatas and KDPRODUK=NVL((SELECT KDPRODUK FROM $DBUser.TABEL_226_BATAS_RESIKO WHERE KDPRODUK='$kdproduk' GROUP BY KDPRODUK),'*')";
	//echo $sql."<br>";
	$DB->parse($sql);
	$DB->execute();
	$res=$DB->nextrow();
...
$premiskg = (!$p1 == 0) ? $p1 : $tarifpremiskg*$juadlrp;			
 ...
 //=================RESIKO AWAL DI TARIF===================
 $sql="SELECT $DBUser.tarif.std('$kdproduk','$kdvaluta','RESIKOAWL',$masa,0,'B','$basis')/100 tarif ".
		  	  "FROM dual";	
	$DB->parse($sql);
	$DB->execute();
	$res=$DB->nextrow();
...
?>	
 ...
 <td class="tblisi1">
   ...
   <td align="right">:</td>
	 ...
	 ?>
   <td align="right"><?echo number_format($rskg*$trf,2);?></td>
...
if ($kdproduk=='JSGTP'){$restot=$rskg*$trf;} else {$restot=$restot;};
//echo $kdproduk.$restot.'||'.$batasresiko;
