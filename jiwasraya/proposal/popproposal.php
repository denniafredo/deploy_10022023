<?  
  ...
  echo "</table>";
	...
	printf("<font face=Verdana size=2><a href=popproposal.php?id=all>All</a></font>&nbsp;");
	$aray=array();
	for($i=0;$i<26;$i++) {
		$aray[]=chr(65+$i);
		printf("<font face=Verdana size=2><a href=popproposal.php?id=".$aray[$i].">".$aray[$i]."</a></font>&nbsp;");
	}
	printf("<font face=Verdana size=2><a href=popproposal.php?id=all>All</a></font>&nbsp;");
	echo "<br><br>";
			if ($id=='all'){
	               ...
	               "from $DBUser.tabel_200_pertanggungan a, $DBUser.tabel_100_klien b ".
			           ...
			           //"and prefixpertanggungan='$prefixpertanggungan' 
								 "and a.kdpertanggungan='1' order by nopertanggungan desc";
			} else {
					 if ($id!=''){ 	
					 $id=$id."%";
			    	$sql="select a.nopertanggungan,a.kdproduk,b.namaklien1,b.gelar,a.userupdated, ".
...
/*					 while($arr=$DB->nextrow()) 
	      		  ...
	      		  print( "<tr>" );
				      ...
				      print( "<tr bgcolor=#e0e0e4>" );
						...
						printf("<td><font face=Verdana size=1><a href=\"#\" onclick=\"javascript:window.opener.document.peliharapolis.nopertanggungan.value='%s';window.close();\">%s</a></font></td>",$arr["NOPERTANGGUNGAN"],$arr["NOPERTANGGUNGAN"]);//,$arr["NAMAKLIEN1"],$arr["TGLLAHIR"]);
