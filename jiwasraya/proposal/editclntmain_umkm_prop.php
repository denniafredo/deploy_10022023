...
<div align="center">
  <table border="0" cellspacing="1" cellpadding="0" class="tblborder">
    <form name="klien" action="updclntmain_umkm_prop.php" method="POST" onSubmit="return OnSumbit(document.klien)">
      <tr>
        <td width="100%"><table width="100%" border="0" cellpadding="4" cellspacing="0" class="tblisi1">
          <tr>
            ...
            <td align="left"><b><font color="red">
              <? $a = ($noklien) ? 'EDIT': 'ENTRY KLIEN';echo $a; ?></td>
          </tr>
          <tr>
        ...
        $query = "select meritalstatus,namameritalstatus from $DBUser.tabel_103_status_pernikahan";
          ...
          <tr>
            ...
            <td class="verdana10blk"><select name="kdkotamadyaKTP" size="1" class="c" id="kdkotamadyaKTP" onFocus="highlight(event)">
              <option></option>
              <?  
        $query = "select kdkotamadya,namakotamadya from $DBUser.tabel_109_KOTAMADYA order by namakotamadya asc";
		    $DB->parse($query);
		    $DB->execute();
    	  while($arr=$DB->nextrow()) {
          ...
          <tr>
            ...
            <td class="verdana10blk"><select name="kdpropinsiKTP" size="1" class="c" id="kdpropinsiKTP" onFocus="highlight(event)">
              <option></option>
              <?  
        $query = "select kdpropinsi,namapropinsi from $DBUser.tabel_108_propinsi order by namapropinsi asc";
		    $DB->parse($query);
		    $DB->execute();
    	  while($arr=$DB->nextrow()) {
        ...
        $query = "select kdpropinsi,namapropinsi from $DBUser.tabel_108_propinsi order by namapropinsi asc";
          ...
          <tr>
            ...
            <td class="verdana10blk"><select class="c" onFocus="highlight(event)" size="1" name="kdpekerjaan">
              <option></option>
              <?  
        $query = "select kdpekerjaan,namapekerjaan from $DBUser.tabel_105_pekerjaan order  by namapekerjaan asc";
		    $DB->parse($query);
		    $DB->execute();
        while($arr=$DB->nextrow()) {
