...
//if($_POST['sbmt'] == '1'){
    ...
    $msg="Yth. Bpk/Ibu ".$arsms["NAMA"].", permohonan asuransi Anda telah disetujui,segera bayar premi via Pos Indonesia/Mandiri/BRI/BNI/Indomaret ".
			 $arsms["H2H"]." Rp. ".$arsms["PREMI"].". Info hub 021-1500151";      			 
		//echo $msg;	 
		...
		//===================SMS AGEN================
		$msgagn="Harap infokan CPP a/n ".$arsms["NAMA"]." utk membayar premi via host to host Pos Indonesia/Mandiri/BNI/BRI/Indomaret no ".$arsms["H2H"]." Rp ".$arsms["PREMI"]." paling lambat ".$arsms["EXPDATE"];
...
<form name="xx" method="POST" action="<?=$_SERVER['PHP_SELF']?>">
<font face="Verdana" size="2"><b>DAFTAR POLIS MEDICAL</b><br>
Pilih Tahun : <select name="tahun">
<option value="2007"><?=$tahun;?></option>
...
</select><input type="submit" name="sumbit" value="Cari">
</form>
...
if($kantor=='KP') {
	$sql= "select ".
    	      ...
    	      "a.nopertanggungan,a.kdproduk,to_char(a.mulas,'DD/MM/YYYY') mulas,a.usia_th,".
				...
				"where ".
    				"a.kdstatusemail='1' ".
					"AND to_char(a.mulas,'yyyy')='$tahun' ".
...
"ORDER BY a.prefixpertanggungan, a.nopertanggungan,a.mulas ";}
		//echo $sql;	
		//die;		
	$DB->parse($sql);
	$DB->execute();
