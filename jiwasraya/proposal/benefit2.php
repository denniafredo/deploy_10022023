<?
  ...
  $prd=$DB->nextrow();
	...
	$sql="select a.kdproduk,a.periodebayar,a.periodebenefit,a.kdbenefit,a.nilaibenefit,a.premi,a.kdjenisbenefit,b.kdrumusbenefit, b.kdrumuspremi, c.namabenefit, c.kdkelompokbenefit ".
...
<?	
  ...
  foreach ($result as $foo => $arr) {
		...
		ECHO "hasil premi  : ". $hasilpremi;
		ECHO "arr premi  : ". $arr["PREMI"];
...
/*********************************************************************/
	  ...
	  echo "<tr class=arial8>";
		...
		echo "   <input type=\"text\" name=bnf".$kdbenefit." value=".$hasilbenefit."></td>";
		echo "<td align=\"right\">".number_format($hasilpremi,2)."";
		echo "   <input type=\"text\" name=prm".$kdbenefit." value=".$hasilpremi." ></td>";
...
//echo $hpremiw."|".$hpremiu."|".$hpremiv."|".$hpremir."|".$hpremix."<br>";
			   ...
			   $premi01 = &$FM->premi1;
				...
				echo 'premistandar '.$premistandar.'<br>';
				echo 'jmlpremi '.$jmlpremi.'<br>';
				
				if (substr($kdproduk, 0, 3) == 'AI0')
					$jua1 = round(1000 * $premistandar / $jmlpremi);
				else
				 ...
				 echo $FM->faktorbayar."|".$premi01."|".$premistandar."|".round($jua1)."AA";
//die;				 
				 $DB->parse($sql);
	  		 $DB->execute();
	  		 $DB->commit();
/*****************************************************************************************************/								
	...
	echo "<td align=right><font face=\"Arial\" size=\"2\"><b>".number_format($jmlpremi,2).'|'.$premistandar.'|'.number_format($jua1,0,"","")."</b></font></td>";
	echo "<td>&nbsp;</td>";
	echo "</tr>";
	echo "</table>";
...
printf ("<form action=\"benefit1.php?vara=$vara&premijua=$premijua&jua1=".number_format($jua1,0,"","")."&prefixpertanggungan=$prefixpertanggungan&nopertanggungan=$nopertanggungan&noproposal=$noproposal&kdper=$kdper&premi1=$premi1\" name=\"porm\" method=\"post\">");
