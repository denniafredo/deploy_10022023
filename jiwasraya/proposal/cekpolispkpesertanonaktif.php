...
<?	
	...
	$sql="select b.nopeserta,b.namapeserta,b.status,b.tgllahir,a.tglasuransi,a.akhirmasa,b.nosertifikat,
			...
			(select status from mutasi where nokontrak=a.nokontrak and nomutasi=a.nomutasi)status,a.nomutasi				
...
?>
	<tr class="hijao">
		<td align=center>Status Mutasi</td>
...
<?		
	while($arr=$DB->nextrow()) 
	{
		...
		if ($status == '0' ) 
		{	$stat = "submit";
			echo("<td class=arial10ungu>".$stat."</td>");}
		elseif ($status == '1') 
		...
		{	$stat = "approved";
			echo("<td class=arial10ungu>".$stat."</td>");}
		echo("<td class=arial10ungu>".$arr["NOMUTASI"]."</td>");
