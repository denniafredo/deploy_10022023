	...
	while($arr=$DB->nextrow()){
		...
		<tr>
			...
			<td bgcolor="#FFFFFF" align="center"><font face="Verdana" size="1">
				<?php if ($arr['SUSPEND']) { ?>
					<input type="button" name="lanjut" onclick="window.location.href='<?="$_SERVER[PHP_SELF]?lanjut=$arr[NOPERTANGGUNGAN]";?>'" value="Lanjut"/>
					<!--input type="hidden" name="lanjut" value="<?=$arr['NOPERTANGGUNGAN'];?>" />
					<input type="submit" name="kirim" value="Lanjut" style="padding:0px;margin:0px;" />
