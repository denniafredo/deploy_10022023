...
//--------------------------- content ------------	
	echo "<font face=\"Verdana\" size=\"2\"><b>Kirim Email Proposal Bayar BP3</b></font>";
	echo "<hr size=\"1\">";
	...
	$sqlx="select a.prefixpertanggungan,a.nopertanggungan,a.kdpertanggungan,a.notertanggung,".
        ...
        "where ".
        "a.prefixpertanggungan='$kantor' and a.notertanggung=c.noklien and ".
				"a.nopemegangpolis=d.noklien and a.nopertanggungan='$nopertanggungan'";
  $DB->parse($sqlx);
	$DB->execute();
			 // echo "<font color=\"black\" face=\"Verdana\" size=\"1\"><b><u>Proposal Medical</u></b></font><br>";	
  $today = date("F j, Y, g:i a");                 
	$isin .= "Kepada Yth.\n".$namakantor."\ndi Tempat \n\n";
  $isin .= "Perihal : Pemberitahuan Lunas BP3 \n\n";
  $isin .= "Berikut ini kami sampaikan proposal yang telah membayar BP3 untuk\ndapat segera diproses kelanjutannya, sebagai berikut :\n\n";
	...
	while($ary=$DB->nextrow()){
	      ...
	      $isin .= "Demikian kami sampaikan atas perhatiannya \nkami ucapkan terima kasih.\n\n\n";
		    ...
		    $isin .= "Branch Manager\n";
			  $isin .= $pejabat;
		    $message = $isin;
  			mail($email,"Pemberitahuan Lunas BP3 ($today)",$message,"From: $emailpengirim\nReply-To: $emailpengirim\nX-Mailer: PHP/" . phpversion());
  			echo "<br>";
