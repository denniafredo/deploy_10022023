	...
	<div class="row" align="center">
		<div class="col-xs-12">
			...
			<div class="well" style="font-weight:bold;font-size:16px">
				JS SMART PROMAPAN (Jiwasraya Smart Proteksi Masa Depan)
			</div>
		</div>
	 </div>
	<hr/>
				...
				<div class="table-responsive">
					<table class="table table-striped table-hover">			
						...
						foreach ($query5->result_array() as $row)
						{
							...
							if($row['IS_HCPCARE']!= '0'){
								echo "<tr>
										<td>JS Hospital Care</td>
										<td align='center'>70</td>
										<td align='right'>".number_format($row['HCPCARE'],0,',','.')."</td>
										<td align='right'>".number_format($row['BIAYA_HCPCARE'],0,',','.')."</td>
										<td align='center'>0%</td>
										<td align='right'>0</td>
									</tr>";
							}
							...
							$persentase_rider_payor = number_format(100/($xp + $yp + $zp),2);
							...
							$persentase_rider_spouse = number_format(100/($xs + $ys + $zs),2);
							...
							if($row['IS_PBD']!=0){
								echo "<tr>
										...
										<td align='center'>".$persentase_rider_payor."%</td>
							...
							if($row['IS_SPD']!=0){
								echo "<tr>
										...
										<td align='center'>".$persentase_rider_spouse."%</td>
							...
							if($row['IS_WPCI51']!=0){
								...
								echo "<tr>
										...
										<td align='center'>".$nilaipersen."</td>
							...
							$total_ua = $row['UADASAR'] + $row['CI53'] + $row['ADDB'] + $row['ADB'] + $row['HCP'] + $row['TPD'] + $row['TR'] + $row['PBD'] + $row['PBCI'] + $row['PBTPD'] + $row['SPD'] + $row['SPCI'] + $row['SPTPD'] + $row['WPTPD'] + $row['WPCI51'] + $row['HCPCARE'] ;
							$total_biaya_new = $row['BIAYA_UADASAR'] + $row['BIAYA_CI53'] + $row['BIAYA_ADDB'] + $row['BIAYA_ADB'] + $row['BIAYA_HCP'] + $row['BIAYA_TPD'] + $row['BIAYA_TR'] + $row['BIAYA_PBD'] + $row['BIAYA_PBCI'] + $row['BIAYA_PBTPD'] + $row['BIAYA_SPD'] + $row['BIAYA_SPCI'] + $row['BIAYA_SPTPD'] + $row['BIAYA_WPTPD'] + $row['BIAYA_WPCI51'] + $row['BIAYA_HCPCARE'];
							...
							$total_ra = $row['UADASAR'] + ($row['CI53']/2) + ($row['TPD']/2) + $row['TR'] + max(($row['WPTPD']*$tarifwpdanspuse/1000), ($row['WPCI51']*$tarifwpdanspuse/1000));
							$total_ra_pp = max(($row['PBD']*$tarifpayor/1000), ($row['PBCI']*$tarifpayor/1000), ($row['PBTPD']*$tarifpayor/1000), ($row['SPD']*$tarifwpdanspuse/1000), ($row['SPCI']*$tarifwpdanspuse/1000), ($row['SPTPD']*$tarifwpdanspuse/1000));
						?>	
...
<br><br><br><hr>
		<!-- <table class="table table-responsive-sm table-bordered">
                          <tr>
                            ...
                            <th>JS SP-TPD</th>
                            <th>TOTAL BIAYA</th>
                            <th>Investasi - Rendah 01</th>
                            <th>Investasi - Sedang 01</th>
                            <th>Investasi - Tinggi 01</th>
                            <th>Investasi - Rendah 02</th>
                            <th>Investasi - Sedang 02</th>
                            <th>Investasi - Tinggi 02</th>
		...
		<!-- Fungsi Untuk Menghitung Ilustasi -->
			<?php
				...
				while($usia <= 99) {
					...
					$sqltarif = 'select * from JAIM.JAIM_300_TARIF_SMART_PROMAPAN where usia = '.$usia;
					...
					foreach ($querytarif->result_array() as $tarif){
						...
						if($ua_hcpcare == 'A' && $jeniskelaminctt == 'M'){
							$tarifriderjshcpcare = $tarif['HC_L_A'];
						}else if($ua_hcpcare == 'B' && $jeniskelaminctt == 'M'){
							$tarifriderjshcpcare = $tarif['HC_L_B'];
						}else if($ua_hcpcare == 'C' && $jeniskelaminctt == 'M'){
							$tarifriderjshcpcare = $tarif['HC_L_C'];
						}else if($ua_hcpcare == 'D' && $jeniskelaminctt == 'M'){
							$tarifriderjshcpcare = $tarif['HC_L_D'];
						}else if($ua_hcpcare == 'E' && $jeniskelaminctt == 'M'){
							$tarifriderjshcpcare = $tarif['HC_L_E'];
						}else if($ua_hcpcare == 'A' && $jeniskelaminctt == 'F'){
							$tarifriderjshcpcare = $tarif['HC_P_A'];
						}else if($ua_hcpcare == 'B' && $jeniskelaminctt == 'F'){
							$tarifriderjshcpcare = $tarif['HC_P_B'];
						}else if($ua_hcpcare == 'C' && $jeniskelaminctt == 'F'){
							$tarifriderjshcpcare = $tarif['HC_P_C'];
						}else if($ua_hcpcare == 'D' && $jeniskelaminctt == 'F'){
							$tarifriderjshcpcare = $tarif['HC_P_D'];
						}else if($ua_hcpcare == 'E' && $jeniskelaminctt == 'F'){
							$tarifriderjshcpcare = $tarif['HC_P_E'];
						}else if($ua_hcpcare == 0){
							$tarifriderjshcpcare = "0,0";
						}
						$tarifriderjshcpcare_	= str_replace(',', '.', $tarifriderjshcpcare);
					...
					for ($y = 1; $y <= 12; $y++) {
						...
						$biaya_pbd		= $ua_pbd*$tarifriderjspbd/12/100*(1+($nilaiekstrapremilife_cpp/1000));
						$biaya_pbci 	= $ua_pbci*$tarifriderjspbci/12/100;
						$biaya_pbtpd	= $ua_pbtpd*$tarifriderjspbtpd/12/100*(1+($nilaiekstrapremitpd_cpp/100));
						$biaya_spd		= $ua_spd*$tarifriderjsspd/12/100*(1+($nilaiekstrapremilife_cpp/1000));
						...
						$biaya_hcpcare	= ($tarifriderjshcpcare_ * 1.12) / 12;

						$biaya_coicor_awal = ($biaya_dasar + $biaya_ci53 + $biaya_adb + $biaya_addb + $biaya_hcp + $biaya_tpd + $biaya_jstr + $biaya_pbd + $biaya_pbci + $biaya_pbtpd + $biaya_spd + $biaya_spci + $biaya_sptpd + $biaya_wpci51 + $biaya_wptpd + $biaya_hcpcare);
						if($tahun_ke == 1){
							$biaya_coicor = $biaya_coicor_awal * 0.6;
							$sisa_biaya_coicor1 = $biaya_coicor_awal * 0.4;
						}elseif($tahun_ke == 2){
							$biaya_coicor = ($biaya_coicor_awal * 0.6) + $sisa_biaya_coicor1;
							$sisa_biaya_coicor2 = $biaya_coicor_awal * 0.4;
						}elseif($tahun_ke == 3){
							$biaya_coicor = ($biaya_coicor_awal * 1) + $sisa_biaya_coicor2;
						}else{
							$biaya_coicor = $biaya_coicor_awal * 1;
						}
						$total_biaya	= $biaya_coa + $biaya_coicor;

						if($y==12){
							if($carabayar==1){
								$iuran = 12*($premi_dasar_input + $premi_topup_input);
							}else{
								$iuran = ($premi_dasar_input + $premi_topup_input);
							}
						...
						if($tahun_ke == 1){
								$akuisisi_premi_dasar = 0.65*$premi_dasar;
								$alokasi_premi_dasar = 0.35*$premi_dasar;
							}
							else if($tahun_ke == 2){
								$akuisisi_premi_dasar = 0.65*$premi_dasar;
								$alokasi_premi_dasar = 0.35*$premi_dasar;
							}else if($tahun_ke == 3){
								$akuisisi_premi_dasar = 0.20*$premi_dasar;
								$alokasi_premi_dasar = 0.80*$premi_dasar;
							}else if($tahun_ke == 4){
								$akuisisi_premi_dasar = 0.20*$premi_dasar;
								$alokasi_premi_dasar = 0.80*$premi_dasar;
							}else if($tahun_ke == 5){
						...
						// 			echo "<td> ".round($biaya_hcpcare)." </td>";
	...
	<div class="row">
		<div class="col-md-12">
			<span style="margin-left:2em">
				<a href='#' onClick="popitup('<?=base_url('jssmartpromapan/CetakPDF?build_id='.$buildid.'&filepdf='.$filepdf.'&kodeprospek='.$kodeprospek);?>')">
