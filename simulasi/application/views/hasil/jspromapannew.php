...
$sql5 = "SELECT B.JENIS,a.NAMA,DESKRIPSI,URUT,SU,B.BUILD_ID,B.NILAI PREMI,C.NILAI UA,nvl(pengakuan,0) pengakuan,
                        nvl(CASE WHEN c.JENIS IN ('WAIVER_CI','WAIVER_TPD') THEN
                          (f.PREMI_BERKALA+f.TOPUP_BERKALA)*(SELECT TARIF FROM TARIF_PROMAPAN WHERE KD_TARIF = 'JRA_WP' AND MASA = 65-e.USIA_TH)
                        ELSE
...
<div class="row">
	<div class="col-md-12">
			...
			<a href="#" onClick="popitup('<?=base_url('jspromapannew/CetakPDF?build_id='.$buildid.'&filepdf='.$filepdf.'&kodeprospek='.$kodeprospek);?>')">
