...
<div style="border: 5px solid #444; padding: 20px; margin-left:20px; margin-right:20px" class="row">
        ...
        <div class="form-group" style="visibility: hidden">
										  ...
										  <select class="form-control select2me" name="pilihananuitas" id="pilihananuitas" onChange="">
                                                <option value="EKSEKUTIF PRIMA">JS Plan Annuity Assurance</option>
										  </select>
										</div>
		</div>
							...
							<table class="table table-striped table-hover">
								...
								<thead>
                                     <tr style="">
										...
										<td>JS Plan Annuity Assurance </td>
                                      	<td>Rp.</td>
										<td>
      ...
      // initiate layout and plugins
       ...
       ComponentsPickers.init();
		...
		$("#premisekaligusjsanuitas").change(function() {
			...
			$.ajax({
				type	: "POST",
				url		: "<?=base_url('anuitasnew/hitungpremisekaligusaspjsanuitas');?>",
			...
			$.ajax({
				type	: "POST",
				url		: "<?=base_url('anuitasnew/hitungpremisekaligusaspjsanuitas');?>",
				data	: "premisekaligusjsanuitas="+premisekaligusjsanuitas+"&saatmulaiasuransijsanuitas="+saatmulaiasuransijsanuitas+"&usiasekarang="+usiasekarang+"&statuskawin="+"bujang"+"&tgllahir="+tgllahir,
				success : function(data) {
					$("#aspbujang").val(data);
				}
			});
			$.ajax({
				type	: "POST",
				url		: "<?=base_url('anuitasnew/hitungpremisekaligusasijsanuitas');?>",
				data	: "premisekaligusjsanuitas="+premisekaligusjsanuitas+"&saatmulaiasuransijsanuitas="+saatmulaiasuransijsanuitas+"&usiasekarang="+usiasekarang+"&statuskawin="+"kawin"+"&tgllahir="+tgllahir,
				success : function(data) {
					$("#asikawin").val(data);
				}
			...
			$.ajax({
				type	: "POST",
				url		: "<?=base_url('anuitasnew/hitungpremisekaligusaepjsanuitas');?>",
				data	: "premisekaligusjsanuitas="+premisekaligusjsanuitas+"&saatmulaiasuransijsanuitas="+saatmulaiasuransijsanuitas+"&usiasekarang="+usiasekarang+"&statuskawin="+"kawin"+"&tgllahir="+tgllahir,
				success : function(data) {
					$("#aepkawin").val(data);
				}
