	...
	<!-- END CONTENT -->
		...
		<script>
			...
			$('#tanggallahircalontertanggung').on('change', function() {
				...
				// var bulan = ((today-dob) / (365.7 * 24 * 60 * 60 * 1000));
				// if(bulan < 0.5 || age > 64){
				// 	alert("Calon Tertanggung Minimal berusia 6 bulan dan Maksimal berusia 64 tahun");
				// 	document.getElementById("tanggallahircalontertanggung").value = "";
				// }
				...
				$.ajax({
					...
					success : function(msg) {
						...
						if(nilaiekstrapremilife_cpp == 'REFER'){
							...
							var tarifriderjsspd_ = ((nilaiekstrapremilife_cpp * 1) + (tarifriderjsspd.replace(",", ".")) * 1) / 12;
			...
			function getAllTarif(){
				...
				if( kdjenispekerjaanctt=="" ||  hubungandenganpempol=="" || jeniskelaminctt=="" || perokokcalontertanggung =="" || premi==0)
				{
					$.LoadingOverlay("hide");
				...
				$.ajax({
					...
					success : function(msg) {
						...
						if(nilaiekstrapremilife_ctt == 'REFER'){
							...
							var tariftermlife_ = ((nilaiekstrapremilife_ctt * 1) + (tariftermlife.replace(",", ".")) * 1) / 12;
						}
						...
						if (checkBox_jstr.checked == true){
							if(nilaiekstrapremilife_ctt == 'REFER'){
								...
								var pilihanjstr = 1;
								var tariftermrider_ = ((nilaiekstrapremilife_ctt * 1) + (tariftermrider.replace(",", ".")) * 1) / 12;
						...
						if(kdjenispekerjaancpp ==169 || kdjenispekerjaancpp ==306){
			...
			$('#submitProposal').on('click', function() {
				...
				var pilihanalokasi = document.getElementById("alokasidana1").value;
				if(UA==0 || pilihanalokasi==""){
					$.LoadingOverlay("hide");
					alert("Nilai Uang Asuransi atau Pilihan Investasi Masih Kosong!");
					exit();
				}
				...
				//var persentasealokasidana2 = document.getElementById("persentasealokasidana1").value;
				...
				$.ajax({
					...
					data	: "alokasidana1="+alokasidana1+"&persentasealokasidana1="+persentasealokasidana1+"&carabayarjspromapannew="+carabayarjspromapannew+"&totalpremi="+totalpremi+"&usiasekarang="+usiasekarang,
					success : function(msg) {
				...
				$.ajax({
					...
					success : function(msg) {				
						...
						window.location.href= "<?=base_url('jspromapannew_2019/hasil')?>?buildid="+<?=$this->session->userdata('build_id');?>+'&filepdf='+filepdf+'&kodeprospek='+kodeprospek; 
						$.LoadingOverlay("hide");
				...
				$.ajax({
					type	: "POST",
					url		: "<?=base_url('jspromapannew_2019/insertDataPDF');?>",
