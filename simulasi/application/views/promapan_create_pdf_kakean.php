...
*/
		...
		foreach($hasilProDataRider as $data) {						
			$this->pdf->Cell(50,3,$data->NAMA,0,0,'L');
			$this->pdf->MultiCell(140,3,$data->DESKRIPSI,0);
			$x = $this->pdf->GetX();			
			$y = $this->pdf->GetY();
			$this->pdf->ln(2);						
		...
		$this->pdf->Cell(60,5,'SALDO DANA (000)','R',0,'C');
		$this->pdf->Cell(60,5,'MANFAAT MENINGGAL DUNIA (000)','R',0,'C');
		$this->pdf->ln();
		
		$this->pdf->SetFont('Arial','',6);
		$this->pdf->Cell(15,3,'Tahun',1,0,'C');
		$this->pdf->Cell(15,3,'Usia ^',1,0,'C');
		$this->pdf->Cell(20,3,'Premi (000)',1,0,'C');
...
//		$this->pdf->Cell(12,5,'dikenakan biaya sebesar 2% dari total dana penarikan.',0,0,'L');
		...
		*/
		...
		$this->pdf->Cell(0);
		$this->pdf->SetFont('Arial','',8);
		$this->pdf->Cell(140,5,'Apabila Tertanggung meninggal dunia dalam masa asuransi sampai dengan ulang tahun Polis diusia Tertanggung 99 Tahun, ','T',0,'L');
		$this->pdf->SetFont('Arial','',8);
		*/
		...
		foreach($hasilProDataRider as $data) {
			...
			{
				//$this->pdf->Cell(60,5,number_format($data->UA,0,'.',','),'T',0,'L');
				//$this->pdf->Write(0, number_format($data->UA,0,'.',',')); 
				$this->pdf->Text(180, 130, "".  number_format($data->UA,0,'.',','));
			}
		}
		...
		$this->pdf->Cell(1);
		$this->pdf->SetFont('Arial','',8);
		$this->pdf->Cell(140,5,'maka akan dibayarkan Uang Asuransi Dasar ditambah dengan Nilai Tunai Polis. Apabila Tertangung hidup sampai  ','B',0,'L');
		$this->pdf->SetFont('Arial','',8);
		$this->pdf->Cell(50,5,'','B',0,'L');
		$this->pdf->ln();
		$this->pdf->Cell(1);
		$this->pdf->SetFont('Arial','',8);
		$this->pdf->Cell(140,5,'berakhirnya Masa Asuransi, maka Jiwasraya akan membayarkan Nilai Tunai (Jumlah Unit x NAB)','B',0,'L');
		$this->pdf->SetFont('Arial','',8);
		$this->pdf->Cell(50,5,'','B',0,'L');
		$this->pdf->ln();
		*/
		...
		$this->pdf->Cell(1);
		$this->pdf->SetFont('Arial','',8);
		$this->pdf->Cell(160,5,'Jika Tertanggung meninggal dunia sebelum berusia 65 Tahun, maka Manfaat yang diterima adalah sejumlah','T',0,'L');
		$this->pdf->SetFont('Arial','',8);
		foreach($hasilProDataRider as $data) {
