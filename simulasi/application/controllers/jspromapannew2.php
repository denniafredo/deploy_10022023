...
class Jspromapannew2 extends CI_Controller{
...
//			'BUILD_ID' => $result['BUILD_ID'],
		...
		$result = $this->ModSimulasiKakean->setUAByRiderv2($datax);
	...
	/*=================Fungsi untuk mendapatkan tarif ProMapan=====================*/

	function getTarifUAdasar(){
		
		$this->load->model('ModSimulasiKakean');

		$usiasekarang  = $this->input->post("usiacalontertanggung");
		$perokokcalontertanggung  = $this->input->post("perokokcalontertanggung");
		//echo $perokokcalontertanggung;
		$result = $this->ModSimulasiKakean->getTarifUA_nonRokok($usiasekarang, $perokokcalontertanggung);
	...
	function getDataProRiderAll(){
		...
		$result = $this->ModSimulasiKakean->getDataProRiderAllv2($buildID);
	...
	function recalcUAByRiderv2()
	{
		...
		switch ($pilihriderlainnya) {
			...
			case "JS Payor Death":
				$fieldName = "IS_PAYOR_DEATH";
				$fieldNameUnset = "IS_CI = '0', IS_TL = '0', IS_ADDB = '0', IS_TPD = '0' ";
				break;
			case "JS Spouse Death":
				$fieldName = "IS_SPOUSE_DEATH";
				$fieldNameUnset = "IS_CI = '0', IS_TL = '0', IS_ADDB = '0', IS_TPD = '0' ";
				break;	
...
//		echo $filepdf;
	...
	function insertProDataRider(){
		...
		$data['datarider'] = array(
			...
			'IS_PAYOR_CI' => $this->input->post("jspcijspromapannew"),
			'PAYOR_CI' => $this->input->post("uangasuransijspcijspromapannew"),
			
			'IS_SPOUSE_CI' => $this->input->post("jsspcijspromapannew"),
			'SPOUSE_CI' => $this->input->post("uangasuransijsspcijspromapannew"),
		...
		$result2 = $this->ModSimulasiKakean->GenerateAllDataV2($this->session->userdata('build_id'));
