...
class Jssiharta extends CI_Controller{

	function hitungpremici53jssiharta(){
		
		$this->load->model('ModSimulasi');
		
		$usiasekarang = $this->input->post("usiasekarang");
		
		$uangasuransici53jssiharta = $this->input->post("uangasuransici53jssiharta");
		...
		$hasil = $result[$jeniskelamin] * ($uangasuransici53jssiharta / 1000);
		
		echo round($hasil);
	}
	
	function hitungpremitermjssiharta(){
		
		$this->load->model('ModSimulasi');
		
		$uangasuransi = $this->input->post("uangasuransi");
		
		$uangasuransitermjssiharta = $this->input->post("uangasuransitermjssiharta");
		
		$usiasekarang = $this->input->post("usiasekarang");
		
		$calonpemegangpolisperokokjssiharta = $this->input->post("calonpemegangpolisperokokjssiharta");
		
		if ($calonpemegangpolisperokokjssiharta == 'Ya')
		{
			$status = 'NONSMOKER';
			$faktorpengali = 3;
		}
		else if ($calonpemegangpolisperokokjssiharta == 'Tidak')
		{
			$status = 'SMOKER';
			$faktorpengali = 2;
		}
		
		$result = $this->ModSimulasi->getBiayaAsuransiPerBulanTerm($usiasekarang, $status, $uangasuransitermjssiharta);
		
		if ($uangasuransitermjssiharta > ($uangasuransi * $faktorpengali))
		{
				$hasil = 'TIDAK BISA';
		}
		else
	...
	function hitungpremijshcp(){
		$this->load->model('ModSimulasi');
		
		$uangasuransihcpjssiharta = $this->input->post("uangasuransihcpjssiharta");
		...
		$result = $this->ModSimulasi->getBiayaAsuransiPerBulanHCP($usiasekarang, $ranap, $statkelaminhcp);
	...
	function hitungpremijshcpb(){
		$this->load->model('ModSimulasi');
		
		$uangasuransihcpbjssiharta = $this->input->post("uangasuransihcpbjssiharta");
		...
		$result = $this->ModSimulasi->getBiayaAsuransiPerBulanHCPBedah($usiasekarang, $statkelaminhcpb, $plafon);
	...
	function hasil(){
		...
		$data['hasil']['xn'] = 65 - $data['hasil']['usiacalontertanggung'];
		...
		$data['hasil']['uangasuransitermjssiharta'] = $this->session->userdata(uangasuransitermjssiharta);
		...
		if (($usiacalontertanggung > 17) && ($usiacalontertanggung < 41))
		{
			$rangeusia = 1;
		}
		...
		$this->load->view('hasil/jssiharta',$data);
	...
	function createPDF($namaFile,$data){
		...
		$this->pdf->Cell(190,5,'JS SIHARTA (Jiwasraya Simpanan Hari Tua)','B',0,'L');
		$this->pdf->ln(10);
		...
		// JAMINAN TAMBAHAN/RIDER
		$this->pdf->SetFont('Arial','B',8);
		$this->pdf->SetFillColor(200,200,200);
		$this->pdf->Cell(190,4,'C. JAMINAN TAMBAHAN/RIDER',1,0,'L', true);
		$this->pdf->ln(10);
		$this->pdf->SetFont('Arial','B',8);
		$this->pdf->Cell(20,5,'1. JS Term Life',0,0,'L');
		$this->pdf->ln();
		$this->pdf->SetFont('Arial','',8);
		$this->pdf->Cell(3,5,'',0,0,'L');
		$this->pdf->Cell(170,5,'Apabila Tertanggung membeli manfaat tambahan (rider) JS Term, maka apabila terjadi risiko meninggal dunia pada Tertanggung, kepada ahli',0,0,'L');
		$this->pdf->ln();
		$this->pdf->SetFont('Arial','',8);
		$this->pdf->Cell(3,5,'',0,0,'L');
		$this->pdf->Cell(140,5,'waris pembayaran manfaat akan ditambahkan 100% Uang Asuransi JS Term Life',0,0,'L');
		$this->pdf->Cell(1,5,'Rp. '.number_format($data['hasil']['uangasuransitermjssiharta'],0,'.',',').' ',0,0,'L');
		$this->pdf->ln();
		$this->pdf->SetFont('Arial','B',8);
		$this->pdf->Cell(20,5,'2. JS CI53',0,0,'L');
		$this->pdf->ln();
		$this->pdf->SetFont('Arial','',8);
		$this->pdf->Cell(3,5,'',0,0,'L');
		$this->pdf->Cell(170,5,'Apabila Tertanggung membeli manfaat tambahan (rider) JS CI53, maka apabila Tertanggung di diagnosa mengalami salah satu dari 53 jenis',0,0,'L');
		$this->pdf->ln();
		$this->pdf->SetFont('Arial','',8);
		$this->pdf->Cell(3,5,'',0,0,'L');
		$this->pdf->Cell(140,5,'penyakit kritis, maka akan dibayarkan Uang Asuransi Critical Illness',0,0,'L');
		$this->pdf->Cell(1,5,'Rp. '.number_format($data['hasil']['uangasuransitermjssiharta'],0,'.',',').' ',0,0,'L');
		$this->pdf->ln(10);
		$this->pdf->SetFont('Arial','',8);
		$this->pdf->Cell(20,5,'Semoga proposal ini dapat memberikan informasi yang memenuhi kebutuhan anda.',0,0,'L');
