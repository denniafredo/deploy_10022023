...
class Linkbalancedfund extends CI_Controller{
	...
	function hasil(){
		...
		if ($idprod == 6)
		{
			$jenis_produk = "JS LINK BALANCED FUND"; 
		}
		...
		$this->load->model('ModSimulasi');
		$detailproduk = $this->ModSimulasi->getDetailProduk(2);
		...
		$this->load->view('hasil/linkbalancedfund',$data);
	...
	function createPDF($namaFile,$data){
		...
		$this->pdf->Cell(190,10,'JS LINK BALANCED FUND',1,0,'C', true);
		$this->pdf->ln(15);
		...
		$this->pdf->ln();
		$this->pdf->Cell(190,5,'JS LINK BALANCED FUND',0,0,'C');
		$this->pdf->ln(10);
		$this->pdf->SetFont('Arial','B',10);
		$this->pdf->Cell(1);
