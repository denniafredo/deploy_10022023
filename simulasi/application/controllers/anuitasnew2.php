...
class Anuitasnew2 extends CI_Controller{
	
	function hitungpremisekaligusanijsanuitas(){
		
		$this->load->model('Modanuitasnew2');
		...
		*/
		...
		$tarifanuitasani1 = $this->Modanuitasnew2->GetTarifAnuitasANI($usiaSekarang, $statuskawin, $selisihBulan, $premisekaligusjsanuitas);
	...
	function hitung(){
		...
		$idNasabah = $this->Modanuitasnew2->GetIdNasabah();
		...
		//$nasabahID = $this->Modanuitasnew2->insertNasabah($data['nasabah']);
		...
		$dataBuildID = $this->Modanuitasnew2->getBuildID();
		...
		//$this->Modanuitasnew2->insertNasabah($data['hitung']);
	...
	function hasil(){
		...
		$dataKantor = $this->Modanuitasnew2->cariDataKantor($kodeKantor);
		...
		$DataAgen = $this->Modanuitasnew2->GetDataAgen($NoProspek);
		...
		$idHit = $this->Modanuitasnew2->GetIdHit();
		...
		$this->Modanuitasnew2->insertSimulasi($data['hitung']);
		...
		$this->load->view('hasil/anuitasnew2',$data);
	...
	function createPDF($namaFile,$data){
		...
		*/
		...
		// PAGE 8
		if (($data['hasil']['pilihananuitas'] == 'IDEAL (SESUAI UU)') && ($data['hasil']['statuskawin'] == 'kawin'))
		{
		$this->pdf->AddPage();
		...
		$this->pdf->SetTextColor(0,0,0);
		$this->pdf->SetFillColor(0,0,0);
		$this->pdf->Cell(190,8,'JS Anuitas Ideal',0,0,'C');
		$this->pdf->ln(10);
		...
		$this->pdf->SetFont('Arial','',8);
		$this->pdf->Cell(6,5,'',0,0,'L');
		$this->pdf->Cell(190,5,'Dibayarkan satu bulan sesudah premi disetor sebesar Rp. '.number_format($data['hasil']['aikawin'],0,'.',',').' setiap bulan.',0,0,'L');
		$this->pdf->ln();
		$this->pdf->SetTextColor(0,0,0);
		$this->pdf->SetFont('Arial','',8);
		$this->pdf->Cell(6,5,'',0,0,'L');
		...
		$this->pdf->SetFont('Arial','',8);
		$this->pdf->Cell(6,5,'',0,0,'L');
		$this->pdf->Cell(190,5,'Dimulai pada bulan berikutnya setelah Tertanggung meninggal dunia sebesar Rp. '.number_format($data['hasil']['aikawin']*0.6,0,'.',',').'',0,0,'L');
		$this->pdf->ln();
		$this->pdf->SetTextColor(0,0,0);
		$this->pdf->SetFont('Arial','',8);
		$this->pdf->Cell(6,5,'',0,0,'L');
		...
		$this->pdf->SetFont('Arial','',8);
		$this->pdf->Cell(6,5,'',0,0,'L');
		$this->pdf->Cell(190,5,'dibayarkan kepada ahli waris jika sudah tidak ada lagi yang berhak menerima pensiun.',0,0,'L');
		$this->pdf->ln(65);
		...
		// PAGE 9
		if (($data['hasil']['pilihananuitas'] == 'IDEAL (SESUAI UU)') && ($data['hasil']['statuskawin'] == 'bujang'))
		{
		$this->pdf->AddPage();
		...
		$this->pdf->SetFont('Arial','',8);
		$this->pdf->Cell(6,5,'',0,0,'L');
		$this->pdf->Cell(190,5,'Dibayarkan satu bulan sesudah premi disetor sebesar Rp. '.number_format($data['hasil']['aibujang'],0,'.',',').' setiap bulan.',0,0,'L');
		$this->pdf->ln();
		$this->pdf->SetTextColor(0,0,0);
		$this->pdf->SetFont('Arial','',8);
		$this->pdf->Cell(6,5,'',0,0,'L');
