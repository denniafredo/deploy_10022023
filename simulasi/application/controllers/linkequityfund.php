...
class Linkequityfund extends CI_Controller{
	...
	function hasil(){
		...
		if ($idprod == 5)
		{
			$jenis_produk = "JS LINK EQUITY FUND"; 
		}
		...
		$this->load->model('ModSimulasi');
		$detailproduk = $this->ModSimulasi->getDetailProduk(1);
		...
		//$data['hasil']['nilai'][$i]['premiribuan'] = $data['hasil']['premiribuan'];
		...
		$this->load->view('hasil/linkequityfund',$data);
	...
	function createPDF($namaFile,$data){
		...
		$this->pdf->Cell(190,10,'JS LINK EQUITY FUND',1,0,'C', true);
		$this->pdf->ln(15);
		...
		$this->pdf->ln();
		$this->pdf->Cell(190,5,'JS LINK EQUITY FUND',0,0,'C');
		$this->pdf->ln(10);
		$this->pdf->SetFont('Arial','B',10);
		$this->pdf->Cell(1);
