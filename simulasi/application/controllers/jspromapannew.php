...
class Jspromapannew extends CI_Controller{
	
	function insertProTertanggung(){
		
		$this->load->model('ModSimulasi');
		
//		$result = $this->ModSimulasi->selectMaxIDTertanggung();
...
//			'BUILD_ID' => $result['BUILD_ID'],
		...
		$result = $this->ModSimulasi->insertProTertanggung($data['tertanggung']);
...
//		$result = $this->ModSimulasi->selectMaxIDPempol();
...
//			'BUILD_ID' => $result['BUILD_ID'],
		...
		$result = $this->ModSimulasi->insertProPempol($data['pempol']);
...
//		$result = $this->ModSimulasi->selectMaxIDAsuransiPokok();
...
//			'MEDICAL' => '0',
		...
		$result = $this->ModSimulasi->insertProAsuransiPokok($data['asuransipokok']);
...
//		$result = $this->ModSimulasi->selectMaxIDAlokasiDana();
...
//			'BUILD_ID' => $result['BUILD_ID'],
		...
		$result = $this->ModSimulasi->insertProAlokasiDana($data['alokasidana']);
...
//		echo $filepdf;
	...
	function insertProDataRider(){
		...
		$result = $this->ModSimulasi->insertProDataRider($data['datarider']);
		
		$result2 = $this->ModSimulasi->GenerateAllData($this->session->userdata('build_id'));
	...
	function hitungsaranua(){
		...
		$result = $this->ModSimulasi->getSaranUA($carabayar, $usiasekarang, $totalpremi);
	...
	function hitungjshcppromapannew(){
		...
		$result = $this->ModSimulasi->getBiayaAsuransiPerBulanHCPJSProMapan($usiasekarang, $uangasuransihcpjspromapannew, $jeniskelamincalontertanggung);
	...
	function hitungjshcpbpromapannew(){
		...
		$result = $this->ModSimulasi->getBiayaAsuransiPerBulanHCPBJSProMapan($usiasekarang, $uangasuransihcpbjspromapannew, $jeniskelamincalontertanggung);
	...
	function hitungjstpdpromapannew() {
		...
		$result = $this->ModSimulasi->getBiayaAsuransiPerBulanTPDJSProMapan($usiasekarang);
...
//		$hasil = (($result[$jeniskelamincalontertanggung]/1000) * $uangasuransijstpdjspromapannew) * $faktorkali;
	...
	function hitungjsci53promapannew() {
		...
		$result = $this->ModSimulasi->getBiayaAsuransiPerBulanCI53JSProMapan($usiasekarang);
...
//		$hasil = (($result[$jeniskelamincalontertanggung]/1000) * $uangasuransijsaddbjspromapannew) * $faktorkali;
	...
	function hitungjstermpromapannew() {
		...
		$result = $this->ModSimulasi->getBiayaAsuransiPerBulanTermJSProMapan($usiasekarang);
...
//		$this->createPDF($filePdf,$data);
		...
		$this->load->view('hasil/jspromapannew',$data);
	...
	function CetakPDF()
	{
		...
		//$ProTertanggung = $this->ModSimulasi->selectProTertanggung($buildid);
		...
		$this->load->view('promapan_create_pdf',$data);
		//$this->createPDF($data);
