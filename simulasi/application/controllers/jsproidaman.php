...
class Jsproidaman extends CI_Controller{
	...
	function hasil(){
		...
		$data['hasil']['biayaAsuransi'] = ((((str_replace(',', '', $biayaAsuransi[$statusperokok])) / 10000) / 1000) * $this->session->userdata('uangpertanggungan'))/12;
		...
		$data['hitung'] = array(
			...
			'JUA'=> $data['hasil']['uangpertanggungan']
		);
		...
		for($i=0;$i<=99;$i++){
			...
			{	
				...
				$premiberkala = $data['hasil']['premisingle']-((5 / 100) * $data['hasil']['premisingle']);
				$topupberkala = $data['hasil']['topupsingle']-((5 / 100) * $data['hasil']['topupsingle']);
				$totalinvestasi = $premiberkala + $topupberkala;
				$totalCOIplusCOA = $totalinvestasi * (5/100);
				$data['hasil']['nilai'][$i]['manfaatinvestasimin1'] = ((1+($data['hasil']['asumsi_inv_min1'] / 100)) * ($totalinvestasi));
				
				
//				$data['hasil']['nilai'][$i]['manfaatinvestasimin1'] = ($totalinvestasi - $totalCOIplusCOA)+(($data['hasil']['asumsi_inv_min1'] / 100) * ($totalinvestasi - $totalCOIplusCOA));
...
//																			- $totalCOIplusCOA) * (1 + (($data['hasil']['asumsi_inv_min1'] / 100) / 12)));
				...
				$data['hasil']['nilai'][$i]['manfaatinvestasimed1'] = ((1+($data['hasil']['asumsi_inv_med1'] / 100)) * ($totalinvestasi));
				
//				$data['hasil']['nilai'][$i]['manfaatinvestasimed1'] = ($totalinvestasi - $totalCOIplusCOA)+(($data['hasil']['asumsi_inv_med1'] / 100) * ($totalinvestasi - $totalCOIplusCOA));
...
//																			- $totalCOIplusCOA) * (1 + (($data['hasil']['asumsi_inv_med1'] / 100) / 12)));
				...
				$data['hasil']['nilai'][$i]['manfaatinvestasimax1'] = ((1+($data['hasil']['asumsi_inv_max1'] / 100)) * ($totalinvestasi));
				
				
//				$data['hasil']['nilai'][$i]['manfaatinvestasimax1'] = ($totalinvestasi - $totalCOIplusCOA)+(($data['hasil']['asumsi_inv_max1'] / 100) * ($totalinvestasi - $totalCOIplusCOA));
...
//																			- $totalCOIplusCOA) * (1 + (($data['hasil']['asumsi_inv_max1'] / 100) / 12)));
				...
				*/
				...
				$data['hasil']['nilai'][$i]['manfaatinvestasimin2'] = ($totalinvestasi - $totalCOIplusCOA)+(($data['hasil']['asumsi_inv_min2'] / 100) * ($totalinvestasi - $totalCOIplusCOA));
...
//																			- $totalCOIplusCOA) * (1 + (($data['hasil']['asumsi_inv_min2'] / 100) / 12)))
//																			- $totalCOIplusCOA) * (1 + (($data['hasil']['asumsi_inv_min2'] / 100) / 12)));
				$data['hasil']['nilai'][$i]['manfaatinvestasimed2'] = ($totalinvestasi - $totalCOIplusCOA)+(($data['hasil']['asumsi_inv_med2'] / 100) * ($totalinvestasi - $totalCOIplusCOA));
...
//																			- $totalCOIplusCOA) * (1 + (($data['hasil']['asumsi_inv_med2'] / 100) / 12)))
//																			- $totalCOIplusCOA) * (1 + (($data['hasil']['asumsi_inv_med2'] / 100) / 12)));
				$data['hasil']['nilai'][$i]['manfaatinvestasimax2'] = ($totalinvestasi - $totalCOIplusCOA)+(($data['hasil']['asumsi_inv_max2'] / 100) * ($totalinvestasi - $totalCOIplusCOA));
...
//																			- $totalCOIplusCOA) * (1 + (($data['hasil']['asumsi_inv_max2'] / 100) / 12)));
		...
		$this->load->view('hasil/jsproidaman',$data);
	...
	function createPDF($namaFile,$data){
		...
		$this->pdf->Cell(190,5,'JS PROIDAMAN (Jiwasraya Proteksi Investasi Dana Aman)','B',0,'L');
		$this->pdf->ln(10);
		...
		$this->pdf->ln();
		$this->pdf->SetFont('Arial','',8);
		$this->pdf->Cell(25,5,'JS PROIDAMAN','LTR',0,'C');
		...
		$this->pdf->Cell(165,5,'Apabila Tertanggung meninggal dunia dalam masa perjanjian Asuransi baik karena sakit atau karena kecelakaan, maka kepada ahli','LR',0,'J');
		$this->pdf->ln();
		$this->pdf->Cell(25,5,'','LBR',0,'C');
		$this->pdf->Cell(165,5,'waris atau yang ditunjuk dibayarkan Uang Asuransi JS PROIDAMAN ditambah Saldo Dana Investasi.','LBR',0,'J');
		$this->pdf->ln(25);
		...
		$this->pdf->SetFont('Arial','',6);
		$this->pdf->cell('3');
		$this->pdf->MultiCell(185,5,'c. Biaya pengelolaan investasi maksimal 2% per tahun tergantung jenis reksadana yang dipilih.',0);
//		$this->pdf->SetFont('Arial','',6);
//		$this->pdf->cell('3');
//		$this->pdf->MultiCell(185,5,'d. Biaya pembelian unit adalah sebesar 5% dari dana investasi.',0);
		$this->pdf->SetFont('Arial','',6);
		$this->pdf->MultiCell(190,5,'3. Unit yang dihitung berdasarkan Harga Unit pada saat tertentu.',0);
		...
		$this->pdf->Cell(190,5,'5. Perubahan harga unit menggambarkan hasil investasi dari dana investasi. Kinerja dari investasi tidak dijamin tergantung dari risiko masing-masing dana investasi. Pemegang Polis diberi keleluasaan',0);
		$this->pdf->ln();
		$this->pdf->Cell(2);
		$this->pdf->Cell(188,5,' untuk menempatkan alokasi dana investasi yang memungkinkan Optimalisasi tingkat pengembalian investasi, sesuai dengan kebutuhan dan profil risiko Pemegang Polis.',0);
		$this->pdf->ln();
		$this->pdf->MultiCell(190,5,'6. Besarnya Nilai Tunai yang dibayarkan (bisa lebih besar atau lebih kecil dari yang diilustrasikan) akan bergantung pada perkembangan dari dana investasi JS PROIDAMAN.',0);
		...
		$this->pdf->MultiCell(190,5,'9.	Polis bebas biaya penarikan dana.',0);
		...
		$this->pdf->Cell(190,5,'10. Penilaian harga unit dilakukan pada setiap hari kerja, Senin sampai dengan Jum`at dengan menggunakan metode harga pasar yang berlaku bagi instrumen investasi yang mendasari',0);
		$this->pdf->ln();
		$this->pdf->Cell(2);
		$this->pdf->Cell(188,5,'  masing-masing alokasi dana investasi yang dipilih.',0);
		$this->pdf->ln();
		$this->pdf->Cell(190,5,'11. Besarnya Nilai Tunai yang terbentuk pada polis ini (dapat lebih besar atau lebih kecil dari dana yang diinvestasikan oleh Pemegang Polis), akan dipengaruhi oleh fluktuasi dari harga unit atau faktor',0);
		$this->pdf->ln();
		$this->pdf->Cell(2);
		$this->pdf->Cell(188,5,'  biaya-biaya sebagaimana disebutkan di atas.',0);
		$this->pdf->ln();
		$this->pdf->Cell(190,5,'12. Harga unit yang digunakan pada Premi Pertama akan terbentuk setelah diterimanya SPAJ dan teridentifikasinya seluruh pembayaran Premi Pertama di Kantor Pusat oleh Jiwasraya. Tanggal ',0);
		$this->pdf->ln();
		$this->pdf->Cell(2);
		$this->pdf->Cell(188,5,'  Perhitungan Harga Unit adalah Tanggal Perhitungan berikutnya setelah diterimanya SPAJ. Atau teridentifikasinya seluruh pembayaran Premi pertama di Kantor Pusat, mana yang paling akhir.',0);
		$this->pdf->ln();
		$this->pdf->Cell(190,5,'13. Memiliki Polis Asuransi Jiwa merupakan komitmen jangka panjang. JS PROIDAMAN adalah suatu produk asuransi jiwa yang dikaitkan dengan investasi. Untuk dapat menikmati manfaat polis ini,',0);
		$this->pdf->ln();
		$this->pdf->Cell(2);
		$this->pdf->Cell(188,5,'  maka kami sarankan Anda untuk melakukan pembayaran Premi selama Masa Asuransi.',0);
