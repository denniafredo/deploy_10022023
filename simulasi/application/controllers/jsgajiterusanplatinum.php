...
class Jsgajiterusanplatinum extends CI_Controller{
	...
	function hasil(){
		...
		$this->load->view('hasil/jsgajiterusanplatinum',$data);
	...
	function createPDF($namaFile,$data){
		...
		$this->pdf->SetTextColor(255,255,255);
		$this->pdf->SetFillColor(65,105,225);
		$this->pdf->Cell(190,5,'JS GAJI TERUSAN PLATINUM',1,0,'C', true);
		$this->pdf->ln(5);
		$this->pdf->SetFont('Arial','I',8);
		...
		$this->pdf->Cell(190,5,'Yth Bapak/Ibu Prospek, berikut ini kami sampaikan ringkasan produk JS Gaji Terusan Platinum untuk membantu buah hati tercinta dalam mewujud-',0,0,'L');
		$this->pdf->ln();
...
//		$this->pdf->ln();
		...
		$this->pdf->Cell(2,5,':',0,0,'L');
		$this->pdf->Cell(3);
		$this->pdf->Cell(32,5,'Rp. '.number_format($data['hasil']['premitahunke6danseterusnya'],0,'.',',').'',0,0,'L');
//		$this->pdf->Cell(32,5,'Rp. '.number_format($data['hasil']['premitahunke6danseterusnya'],0,'.',',').' premi tahun ke-6 dan seterusnya ',0,0,'L');
		...
		$this->pdf->ln();
		$this->pdf->SetFont('Arial','B',8);
		$this->pdf->Cell(50,5,'b. Meninggal Dunia Asumsi Meninggal di akhir tahun ke-'.($data['hasil']['masaasuransijsgajiterusan']).'',0,0,'L');
		$this->pdf->ln();
		$this->pdf->SetFont('Arial','',8);
