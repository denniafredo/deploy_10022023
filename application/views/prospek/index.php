                    ...
                    <div class="form-group">
                        <div class="input-group">
                            ...
                            <span class="input-group-btn">
                                ...
                                <a href="<?="$this->url/add"?>" class="btn yellow-casablanca tooltips" data-container="body" data-placement="top" data-original-title="<?=C_BUTTON_ADD?>"><i class="fa fa-plus"></i></a>
                            </span>
                        </div>
                    </div>
                    </form>
                    ...
                    <div class="table-scrollable">
                        <table class="table table-bordered table-hover" style="font-size: 12px;">
                            ...
                            <?php foreach($prospek as $i => $v) { ?>
                                <tr>
                                    <td align="center"><?=$v['NOID']?></td>
                                    <td><?=$v['NAMAKLIEN']?></td>
                                    <td><?=$v['NAMAJENISKELAMIN']?></td>
                                    <td><?=$v['ALAMAT']?></td>
                                    <td align="center"><?=$v['TGLLAHIR']?></td>
                                    <td align="center"><?=$v['HP']?></td>
                                    <td align="center"><?=$v['TELEPON']?></td>
                                    <td><?=$v['EMAIL']?></td>
                                    <td class="text-center" width="95">
                                        <a href="<?="$this->url/edit?id=$v[NOID]"?>" class="btn btn-xs yellow-gold tooltips" style="margin:0px;" data-container="body" data-placement="top" data-original-title="<?=C_BUTTON_EDIT?>"><i class="fa fa-pencil btn-xs-tbl"></i></a>
                                        <a href="<?="$this->url/proposal?id=$v[NOID]"?>" class="btn btn-xs blue tooltips" style="margin:0px;" data-container="body" data-placement="top" data-original-title="Proposal"><i class="fa fa-search-plus btn-xs-tbl"></i></a>
                                        <!--a class="btn btn-xs red-flamingo tooltips btn-delete" data-value="<?=$v['NOID']?>" style="margin:0px;" data-container="body" data-placement="top" data-original-title="<?=C_BUTTON_DELETE?>"><i class="fa fa-trash-o btn-xs-tbl"></i></a-->
                                    </td>
                                </tr>
                            <?php } ?>
                            </tbody>
...
<script>
    jQuery(document).ready(function() {
        ...
        $('.btn-delete').click(function(){
            ...
            bootbox.confirm({
                ...
                callback: function(result){
                    if (result)
                        window.location.href = "<?="$this->url/delete?id="?>"+noprospek;
                }
            });
        });
        ...
        if ("<?=$status?>" == "<?=C_STATUS_SUKSES_SIMPAN?>")
            ...
            toastr.error('Oo tidaak, data gagal disimpan. <?=$message?>');
