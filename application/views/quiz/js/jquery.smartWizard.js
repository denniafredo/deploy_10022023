...
function SmartWizard(target, options) {
    ...
    this.buttons = {
        ...
        finish  : $('<span>'+options.labelFinish+'</span>').addClass("buttonFinish"),
        cont  : $('<span>'+options.labelContinue+'</span>').addClass("buttonFinish")
    };
    ...
    var _init = function($this) {
        var elmActionBar = $('<div></div>').addClass("actionBar");
		 elmActionBar.prepend($('#meButton').html());
        elmActionBar.append($this.msgBox);
        $('.close',$this.msgBox).click(function() {
            $this.msgBox.fadeOut("fast");
            return false;
        });
		...
		if ($this.options.includeContinueButton){
            elmActionBar.append($this.buttons.cont)
        }
		...
		$($this.buttons.cont).click(function() {
            if(!$(this).hasClass('buttonDisabled')){
                if($.isFunction($this.options.onContinue)) {
                    var context = { fromStep: $this.curStepIdx + 1 };
                    if(!$this.options.onContinue.call(this,$($this.steps), context)){
                        return false;
                    }
                }else{
                    var frm = $this.target.parents('form');
    ...
    var _adjustButton = function($this) {
		...
		$this.enableContinue($this.options.enableContinueButton);
    };
	...
	SmartWizard.prototype.enableContinue = function (enable) {
        // Controll status of finish button dynamically
        // just call this with status you want
        this.options.enableContinueButton = enable;
        if (this.options.includeContinueButton){
            if (!this.steps.hasClass('disabled') || this.options.enableContinueButton){
                $(this.buttons.cont).removeClass("buttonDisabled");
                if (this.options.hideButtonsOnDisabled) {
                    $(this.buttons.cont).show();
                }
            }else{
                $(this.buttons.cont).addClass("buttonDisabled");
                if (this.options.hideButtonsOnDisabled) {
                    $(this.buttons.cont).hide();
                }
            }
        }
        return this.options.enableContinueButton;
    }
...
// Default Properties and Events
    $.fn.smartWizard.defaults = {
        ...
        labelFinish:'Selesai',
        labelContinue:'Mulai Survei',
        noForwardJumping: false,
        ajaxType: "POST",
        onLeaveStep: null, // triggers when leaving a step
        onShowStep: null,  // triggers when showing a step
        onFinish: null,  // triggers when Finish button is clicked
        onContinue: null,  // triggers when Finish button is clicked
        includeFinishButton : true,   // Add the finish button
        includeContinueButton : false   // Add the finish button
    };
