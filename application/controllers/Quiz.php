...
class Quiz extends CI_Controller {
	...
	function lisensi(){
		$data = null;
		$quiz = $this->db->query('select * from JAIM_000_QUIZ where enabled = 1 
			and quizid not in (4) order by quizid')->result_array();
		
		$data['quiz'] = $quiz;
		
		//$this->load->view('quiz/__head',$data);
		$this->load->view('quiz/lisensi',$data);
	...
	function index(){
		...
		$quiz = $this->db->query(' select * from JAIM_000_QUIZ where enabled = 1 and quizid not in (4)  order by quizid')->result_array();
		...
		$this->load->view('quiz/index_quiz',$data);
	}
	...
	foreach($inp as $re => $val){
			...
			if($result){
				...
				//todo: Get scoring from DB JAIM_000_QUIZ field PASSINGSCORE
				if($score < 50 ){
					$array = array(
						'status'=>TRUE,
						'code'=>3,
