...
class Auth_Ldap {
    ...
    private function _init() {
        ...
        // Verify that the LDAP extension has besen loaded/built-in
    ...
    private function _authenticate($username, $password) {
        ...
        $search = ldap_search($this->ldapconn, $this->basedn, $filter, 
                array('dn', $this->login_attribute, 'cn','uidNumber'));
		...
		if($entries['count'] > 0){
	        ...
	        if(!$bind) {
	            ...
	            //return FALSE;
	            return array('status'=>FALSE,'cn' => '', 'dn' => '', 'id' => '','nik'=>'');
	        }else{
				$cn = $entries[0]['cn'][0];
				
				//Dimas
				$nik = $entries[0]['uidnumber'][0];
				//Dimas
		       ...
		       /* return array('cn' => $cn, 'dn' => $dn, 'id' => $id,'nik'=>$nik,
		            'role' => $this->_get_role($get_role_arg));*/
	        
	        	return array('status'=>TRUE,'cn' => $cn, 'dn' => $dn, 'id' => $id,'nik'=>$nik);
	        }
    ...
    private function _get_role($username) {
        ...
        if($results['count'] != 0) {
            for($i = 0; $i < $results['count']; $i++) {
                $role = array_search($results[$i]['ou'][0], $this->roles);
                if($role !== FALSE) {
                    return $role;
                }
            }
