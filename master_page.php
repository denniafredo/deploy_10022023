                    ...
                    <ul class="nav navbar-nav pull-right">
                        ...
                        <li class="dropdown dropdown-extended dropdown-dark dropdown-notification" id="header_notification_bar">
                            ...
                            <?php
								...
								$data5 = $this->prospek_model->get_list_histories();
                                $nonotif = count($data5) + count($data4) + count($data3) + count($data) + ($data2['SISAPKAJ'] <= 30 ? 1 : 0) + ($this->session->USERNAME == $this->session->PASSWORD ? 1 : 0);
                                // countdown expired
                                ...
                                $expiredUser && preg_match('/\/prospek.*/', $_SERVER["REQUEST_URI"]) ? header('Location: ' . base_url()) : '';
                                ...
                                <li>
                                    <ul class="dropdown-menu-list scroller" style="height: 275px;" data-handle-color="#637283">
										...
										<?php if (count($data5 > 0)) {
											foreach ($data5 as $i => $v) {
												if($v['KDSTATUS'] == 1){
													$icon = "fa-check";
													$label = "label-success";
													$ket = "Success melakukan Welcoming Call";
												}elseif($v['KDSTATUS'] == 2){
													$icon = "fa-comments";
													$label = "label-info";
													$ket = "Success melakukan Welcoming Call dengan catatan.";
												}else{
													$icon = "fa-warning";
													$label = "label-danger";
													$ket = "Gagal Welcoming Call, Mohon untuk menginformasikan jadwal terbaru. ";
												}
										?>
												<li>
													<a href="<?=base_url("prospek/follow-up?buildid=$v[BUILDID]")?>">
														...
														<span class="label label-sm label-icon <?= $label ?>">
															<i class="fa <?= $icon ?>"></i>
														</span>Build ID <?=$v['BUILDID']?> atas nama <?=$v['NMPEMPOL']." ".$ket; ?> </span>
													</a>
												</li>
											<?php }
										} ?>
                        ...
                        <li class="dropdown dropdown-user dropdown-dark">
                            <a href="javascript:;" class="dropdown-toggle" data-toggle="dropdown" data-hover="dropdown" data-close-others="true">
                                ...
                                <span class="username username-hide-mobile"><?php
									if ($this->session->KDROLE == '5' || $this->session->KDROLE == '6' || $this->session->KDROLE == '7' || $this->session->KDROLE == '8')
										{
											if($this->session->KDROLE == '5') {
												echo "Super User Agency";
											} else if($this->session->KDROLE == '6') {
												echo "Super Admin Agency";
											} else if($this->session->KDROLE == '7') {
												echo "ARM LPA";
											} else if($this->session->KDROLE == '8') {
												echo "BANCAS LPA";
											}
										}
									else
            ...
            <div class="container">
                <div class="hor-menu ">
                    <ul class="nav navbar-nav"> <?php
                                    ...
                                    <a data-hover='megamenu-dropdown' data-close-others='true' data-toggle='dropdown' href='$url'>
                                        $r[MENU] <i class='fa fa-angle-down'></i></a>
                                            <ul class='dropdown-menu pull-left'>";
                                                get_submenu($r['CHILDREN'], $this->session->USERNAME, $cd);
                                            echo "</ul>
                                    </li>";
                            }
                        }
                        ...
                        function get_submenu($submenu, $username, $countdown) {
                            foreach ($submenu as $i => $r) {
                                ...
                                if (empty($r['CHILDREN'])) {
                                    ...
                                    echo "<li class='dropdown-submenu'>
                                        ...
                                        <ul class='dropdown-menu'>";
                                    get_submenu($r['CHILDREN'], $username, $countdown);
                                    echo "</ul></li>";
                                }
                            }
                        } ?>
