    ...
    <div class="page-header">
        ...
        <div class="page-header-top">
            <div class="container">
                <!-- BEGIN LOGO -->
                <div class="page-logo">
                        <a href="<?=base_url()?>" style="text-decoration:none;" class="tooltips" data-container="body" data-placement="right" data-original-title="Jiwasraya Agency Information Management"><!--img src="<?=base_url()?>asset/img/logo-default.png" alt="logo" class="logo-default"--><span class="logo-default">JAiM</span></a>
                </div>
                <!-- END LOGO -->
				...
				$data3 = $this->user_model->get_agen_kickoff($this->session->USERNAME);
                                $nonotif = count($data3) + count($data) + ($data2['SISAPKAJ'] <= 30 ? 1 : 0) + ($this->session->USERNAME == $this->session->PASSWORD ? 1 : 0);
                                //$nonotif += file_get_contents(C_URL_API_JAIM."/topagenaward.php?r=1&p=".$this->session->USERNAME);
                                ...
                                if ($pbeUser &&
                                    in_array($_SERVER["REQUEST_URI"], [
                                        '/pendapatan/agen',
                                        '/evaluasi/monitor_proposal',
                                        '/agenda/pribadi',
                                        '/prospek',
                                        '/nasabah',
                                        '/nasabah/ulangtahun',
                                        '/nasabah/jatuhtempo',
                                        '/nasabah/jatuhtempopremi',
                                    ])
                                ) {
                                    header('Location: ' . base_url());
                                }

                                $expiredUser && $_SERVER["REQUEST_URI"] == '/prospek' ? header('Location: ' . base_url()) : '';
                            ?>
					...
					<?php if ($data3['USERNAME'] == $this->session->USERNAME) { ?>
                        ...
                        <li class="dropdown dropdown-user dropdown-dark">
                            <a href="javascript:;" class="dropdown-toggle" data-toggle="dropdown" data-hover="dropdown" data-close-others="true">
                                ...
                                <span class="username username-hide-mobile"><?=$this->session->NAMALENGKAP?></span>
                            </a>
            ...
            <div class="container">
                <div class="hor-menu ">
                    <ul class="nav navbar-nav"> <?php
                        // jika status join team == (0 || null ) & kdjabatanagen = 24

                        if ( $this->menu->getstatusjointeam($this->session->USERNAME) ) {
                           ...
                           foreach ($this->menu->l_menu() as $i => $r) {
                               $url = $r['URL'] ? base_url($r['URL']) : "javascript:;";
                                       if (empty($r['CHILDREN'])) {
                                        ...
                                        // HIDE ESPAJ IF EXPIRED USER
                                        if (($r['KDMENU'] == 'MN18030001') && $expiredUser) continue;
                                        // HIDE ESPAJ IF PBE USER
                                        if ($pbeUser && $r['KDMENU'] == 'MN18030001') continue;
                                           echo "<li><a href='$url'>$r[MENU]</a></li>";
                                       } else {
                                        // HIDE PENCAPAIAN, WORKBOOK IF PBE USER
                                        if ($pbeUser && in_array($r['KDMENU'], ['MN15060002', 'MN15060004'])) continue;
                                           ...
                                           echo "<li class='menu-dropdown classic-menu-dropdown'>
                                               <a data-hover='megamenu-dropdown' data-close-others='true' data-toggle='dropdown' href='$url'>
                                                   $r[MENU] <i class='fa fa-angle-down'></i></a>
                                                       ...
                                                       get_submenu($r['CHILDREN'], $cd);
                                                       echo "</ul>
                                               </li>";
                                       }
                               }
                        // ngecek jika KDJABATANAGEN != 24
                        }
                        else {
                             //foreach ($this->menu->l_menu() as $i => $r) {
                           foreach ($this->menu->l_menu_join() as $i => $r) {
                               $url = $r['URL'] ? base_url($r['URL']) : "javascript:;";
                        ...
                        function get_submenu($submenu, $countdown) {
                            foreach ($submenu as $i => $r) {
                                ...
                                if (empty($r['CHILDREN'])) {
                                    ...
                                    $expiredUser = strtotime($countdown['TGLAKHIRLISENSI']) < strtotime(date('d-M-y'));
                                    if (($r['KDMENU'] == 'MN21070001') && $expiredUser) continue;
                                    echo "<li>
                                        <a href='$url'>
                                        <i class='$r[ICON]'></i>
                                        $r[MENU]
                                    ...
                                    echo "<li class='dropdown-submenu'>
                                        ...
                                        <ul class='dropdown-menu'>";
                                        get_submenu($r['CHILDREN'], $countdown);
                                    echo "</ul></li>";
                                }
                            }
                        } ?>
        ...
        <div class="container">
            <div class="row">
                <div class="col-md-6 col-sm-6 col-xs-12 footer-block">
                    <h2>Tentang</h2>
                    <p>
                        JAIM (Jiwasraya Agency Information Management) adalah aplikasi yang diperuntukan mitra kerja agen jiwasraya.
                    </p>
                </div>
