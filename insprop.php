...
//echo $userid.'xxxx';
  ...
  include "./includes/database.php"; 
	...
	if($notertanggung2=="" && $namatertanggung2==""){
		$sql = "update $DBUser.tabel_200_temp set premiumholiday='$premiumholiday',nopolswitch='$prefix$noper',taltup='$taltup',".
			 	 				 "tgltransfer=to_date('$tgltransfer','DD/MM/YYYY'), jmltransfer='$jmltransfer',kdbank='$kdbank',norekeningdebet='$norekening' ".
  		 	 "where prefixpertanggungan='$prefixpertanggungan' and nopertanggungan='$nopertanggungan'";			 
	}else{
	//$sql = "update $DBUser.tabel_200_pertanggungan set notertanggung2='$notertanggung2',taltup='$taltup' where prefixpertanggungan='$prefixpertanggungan' and nopertanggungan='$nopertanggungan'";	
	$sql = "update $DBUser.tabel_200_pertanggungan set taltup='$taltup' where prefixpertanggungan='$prefixpertanggungan' and nopertanggungan='$nopertanggungan'";	
	}
  //echo $sql;
//  die;
  $DB->parse($sql);
...
<table border="0" cellpadding="1" width="700" cellspacing="1" class="tblhead">
	<tr>
		<td class="tblisi">
			<table width="100%" border="0" cellpadding="1" cellspacing="1">
                ...
                ?>
							...
							<tr>
							  <td >Tert. Tambahan</td>
							  <td >:</td>
							  <td ><? echo $notertanggung2; ?></td>
							  <td >Nama</td>
							  <td >:</td>
							  <td ><?echo $namatertanggung2; ?></td>
						  </tr>
							<tr>
							...
							<tr>
								...
								<td width="2%" >:</td>
								<td width="30%" ><? echo abs($ara["IDENTITAS"]); ?></td>
					  ...
					  <table width="95%" border="0" cellpadding="0" cellspacing="0" class="verdana8">	
						  <tr>
							  <td width="18%" >Kode Produk</td>
								...
								<td width="2%" >:</td>
                <?
								$sql = "select premi1, kdstatusfile from $DBUser.tabel_200_temp ".
                			   "where prefixpertanggungan='$prefixpertanggungan' and nopertanggungan='$nopertanggungan'";
                //echo $sql;
                	$DB->parse($sql);
                	$DB->execute();
            ...
            <?
            	$sql="select a.namabenefit,b.kdbenefit,decode(b.kdjenisbenefit,'R',0,b.premi) premi,b.nilaibenefit,b.kdjenisbenefit,to_char(b.expirasi,'DD/MM/YYYY') expirasi ".
              ...
              if(substr($kdproduk,0,3)=='JL4' && (($ara["KDJENISBENEFIT"]=='R' && substr($ara["KDBENEFIT"],0,4)!='RISK') || $ara["KDJENISBENEFIT"]=='I'))
                  $jmlpremi = $jmlpremi ;
              else
                  $jmlpremi+=$ara["PREMI"];
    // ---------------------------------------------------   akhir tambahan untuk new js link$kdproduk
                //$jmlpremi+=$ara["PREMI"];
            		...
            		echo "<tr>";
            		echo "<td colspan=3>"; 
					if($ara["KDBENEFIT"]=="RISKER") 
					echo "EXTRA PREMI" ;
					else 
					echo $ara["NAMABENEFIT"];
					echo "</td>";
            ...
            ?>
              ...
              ?>
			 				...
			 				<tr>
<?
	$sql="select kdstatusfile,namastatusfile from $DBUser.tabel_299_status_file WHERE kdstatusfile = '$kdstatusfile'";
	$DB->parse($sql);			
	$DB->execute();	    
	$ara=$DB->nextrow();			    
?>	
...
<?
	$sql="select a.thnkomisi,a.komisiagen,b.namakomisiagen,b.kdkomisiagen, ".
	     ...
	     "decode(b.kdkomisiagen,'02',a.komisiagen,decode('$kprod','1',decode('$kdjeniscb','B',a.komisiagen/$faktorkomisi,'X',a.komisiagen),decode('$kdjeniscb','B',a.komisiagen/$faktorkomisi,'X',a.komisiagen*$faktorkomisi) ) ) k ".  
			 ...
			 "where a.kdkomisiagen not in ('24') and a.kdkomisiagen=b.kdkomisiagen and ".
...
<tr>
 <td align="center" colspan="2"><input type="submit" value="SUBMIT & BUAT SKK" name="submit"></td>
</tr>
</form>
<tr>
 <form action="ntryprop_ulink.php" method="post" name="ediprop"> 
